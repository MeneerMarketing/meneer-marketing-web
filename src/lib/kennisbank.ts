...
import {
  getZoekenLinksForArticleSlug,
  getZoekenLinksForDienstSlug,
...
/** Voor schema en metadata: expliciete update-datum of fallback op publicatie. */
export function getArticleModifiedAt(article: KennisbankArticle): string {
  return article.modifiedAt ?? article.publishedAt;
}
...
export function getZoekenLinksForArticle(
  ...
  return getZoekenLinksForArticleSlug(slug, limit);
}
...
export function getZoekenLinksForDienst(
  ...
  return getZoekenLinksForDienstSlug(dienstSlug, limit);
}
