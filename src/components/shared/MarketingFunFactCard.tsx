      ...
      >
        <div className="absolute inset-0 flex flex-col justify-between rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_10px_32px_-14px_rgba(15,23,42,0.14)] [backface-visibility:hidden]">
          <div className="flex items-start justify-between">
            <span
              className="inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-white"
          ...
          <div>
            <p className="text-5xl font-black tracking-tighter" style={{ color: fact.accent }}>
              {fact.stat}
            </p>
        ...
        >
          <p className="text-[11px] font-bold uppercase tracking-[0.16em]" style={{ color: fact.accent }}>
            {fact.title}
          </p>
...
export function MarketingFunFactsRow({ facts, title }: MarketingFunFactsRowProps) {
  return (
    <section className="border-b border-slate-200 bg-slate-50/80" aria-labelledby={title ? "fun-facts-row-heading" : undefined}>
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        {title ? (
          <h2 id="fun-facts-row-heading" className="mb-8 text-center text-xl font-extrabold text-slate-900 sm:text-2xl">
            {title}
          </h2>
        ) : null}
        <ul className={`grid gap-4 ${facts.length === 1 ? "max-w-sm mx-auto" : facts.length === 2 ? "sm:grid-cols-2" : "sm:grid-cols-2 lg:grid-cols-3"}`}>
          {facts.map((fact) => (
            <li key={fact.stat}>
              <MarketingFunFactCard fact={fact} />
            </li>
          ))}
        </ul>
      </div>
