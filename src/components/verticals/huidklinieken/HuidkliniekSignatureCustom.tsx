...
const sig = HUIDKLINIEKEN_VERTICAL.pricing.signatureCustom;

interface HuidkliniekSignatureCustomProps {
  campaignRef?: string | null;
  onSelect?: () => void;
}

export function HuidkliniekSignatureCustom({
  campaignRef = null,
  onSelect,
}: HuidkliniekSignatureCustomProps) {
  return (
    <section
      id="signature-custom"
      className="relative overflow-hidden border-b border-slate-200 bg-[#0c1222] text-white"
      aria-labelledby="Huidkliniek-signature-heading"
    >
      <div
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="shrink-0 rounded-3xl border border-white/15 bg-white/5 p-6 backdrop-blur-sm lg:min-w-[240px] lg:text-right">
              ...
              <a
                href="#aanvraag"
                onClick={() => {
                  trackHuidkliniekEvent("huidkliniek_custom_click");
                  onSelect?.();
