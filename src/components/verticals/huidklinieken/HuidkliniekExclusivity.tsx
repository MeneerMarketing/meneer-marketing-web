...
interface HuidkliniekExclusivityProps {
...
const PILLARS = [
  {
    title: "Jouw stad",
    text: "Ik help niet tegelijk jouw directe huidkliniek-concurrent.",
  },
  {
    title: "Jouw positie",
...
export function HuidkliniekExclusivity({ personalization }: HuidkliniekExclusivityProps) {
  const { exclusivity } = HUIDKLINIEKEN_VERTICAL;
  ...
  return (
    <section
      ...
      aria-labelledby="Huidkliniek-exclusivity-heading"
    >
      <div
          ...
          <Reveal delay={0.12}>
            <p className="mt-8 inline-flex rounded-2xl border border-white/20 bg-white/5 px-4 py-2.5 text-sm font-medium text-white">
              {personalization.cityStatus === "RESERVED"
                ...
                : personalization.cityStatus === "EXCLUSIVE"
                  ? `${personalization.city} valt binnen jullie exclusieve huidkliniek-traject.`
