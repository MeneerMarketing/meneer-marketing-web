...
import { HuidkliniekPriceHeadline } from "@/components/verticals/huidklinieken/HuidkliniekPriceHeadline";
...
const HEADING_ID = "huidkliniek-why-price-heading";
...
const HUIDKLINIEK_CONTACT_HINTS: Record<ContactChannel["id"], string> = {
  whatsapp:
...
const HUIDKLINIEK_WHATSAPP =
  "Hoi Meneer Marketing! Ik heb een huidkliniek en wil eerst even sparren voordat ik start.";

function huidkliniekChannelHref(channel: ContactChannel): string {
  if (channel.id === "whatsapp") {
    return whatsappHref(HUIDKLINIEK_WHATSAPP) ?? channel.href;
  }
  if (channel.id === "email") {
    return mailtoHref({
      subject: "Huidkliniek · even sparren",
...
export function HuidkliniekWhyPrice() {
  const reduce = useReducedMotion();
      ...
      <div className="relative mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-[1.12fr_0.88fr] lg:items-start lg:gap-12">
                  ...
                  >
                    ...
                    <p className="mt-2 text-sm leading-relaxed text-orange-50">
                      {selectedPackage === "studio-edition"
                        ? "Je site live, gevonden in jouw stad. Hoe eerder we starten, hoe eerder Google je kliniek kent."
                        : selectedPackage === "local-growth"
                          ? "Local Growth · ik stuur actief op intakes uit Google. Laten we meteen plannen."
                ...
                <div className="flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-[#fffef9] shadow-[0_24px_50px_-36px_rgba(15,23,42,0.22)] ring-1 ring-slate-900/[0.04]">
                  ...
                  <div className="flex flex-col p-4 sm:p-5">
                          ...
                          <div className="flex items-start gap-2.5">
                            ...
                            <div className="max-w-[90%] rounded-2xl rounded-tl-md bg-white px-3.5 py-2.5 shadow-sm ring-1 ring-slate-200/80">
                              <p className="text-[13px] leading-relaxed text-slate-700">
                                {HUIDKLINIEK_CONTACT_HINTS[contactMode]}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="mt-auto flex justify-end pt-3">
                            <div className="max-w-[85%] rounded-2xl rounded-tr-md bg-[#FF5722] px-3.5 py-2 text-[13px] font-semibold text-white shadow-sm">
                              {contactMode === "whatsapp"
                                ? "WhatsApp me even over mijn kliniek"
                                : contactMode === "phone"
                                  ? "Zullen we bellen?"
                    ...
                    {activeChannel.external ? (
                      <a
                        href={huidkliniekChannelHref(activeChannel)}
                        target={
          ...
          <div className="flex w-full justify-center lg:sticky lg:top-24 lg:ml-4 lg:justify-start xl:ml-6">
            <HuidkliniekClinicReceipt packageId={selectedPackage} />
          </div>
        </div>
      </div>
    </section>
