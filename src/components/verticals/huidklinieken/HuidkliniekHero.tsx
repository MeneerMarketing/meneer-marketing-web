...
const whatsappLink =
  whatsappHref(
    "Hoi Meneer Marketing! Ik heb een huidkliniek en wil graag even schakelen over website, lokale SEO en afspraken.",
  ) ?? "/contact";

const pageSummary = [
  { label: "Website", hint: "From scratch" },
  { label: "Lokaal SEO", hint: "Kliniek + stad" },
  { label: "Afspraken", hint: "Klik → intake" },
      ...
      <div className="relative z-10 mx-auto grid max-w-6xl gap-10 px-4 pb-16 pt-14 sm:px-6 sm:pb-20 sm:pt-16 lg:grid-cols-2 lg:items-stretch lg:gap-12 lg:px-8 lg:pb-24 lg:pt-20">
        <Reveal className="flex h-full flex-col">
          ...
          <h1 className="mt-5 max-w-[18ch] text-[2.45rem] font-extrabold leading-[1.05] tracking-tight sm:max-w-none sm:text-[2.85rem] lg:text-[3.05rem] lg:leading-[1.05]">
            Ze zoeken een kliniek.
            <br />
            <span className="text-[#FF5722]">Ik zorg dat ze een intake boeken.</span>
          </h1>

          <p className="mt-5 max-w-lg text-[15px] leading-relaxed text-slate-300 sm:text-base">
            Custom website, lokale SEO en een pad van Google naar afspraak.
            Optioneel Google Ads of Meta Ads. Vanaf {fromMonthly}/m
            {launchPromo ? (
              <>
                , launch{" "}
              ...
              <div className="flex shrink-0 items-center gap-1.5 border-b border-white/10 bg-slate-900/95 px-3 py-2.5">
                ...
                <span className="ml-2 flex min-w-0 flex-1 items-center gap-1.5 truncate rounded-md bg-white/5 px-2 py-1 text-[10px] text-slate-400">
                  ...
                  <span className="truncate">
                    jouwkliniek.nl · behandelingen · boeken
                  </span>
                </span>
              </div>
              <div
                className="relative min-h-[240px] flex-1 sm:min-h-[280px] lg:min-h-0"
                aria-hidden
              >
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(145deg, #1a1520 0%, #2c2217 38%, #45382c 62%, #f5f0ea 100%)",
                  }}
                />
                <div
                  ...
                  style={{
                    backgroundImage:
                      "radial-gradient(ellipse 50% 40% at 70% 30%, rgba(255,87,34,0.35), transparent 55%), radial-gradient(circle at 20% 80%, rgba(245,240,234,0.2), transparent 45%)",
                  }}
                />
                <div className="absolute inset-x-6 top-8 bottom-10 flex flex-col justify-between sm:inset-x-8 sm:top-10">
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-orange-200/90">
                      Huidkliniek
                    </p>
                    <p className="mt-2 max-w-[14ch] text-2xl font-extrabold leading-tight tracking-tight text-white sm:text-3xl">
                      Behandelingen die rust en precisie uitstralen.
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {["Intake", "Huidverbetering", "Consult"].map((label) => (
                      <span
                        key={label}
                        className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-[11px] font-semibold text-white/90 backdrop-blur-sm"
                      >
                        {label}
                      </span>
                    ))}
