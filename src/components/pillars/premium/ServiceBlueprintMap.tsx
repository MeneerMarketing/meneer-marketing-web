...
/** Zone op de bouwtekening, posities in procenten van het canvas-lichaam. */
interface Zone {
  href: string;
  short: string;
  x: number;
...
const ZONES: Zone[] = [
  { href: "/diensten/webdevelopment", short: "Site", x: 0, y: 0, w: 100, h: 11 },
  { href: "/diensten/webdesign", short: "UI/UX", x: 3, y: 15, w: 45, h: 30 },
  { href: "/diensten/shopify-enterprise", short: "Shopify", x: 52, y: 15, w: 45, h: 36 },
  { href: "/diensten/web-apps", short: "Portal", x: 3, y: 49, w: 45, h: 29 },
  { href: "/diensten/optimalisatie", short: "Snelheid", x: 52, y: 55, w: 21, h: 26 },
  { href: "/diensten/animaties", short: "Motion", x: 76, y: 55, w: 21, h: 26 },
  { href: "/diensten/branding", short: "Merk", x: 3, y: 82, w: 30, h: 13 },
];
    ...
    case "/diensten/shopify-enterprise":
      return (
        <div className="grid h-full grid-cols-3 gap-1.5 p-2.5">
          {[0, 1, 2].map((i) => (
            <div key={i} className="flex flex-col gap-1 rounded-md border border-slate-200/80 bg-white/70 p-1.5">
        ...
        <div className="flex h-full items-center justify-center">
          <svg viewBox="0 0 60 34" className="w-4/5" aria-hidden>
            ...
            <path
              ...
              strokeLinecap="round"
            />
            <line x1="30" y1="30" x2="42" y2="14" stroke="#0F172A" strokeWidth="2.5" strokeLinecap="round" />
          </svg>
        </div>
      );
    case "/diensten/animaties":
      return (
        <div className="relative flex h-full items-center justify-center">
          ...
          <svg viewBox="0 0 24 24" className="absolute bottom-2 right-3 size-4 text-slate-700" fill="currentColor" aria-hidden>
...
/**
 * De bouwtekening: een wireframe van een site waarin elk onderdeel een dienst
 * is. Hover of tik op een zone en de bijbehorende dienst licht op, en andersom.
 * Het hub-element van de Bouwen-pagina.
 */
      ...
      <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1.15fr_1fr] lg:items-start">
          ...
          <div className="relative">
            {/* Blauwdruk-raster */}
            <div
              className="absolute -inset-4 rounded-3xl bg-[linear-gradient(to_right,rgba(56,189,248,0.07)_1px,transparent_1px),linear-gradient(to_bottom,rgba(56,189,248,0.07)_1px,transparent_1px)] bg-[size:26px_26px]"
              aria-hidden
            />
            {/* Crosshairs */}
            {["-left-2 -top-2", "-right-2 -top-2", "-left-2 -bottom-2", "-right-2 -bottom-2"].map(
              (pos) => (
                <span
                  key={pos}
                  className={`absolute ${pos} text-sky-500/60`}
                  aria-hidden
                >
                  +
                </span>
            ...
            <div className="relative overflow-hidden rounded-2xl border border-sky-500/25 bg-slate-900/80 shadow-[0_32px_64px_-28px_rgba(2,6,23,0.9)] backdrop-blur">
            ...
            <p className="mt-4 text-center text-[11px] font-semibold uppercase tracking-[0.16em] text-sky-400/60">
          ...
          <ul className="space-y-2">
            {services.map((service, index) => {
              ...
              const hasZone = zoneByHref.has(service.href);
              ...
              return (
                ...
                >
                  <Link
                    href={service.href}
                    onMouseEnter={hasZone ? () => setActive(service.href) : undefined}
                    onMouseLeave={hasZone ? () => setActive(null) : undefined}
                    onFocus={hasZone ? () => setActive(service.href) : undefined}
                    onBlur={hasZone ? () => setActive(null) : undefined}
                    className={`group flex items-center gap-3.5 rounded-2xl border px-4 py-3.5 transition-all duration-300 ${
                      isActive
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span
                        className={`block truncate text-sm font-extrabold tracking-tight transition-colors ${
