...
import { ArrowRight, BarChart3, Route, Target } from "lucide-react";
import { useState } from "react";

interface StrategieContextSectionProps {
...
const INSIGHTS = [
  {
    ...
    icon: Target,
    label: "Prioriteit",
    title: "De slimste euro is niet altijd een ad",
    preview: "Site, e-mail of vindbaarheid eerst?",
    body: "Iedereen kan een advertentie aanzetten. De vraag is of dat op dit moment de slimste euro is. Soms verdient je site eerst aandacht, soms je e-mail, soms je vindbaarheid.",
  },
  {
    id: "route",
    icon: Route,
...
export function StrategieContextSection({
  introParagraphs,
  angleTitle,
  angleBody,
  funFact,
  funFactSource,
  funFactStat,
}: StrategieContextSectionProps) {
  const reduce = useReducedMotion();
  const [active, setActive] = useState<InsightId>("euro");
  const [mode, setMode] = useState<"ads" | "plan">("ads");

  const activeInsight = INSIGHTS.find((i) => i.id === active)!;
  const roi = mode === "plan" ? "+34%" : "-18%";
  const cac = mode === "plan" ? "€ 42" : "€ 89";

  return (
    <section
      className="relative overflow-hidden border-b border-slate-200 bg-white"
      aria-labelledby="strategie-context-heading"
    >
      <div
          ...
          <div className="flex flex-col gap-6">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {introParagraphs[1] && active === "route" ? (
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6">
              <p className="text-sm font-extrabold tracking-tight text-slate-900">
                Ads eerst of fundament eerst?
              </p>
              <p className="mt-1 text-xs text-slate-500">
                Tik en zie wat er gebeurt met ROI en acquisitiekosten.
              </p>
              ...
              <div className="mt-5 grid grid-cols-2 gap-3">
                <div className="rounded-2xl border border-slate-100 bg-slate-50/80 px-4 py-3">
                  ...
                  <motion.p
                    ...
                    className={`mt-1 text-xl font-extrabold ${
                      mode === "plan" ? "text-emerald-500" : "text-red-400"
                    }`}
                  >
                    {roi}
                  </motion.p>
                ...
                <div className="rounded-2xl border border-slate-100 bg-slate-50/80 px-4 py-3">
                  ...
                  <motion.p
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className={`mt-1 text-xl font-extrabold ${
                      mode === "plan" ? "text-emerald-500" : "text-amber-500"
                    }`}
                  >
                    {cac}
                  </motion.p>
