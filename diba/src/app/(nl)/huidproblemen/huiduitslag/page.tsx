    ...
    <main className="figma-home bg-[var(--g-010)] text-[var(--t-strong)]">
      ...
      <section className="bg-[var(--g-700)] text-[var(--on-dark)] px-5 sm:px-9 lg:px-[7.5vw]">
        <div className="py-10 sm:py-14 lg:py-20">
          ...
          <h1 className="diba-display-l mt-6 max-w-[18ch]">
            {t("Huiduitslag, en wanneer")}
            <br />
            <span className="diba-accent-on-dark">{t("het haast heeft")}</span>
          </h1>
          ...
          <div className="mt-9">
            <Button variant="primair-op-donker" href="#alarm">
              {t("Kijk eerst naar de alarmsignalen")}
            </Button>
          </div>
        </div>
      </section>
      ...
      >
        <div className="mx-auto">
          <Label opDonker>{t("Wanneer je niet afwacht")}</Label>
          <h2 className="diba-display-s mt-5 max-w-[22ch]">
            {t("Zes signalen waarbij")}
            <br />
            <span className="diba-accent-on-dark">{t("je vandaag belt.")}</span>
          </h2>
          ...
          <div className="mt-10 rounded-[var(--r-md)] bg-[var(--g-800)] p-6 sm:p-8">
            <Label opDonker>{t("Waar je dan naartoe belt")}</Label>
            <dl className="mt-5 grid gap-4 sm:grid-cols-3 sm:gap-6">
              ...
              {ALARM_ROUTE.map((r) => (
                <div key={tc(r.waar)} className="flex h-full flex-col">
          ...
          <ul className="mt-8 sm:mt-12 grid gap-px overflow-hidden rounded-[var(--r-md)] bg-[var(--g-100)] md:grid-cols-2 lg:grid-cols-4">
            {OORZAKEN.map((o) => (
              <li key={o.naam} className="flex flex-col bg-white p-6 sm:p-7">
                <h3 className="diba-card-title">{tc(o.naam)}</h3>
      ...
      <section className="mx-5 mt-16 mb-5 rounded-[var(--r-xl)] lg:mt-20 bg-[var(--g-050)] px-7 py-10 sm:py-14 sm:mx-9 sm:px-12 lg:mx-[7.5vw] lg:px-16 lg:py-20">
        <div className="mx-auto">
          ...
          <h2 className="diba-display-s mt-5 max-w-[22ch]">
            {t("Naar je huisarts.")}
            <br />
            <span className="diba-accent">{t("Bij twijfel vandaag nog.")}</span>
          </h2>
          ...
          <div className="mt-8 flex flex-wrap gap-x-8 gap-y-3">
            ...
            >
              {t("Misschien is het eczeem")}
            </Link>
            <Link
              href="/huidproblemen/gevoelige-huid"
              className="diba-label text-[var(--g-700)] underline underline-offset-4"
            >
              {t("Of reageert je huid op je routine")}
            </Link>
          </div>
        </div>
      </section>
