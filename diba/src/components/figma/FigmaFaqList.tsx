...
export default function FigmaFaqList({ items }: { items: FigmaFaqItem[] }) {
  return (
    <div className="divide-y divide-[#dce5d6] border-t border-[#dce5d6]">
      {items.map((item) => (
        <details key={item.question} className="group py-6">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-6 text-lg tracking-[-.035em] text-[#2c3426] sm:text-xl">
