  ...
  return (
    <div>
      ...
      <ol className={`${kop ? "mt-8" : ""} flex flex-col gap-8`}>
        {momenten.map((m, i) => {
          const pct = momenten.length <= 1 ? 100 : Math.round((i / (momenten.length - 1)) * 100);
          return (
            <li key={m.label} className="grid gap-3 md:grid-cols-[140px_1fr] md:gap-8">
              <div>
              ...
              <p className={figmaBody}>{m.tekst}</p>
            </li>
          );
        })}
      </ol>
