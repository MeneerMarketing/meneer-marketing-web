...
export function buildPageRepresentation(input) {
    ...
    const subtitle = $("h1").next("p, h2, .subtitle, .product__subtitle").first().text().trim() ||
        $("meta[name='description']").attr("content")?.trim()?.slice(0, 180) ||
        null;
    const primaryCta = findFirstText($, [
...
function collectListItems($, selectors) {
    const items = [];
    for (const selector of selectors) {
        $(selector).each((_, el) => {
...
function collectTextMatches(text, needles) {
...
//# sourceMappingURL=pageRepresentation.js.map