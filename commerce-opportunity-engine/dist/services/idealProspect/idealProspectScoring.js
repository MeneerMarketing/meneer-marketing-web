 ...
 */
import { IDEAL_CATALOG_SWEET_SPOT, IDEAL_OWN_BRAND_SCORES, IDEAL_PLATFORM_SCORES, IDEAL_PRE_SCORE_WEIGHTS, } from "../../config/idealProspectProfile.js";
...
export function scoreCatalogSweetSpotIdeal(estimatedProducts) {
...
export function scoreDeepDiveFitProxy(input) {
    const evidence = [];
    let score = 38;
    ...
    if (input.pdpWeaknessProxy >= 60)
        evidence.push("current_pdp_underexplained");
    return { score: clamp(score), evidence };
}
export function scoreAssetReadinessProxy(input) {
...
export function scoreGoogleAdsSignalIdeal(input) {
    if (input.confirmedGoogleAdvertiser && input.paidConfirmed)
        return 95;
    if (input.confirmedGoogleAdvertiser)
        return 88;
    if (input.transparencyConfirmed)
        return 78;
    if (input.paidConfirmed)
        return 68;
    return 18;
}
export function computeIdealProspectPreScore(input) {
    ...
    const platform = IDEAL_PLATFORM_SCORES[(input.platform ?? "UNKNOWN").toUpperCase()] ??
        IDEAL_PLATFORM_SCORES.UNKNOWN;
    const ownBrand = IDEAL_OWN_BRAND_SCORES[String(input.brandCommerceModel)] ??
        IDEAL_OWN_BRAND_SCORES.UNKNOWN;
    let retailerPenalty = 0;
    if ((input.retailerScaleScore ?? 0) >= 75)
        retailerPenalty = 28;
    else if ((input.retailerScaleScore ?? 0) >= 65)
        retailerPenalty = 16;
    else if ((input.retailerScaleScore ?? 0) >= 55)
        retailerPenalty = 8;
    const components = {
    ...
    let score = components.googleAdsSignal * w.googleAdsSignal +
...
//# sourceMappingURL=idealProspectScoring.js.map