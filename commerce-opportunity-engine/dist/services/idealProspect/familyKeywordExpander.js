...
function activeFamilies(branch) {
...
function matchFamily(keyword, families) {
...
function familyRejects(keyword, family) {
    const lower = keyword.toLowerCase();
    for (const token of family.rejectTokens) {
        if (lower.includes(token.toLowerCase()))
            return `family_reject_token:${token}`;
    }
    return null;
}
function buildKeyword(keyword, family, archetypeId, keywordCategory, seed, source, volume, cpc) {
...
export async function expandFamilyKeywords(input) {
    ...
    for (const entry of familyIndex) {
        for (const seed of entry.family.seeds) {
            ...
            if (seen.has(key))
                continue;
            seen.add(key);
            const built = buildKeyword(seed, entry.family, entry.branch.archetypeId, entry.keywordCategory, entry.family.seeds[0], "family_seed", null, null);
            if ("keyword" in built)
                keywords.push(built.keyword);
            else
    ...
    if (input.allowIdeas && input.client && input.env) {
        ...
        for (const item of ideas.items) {
            ...
            const built = buildKeyword(candidate, family, owner.branch.archetypeId, owner.keywordCategory, family.seeds[0], "dataforseo_ideas", item.searchVolume, item.cpc);
            if ("keyword" in built)
                keywords.push(built.keyword);
            else
                rejected.push(built.rejected);
 ...
 */
export function selectProductionKeywords(keywords, options) {
    const byFamily = new Map();
    for (const keyword of keywords) {
        if (!byFamily.has(keyword.familyId))
            byFamily.set(keyword.familyId, []);
        byFamily.get(keyword.familyId).push(keyword);
    }
    const shareByArchetype = new Map(options.branches.map((branch) => [branch.archetypeId, branch.keywordShare]));
    const queues = [];
...
export async function persistFamilyKeywords(client, keywords) {
    ...
    const rows = keywords.map((keyword) => ({
        ...
        keyword_source: keyword.generationSource === "family_seed" ? "ARCHETYPE_SEED" : "ARCHETYPE_EXPANSION",
    ...
    if (error)
        throw new Error(`Failed to persist family keywords: ${error.message}`);
    const { data, error: loadError } = await client
        .from("keywords")
        .select("id, keyword, search_volume, cpc")
        .in("keyword", keywords.map((keyword) => keyword.keyword))
        .eq("locale", KEYWORD_LOCALE);
    if (loadError)
        throw new Error(`Failed to reload family keywords: ${loadError.message}`);
    ...
    return keywords.map((keyword) => {
        ...
        return {
            ...
            searchVolume: keyword.searchVolume ?? (row?.search_volume ?? null),
            cpc: keyword.cpc ?? (row?.cpc ?? null),
        };
    });
}
//# sourceMappingURL=familyKeywordExpander.js.map