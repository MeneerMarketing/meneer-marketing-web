 ...
 */
import { IDEAL_KEYWORD_INTENT_BLOCK, IDEAL_KEYWORD_INTENTS, IDEAL_PROSPECT_CATEGORIES, M93_DEFAULTS, } from "../../config/idealProspectProfile.js";
...
export async function selectIdealProspectKeywords(client, options) {
    ...
    const activeCategories = enabledCategories.length
        ? enabledCategories.filter((c) => IDEAL_PROSPECT_CATEGORIES.includes(c))
    ...
    if (error)
        throw new Error(`Failed to load ideal keywords: ${error.message}`);
    const scored = [];
    const preGateRejected = [];
    ...
    for (const row of data ?? []) {
        ...
        if (IDEAL_KEYWORD_INTENT_BLOCK.includes(intent))
            continue;
        if (intent &&
            !IDEAL_KEYWORD_INTENTS.includes(intent) &&
            intent !== "OTHER") {
            continue;
        }
        ...
        const idealBoost = Math.round(preGate.prospectingValue * 0.12) +
    ...
    for (const kw of scored) {
        ...
        if (clusterCount >= M93_DEFAULTS.maxClusterPerCategory)
            continue;
        const dupe = nearDupeKey(kw.keyword);
        if (dupeKeys.has(dupe))
            continue;
        if (kw.keyword_intent_type === "PRODUCT_BRANDED") {
            if (brandedCount >= maxBranded)
                continue;
            brandedCount += 1;
        }
        selected.push(kw);
...
//# sourceMappingURL=idealKeywordSelector.js.map