 ...
 */
import { ENGINEERING_FIXTURE_DOMAINS, } from "../../config/outreachScoring.js";
import { STRONG_CONTRAST_GATE, } from "../../config/idealProspectProfile.js";
import { scoreEngineeringPilotRow } from "../concept/selectPremiumDtcPilot.js";
import { scoreOutreachConceptFit, } from "../concept/outreachScoring.js";
export function evaluateStrongContrastGate(input) {
    ...
    if (input.deepDiveFit < t.minDeepDiveFit)
        blocked.push("insufficient_deep_dive_fit");
    if (input.salesFit < t.minSalesFit)
        blocked.push("insufficient_sales_fit");
    if (!input.confirmedAdvertiser)
        blocked.push("missing_ads_signal");
    return { passes: blocked.length === 0, blocked };
}
export function rankTrueSalesCandidates(entries, idealPreScores, newBrandIds) {
    const ranked = [];
    for (const entry of entries) {
        ...
        const contrast = evaluateStrongContrastGate({
            ...
            confirmedAdvertiser: entry.outreachInput.confirmedGoogleAdvertiser ||
                entry.outreachInput.paidConfirmed,
        });
        const engineeringFixture = ENGINEERING_FIXTURE_DOMAINS.includes(row.normalized_domain);
        ranked.push({
...
export function selectTrueSalesDesignTarget(ranked) {
    const eligible = ranked.filter((r) => r.contrastGatePass &&
        r.croDataSource === "AUDITED" &&
        !r.engineeringFixture);
    if (!eligible.length) {
        const bestAuditedNonFixture = ranked.find((r) => r.croDataSource === "AUDITED" && !r.engineeringFixture);
    ...
    if (runnerUps[0]) {
        why.push(`Above #2 ${runnerUps[0].domain}: sales +${winner.salesFit - runnerUps[0].salesFit}, transform delta ${(winner.transformation ?? 0) - (runnerUps[0].transformation ?? 0)}`);
    }
    return {
        recommended: winner,
        runnerUps,
...
//# sourceMappingURL=trueSalesRanking.js.map