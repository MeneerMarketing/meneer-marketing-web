...
import { scoreProductArchetypeFit } from "./productArchetypeFit.js";
...
function findToken(haystack, tokens) {
    for (const token of tokens) {
        if (hasToken(haystack, token))
            return token;
    }
    return null;
}
...
export function evaluateKeywordPreGate(input) {
    ...
    let preGateClass = "NON_BRANDED_SPECIFIC_PRODUCT";
    ...
    const prospectingValue = accepted
        ? Math.max(0, Math.min(100, Math.round(archetype.productArchetypeFitScore * 0.8 + commercialIntent * 0.2 + priorityBonus)))
...
//# sourceMappingURL=keywordPreGate.js.map