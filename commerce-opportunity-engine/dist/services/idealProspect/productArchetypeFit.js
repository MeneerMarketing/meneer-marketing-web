 ...
 */
import { ARCHETYPE_BY_ID, IDEAL_PRODUCT_ARCHETYPES_V1, MIN_ARCHETYPE_FIT_FOR_DISCOVERY, } from "../../config/idealProductArchetypes.js";
...
export function matchArchetype(keyword) {
    const normalized = normalize(keyword);
    let best = null;
    let bestLength = 0;
    for (const archetype of IDEAL_PRODUCT_ARCHETYPES_V1) {
        for (const family of archetype.families) {
            if (family.rejectTokens.some((token) => containsToken(normalized, token)))
                continue;
...
export function scoreProductArchetypeFit(keyword) {
    const normalized = normalize(keyword);
    ...
    if (archetype) {
        ...
        if (family)
            positiveSignals.push(`familie ${family.label}`);
    }
    else {
    ...
    if (!archetype)
        rejectReason = "NO_ARCHETYPE_MATCH";
    else if (!archetype.enabled)
        rejectReason = "ARCHETYPE_DISABLED";
    else if (broadHit)
        rejectReason = "TOO_BROAD";
    else if (productArchetypeFitScore < MIN_ARCHETYPE_FIT_FOR_DISCOVERY)
        rejectReason = "LOW_ARCHETYPE_FIT";
    return {
        ...match,
...
export function buildKeywordLineage(keyword, source, seed) {
...
//# sourceMappingURL=productArchetypeFit.js.map