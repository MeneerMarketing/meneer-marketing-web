 ...
 */
export async function seedArchetypeKeywords(client, archetypeIds) {
    const skipped = [];
    const rows = [];
    const meta = new Map();
    ...
    if (error)
        throw new Error(`Failed to seed archetype keywords: ${error.message}`);
    const { data, error: loadError } = await client
        .from("keywords")
        .select("id, keyword, category, product_archetype_id, product_family_id, product_archetype_fit_score, keyword_pre_gate_class, prospecting_value_score, search_volume, cpc, last_scanned_at")
        .in("keyword", rows.map((r) => String(r.keyword)))
        .eq("locale", KEYWORD_LOCALE);
    if (loadError)
        throw new Error(`Failed to reload archetype keywords: ${loadError.message}`);
    const seeded = [];
 ...
 */
export function selectCalibrationKeywords(seeded, options) {
    const byBranch = new Map();
    for (const keyword of seeded) {
        if (!byBranch.has(keyword.archetypeId))
            byBranch.set(keyword.archetypeId, []);
    ...
    for (const [, keywords] of byBranch) {
        ...
        for (const keyword of ranked) {
            if (branchPicks.length >= options.maxPerBranch)
                break;
            const used = perFamily.get(keyword.familyId) ?? 0;
            if (used >= options.maxPerFamily)
                continue;
        ...
        for (const keyword of ranked) {
            if (branchPicks.length >= options.maxPerBranch)
                break;
            if (branchPicks.includes(keyword))
                continue;
            branchPicks.push(keyword);
        }
...
//# sourceMappingURL=archetypeKeywordSeeder.js.map