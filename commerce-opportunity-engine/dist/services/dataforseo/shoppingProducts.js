...
function parsePaidItem(keyword, item, itemType) {
    return {
        itemType,
        keyword,
 ...
 */
export function extractPaidShoppingListings(keyword, items) {
    const out = [];
        ...
        if (type === "google_shopping_sponsored_carousel") {
            ...
            for (const child of nested) {
                ...
                if (childType === "google_shopping_sponsored_carousel_element") {
                    out.push(parsePaidItem(keyword, el, "google_shopping_sponsored_carousel_element"));
                }
            }
        }
    }
...
async function postShoppingProductsTask(input) {
    ...
    const response = await input.client.post("/merchant/google/products/task_post", body, { timeout: 60000 });
    const data = response.data;
    const task = data.tasks?.[0];
    const cost = task?.cost ?? data.cost ?? 0;
    if (data.status_code && data.status_code !== 20000) {
        throw new Error(`Shopping products task_post failed: ${data.status_message ?? data.status_code}`);
    }
    if (task?.status_code && task.status_code !== 20000 && task.status_code !== 20100) {
        throw new Error(`Shopping products task_post task error: ${task.status_message ?? task.status_code}`);
    }
    if (!task?.id) {
...
async function getShoppingProductsAdvanced(client, taskId) {
    const response = await client.get(`/merchant/google/products/task_get/advanced/${taskId}`, { timeout: 60000 });
    const data = response.data;
    const task = data.tasks?.[0];
    const cost = task?.cost ?? data.cost ?? 0;
    ...
    throw new Error(`Shopping products task_get failed: ${task?.status_message ?? data.status_message ?? status}`);
}
/**
...
export async function fetchShoppingPaidListings(input) {
    ...
    if (!result) {
        throw new Error(`Shopping products task ${posted.taskId} not ready within ${maxWait}ms (cost so far $${totalCost})`);
    }
    const items = Array.isArray(result.items) ? result.items : [];
    const itemTypes = Array.isArray(result.item_types)
        ? result.item_types.filter((t) => typeof t === "string")
        : [];
...
//# sourceMappingURL=shoppingProducts.js.map