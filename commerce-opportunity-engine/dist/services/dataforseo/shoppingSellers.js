...
export async function fetchShoppingSellers(input) {
    ...
    const post = await input.client.post("/merchant/google/sellers/task_post", body, { timeout: 60000 });
    ...
    if (post.data.status_code && post.data.status_code !== 20000) {
        throw new Error(`Shopping sellers task_post failed: ${post.data.status_message ?? post.data.status_code}`);
    }
    ...
    while (Date.now() - started < maxWait) {
        await sleep(2500);
        const get = await input.client.get(`/merchant/google/sellers/task_get/advanced/${task.id}`, { timeout: 60000 });
        ...
        if (status !== 20100 && status !== 40601 && status !== 40602) {
            throw new Error(`Shopping sellers task_get failed: ${t?.status_message ?? get.data.status_message ?? status}`);
        }
    }
    if (!result) {
    ...
    for (const raw of items) {
        ...
        if (!item)
            continue;
        if (asString(item.type) !== "shops_list")
            continue;
...
export function pickMatchingSellerOffer(offers, preferredSeller) {
