...
function pickRank(item, parentRank) {
...
function buildPaidAd(keyword, timestamp, fields) {
    const domainParts = normalizeDomainFromParts(fields.landingUrl, fields.displayedUrl, fields.domainHint);
    if (!domainParts) {
        return null;
    }
    return {
...
function extractTextPaidAds(keyword, items, timestamp) {
    const paidAds = [];
    if (!items) {
        return paidAds;
    }
...
function isLikelyRetailerDomain(domain, title) {
    const lower = domain.toLowerCase();
    if (M96_RETAILER_DOMAIN_HINTS.some((hint) => lower.includes(hint)))
        return true;
    if (isBlacklistedDomain(lower))
        return true;
    const titleLower = (title ?? "").toLowerCase();
    if (/vergelijk|kopen bij|webshop met|alle merken/i.test(titleLower))
        return true;
    if (titleSuggestsThirdPartyProduct(lower, title))
        return true;
    return false;
}
function extractOrganicProductResults(keyword, items, timestamp) {
    const results = [];
    if (!items)
        return results;
    for (const item of items) {
...
function extractShoppingPaidAds(keyword, items, timestamp, unresolvedAds) {
    const paidAds = [];
    if (!items) {
        return paidAds;
    }
...
function extractPaidAdsFromItems(keyword, items, timestamp, includeShopping) {
    const unresolvedShoppingAds = [];
...
function parseSerpResponse(keyword, responseData, includeShopping) {
    ...
    const { paidAds, shoppingAdCount, otherPaidLikeCount, unresolvedShoppingAds, organicResults } = extractPaidAdsFromItems(keyword, result?.items, timestamp, includeShopping);
...
export async function fetchGooglePaidAds(options, keyword, fetchOptions) {
    ...
    if (env.DATAFORSEO_SERP_FIXTURE_PATH) {
        ...
        const parsed = parseSerpResponse(keyword, fixtureData, env.GOOGLE_DISCOVERY_INCLUDE_SHOPPING_ADS);
        return {
            ...parsed,
            cost: 0,
    ...
    const responseData = await withRetry(async () => {
        const response = await client.post(SERP_LIVE_ADVANCED_PATH, payload);
    ...
    const parsed = parseSerpResponse(keyword, responseData, env.GOOGLE_DISCOVERY_INCLUDE_SHOPPING_ADS);
...
export async function saveSerpFixture(path, responseData) {
    const { writeFile, mkdir } = await import("node:fs/promises");
    const { dirname } = await import("node:path");
    await mkdir(dirname(path), { recursive: true });
    await writeFile(path, JSON.stringify(responseData, null, 2), "utf8");
}
//# sourceMappingURL=googleSerp.js.map