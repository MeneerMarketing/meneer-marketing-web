...
export function computeMaturityScore(input) {
    ...
    const reviewVolumeSignal = scoreReviews(input.productPage.reviewCount, input.productPage.rating);
    const catalogDepthSignal = scoreCatalog(input.signals.estimatedProductLinks, input.signals.estimatedCategoryLinks);
    ...
    const paymentSignal = input.signals.paymentSignals.length + input.productPage.paymentSignals.length > 0
        ? Math.min(1, (new Set([
    ...
    const platformMaturitySignal = input.platform !== "UNKNOWN"
...
function scoreCatalog(productLinks, categoryLinks) {
...
function scoreBranding(signals) {
    let observed = 0;
    let score = 0;
    if (signals.title) {
        observed += 1;
...
function scoreShippingReturns(signals, product) {
...
function scoreProductDataQuality(product) {
...
//# sourceMappingURL=maturityScorer.js.map