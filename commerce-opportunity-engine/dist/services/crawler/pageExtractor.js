...
function parseJsonLdProduct(node) {
    ...
    if (typeof brandNode === "string") {
        ...
        const brandName = brandNode.name;
...
function collectJsonLd(node, products, types) {
...
export function extractPageSignals(html, baseUrl) {
    ...
    const hasCartLink = htmlLower.includes("/cart") ||
        htmlLower.includes("winkelwagen") ||
        htmlLower.includes("cart.js") ||
        htmlLower.includes("mini-cart");
    const hasCheckoutLink = htmlLower.includes("/checkout") || htmlLower.includes("afrekenen");
    const hasAddToCart = htmlLower.includes("add-to-cart") ||
        ...
        $("button, input, a").filter((_, el) => {
            ...
            return (text.includes("winkelwagen") ||
    ...
    const productGridHints = ($(".product-grid, .product-list, .products-grid, [class*='product-card'], [class*='product-item'], [data-product-id]").length > 0
        ? 1
        : 0) +
    ...
    if (bodyTextSample.includes("ideal"))
        paymentSignals.push("ideal");
    if (bodyTextSample.includes("paypal"))
        paymentSignals.push("paypal");
    if (bodyTextSample.includes("klarna"))
