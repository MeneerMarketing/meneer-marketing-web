...
async function fetchWithAxios(url, timeoutMs) {
    const response = await axios.get(url, {
        timeout: timeoutMs,
        maxRedirects: 5,
        headers: {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) CommerceOpportunityEngine/1.0",
...
async function fetchWithPlaywright(url, timeoutMs) {
...
export async function crawlWebsite(url, timeoutMs) {
    const normalized = normalizeUrl(url);
    try {
        const axiosResult = await withRetry(() => fetchWithAxios(normalized, timeoutMs), { maxAttempts: 2, delayMs: 500, backoffFactor: 2 });
        ...
        try {
            ...
            return {
                ...
                html: "",
                status: "failed",
                errorMessage: playwrightError instanceof Error ? playwrightError.message : "Crawl failed",
                usedPlaywright: true,
            };
        }
    }
