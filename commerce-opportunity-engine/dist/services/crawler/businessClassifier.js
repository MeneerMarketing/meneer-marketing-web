...
const INVESTOR_RELATIONS_PATTERN = /\b(investeerders|investor relations|aandeelhouders|jaarverslag|annual report|newsroom|persberichten)\b/i;
const PROFESSIONAL_DIVISION_PATTERN = /\b(b2b|zakelijke markt|business solutions|professionele gezondheidszorg|for professionals|zakelijke oplossingen)\b/i;
export function classifyBusinessFromWebsite(domain, signals, ecommerce) {
    const breadth = computeBreadthSignals(signals);
    const decide = (businessType, businessTypeConfidence, businessTypeReasoning) => ({
        businessType,
    ...
    if (structural) {
        return decide(structural.businessType, 0.95, `Structureel domeinoordeel: ${structural.matchedSignal}`);
    }
    // Service-first orgs (membership, insurance, roadside) even if they also sell products
    if (signals.insuranceServiceMentions >= 2) {
        const serviceDominant = !ecommerce.isEcommerce ||
            ecommerce.ecommerceConfidence < 0.7 ||
            signals.storeLocatorMentions >= 1 ||
            countCorporateServiceHints(signals.bodyTextSample) >= 2;
        if (serviceDominant) {
            return decide("SERVICE_BUSINESS", ecommerce.isEcommerce ? 0.78 : 0.88, "Sterke service/verzekering/membership signalen; eventuele shop is secundair.");
        }
    }
    if (!ecommerce.isEcommerce && ecommerce.ecommerceConfidence < 0.35) {
        return decide("NON_ECOMMERCE", 0.75, "Geen betrouwbare ecommerce signalen op de website.");
    }
    ...
    const navBreadth = signals.estimatedCategoryLinks >= 40
        ? 0.15
    ...
    if (/grootste|europe'?s largest|filialen|vestigingen|warenhuis|hypermarkt|keten|landelijke dekking/i.test(signals.bodyTextSample)) {
    ...
    if (/kampeerwinkel|campingwinkel|outdoorwinkel|kampeer specialist/i.test(titleMeta) ||
    ...
    const best = scores[0];
    if (INVESTOR_RELATIONS_PATTERN.test(signals.bodyTextSample) &&
        PROFESSIONAL_DIVISION_PATTERN.test(signals.bodyTextSample)) {
        return decide("MASS_RETAILER", 0.9, "Investor relations naast een zakelijke divisie: corporate onderneming, geen webshop-prospect");
    }
    ...
    if (qualifiesAsMassRetailer(breadth)) {
        return decide("MASS_RETAILER", 0.85, [`assortimentsbreedte ${breadth.retailerBreadthScore}/100`, ...breadth.evidence].join("; "));
    }
    // Require stronger evidence for GENERAL_RETAILER vs specialist default
    if (best && best.type === "GENERAL_RETAILER" && best.score >= 0.5) {
        return decide("GENERAL_RETAILER", Math.min(best.score, 0.95), reasons.join("; ") || "GENERAL_RETAILER");
    }
    if (best && best.type !== "GENERAL_RETAILER" && best.score >= 0.45) {
        return decide(best.type, Math.min(best.score, 0.95), reasons.join("; ") || `Score ${best.type}`);
    }
    if (ecommerce.isEcommerce) {
        // Prefer specialist over weak general_retailer
        if (generalRetailerScore >= 0.5 && generalRetailerScore > specialistScore) {
            return decide("GENERAL_RETAILER", Math.min(generalRetailerScore, 0.95), reasons.join("; "));
        }
        return decide("SPECIALIST_WEBSHOP", Math.max(0.55, specialistScore), reasons.join("; ") ||
            "Ecommerce shop zonder sterke marketplace/retailer/comparison signalen.");
    }
    return decide("UNKNOWN", 0.4, "Onvoldoende sterke website signalen voor harde classificatie.");
}
export function needsHaikuFallback(classification) {
    return (classification.businessTypeConfidence < 0.55 ||
        classification.businessType === "UNKNOWN");
}
function countCorporateServiceHints(text) {
    const phrases = [
        "lid worden",
        "membership",
        "vereniging",
...
//# sourceMappingURL=businessClassifier.js.map