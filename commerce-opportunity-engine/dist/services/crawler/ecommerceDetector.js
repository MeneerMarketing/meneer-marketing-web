export function detectEcommerce(signals, platform, secondarySignals = []) {
    ...
    if (platform.platform !== "UNKNOWN" ||
        ...
        score += 0.2;
        evidence.add(`platform_${(platform.platform !== "UNKNOWN" ? platform.platform : platform.platformCandidate).toLowerCase()}`);
    }
    // Secondary crawl bonus: ecommerce routes confirmed beyond homepage
    if (secondarySignals.length > 0) {
        const secondaryHasShop = secondarySignals.some((s) => s.hasAddToCart ||
            s.hasCartLink ||
...
//# sourceMappingURL=ecommerceDetector.js.map