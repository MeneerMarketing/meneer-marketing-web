...
export function evaluateAnthropicBudgetGate(input) {
    const remaining = Math.max(0, input.configuredCap - input.currentRunCost);
    const estimatedAfter = input.currentRunCost + input.conservativeNextCallCost;
...
export function assertAnthropicBudgetOrThrow(input) {
...
export function estimateAnthropicCostFromUsage(input) {
    ...
    return ((input.inputTokens / 1_000_000) * inRate +
        (input.outputTokens / 1_000_000) * outRate);
}
//# sourceMappingURL=anthropicBudget.js.map