...
import { OutreachDraftAiSchema, validateOutreachDraft, } from "./claimValidation.js";
import { pickPrimaryObservation, pickPrimaryStrength, } from "./allowedClaims.js";
import { MM_COMPANY, appendTextSignature, renderOutreachHtml, } from "./emailRender.js";
import { countWords } from "./contentHash.js";
import { logger } from "../../utils/logger.js";
const INPUT_COST = 3.0;
const OUTPUT_COST = 15.0;
function humanCategoryLabel(category, _domain) {
...
function humanObservationLine(observation, productName) {
...
function humanStrengthLine(strength) {
...
export async function generateOutreachDraft(input) {
    ...
    const brandLabel = input.data.brandName?.replace(/\.(nl|com|be|eu)$/i, "") ??
        input.data.brandDomain.replace(/\.(nl|com|be|eu)$/i, "");
    const categoryLabel = humanCategoryLabel(input.data.category, input.data.brandDomain);
    const sourceRules = input.data.auditType === "EXACT_PAID_FUNNEL"
...
Claim geen advertentieverkeer naar deze pagina.`;
    const exampleObservation = humanObservationLine(observation, input.data.productName);
...
Return ONLY valid JSON:
{
  ...
  "claims_used": ["observation:${observation.id}"${strength ? `, "strength:${strength.id}"` : ""}]
    ...
    const strengthTitles = input.data.allowedClaims
        ...
        .map((c) => c.source_title);
    const availabilityProven = input.data.allowedClaims.some((c) => c.type === "OBSERVATION" &&
    ...
    const estimatedCost = (response.usage.input_tokens / 1_000_000) * INPUT_COST +
...
function parseJson(text) {
    ...
    if (!candidate)
        throw new Error("No JSON in outreach response");
    return JSON.parse(candidate);
}
//# sourceMappingURL=outreachDraftGenerator.js.map