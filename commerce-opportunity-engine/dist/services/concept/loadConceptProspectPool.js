 ...
 */
import { computeCurrentPdpQualityScore, } from "./currentPdpQuality.js";
function num(v) {
    if (v == null || v === "")
        return null;
    const n = Number(v);
...
function auditUrl(audit) {
    if (!audit?.page_representation)
        return null;
    const rep = audit.page_representation;
    return rep.url ?? null;
}
function resolveCroDataSource(audit, productUrl) {
    if (!audit?.audit_valid || !audit.cro_scores)
        return "MISSING";
    ...
    if (audit.status === "COMPLETED" || audit.audit_valid)
        return "AUDITED";
    return "PROXY";
}
function conceptFirstFromAudit(audit) {
...
function pageHealthUsable(audit, hasProductUrl) {
    if (!hasProductUrl)
        return false;
    ...
    if (auditStatus.includes("FAILED"))
        return false;
    return true;
}
function isExactPaidFunnel(opp, page, productUrl) {
    if (!opp?.paid_confirmed || !productUrl)
        return false;
...
export async function loadConceptProspectPool(supabase) {
    const { data: candidatesRaw, error: candErr } = await supabase
        .from("coe_concept_candidates")
        .select(`id, brand_id, status, concept_ready_score, brand_commerce_model,
    ...
    const [{ data: brandsRaw }, { data: oppsRaw }, { data: pagesRaw }, { data: auditsRaw }] = await Promise.all([
        supabase
            .from("brands")
            .select(`id, name, normalized_domain, business_type, platform, business_maturity_score,
           retailer_scale_score, confirmed_google_advertiser, transparency_confirmed,
           manual_excluded, do_not_contact, eligibility_status`)
            .in("id", brandIds),
        supabase
            .from("opportunities")
            .select(`id, brand_id, paid_confirmed, resolved_page_id, landing_url,
        ...
        supabase
            .from("audits")
            .select(`id, brand_id, opportunity_id, status, audit_valid, audit_confidence, cro_scores,
           conversion_leaks, strengths, meneer_marketing_fit_score, page_health_status,
           page_representation, findings`)
            .in("brand_id", brandIds),
    ]);
    const brandById = new Map((brandsRaw ?? []).map((b) => [b.id, b]));
    const oppsByBrand = new Map();
    for (const o of (oppsRaw ?? [])) {
        const list = oppsByBrand.get(o.brand_id) ?? [];
        list.push(o);
        oppsByBrand.set(o.brand_id, list);
    }
    const pageById = new Map((pagesRaw ?? []).map((p) => [p.id, p]));
    const auditsByBrand = new Map();
    ...
    for (const c of candidates) {
        ...
        let adsStatus = "none";
        if (brand.confirmed_google_advertiser && exactPaid)
            adsStatus = "confirmed_exact_paid";
        else if (brand.confirmed_google_advertiser)
            adsStatus = "confirmed_advertiser";
        else if (opp?.paid_confirmed)
            adsStatus = "paid_opportunity";
        else if (brand.transparency_confirmed)
            adsStatus = "transparency_confirmed";
        ...
        let currentPdpQualityScore = null;
        if (croDataSource === "AUDITED" && cro) {
            const quality = computeCurrentPdpQualityScore(cro, conceptSignals);
        ...
        const outreachInput = {
            ...
            visualDesignQuality: conceptSignals?.premium_design_perception ?? num(cro?.visual_design_quality),
            deepDiveQuality: conceptSignals?.deep_dive_quality ?? num(cro?.product_presentation_quality),
            conversionLeakCount: arrLen(audit?.conversion_leaks),
            strengthCount: arrLen(audit?.strengths),
            siteTechnicallyBroken: siteBroken,
            mmFitScore: num(opp?.meneer_marketing_fit_score) ?? num(audit?.meneer_marketing_fit_score),
...
//# sourceMappingURL=loadConceptProspectPool.js.map