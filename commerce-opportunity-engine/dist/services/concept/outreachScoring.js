 ...
 */
import { CATALOG_SWEET_SPOT_BANDS, CRO_EXCEPTIONAL_THRESHOLD, CRO_STRONG_THRESHOLD, OUTREACH_FIT_WEIGHTS, OUTREACH_GATE_THRESHOLDS, OUTREACH_PENALTIES, OWN_BRAND_FIT_SCORES, PLATFORM_FIT_SCORES, } from "../../config/outreachScoring.js";
import { croAlreadyStrongPenaltyFromQuality, } from "./currentPdpQuality.js";
...
export function scoreConceptContrast(input) {
...
export function scoreCatalogSweetSpot(estimatedProducts) {
...
export function scoreCurrentPdpWeakness(input) {
    ...
    let score = 55;
    if (input.aboveFoldQuality != null && input.aboveFoldQuality < 55)
        score += 12;
...
export function scoreCroAlreadyStrongPenalty(croComposite, currentPdpQuality, croDataSource) {
    if (croDataSource === "AUDITED" && currentPdpQuality != null) {
        return croAlreadyStrongPenaltyFromQuality(currentPdpQuality);
    }
    if (croComposite == null)
        return 0;
    ...
    if (croComposite >= CRO_STRONG_THRESHOLD) {
        const ramp = ((croComposite - CRO_STRONG_THRESHOLD) /
...
export function scoreDeepDivePdpFit(input) {
    const evidence = [];
    let score = 42;
    ...
    if (weakness >= 60)
        evidence.push("current_pdp_underutilized");
...
export function scoreProjectEconomicFit(input) {
    const evidence = [];
    let score = 28;
    ...
    const ownBrand = OWN_BRAND_FIT_SCORES[String(input.brandCommerceModel)] ?? OWN_BRAND_FIT_SCORES.UNKNOWN;
    ...
    const platform = PLATFORM_FIT_SCORES[(input.platform ?? "UNKNOWN").toUpperCase()] ??
...
export function scoreGoogleAdsConfirmation(input) {
    if (input.confirmedGoogleAdvertiser && input.exactPaidFunnelLikely)
        return 95;
    if (input.confirmedGoogleAdvertiser && input.paidConfirmed)
        return 88;
    if (input.confirmedGoogleAdvertiser || input.transparencyConfirmed)
        return 78;
...
function computePenalties(input) {
    const croAlreadyStrong = scoreCroAlreadyStrongPenalty(input.croQualityComposite, input.currentPdpQualityScore, input.croDataSource);
    let hugeCatalog = 0;
    ...
    if (model === "GENERAL_RESELLER")
        generalRetailer = OUTREACH_PENALTIES.generalRetailerMax;
...
export function computeOutreachScoreConfidence(input) {
    if (input.croDataSource === "AUDITED") {
    ...
    if (input.croDataSource === "PROXY") {
        ...
        if (input.croQualityComposite != null)
            c += 12;
        if (num(input.conceptAssetReadinessScore) >= 60)
            c += 8;
        return clamp(c);
    }
    return 28;
}
export function scoreOutreachConceptFit(input, engineeringScore) {
    const evidence = [];
    const catalogSweet = scoreCatalogSweetSpot(input.estimatedProductCount);
    ...
    const components = {
        ...
        ownBrandFit: OWN_BRAND_FIT_SCORES[String(input.brandCommerceModel)] ?? OWN_BRAND_FIT_SCORES.UNKNOWN,
        ...
        platformFit: PLATFORM_FIT_SCORES[(input.platform ?? "UNKNOWN").toUpperCase()] ??
            PLATFORM_FIT_SCORES.UNKNOWN,
        projectEconomicFit: projectEconomic.score,
        deepDivePdpFit: deepDive.score,
    };
    const w = OUTREACH_FIT_WEIGHTS;
    const weighted = components.pdpTransformation * w.pdpTransformation +
    ...
    return {
        ...
        contrast,
        evidence,
        formula: "weighted(OUTREACH_FIT_WEIGHTS) - penalties; confidence from audit coverage",
    };
}
//# sourceMappingURL=outreachScoring.js.map