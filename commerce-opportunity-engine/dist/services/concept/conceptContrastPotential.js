 ...
 */
import { CONTRAST_ADJUSTMENTS, CONTRAST_BANDS, CONTRAST_CAPABILITY_RANGE, CONTRAST_CAPABILITY_WEIGHTS, CONTRAST_CONFIDENCE, CONTRAST_GATE_THRESHOLDS, CONTRAST_ROOM_WEIGHTS, CURRENT_QUALITY_CEILINGS, PREMIUM_DESIGN_CEILINGS, contrastBandFor, } from "../../config/conceptContrast.js";
...
function computeRoomScore(input) {
    const evidence = [];
    const w = CONTRAST_ROOM_WEIGHTS;
    const quality = input.croDataSource === "AUDITED" && input.currentPdpQuality != null
    ...
    const score = (100 - qualityValue) * w.currentPdpQuality +
    ...
    if (design >= 75)
        evidence.push(`huidige vormgeving al premium (${Math.round(design)})`);
    if (storytelling <= 45)
        evidence.push(`product wordt nauwelijks uitgelegd (${Math.round(storytelling)})`);
    if (deepDive <= 45)
        evidence.push(`geen echte verdieping op de pagina (${Math.round(deepDive)})`);
    if (buyblock <= 50)
        evidence.push(`koopmoment nog onbenut (${Math.round(buyblock)})`);
    if (mobile <= 45)
        evidence.push(`mobiel kopen schiet tekort (${Math.round(mobile)})`);
    return { score: clamp(score), quality, design, evidence };
}
/** Capability is what we can actually build with, given this shop's material. */
function computeCapabilityScore(input) {
    const evidence = [];
    ...
    const score = assets * w.assetReadiness +
        commercial * w.commercialSignal +
        ownBrand * w.ownBrandFit +
        focus * w.catalogFocus +
        maturity * w.businessMaturity;
    if (assets < 45)
        evidence.push(`te weinig materiaal voor een premium versie (${Math.round(assets)})`);
    if (ownBrand >= 88)
        evidence.push("eigen merk: volledige verhaallijn mogelijk");
    return { score: clamp(score), evidence };
}
/** Maps the capability score onto the multiplier range. */
function capabilityFactor(capabilityScore) {
    const { min, max, fullAt, floorAt } = CONTRAST_CAPABILITY_RANGE;
    if (capabilityScore >= fullAt)
        return max;
    if (capabilityScore <= floorAt)
        return min;
 ...
 */
function applyCeilings(score, quality, design) {
    let capped = score;
    let ceiling = null;
...
function computeConfidence(input) {
    ...
    if (input.croQualityComposite != null)
        return CONTRAST_CONFIDENCE.proxy;
    return CONTRAST_CONFIDENCE.missing;
}
export function computeConceptContrastPotential(input) {
...
export function contrastBlocksDesignTarget(score) {
...
//# sourceMappingURL=conceptContrastPotential.js.map