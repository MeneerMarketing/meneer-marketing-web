 ...
 */
import { DESIGN_TARGET_GATE, TRUE_SALES_DESIGN_WEIGHTS, TRUE_SALES_DESIGN_FORMULA, } from "../../config/newProspectAudit.js";
export function evaluateDesignTargetGate(input) {
    const blockers = [];
    const warnings = [];
    if (input.excluded) {
    ...
    if (DESIGN_TARGET_GATE.blockedBusinessTypes.includes(businessType)) {
    ...
    if (input.currentPdpQuality == null) {
        ...
        blockers.push(`huidige PDP te sterk (${input.currentPdpQuality} > ${DESIGN_TARGET_GATE.hardMaxCurrentPdpQuality})`);
    }
    else if (input.currentPdpQuality > DESIGN_TARGET_GATE.preferredMaxCurrentPdpQuality) {
        warnings.push(`PDP boven de voorkeursgrens (${input.currentPdpQuality} > ${DESIGN_TARGET_GATE.preferredMaxCurrentPdpQuality})`);
    }
    if ((input.transformation ?? 0) < DESIGN_TARGET_GATE.minTransformation) {
        blockers.push(`transformatie te laag (${input.transformation ?? 0} < ${DESIGN_TARGET_GATE.minTransformation})`);
    }
    if ((input.conceptContrast ?? 0) < DESIGN_TARGET_GATE.minConceptContrast) {
        blockers.push(`concept contrast te laag (${input.conceptContrast ?? 0} < ${DESIGN_TARGET_GATE.minConceptContrast})`);
    }
    if ((input.assetReadiness ?? 0) < DESIGN_TARGET_GATE.minAssetReadiness) {
        blockers.push(`te weinig materiaal (${input.assetReadiness ?? 0} < ${DESIGN_TARGET_GATE.minAssetReadiness})`);
    }
    if ((input.deepDiveFit ?? 0) < DESIGN_TARGET_GATE.minDeepDiveFit) {
        blockers.push(`deep-dive fit te laag (${input.deepDiveFit ?? 0} < ${DESIGN_TARGET_GATE.minDeepDiveFit})`);
    }
    ...
    if (input.businessMaturity == null) {
        ...
        blockers.push(`business te onvolwassen (${input.businessMaturity} < ${DESIGN_TARGET_GATE.minBusinessMaturity})`);
    }
    if ((input.auditConfidence ?? 0) < DESIGN_TARGET_GATE.minAuditConfidence) {
        blockers.push(`auditvertrouwen te laag (${input.auditConfidence ?? 0} < ${DESIGN_TARGET_GATE.minAuditConfidence})`);
    }
 ...
 */
function measurementConfidence(input) {
    const parts = [
...
export function computeTrueSalesDesignScore(input) {
...
//# sourceMappingURL=trueSalesDesignScore.js.map