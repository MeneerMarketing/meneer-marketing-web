...
export function computeDesignInterventionOpportunity(input) {
    const evidence = [];
    const subs = Object.entries(input.subScores);
    let weaknessSum = 0;
    let weaknessCount = 0;
    ...
    const avgWeakness = weaknessCount > 0 ? weaknessSum / weaknessCount : input.currentPdpQuality != null ? 100 - input.currentPdpQuality : 50;
    const leakBoost = Math.min(18, input.leaks.filter((leak) => leak.severity === "HIGH" || leak.severity === "CRITICAL").length * 6 +
...
//# sourceMappingURL=designInterventionOpportunity.js.map