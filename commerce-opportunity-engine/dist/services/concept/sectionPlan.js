 ...
 */
function hintsInclude(hints, ...tokens) {
    const h = hints.map((x) => x.toLowerCase()).join(" ");
    return tokens.some((t) => h.includes(t));
}
export function buildRecommendedSectionPlan(input) {
    const plan = [];
    const beauty = hintsInclude(input.categoryHints, "beauty", "skin", "skincare", "cosmetic", "verzorging");
    const pets = hintsInclude(input.categoryHints, "pet", "hond", "kat", "dier", "dog", "cat");
    const sleep = hintsInclude(input.categoryHints, "sleep", "matras", "slaap", "bedding");
    plan.push({
    ...
    plan.push({
        ...
        content_source: input.hasDeliveryReturns || input.hasGuarantee || input.hasReviews
            ? "SOURCE_CONTENT"
    ...
    if (input.hasBenefits === true || input.descriptionLength >= 80) {
        plan.push({
            ...
            content_source: input.hasBenefits === true ? "SOURCE_CONTENT" : "DERIVED_COPY",
        });
    }
    ...
    if (input.hasHowItWorks === true || beauty) {
        plan.push({
            ...
            content_source: input.hasHowItWorks === true ? "SOURCE_CONTENT" : "PLACEHOLDER_REQUIRED",
        });
    }
    if (input.hasHowToUse === true || beauty || pets) {
        plan.push({
            ...
            content_source: input.hasHowToUse === true ? "SOURCE_CONTENT" : "PLACEHOLDER_REQUIRED",
        });
    }
    if (input.hasIngredients === true || (beauty && input.hasIngredients !== false)) {
        plan.push({
            ...
            content_source: input.hasIngredients === true ? "SOURCE_CONTENT" : "PLACEHOLDER_REQUIRED",
        });
    }
    if (input.hasMaterials === true || pets || sleep) {
        plan.push({
            ...
            content_source: input.hasMaterials === true ? "SOURCE_CONTENT" : "PLACEHOLDER_REQUIRED",
        });
    }
    ...
    if (input.hasSizeGuide === true || pets || sleep) {
        plan.push({
            ...
            content_source: input.hasSizeGuide === true ? "SOURCE_CONTENT" : "PLACEHOLDER_REQUIRED",
        });
    }
...
//# sourceMappingURL=sectionPlan.js.map