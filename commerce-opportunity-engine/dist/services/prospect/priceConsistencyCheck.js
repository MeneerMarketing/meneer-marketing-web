...
export function parseEuropeanMoneyAmount(raw) {
...
function pricesConflict(a, b) {
    const max = Math.max(a, b);
    const min = Math.min(a, b);
    if (max <= 0)
        return false;
    const ratio = max / min;
    if (ratio >= 8 && max >= 500 && min < 250)
        return true;
    if (ratio >= 3 && max - min >= 150)
        return true;
    return false;
}
function isLikelyDecimalParseError(high, low) {
    if (high < 500 || low >= 250)
        return false;
...
export function assessPriceConsistency(input) {
    ...
    const metaRaw = $('meta[property="product:price:amount"]').attr("content") ??
        $('meta[itemprop="price"]').attr("content") ??
        null;
    if (metaRaw) {
        const metaValue = parseEuropeanMoneyAmount(metaRaw);
        const metaSuspectBareInteger = metaValue != null &&
            metaValue > 10000 &&
            !metaRaw.includes(",") &&
            !metaRaw.includes(".");
    ...
    let canonicalPrice = input.primaryPrice;
    ...
    if (conflict) {
        ...
        if (isLikelyDecimalParseError(high, low)) {
            ...
            const domOrVisible = sources.find((s) => s.source === "dom_selector" || s.source === "visible_text");
...
//# sourceMappingURL=priceConsistencyCheck.js.map