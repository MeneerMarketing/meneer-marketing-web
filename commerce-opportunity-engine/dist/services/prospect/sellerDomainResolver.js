...
const TLD_ORDER = ["nl", "com", "be", "eu"];
function stripDiacritics(value) {
    return value.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
}
function sellerTokens(seller) {
    return stripDiacritics(seller.toLowerCase())
        .split(/[^a-z0-9]+/)
        .filter((token) => token.length > 0 && !NOISE_TOKENS.has(token));
}
export function buildSellerDomainCandidates(seller) {
 ...
 */
function confirmSeller(stemTokens, title, bodyTextSample, finalHost) {
    ...
    if (!finalHost.replace(/[^a-z0-9]/g, "").includes(compact))
        return "rejected";
    if (PARKED_PATTERNS.some((pattern) => haystack.includes(pattern)))
        return "rejected";
    if (haystack.includes(compact) || haystack.includes(spaced))
        return "content";
    ...
    if (distinctive && hostStem(finalHost) === compact && challenged)
        return "host_only";
    return "rejected";
}
export async function resolveSellerDomain(seller, options) {
    ...
    for (const candidate of buildSellerDomainCandidates(seller)) {
        ...
        const confirmation = confirmSeller(candidate.stemTokens, signals.title, signals.bodyTextSample, finalHost);
...
//# sourceMappingURL=sellerDomainResolver.js.map