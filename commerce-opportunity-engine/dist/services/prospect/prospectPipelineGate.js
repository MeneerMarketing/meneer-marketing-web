...
import { ALWAYS_EXCLUDED_BUSINESS_TYPES, COMPARISON_DOMAIN_PATTERNS, MARKETPLACE_DOMAIN_PATTERNS, MASS_RETAIL_OPERATOR_TOKENS, MASS_RETAIL_SIGNAL_THRESHOLDS, NON_PROSPECT_BUSINESS_TYPES, PROSPECT_EXCLUSION_VERSION, } from "../../config/prospectExclusion.js";
 ...
 */
export function classifyProspectExclusion(signals) {
    const evidence = [];
    ...
    const deny = (reason, prospectClass) => ({
        eligible: false,
        reason,
        evidence,
        prospectClass,
    ...
    if (ALWAYS_EXCLUDED_BUSINESS_TYPES.includes(type)) {
        evidence.push(`business_type=${type}`);
        const cls = type === "COMPARISON_SITE"
            ? "COMPARISON_SITE"
            : type === "MARKETPLACE"
                ? "MARKETPLACE"
                : type === "MASS_RETAILER"
    ...
    const breadthHit = categorySpread >= MASS_RETAIL_SIGNAL_THRESHOLDS.categorySpread ||
    ...
    if (breadthHit) {
        evidence.push(`adverteert breed: ${categorySpread} archetype-categorieën, ${keywordSpread} keywords`);
    ...
    if (signals.isEcommerce === false && NON_PROSPECT_BUSINESS_TYPES.includes(type)) {
        evidence.push("geen ecommerce signalen");
        return deny("non_ecommerce", "NON_COMMERCE");
    }
    const prospectClass = type === "BRAND" || model === "DTC_OWN_BRAND" || model === "MOSTLY_OWN_BRAND"
        ? "NICHE_BRAND"
...
export function passesProspectPipelineGate(signals) {
 ...
 */
export function classifySerpDomain(domain, businessType) {
 ...
 */
export function structuralDomainClass(domain) {
...
//# sourceMappingURL=prospectPipelineGate.js.map