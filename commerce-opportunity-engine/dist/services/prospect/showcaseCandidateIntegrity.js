...
const CATEGORY_PATH = /\/c\/|\/categor(?:y|ie)\/|\/collectie\/|\/collection\/|\/accessoires\/[^/]+\.html$/i;
const LISTING_SLUG = /\/(keukenaccessoires|deodorant|handverzorging|keuken-en-kantine|keukenstijlen)\/?$/i;
const PLURAL_LISTING_TITLE = /\d+\s+producten|online bestellen|categorie|overzicht|accessoires$/i;
...
export function extractProductIdentityFingerprint(html, productUrl, domain, productTitle) {
    ...
    const sku = json?.sku ??
        ...
        null;
    const gtin = json?.gtin ??
        json?.gtin13 ??
        $('meta[property="product:gtin"]').attr("content") ??
        null;
    const manufacturer = json?.manufacturer?.name ??
...
export function hardenShowcasePageEntity(input) {
    ...
    const hasSingleCta = (input.html.match(/add-to-cart|winkelwagen|in winkelwagen|koop nu/i) ?? []).length <= 4;
    if (isUsableHeroUrl(input.productUrl, input.domain) &&
        hasProductSchema &&
        hasSingleCta &&
        productLinks <= 6) {
...
export function assessCrossDomainMatches(fingerprints) {
    ...
    for (let i = 0; i < fingerprints.length; i += 1) {
        for (let j = i + 1; j < fingerprints.length; j += 1) {
            ...
            if (cmp.verdict === "SAME_PRODUCT" || cmp.verdict === "LIKELY_SAME_PRODUCT") {
                const level = cmp.verdict === "SAME_PRODUCT" ? "CONFIRMED" : "LIKELY";
...
export function assessBrandOwnership(input) {
    ...
    const genericDistributedTitle = /\b(handzeep|de luxe|euro)\b/i.test(titleLower) && titleCore(titleLower).length < 24;
    ...
    const externalBreadth = EXTERNAL_BRAND_HINTS.filter((b) => titleLower.includes(b)).length * 25;
    return {
...
export function deriveCurrentSiteImpression(currentVisualQualityScore) {
    const cvq = currentVisualQualityScore ?? 55;
    if (cvq < 45)
        return "CLEARLY_UNDERDESIGNED";
    if (cvq < 55)
        return "BASIC_BUT_ACCEPTABLE";
    if (cvq < 70)
        return "MODERN_ENOUGH";
    return "PREMIUM";
}
...
export function passesShowcaseSalesCandidate(input) {
    ...
    if (input.currentSiteImpression === "MODERN_ENOUGH" ||
...
export function withProvenance(value, inferredDefault = null) {
    if (value != null)
        return { value, provenance: "MEASURED" };
    if (inferredDefault != null)
        return { value: inferredDefault, provenance: "DEFAULT" };
    return { value: null, provenance: "UNKNOWN" };
}
//# sourceMappingURL=showcaseCandidateIntegrity.js.map