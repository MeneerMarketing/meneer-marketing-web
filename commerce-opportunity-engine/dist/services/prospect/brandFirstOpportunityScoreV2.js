...
export function computeBrandFirstOpportunityScoreV2(input) {
    const brandQuality = norm(norm(input.firstPartyConfidence) * 0.55 + norm(input.brandScaleFit) * 0.45);
    ...
    if (price != null) {
        if (price >= 150 && price <= 750)
            ...
            heroEconomics = 24;
        if (price > 3000)
            heroEconomics = Math.max(20, heroEconomics - 12);
    }
    ...
    const presentationGap = Math.max(0, Math.min(100, Math.round(visualGap * 0.42 + purchaseGap * 0.33 + mobileGap * 0.2 - presentation * 0.12)));
    const score = Math.round(brandQuality * 0.13 +
    ...
    if (businessHigh && gapHigh && !presentationHigh)
        sweetSpotProfile = "IDEAL";
    else if (businessHigh && presentationHigh)
        sweetSpotProfile = "STRONG_BUSINESS_WEAK_GAP";
    else if (!businessHigh && gapHigh)
        sweetSpotProfile = "STRONG_GAP_WEAK_BUSINESS";
    return {
...
export function manualReviewVerdict(input) {
    if ((input.opportunityScoreV2 ?? 0) < 52)
        return "NO_TARGET";
    if (input.sweetSpotProfile === "STRONG_BUSINESS_WEAK_GAP")
        return "NO_TARGET";
...
//# sourceMappingURL=brandFirstOpportunityScoreV2.js.map