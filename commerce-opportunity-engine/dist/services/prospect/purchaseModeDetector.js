...
export function detectPurchaseMode(input) {
    ...
    const hasAddToCart = /add-to-cart|add_to_cart|winkelwagen|in winkelwagen|koop nu|buy now|cart\.add|product-form/i.test(textSample);
    const hasConfigurator = /configurator|configure|customize|op maat|maatwerk|build your|samengesteld/i.test(textSample);
    const hasLeadGen = /offerte aanvragen|request quote|contact us|neem contact|bel voor|plan een|book a call|schedule/i.test(textSample);
    const hasShowroom = /showroom|bezoek onze winkel|visit our store|afspraak maken|dealer locator|showroom appointment/i.test(textSample);
    if (hasConfigurator)
        evidence.push("configurator_language");
    if (hasLeadGen)
        evidence.push("lead_generation_language");
    if (hasShowroom)
        evidence.push("showroom_language");
    if (hasAddToCart)
        evidence.push("add_to_cart_present");
    if (hasPrice)
        evidence.push("hero_price_present");
...
export function purchaseModeScore(mode) {
    switch (mode) {
...
//# sourceMappingURL=purchaseModeDetector.js.map