...
export function computeBusinessEconomicFit(input) {
    ...
    if (input.paidAcquisitionLevel === "CONFIRMED")
        ...
        paidBonus = 3;
    const score = clamp(brandQuality * 0.22 +
        ...
        purchaseFit * 0.06 +
        paidBonus);
    if (catalogBand.label === "sweet_spot")
        evidence.push("catalog_sweet_spot");
    if (ownBrand >= 65)
        evidence.push("own_brand_strong");
...
export function computeGapFirstSalesPotential(input) {
    ...
    const allSufficient = gap >= 58 && material >= 55 && business >= 58 && economics >= 55;
    const weakBusiness = business < 45 || economics < 40;
    const strongBusinessWeakGap = gap < 45 && business >= 70;
    let profile = "MIXED";
    if (allSufficient)
        profile = "IDEAL_GAP_FIRST";
    else if (strongBusinessWeakGap)
        profile = "STRONG_BUSINESS_WEAK_GAP";
    else if (gap >= 65 && weakBusiness)
        profile = "STRONG_GAP_WEAK_BUSINESS";
    let confidence = "LOW";
    if (allSufficient && score >= 72)
        confidence = "HIGH";
    else if (score >= 58 && gap >= 55)
        confidence = "MEDIUM";
    return { score, confidence, profile };
}
export function manualReviewVerdictM98(input) {
    if (input.profile === "STRONG_BUSINESS_WEAK_GAP")
        return "NO_TARGET";
    if (input.purchaseMode === "LEAD_GENERATION" || input.purchaseMode === "SHOWROOM_ASSISTED") {
        return "NO_TARGET";
    }
    if (input.rawPdpRedesignOpportunity < 52)
        return "NO_TARGET";
    if (input.materialQualityScore < 40)
        return "NO_TARGET";
    if (input.gapFirstSalesPotential >= 62 && input.rawPdpRedesignOpportunity >= 58) {
        return "TRUE_MANUAL_REVIEW_CANDIDATE";
    }
    if (input.gapFirstSalesPotential >= 54)
        return "PROMISING";
    return "NO_TARGET";
}
//# sourceMappingURL=gapFirstSalesPotential.js.map