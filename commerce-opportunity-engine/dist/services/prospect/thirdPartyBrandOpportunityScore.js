...
export function computeThirdPartyBrandOpportunityScore(input) {
    const marketPresence = norm(input.brandMarketPresenceScore);
    const firstParty = norm((norm(input.firstPartyConfidence) * 0.6 + norm(input.officialDomainConfidence) * 0.4));
    ...
    const presentationGap = Math.max(0, Math.min(100, Math.round(visualGap * 0.42 + purchaseGap * 0.33 + mobileGap * 0.2 - presentation * 0.1)));
    const independentBoost = Math.min(10, input.independentSourceCount * 2);
    const score = Math.round(marketPresence * 0.1 +
        firstParty * 0.12 +
        scaleFit * 0.08 +
        catalogFocus * 0.06 +
        ownBrand * 0.07 +
    ...
    let confidence = "LOW";
    if (firstParty >= 70 && input.independentSourceCount >= 2)
        confidence = "MEDIUM";
    if (firstParty >= 75 &&
        presentationGap >= 50 &&
...
export function manualReviewVerdictM97(input) {
    ...
    if (visualBand === "LOW" && purchaseBand === "LOW")
        return "NO_TARGET";
    return "TRUE_MANUAL_REVIEW_CANDIDATE";
}
//# sourceMappingURL=thirdPartyBrandOpportunityScore.js.map