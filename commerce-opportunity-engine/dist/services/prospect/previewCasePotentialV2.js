...
function norm(value, fallback = 48) {
    if (value == null || !Number.isFinite(value))
        return fallback;
    return clamp(value);
}
function confidenceFromVerified(verified, score) {
    if (!verified || score == null)
        return "UNKNOWN";
    return score >= 70 ? "HIGH" : score >= 50 ? "MEDIUM" : "LOW";
}
export function computePreviewCasePotentialV2(input) {
    ...
    const catalogConf = input.catalogVerified
        ...
        : "UNKNOWN";
    const ownBrandConf = input.ownBrandSignal != null
        ? input.ownBrandSignal >= 65
            ...
            : input.ownBrandSignal >= 45
                ...
                : "LOW"
        : "UNKNOWN";
    const maturityConf = input.businessMaturity != null
        ? input.businessMaturity >= 60
            ...
            : input.businessMaturity >= 40
                ...
                : "LOW"
        : "UNKNOWN";
    const catalogSizeScore = input.estimatedCatalogSize != null
        ? input.estimatedCatalogSize <= 50
            ...
            : input.estimatedCatalogSize <= 100
                ...
                : input.estimatedCatalogSize <= 200
                    ...
                    : 28
        : 50;
    const economicFit = clamp(norm(input.highTicketFocusedFit, 50) * 0.32 +
    ...
    if (input.heroPrice != null) {
        if (input.heroPrice >= 500)
            ...
            heroEconomics = 78;
        else if (input.heroPrice >= 80)
            heroEconomics = 65;
        else
            heroEconomics = 38;
    }
    const adsBoost = input.adKeywordCount >= 2 ? 8 : input.adKeywordCount >= 1 ? 4 : 0;
    const preview = clamp(gap * 0.38 +
        visual * 0.14 +
        purchase * 0.1 +
        economicFit * 0.28 +
        heroEconomics * 0.06 +
        adsBoost);
    const unknownCount = [catalogConf, ownBrandConf, maturityConf, input.heroPriceConfidence].filter((entry) => entry === "UNKNOWN").length;
    const overallConfidence = unknownCount >= 3 ? "LOW" : unknownCount >= 2 ? "MEDIUM" : unknownCount === 1 ? "MEDIUM" : "HIGH";
    return {
...
//# sourceMappingURL=previewCasePotentialV2.js.map