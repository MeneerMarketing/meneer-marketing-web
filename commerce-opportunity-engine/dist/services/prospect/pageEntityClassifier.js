...
const CATEGORY_PATH = /\/product-categorie\/|\/product-categor|\/categor(?:y|ie)\/|\/shop\/[a-z0-9-]+$/i;
const COLLECTION_PATH = /\/collections?\/|\/collectie\//i;
const BRAND_OVERVIEW_PATH = /\/merken\/|\/brands\/|\/brand\//i;
const LISTING_PATH = /\/koopgids\/|\/gids\/|\/overzicht\/|\/producten\/[^/]+\/[^/]+\/?$/i;
...
const PLURAL_CATEGORY_TITLES = /^(waterflossers|tandenborstels|mondverzorging|elektrische tandenborstels|sonicare|compressie|recovery|massage guns?|waterflosser kopen)/i;
...
function countProductCardSignals(html) {
...
export function classifyPageEntityFromUrl(productUrl, domain) {
    const evidence = [];
    try {
...
export function classifyPageEntity(input) {
    ...
    const hasSingleProductEvidence = isUsableHeroUrl(input.productUrl, input.domain) ||
...
//# sourceMappingURL=pageEntityClassifier.js.map