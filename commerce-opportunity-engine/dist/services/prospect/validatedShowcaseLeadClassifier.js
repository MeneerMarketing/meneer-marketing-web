 ...
 */
const OWNERSHIP_SALES_MODELS = [
    "DTC_OWN_BRAND",
    "MOSTLY_OWN_BRAND",
    "MANUFACTURER_DTC",
    "FOCUSED_PRIVATE_LABEL_BRAND",
];
function ownershipSalesOk(model, confidence) {
    if (model === "GENERAL_RETAILER" || model === "GENERAL_RESELLER")
        return false;
    if (model === "FOCUSED_SPECIALIST_RESELLER")
        return false;
    if (OWNERSHIP_SALES_MODELS.includes(model))
...
export function classifyM994LeadType(input) {
    ...
    if (input.showcaseIntegrityPass &&
...
//# sourceMappingURL=validatedShowcaseLeadClassifier.js.map