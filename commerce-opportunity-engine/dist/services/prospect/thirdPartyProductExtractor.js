...
export function isPlausibleMinedBrand(name) {
    if (!name || name.length < 3)
        return false;
    const lower = name.toLowerCase().replace(/[^a-z0-9]/g, "");
    if (GENERIC_BRAND_BLOCKLIST.has(lower))
        return false;
    if (lower.length < 3)
        return false;
    return true;
...
export function extractThirdPartyProductEntity(input) {
    ...
    const schemaBrand = typeof raw.brand === "string"
        ? raw.brand
        : typeof raw.product?.brand === "string"
            ? raw.product.brand
            : null;
    ...
    let confidence = 45;
    if (evidence.includes("schema_brand"))
        confidence += 25;
    if (evidence.includes("title_brand_pattern"))
        confidence += 18;
...
//# sourceMappingURL=thirdPartyProductExtractor.js.map