 ...
 */
export function classifyMaterialQuality(input) {
...
export function assessBeforeAfterObvious(input) {
    const cvq = input.currentVisualQualityScore;
    const impression = input.currentSiteImpression ?? "";
    const material = input.materialFeasibility ?? 0;
    if (!input.visionScoreAllowed || cvq == null)
        return "MAYBE";
    if (cvq >= 58 && impression !== "CLEARLY_UNDERDESIGNED")
        return "NO";
    if (cvq >= 55 && impression === "BASIC_BUT_ACCEPTABLE")
        return "MAYBE";
    if (cvq <= 40 && impression === "CLEARLY_UNDERDESIGNED" && material >= 80) {
        return "YES_STRONG";
    }
    if (cvq <= 45 && impression === "CLEARLY_UNDERDESIGNED")
        return "YES";
    if (cvq <= 52 && impression === "CLEARLY_UNDERDESIGNED")
        return "YES";
    if (impression === "BASIC_BUT_ACCEPTABLE")
        return "MAYBE";
    return "MAYBE";
}
export function synthesizeHomepageReview(input) {
    ...
    if (scale >= 70)
        parts.push("Schaal oogt professioneel genoeg om serieus te benaderen.");
    else if (scale >= 45)
        parts.push("Middelgrote shop, professioneel maar niet corporate.");
    else
        parts.push("Kleinere operator; check of budget en urgentie passen.");
    if (cvq != null && cvq < 45)
        parts.push("Visueel niveau homepage past bij een zwakkere PDP.");
    return parts.join(" ");
}
export function synthesizePdpReview(input) {
    const look = input.manualRationale?.currentLook ?? "";
    const template = /template|shopify|woocommerce|generic|standard|plain|basic/i.test(look)
        ? "Standaard template / theme-layout"
        : "Custom elementen, maar nog niet premium";
    const gallery = /gallery|thumbnail|single angle|plain product image/i.test(look)
        ? "Basic gallery, weinig zoom/lifestyle of editorial diepte"
        : "Functionele productbeelden, weinig premium presentatie";
    const typography = /typography|hierarchy|sans-serif|utilitarian/i.test(look)
        ? "Zwakke typografie-hiërarchie, generieke fonts"
        : "Typografie functioneel, niet distinctive";
    const buyblock = /buyblock|cta|yellow button|option selectors/i.test(look)
...
export function listRedesignChanges(input) {
    const changes = [];
    const look = input.manualRationale?.currentLook ?? "";
...
export function assessBusinessRisk(input) {
...
export function classifyMaterialBreakdown(input) {
    const assets = input.assetQuality ?? 0;
    const content = input.contentAvailable ?? 0;
    return {
        productPhotography: assets >= 75 ? "Productfoto's bruikbaar als basis" : "Productfoto's basic, lifestyle ontbreekt",
        lifestylePhotography: assets >= 80 ? "Enige lifestyle potentie" : "Weinig lifestyle beelden zichtbaar",
        ...
        reviewsProof: (input.purchaseGap ?? 0) >= 55 ? "Proof/trust kan visueel sterker" : "Reviews/proof niet prominent",
        technicalContent: content >= 70 ? "Technische content bruikbaar" : "Specs basic",
    };
}
export function assignFinalClassification(candidates) {
    const results = [];
...
export function visualFitScoreForRanking(input) {
    const before = assessBeforeAfterObvious(input);
    const beforeWeight = before === "YES_STRONG" ? 40 : before === "YES" ? 28 : before === "MAYBE" ? 12 : 0;
...
//# sourceMappingURL=humanShowcaseReview.js.map