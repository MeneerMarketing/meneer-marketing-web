 ...
 */
import { PREVIEW_CASE_WEIGHTS, PREAUDIT_GATE_THRESHOLDS, } from "../../config/designGapProspect.js";
function clamp(n) {
    return Math.max(0, Math.min(100, Math.round(n)));
}
function norm(value, fallback = 45) {
    if (value == null || !Number.isFinite(value))
        return fallback;
    return clamp(value);
}
export function computePreviewCasePotential(input) {
    const evidence = [];
    const businessStrength = clamp((norm(input.businessMaturity, 50) * 0.45 +
        norm(input.ownBrandSignal, 50) * 0.35 +
        norm(input.highTicketFocusedFit, 50) * 0.2));
    let productEconomics = 45;
    if (input.heroPrice != null) {
        if (input.heroPrice >= 500)
            productEconomics = 100;
        else if (input.heroPrice >= 150)
            productEconomics = 92;
        else if (input.heroPrice >= 100)
            productEconomics = 82;
        else if (input.heroPrice >= 60)
            productEconomics = 62;
        else
            productEconomics = 38;
        evidence.push(`hero_price_${input.heroPrice}`);
    }
    const assetContent = clamp((norm(input.assetReadinessProxy) * 0.55 + norm(input.contentAvailableScore) * 0.45));
    const presentationDrag = input.contentPresentationQuality != null && input.contentPresentationQuality >= 72
        ? 12
        : input.contentPresentationQuality != null && input.contentPresentationQuality >= 60
            ? 6
            : 0;
    const raw = businessStrength * PREVIEW_CASE_WEIGHTS.businessStrength +
    ...
    if (presentationDrag > 0)
        evidence.push(`presentation_drag_${presentationDrag}`);
    const gateFailures = [];
...
//# sourceMappingURL=previewCasePotential.js.map