 ...
 */
export async function buildRetailerNameTokens(client) {
    const tokens = new Set();
    const addDomainTokens = (domain) => {
        const base = domain
            .toLowerCase()
            .replace(/^www\./, "")
    ...
    for (const row of data ?? []) {
        if (row.normalized_domain)
            addDomainTokens(String(row.normalized_domain));
...
export async function buildProductBrandTokens(client) {
    const tokens = new Set([
        "cerave",
        "la roche posay",
        "la roche-posay",
...
export function findMatchingToken(keywordNormalized, tokens) {
    ...
    for (const token of sorted) {
        if (!token || token.length < 2)
            continue;
        if (token.includes(" ")) {
            if (keywordNormalized.includes(token))
                return token;
        }
        else {
            const re = new RegExp(`(?:^|\\s)${escapeRegExp(token)}(?:\\s|$)`);
            if (re.test(keywordNormalized))
                return token;
        }
    }
    return null;
...
//# sourceMappingURL=retailerNameDetector.js.map