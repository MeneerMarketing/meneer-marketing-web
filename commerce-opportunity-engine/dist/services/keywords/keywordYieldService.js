 ...
 */
export async function computeKeywordYield(client, keywordId, serpCostHint) {
    ...
    for (const row of ads ?? []) {
        ...
        if (!brand?.id)
            continue;
        const confirmedFlag = row.ad_signal_type === "CONFIRMED_PAID" ||
        ...
        if (!brandMap.has(brand.id)) {
            brandMap.set(brand.id, {
                ...
                firstSeenAt: brand.first_seen_at ?? null,
    ...
    for (const opp of opps ?? []) {
        ...
        if (auditType === "HIGH_CONFIDENCE_PRODUCT_TARGET")
            highConfidence += 1;
        if (auditType === "EXACT_PAID_FUNNEL")
            exactPaid += 1;
    }
    let retailers = 0;
    let comparison = 0;
    ...
    for (const brand of brandMap.values()) {
        ...
        if (type === "GENERAL_RETAILER")
            retailers += 1;
        if (type === "COMPARISON_SITE")
            comparison += 1;
        if (type === "MARKETPLACE")
            marketplaces += 1;
        if (brand.leadEligible)
            eligible += 1;
    ...
    const retailerRatio = uniqueDomains > 0 ? Number((retailerHeavy / uniqueDomains).toFixed(4)) : null;
    ...
    if (serpCost != null && serpCost > 0 && uniqueDomains >= 3) {
        ...
        efficiency = Math.max(0, Math.min(100, Math.round(40 + good * 15 - waste * 8 - serpCost * 200)));
    }
    return {
        serpCost,
...
//# sourceMappingURL=keywordYieldService.js.map