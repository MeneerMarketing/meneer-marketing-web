import { PRIMARY_KEYWORD_WEIGHTS, SOURCE_QUALITY_SCORE_CAPS, SOURCE_TYPE_SCORE_CAPS, } from "../../config/sourceIntegrityWeights.js";
...
export function scoreKeywordCandidate(input) {
    ...
    let score = 0;
    if (input.isPrimaryAd)
        score += w.linkedToPrimaryAd;
    if (input.ad.adSignalType === "CONFIRMED_PAID")
        score += w.confirmedPaidSignal;
    else if (input.ad.adSignalType === "PAID_CANDIDATE")
        score += w.paidCandidateSignal;
    if (input.ad.landingUrl)
        score += w.landingUrlPresent;
    ...
    if (conf != null)
        score += Math.min(w.productResolutionConfidence, conf * w.productResolutionConfidence);
    return Math.round(Math.min(100, score));
}
export function selectPrimaryKeyword(input) {
    const byKeyword = new Map();
    for (const ad of input.ads) {
        if (!ad.keywordId || !ad.keyword)
            continue;
    ...
    if (winner.score >= 70)
        reasons.push("strong_product_or_headline_overlap");
    else if (winner.score >= 45)
        reasons.push("moderate_relevance");
    else
        reasons.push("weak_relevance_best_available");
    return {
...
export function classifySourceType(ad) {
    ...
    if (conf.includes("labs") || conf.includes("ranked_keywords"))
        return "LABS_PAID_KEYWORD";
    ...
    if (conf.includes("transparency"))
        return "TRANSPARENCY_CONFIRMED";
    if (ad.adSignalType === "PAID_CANDIDATE")
        return "PAID_CANDIDATE";
    if (ad.adSignalType === "CONFIRMED_PAID")
        return "TRANSPARENCY_CONFIRMED";
    return "UNKNOWN";
}
export function computeSourceQuality(input) {
    ...
    if (hasLanding &&
        ...
        if (keywordProductMatch >= 0.4 || adProductMatch >= 0.4)
            score += 5;
    }
    ...
    const brokenAssociation = Boolean(ad?.headline) &&
...
export function applySourceQualityCap(opportunityScore, sourceQualityScore, sourceType) {
...
export function explainOpportunityScore(components, penalty, weightOverrides) {
...
export function validateConversionLeaks(input) {
    ...
    return input.leaks.map((leak) => {
        ...
        if (titleLower.includes("mismatch") ||
            titleLower.includes("ad-landing") ||
            titleLower.includes("ad →") ||
            titleLower.includes("wrong product")) {
            const mismatch = overlapRatio(tokenize(headline), tokenize(product)) < 0.2 &&
                Boolean(headline) &&
                Boolean(product);
            return {
                title,
                status: (mismatch ? "SUPPORTED" : "QUESTIONABLE"),
                reason: mismatch
...
//# sourceMappingURL=sourceIntegrity.js.map