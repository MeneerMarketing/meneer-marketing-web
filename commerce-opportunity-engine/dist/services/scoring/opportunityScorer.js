import { EXACT_PAID_FUNNEL_WEIGHTS, HIGH_CONFIDENCE_PRODUCT_TARGET_WEIGHTS, HIGH_CONFIDENCE_SCORE_CAP, KEYWORD_INTENT_PENALTY, OPPORTUNITY_VERDICT_BANDS, RETAILER_SCALE_PENALTY, } from "../../config/scoringWeights.js";
...
function scoreExactPaidFunnel(input) {
    const scores = input.ai.scores;
    const croGap = averageGap(croQualityParts(scores));
    const adLanding = scores.ad_landing_match_quality == null ? 50 : scores.ad_landing_match_quality;
    ...
    const productCommercialSignal = scoreProductCommercial(input.productPrice, input.reviewCount, input.hasProductPage);
    ...
    const weighted = paidAcquisitionStrength * weights.paidAcquisitionStrength +
    ...
    const capped = applySourceQualityCap(uncapped, input.sourceQualityScore ?? 100, input.sourceType);
    return {
        ...
        formula: "EXACT_PAID: paid15 + maturity15 + croGap20 + adGap15 + platform10 + rebuild10 + product5 + designTrust5 + source5 − retailerScalePenalty − keywordIntentPenalty, then sourceQualityCap",
        uncappedScore: uncapped,
        sourceQualityCap: capped.capApplied,
        formulaLines: explained.lines,
    };
}
function scoreHighConfidence(input) {
    const scores = input.ai.scores;
    ...
    const confirmedPaidBrandStrength = input.confirmedGoogleAdvertiser || input.confirmedPaid ? 92 : 40;
    const businessMaturity = clamp(input.businessMaturityScore ?? 35);
    const rebuildPotential = clamp(input.ai.custom_shopify_rebuild_potential);
    const platformFit = scorePlatform(input.platform, input.platformCandidate);
    const productCommercialSignal = scoreProductCommercial(input.productPrice, input.reviewCount, input.hasProductPage);
    ...
    const weighted = confirmedPaidBrandStrength * weights.confirmedPaidBrandStrength +
        businessMaturity * weights.businessMaturity +
    ...
    const capped = applySourceQualityCap(uncappedRaw, input.sourceQualityScore ?? 100, input.sourceType);
...
export function computeAuditConfidence(input) {
    ...
    if (!input.screenshotOk)
        score -= 25;
    if (input.productResolutionConfidence == null ||
        input.productResolutionConfidence < 0.5) {
        score -= input.auditType === "EXACT_PAID_FUNNEL" ? 10 : 8;
    }
    if (input.representationSparse)
        score -= 15;
    if (input.blockedHints)
        score -= 30;
    return Math.round(clamp(score));
}
function croQualityParts(scores) {
    return [
...
function scorePaid(input) {
    if (input.confirmedPaid)
        return 95;
    if (input.paidSignalType === "CONFIRMED_PAID")
        return 90;
    if (input.paidSignalType === "PAID_CANDIDATE")
        return 55;
    return 20;
}
function scorePlatform(platform, candidate) {
    if (platform === "SHOPIFY")
        return 100;
...
function scoreProductCommercial(price, reviews, hasProductPage) {
    if (!hasProductPage)
        return 15;
    let score = 40;
    if (price != null) {
