...
import { runConceptContrastRegression, } from "../services/concept/conceptContrastRegression.js";
...
const DASHBOARD_REPORT_PATH = resolve(projectRoot, "dashboard/src/preview/concepts/data/concept-contrast-report.json");
export async function recomputeConceptContrast() {
    const env = loadEnv();
    ...
    for (const entry of entries) {
        ...
        const contrastBlocks = gate.blockedReasons.filter((reason) => reason.startsWith("insufficient_concept_contrast"));
        const otherBlocks = gate.blockedReasons.filter((reason) => !reason.startsWith("insufficient_concept_contrast"));
        ...
        rows.push({
            ...
            designTargetEligible: contrast.concept_contrast_potential >= CONTRAST_GATE_THRESHOLDS.minDesignTargetContrast,
...
async function main() {
    ...
    console.log(`  regressie ${report.regression.passed}/${report.regression.total} · drempel design target ${report.thresholds.designTarget}, outreach ${report.thresholds.outreach}`);
    for (const entry of report.regression.cases) {
        console.log(`    ${entry.passed ? "ok  " : "FOUT"} ${entry.label} → ${entry.score} ${entry.band}${entry.ceilingApplied ? ` (plafond: ${entry.ceilingApplied})` : ""}`);
    }
    console.log(`\n  ${report.scored} kandidaten gescoord, ${report.persisted} opgeslagen`);
    console.log(`  banden: ${Object.entries(report.bandCounts)
    ...
    if (report.newlyBlocked.length > 0) {
        ...
        for (const row of report.newlyBlocked) {
            console.log(`    ${row.domain} · contrast ${row.contrast} (${row.band})${row.ceiling ? ` · plafond: ${row.ceiling}` : ""} · sales fit ${row.salesFit}`);
        }
    }
    console.log("\n  Top contrast:");
    for (const row of report.rows.slice(0, 12)) {
        console.log(`    ${row.domain} · ${row.contrast} ${row.band} · ruimte ${row.roomScore} · materiaal ${row.capabilityScore} · PDP ${row.currentPdpQuality ?? "?"} · ${row.croDataSource}`);
    }
    console.log(`\nRapport: ${REPORT_PATH}\n`);
}
...
//# sourceMappingURL=recomputeConceptContrast.js.map