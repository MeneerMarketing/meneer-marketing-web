...
import { ENGINEERING_FIXTURE_DOMAINS, OUTREACH_FIT_WEIGHTS, OUTREACH_GATE_THRESHOLDS, OUTREACH_PENALTIES, } from "../config/outreachScoring.js";
...
async function captureScreenshots(baseUrl, conceptId, productUrl, outDir) {
    ...
    await shot(desktop, `${baseUrl}/preview/concept/${conceptId}`, "concept-desktop-1440.png", false);
    await shot(desktop, `${baseUrl}/preview/concept/${conceptId}`, "concept-desktop-full.png", true);
    try {
    ...
    await shot(mobile, `${baseUrl}/preview/concept/${conceptId}`, "concept-mobile-390.png", false);
    try {
...
async function main() {
    ...
    try {
        ...
        if (runnerUp[0]) {
            whyWins.push(`Beats #2 ${runnerUp[0].domain} by ${winnerBundle.outreach.outreachConceptFitScore - runnerUp[0].outreachScore} outreach points`);
        }
        ...
        let screenshotPaths = {};
        try {
            screenshotPaths = await captureScreenshots(previewBase, winner.id, hydration.productUrl, screenshotDir);
        }
        catch (e) {
        ...
        const report = {
            ...
            tensfactComparison: tensfact
                ? {
                    ...
                    outreachRank: ranked.findIndex((r) => r.domain === "tensfact.com") + 1,
                    totalPool: ranked.length,
                    whyDifference: "Outreach mode weights transformation + current PDP weakness and penalizes cro_already_strong; engineering mode favors brief quality + assets.",
            ...
            winner: {
                ...
                runnerUp,
                visualContrastNote: "Review current vs concept screenshots manually. Mark INSUFFICIENT_TRANSFORMATION_CONTRAST if pitch is weak.",
            },
            hydration: {
        ...
        const reportPath = path.resolve(projectRoot, "dashboard/src/preview/concepts/data/outreach-pilot-report.json");
        ...
        for (const r of top10.slice(0, 3)) {
            console.log(`${r.outreachConceptFit}\teng=${r.engineeringScore}\t${r.domain}\t${r.status}`);
        }
        console.log("\nPreview:", hydration.previewPath);
...
//# sourceMappingURL=selectOutreachPilot.js.map