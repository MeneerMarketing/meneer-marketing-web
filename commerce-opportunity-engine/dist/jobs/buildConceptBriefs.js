...
import { BRIEF_READY_MIN_ASSET_SCORE, BRIEF_READY_MIN_CONCEPT_SCORE, CONCEPT_TEMPLATE_REGISTRY, } from "../config/conceptScoring.js";
...
import { generateConceptBrief, suggestTemplateFamily, } from "../services/concept/conceptBriefGenerator.js";
    ...
    try {
        ...
        for (const c of candidates) {
            ...
            const { data: brand } = await supabase
                .from("brands")
                .select("id, name, domain, normalized_domain, platform, business_type, do_not_contact, manual_excluded, eligibility_status, lead_eligible")
                .eq("id", c.brand_id)
                .maybeSingle();
            if (!brand)
                continue;
            ...
            if (c.page_id) {
                const { data } = await supabase
                    .from("pages")
                    .select("product_description, review_count, rating, currency, extracted_data, url")
                    .eq("id", c.page_id)
                    .maybeSingle();
                page = data;
            }
            ...
            const family = (c.manual_template_family ||
                ...
                suggestTemplateFamily([brand.normalized_domain ?? ""], c.brand_commerce_model ?? "UNKNOWN"));
            const registry = CONCEPT_TEMPLATE_REGISTRY.find((t) => t.template_family === family);
            ...
            const strengthLabels = Array.isArray(audit?.strengths)
                ...
                : [];
            const missingAssets = Object.entries((c.asset_readiness_components ?? {}))
...
//# sourceMappingURL=buildConceptBriefs.js.map