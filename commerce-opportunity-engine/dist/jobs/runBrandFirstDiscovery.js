...
import { BRAND_FIRST_HIGH_TICKET_PROFILE_VERSION, BRAND_FIRST_PRODUCT_FAMILIES, M96_DISCOVERY, M96_DISCOVERY_VERSION, ORGANIC_QUERY_REJECT, } from "../config/brandFirstHighTicket.js";
...
import { evaluateBrandFirstEarlyGate, passesBrandFirstEconomicQualified, } from "../services/prospect/brandFirstEarlyGate.js";
import { computeBrandFirstOpportunityScore } from "../services/prospect/brandFirstOpportunityScore.js";
import { paidAcquisitionBonus, validatePaidAcquisition, } from "../services/prospect/paidAcquisitionValidation.js";
...
const DASHBOARD_REPORT_PATH = resolve(projectRoot, "dashboard/src/preview/concepts/data/brand-first-discovery-report.json");
...
export async function runBrandFirstDiscovery(options) {
    const env = loadEnv();
    const supabase = createSupabaseServerClient(env);
    const dataForSeo = createDataForSeoClient(env);
    const dfsBudget = { spent: 0, cap: env.M96_MAX_DATAFORSEO_COST };
    const anthropicBudget = { spent: 0, cap: env.M96_MAX_ANTHROPIC_COST };
    ...
    while (queries.length < M96_DISCOVERY.maxOrganicQueries) {
        ...
        for (const row of familyQueryRows) {
            ...
            added = true;
            if (queries.length >= M96_DISCOVERY.maxOrganicQueries)
                break;
        }
        if (!added)
            break;
    ...
    for (const entry of queries) {
        ...
        if (familyCount >= M96_DISCOVERY.maxBrandsPerFamily)
            continue;
        if (discovered.size >= M96_DISCOVERY.maxBrandCandidates)
            break;
        if (!canSpend(dfsBudget, M96_DISCOVERY.estimatedSerpCostPerKeyword))
            break;
        ...
        for (const organic of serp.organicResults) {
            organicRows += 1;
            if (isBlacklistedDomain(organic.normalizedDomain))
                continue;
            ...
            if (!officialDomain || extraction.entityRole !== "OFFICIAL_BRAND_DOMAIN") {
                if (extraction.merchantDomain)
                    retailerExtractions += 1;
                continue;
            }
            if (isBlacklistedDomain(officialDomain))
                continue;
    ...
    for (const candidate of gapPool) {
        ...
        if (visionScreens < M96_DISCOVERY.maxVisionScreens &&
    ...
    for (const candidate of gapPool) {
        if (!candidate.designGapScreened)
            continue;
    ...
    for (const candidate of paidPool) {
        if (!canSpend(dfsBudget, M96_DISCOVERY.estimatedSerpCostPerKeyword * 2))
            break;
        const paid = await validatePaidAcquisition({
            domain: candidate.domain,
            brandName: candidate.brandName,
            productKeywords: [candidate.sourceQuery, candidate.heroTarget.heroTitle ?? candidate.sourceQuery].filter(Boolean),
            serpOptions,
    ...
    const report = {
        ...
        funnel: {
            ...
            first_party_passed: [...discovered.values()].filter((c) => c.firstPartyConfidence >= M96_DISCOVERY.firstPartyMinConfidence).length,
...
function printReport(report) {
    ...
    for (const c of report.top10 ?? []) {
        ...
        console.log(`     first-party ${c.firstPartyConfidence} · scale ${c.brandScaleFit} · visual ${c.preauditVisualGap} (${c.visualGapBand}) · paid ${c.paidAcquisition}`);
...
//# sourceMappingURL=runBrandFirstDiscovery.js.map