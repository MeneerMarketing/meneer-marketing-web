...
async function main() {
    ...
    const { data: brand } = await supabase
        .from("brands")
        .select("id, name, normalized_domain, confirmed_google_advertiser, preferred_contact_id")
        .eq("normalized_domain", "huisdierspullen.nl")
        .single();
    if (!brand)
        throw new Error("huisdierspullen brand missing");
    ...
    const allowedClaims = buildAllowedClaims({
        ...
        supportedOnly: false,
    }).map((c) => c.scope === "MULTI_PAGE" || c.scope === "SITE_WIDE"
        ? c
        : { ...c, scope: "PAGE_SPECIFIC" });
    const priceFirst = [
        ...allowedClaims.filter((c) => c.type === "OBSERVATION" && /price|prijs/i.test(c.subject)),
        ...allowedClaims.filter((c) => !(c.type === "OBSERVATION" && /price|prijs/i.test(c.subject))),
    ];
    ...
    try {
        ...
        if (anthropicCost > costCap + 1e-9) {
            throw new Error(`HARD CAP VIOLATION: anthropicCost $${anthropicCost} > cap $${costCap}`);
        }
        ...
        await syncOutreachMessagesForBrand(supabase, String(brand.id), "m8.2 copy fixture harvest-excluded");
        ...
        console.log(`  Old overrun cause: post-call accounting; headroom checked after spend started / estimate too low for 2 full-mail calls`);
        console.log(`  Gate demo under cap: allowed=${gateDemoAllowed.allowed}`);
        console.log(`  Gate demo over cap: allowed=${gateDemoBlocked.allowed} status=${!gateDemoBlocked.allowed ? gateDemoBlocked.status : "n/a"}`);
        console.log(`  Conservative next-call estimate: $${CONSERVATIVE_PERSONALISATION_CALL_COST}`);
        console.log("");
        for (const r of results) {
            console.log(`--- ${r.variant} · mode=${r.mode} · ${r.wordCount}w · ${r.validation} ---`);
            console.log(`subject: ${r.subject}`);
            console.log(`claims: ${r.claimsUsed.join(", ")}`);
            console.log(`notes: ${r.notes.join("; ")}`);
            if (r.budgetBlocked)
                console.log(`BUDGET_BLOCKED: ${r.budgetBlockReason}`);
            console.log("");
        ...
        console.log("  Deterministic is simpler/more consistent for first-touch; AI only tweaks observation/strength wording.");
        console.log("STOP — geen Resend-send, geen prospectmail.");
        if (provider.status !== "READY") {
            console.log("");
            console.log("NEXT: zet RESEND_API_KEY, RESEND_FROM_EMAIL, OUTREACH_TEST_EMAIL in .env voor test-send.");
        }
        process.exit(0);
    }
    catch (err) {
...
async function insertMessage(supabase, input) {
    ...
    const status = input.result.validation.status === "PASSED"
        ? "READY_FOR_REVIEW"
        : "DRAFT_INVALID";
    const { data, error } = await supabase
        ...
        selected_strength_title: input.result.strengthClaim?.source_title ?? null,
...
main();
//# sourceMappingURL=humanFirstHuisdierspullenM82.js.map