...
import { M951_DISCOVERY_VERSION, M951_WIDE_SCREEN, gapScoreBand, } from "../config/designGapWideScreen.js";
import { BRAND_FIRST_DISCOVERY_VERSION, BRAND_FIRST_PIPELINE_STAGES, BRAND_FIRST_SOURCE_ADAPTERS, BRAND_FIRST_PARKED, BRAND_FIRST_LEAD_PROFILE, } from "../config/brandFirstDiscovery.js";
...
import { evaluateHardExclusion, passesOldEconomicPrescreen, softWideScreenRank, } from "../services/prospect/wideScreenSelection.js";
...
const DASHBOARD_REPORT_PATH = resolve(projectRoot, "dashboard/src/preview/concepts/data/design-gap-wide-screen-report.json");
...
export async function runDesignGapWideScreen() {
    const env = loadEnv();
    const supabase = createSupabaseServerClient(env);
    const anthropicBudget = { spent: 0, cap: env.M951_MAX_ANTHROPIC_COST };
    const startedAt = new Date().toISOString();
    const m95Raw = await readFile(M95_REPORT_PATH, "utf8");
    const m95Report = JSON.parse(m95Raw);
    const sourceRunId = m95Report.runId;
    if (!sourceRunId)
        throw new Error("M9.5 report missing runId");
    ...
    for (const entry of prospectEligible) {
        ...
        try {
            const catalog = await runCatalogFocusCheck(entry.domain, crawlTimeout, homepageProductLinks, homepageCategoryLinks);
        ...
        let heroSource = hero?.source ?? "unknown";
        ...
        let visualGap = null;
        let purchaseGap = null;
        let mobileGap = null;
        let contentAvailable = null;
        let contentPresentation = null;
        let rawGap = null;
        ...
        if (!hard.excluded && heroUrl) {
            ...
            if (crawl.status === "success" && crawl.html.length > 200) {
                ...
                const weakness = computeCurrentPdpWeaknessProxy(pdpWeaknessSignalsFromHtml(html, heroUrl, platform));
                ...
                mobileGap = Math.max(0, Math.min(100, Math.round(purchaseGap * 0.55 + 18 + (purchaseSignals.mobileAtcSignal ? 0 : 12))));
        ...
        enriched.push({
            ...
            ownBrand: ownBrand,
            ownBrandConfidence: ownBrand != null ? (ownBrand >= 65 ? "HIGH" : ownBrand >= 45 ? "MEDIUM" : "LOW") : "UNKNOWN",
            maturity: maturity,
    ...
    for (const candidate of softRanked) {
        ...
        if (visionScreens < M951_WIDE_SCREEN.maxVisionScreens &&
            ...
            if (candidate.preauditVisualGap != null) {
                candidate.preauditVisualGap = Math.max(0, Math.min(100, candidate.preauditVisualGap + vision.visualAdjustment));
            }
            if (candidate.preauditPurchaseGap != null) {
                candidate.preauditPurchaseGap = Math.max(0, Math.min(100, candidate.preauditPurchaseGap + vision.purchaseAdjustment));
            }
            if (candidate.mobileGap != null) {
                candidate.mobileGap = Math.max(0, Math.min(100, candidate.mobileGap + vision.mobileAdjustment));
            }
    ...
    const combinedSorted = [...screened].sort((a, b) => (b.previewCasePotentialV2 ?? 0) - (a.previewCasePotentialV2 ?? 0));
    ...
    const falseNegatives = screened
        .filter((c) => !c.oldEconomicPrescreenPass &&
    ...
    for (const candidate of combinedTop10) {
        if (candidate.screenshots?.["pdp-desktop-1440x1000"])
            continue;
        const heroUrl = candidate.heroTarget.heroProductUrl;
        if (!heroUrl)
            continue;
    ...
    const report = {
        ...
        dataConsistency: {
            ...
            assessment: consistency.cases.find((c) => c.domain === "vitalwave.nl")?.likelyCause ??
...
function explainOldGateMiss(candidate) {
    const reasons = [];
    if ((candidate.economicFit ?? 0) < 52)
        reasons.push("high_ticket_fit_below_52");
    if ((candidate.ownBrand ?? 0) < 42)
        reasons.push("own_brand_below_42");
    const price = candidate.heroTarget.heroPrice ?? 0;
    if (price > 0 && price < 60)
        reasons.push("hero_price_below_60");
    if (candidate.catalogEstimateConfidence === "UNKNOWN")
        reasons.push("catalog_unknown_not_fail_but_ranked_down");
    return reasons.join(" · ") || "old_gate_rules";
}
function printReport(report) {
    const funnel = report.funnel;
    console.log("\n=== DATA CONSISTENCY ===");
    console.log(`  ${report.dataConsistency.vitalwaveNote}`);
    ...
    for (const row of report.falseNegatives ?? []) {
        console.log(`  ${row.domain} · visual ${row.visualGap} (${row.visualGapBand}) · purchase ${row.purchaseGap} (${row.purchaseGapBand})`);
        console.log(`     miss: ${row.oldGateMissReason}`);
    }
    console.log("\n=== COMBINED TOP 10 ===");
    for (const c of report.combinedTop10 ?? []) {
        console.log(`\n  #${c.combinedRank} ${c.domain} · preview v2 ${c.previewCasePotentialV2} · raw gap ${c.rawDesignGapOpportunity}`);
        console.log(`     design rank #${c.designGapRank} · economic rank #${c.economicFitRank} · visual ${c.preauditVisualGap} (${c.visualGapBand})`);
    ...
    console.log("\n=== COST ===");
    console.log(`  DataForSEO $0 · Anthropic $${cost.anthropic.toFixed(4)} / $${cost.anthropicCap.toFixed(3)} (${cost.visionScreens} vision)`);
    console.log(`\nRapport: ${REPORT_PATH}\n`);
}
...
//# sourceMappingURL=runDesignGapWideScreen.js.map