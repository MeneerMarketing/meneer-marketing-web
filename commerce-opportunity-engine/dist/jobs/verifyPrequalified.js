...
import { categoryStatsToJson, computeCategoryUniqueStats, } from "../services/qualification/categoryStats.js";
import { computeIntelligenceCompleteness, rankPrequalifiedForTransparency, scoreConfirmedForTargetResolution, scoreTargetPriority, } from "../services/qualification/verificationRanking.js";
...
async function main() {
    ...
    const { data: scaleRun, error: scaleErr } = await supabase
        ...
        .maybeSingle();
    if (scaleErr)
        throw new Error(scaleErr.message);
    if (!scaleRun)
        throw new Error("No completed controlled_scale_runs found. Run scale:controlled first.");
    const keywordIds = scaleRun.selected_keyword_ids ?? [];
    if (!keywordIds.length)
        throw new Error("Latest controlled scale run has no selected_keyword_ids.");
    ...
    try {
        ...
        const { data: keywords, error: kwErr } = await supabase
            .from("keywords")
            .select(`id, keyword, category, cluster, prospecting_value_score, keyword_quality_score,
         ...
         unique_domains_found, retailer_ratio, prospecting_tier, keyword_intent_type`)
            .in("id", keywordIds);
        if (kwErr)
            throw new Error(kwErr.message);
        const relevanceExamples = [];
        let demoted = 0;
        for (const kw of keywords ?? []) {
            const relevance = scoreCategoryRelevance(kw.keyword, kw.category, kw.cluster);
            ...
            if (relevance.score < PAID_VERIFY_DEFAULTS.minRelevanceForPrimary &&
        ...
        console.log(`Category relevance: ${keywords?.length ?? 0} M7.2 keywords + ${allReclassified} others; demoted PRIMARY→LOW_VALUE: ${demoted}`);
        // ------------------------------------------------------------------
        // FASE A3 — Fix UNIQUE category stats
        // ------------------------------------------------------------------
        const categoryKeywordIds = {};
        for (const cat of CONTROLLED_SCALE_CATEGORIES) {
            categoryKeywordIds[cat] = (keywords ?? [])
                .filter((k) => k.category === cat)
                .map((k) => k.id);
        }
        const oldStats = (scaleRun.category_stats ?? {});
        const serpCostByCategory = {};
        for (const cat of CONTROLLED_SCALE_CATEGORIES) {
        ...
        report.faseA = {
            petsBugCause: "prequalified/specialists/shopify counted per ad_occurrence row; domainsFound used a Set",
            petsBefore: {
        ...
        for (const cat of CONTROLLED_SCALE_CATEGORIES) {
            ...
            console.log(`  ${cat}: domains=${m.uniqueDomains} ecommerce=${m.uniqueEcommerceDomains} brand/spec=${m.uniqueBrandSpecialistDomains} preq=${m.uniquePrequalifiedDomains} shopify=${m.uniqueShopifyDomains}`);
        }
        // ------------------------------------------------------------------
        ...
        for (const ad of ads ?? []) {
            ...
            if (!agg) {
                agg = {
                    ...
                    transparencyStatus: b.transparency_status ?? null,
                    transparencyCheckedAt: b.transparency_checked_at ?? null,
                    transparencyApiStatus: b.transparency_api_status ?? null,
                    manualExcluded: Boolean(b.manual_excluded),
                    lastCrawledAt: b.last_crawled_at ?? null,
                    paidTargetStatus: b.paid_target_status ?? null,
            ...
            if (kwRaw && typeof kwRaw === "object") {
                ...
                if (!agg.sourceKeywords.has(kid)) {
                    agg.sourceKeywords.set(kid, {
                        ...
                        prospecting: kw.prospecting_value_score ?? null,
                        relevance: kw.category_relevance_score ?? null,
                        intent: kw.keyword_intent_type ?? null,
                    });
                }
            }
        }
        const sourcesByBrand = new Map();
        const paidStatusByBrand = new Map();
        const rankInputs = [...brandMap.values()]
            .filter((b) => {
            const type = (b.businessType ?? "").toUpperCase();
            return (b.isEcommerce &&
        ...
        const needsTransparency = ranked.filter((b) => {
            if (b.confirmedAdvertiser)
                return false;
            if (b.transparencyStatus === "CONFIRMED")
                return false;
            if (b.transparencyStatus === "NOT_CONFIRMED" && b.transparencyCheckedAt) {
                const age = daysAgo(b.transparencyCheckedAt);
                if (age != null && age < PAID_VERIFY_DEFAULTS.confirmationCooldownDays)
                    return false;
            }
            // Successful unresolved (semantic) within cooldown — skip
            if (b.transparencyStatus === "NOT_RESOLVED" &&
                b.transparencyApiStatus === "SUCCESS" &&
                b.transparencyCheckedAt) {
                const age = daysAgo(b.transparencyCheckedAt);
                if (age != null && age < PAID_VERIFY_DEFAULTS.unresolvedCooldownDays)
                    return false;
            }
        ...
        for (const c of toCheck) {
            console.log(`  ${c.domain} preFit=${c.preFit} verifyScore=${c.verificationPriorityScore} shopify=${(c.platform ?? "").toUpperCase() === "SHOPIFY"} sources=${c.uniqueSourceKeywords}`);
        }
        // ------------------------------------------------------------------
        ...
        const checkOne = async (brand) => {
            ...
            try {
                const result = await checkGoogleAdsTransparency({ client: dfs, env }, brand.domain);
                ...
                const advertiserStatus = result.confirmedAdvertiser
                    ...
                    : result.evidenceStrength === "EXPLICIT_NEGATIVE"
                        ? "NOT_CONFIRMED"
                        : "NOT_RESOLVED";
                if (advertiserStatus === "CONFIRMED")
                    confirmed += 1;
                else if (advertiserStatus === "NOT_CONFIRMED")
                    notConfirmed += 1;
                else
                    notResolved += 1;
                const row = {
        ...
        const confirmedCandidates = ranked
            .map((r) => {
            const fresh = transparencyResults.find((t) => t.brandId === r.id && t.advertiserStatus === "CONFIRMED");
        ...
        for (const brand of targetPick) {
            ...
            try {
                ...
                const { data: opps } = await supabase
                    .from("opportunities")
                    .select("id, source_quality_score, cro_readiness_level, landing_url, ad_headline, ground_truth_source_type, keyword_id")
        ...
        for (const r of sortedForReport.slice(0, 10)) {
            const { data: brandFresh } = await supabase
                .from("brands")
                .select("transparency_status, confirmed_google_advertiser, confirmation_source, paid_target_status, platform, business_type, pre_fit_score, business_maturity_score, retailer_scale_score")
                .eq("id", r.id)
                .maybeSingle();
            const { data: topOpp } = await supabase
                .from("opportunities")
                .select("landing_url, source_quality_score, cro_readiness_level, target_priority_score, ad_headline")
                .eq("brand_id", r.id)
        ...
        // ------------------------------------------------------------------
        const oldFunnel = (scaleRun.funnel ?? {});
        ...
        prequalifiedList.sort((a, b) => (b.verificationPriority ?? 0) - (a.verificationPriority ?? 0));
        const updatedFunnel = {
            ...oldFunnel,
...
//# sourceMappingURL=verifyPrequalified.js.map