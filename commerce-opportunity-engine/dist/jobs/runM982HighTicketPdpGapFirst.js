...
import { buildM982HarvestQueries, buildM983CompletionQueries, inferExecutedQueriesFromCount, M982_DISCOVERY, M983_DISCOVERY, M983_DISCOVERY_VERSION, HAIR_SCALP_M983_STATUS, M982_DISCOVERY_ROUTE, M982_DISCOVERY_VERSION, HIGH_TICKET_GAP_FIRST_TARGET_V1, M982_PARKED_FAMILIES, M982_PRODUCT_QUERIES, } from "../config/highTicketPdpGapFirst.js";
...
import { serpPositionBand } from "../services/prospect/serpPositionBand.js";
...
import { computeBusinessEconomicFit, computeMaterialQualityScore, } from "../services/prospect/gapFirstSalesPotential.js";
import { classifyPageEntity, classifyPageEntityFromUrl } from "../services/prospect/pageEntityClassifier.js";
...
import { computeShowcaseGapPotential, isHighGapCandidate, } from "../services/prospect/showcaseGapPotential.js";
import { computeProductEconomicFit, computeHighTicketGapSalesFit, passesHighTicketFinalistGate, meetsEarlySuccessPartialCriteria, meetsEarlySuccessFinalistCriteria, isStrongFinalistForEarlyStop, evaluatePriceGate, inferPriceConfidence, } from "../services/prospect/highTicketGapSalesFit.js";
import { assessPriceConsistency } from "../services/prospect/priceConsistencyCheck.js";
import { classifyOpportunityTierLegacy, } from "../services/prospect/opportunityTierClassifier.js";
const scriptDir = dirname(fileURLToPath(import.meta.url));
const projectRoot = resolve(scriptDir, "../../");
config({ path: resolve(projectRoot, ".env"), quiet: true });
const M982_REPORT_PATH = resolve(projectRoot, "reports/high-ticket-pdp-gap-first-report.json");
const M983_REPORT_PATH = resolve(projectRoot, "reports/high-ticket-gap-completion-report.json");
const M982_DASHBOARD_REPORT_PATH = resolve(projectRoot, "dashboard/src/preview/concepts/data/high-ticket-pdp-gap-first-report.json");
const M983_DASHBOARD_REPORT_PATH = resolve(projectRoot, "dashboard/src/preview/concepts/data/high-ticket-gap-completion-report.json");
...
function screenPriority(input) {
    ...
    if (input.serpPositionBand === "21_50")
        score += 14;
    else if (input.serpPositionBand === "11_20")
        score += 10;
    else if (input.serpPositionBand === "51_PLUS")
        score += 8;
...
export async function runM982HighTicketPdpGapFirst(options) {
    ...
    let priorExecutedQueries = [];
    if (completionPass) {
        try {
            const priorRaw = JSON.parse(await readFile(M982_REPORT_PATH, "utf8"));
    ...
    let hpltPriceRegression = { note: "not_run" };
    if (completionPass) {
        ...
        if (hpltCrawl.status === "success") {
            ...
            console.log(`HPLT price regression: extracted=${v.observedPrice} canonical=${pc.canonicalPrice} confidence=${pc.priceConfidence}`);
        }
    }
    const run = await createRun(supabase, completionPass ? "high_ticket_gap_completion" : "high_ticket_pdp_gap_first", {
    ...
    for (const candidate of candidateMap.values()) {
        ...
        if (price == null)
            priceDistribution.UNKNOWN = (priceDistribution.UNKNOWN ?? 0) + 1;
        else if (price < 60)
            priceDistribution.UNDER_60 = (priceDistribution.UNDER_60 ?? 0) + 1;
        else if (price < 100)
            priceDistribution["60_99"] = (priceDistribution["60_99"] ?? 0) + 1;
        else if (price < 150)
            priceDistribution["100_149"] = (priceDistribution["100_149"] ?? 0) + 1;
        else if (price <= 750)
            priceDistribution["150_750"] = (priceDistribution["150_750"] ?? 0) + 1;
        else
            priceDistribution["750_PLUS"] = (priceDistribution["750_PLUS"] ?? 0) + 1;
    ...
    for (let i = 0; i < screenQueue.length; i += 1) {
        ...
        candidate.mobileGap = Math.max(0, Math.min(100, Math.round(purchase.score * 0.55 +
            18 +
        ...
        if (candidate.pageEntityType === "PRODUCT_DETAIL" &&
            ...
            try {
                ...
                if (quickPaths["pdp-desktop-1440x1000"]) {
                    const vision = await screenPdpViewportWithVision(env, candidate.domain, quickPaths["pdp-desktop-1440x1000"]);
                ...
                console.warn(`  vision skip ${candidate.domain}: ${error.message}`);
            }
        }
    }
    const screened = screenQueue.filter((c) => c.screened);
    const highGapShortlist = screened
        .filter((c) => isHighGapCandidate({
    ...
    for (const candidate of highGapShortlist) {
        ...
        if (!hero?.url) {
            const flagship = await resolveFlagshipProduct(candidate.domain, crawlTimeout, candidate.productTitle, [], [candidate.sourceQuery]);
        ...
        if (meetsEarlySuccessFinalistCriteria({
            pageEntityType: candidate.pageEntityType,
            heroPrice,
            priceConfidence: candidate.priceConfidence,
            showcaseGapPotential: candidate.showcaseGapPotential ?? 0,
        ...
        if (isStrongFinalistForEarlyStop({
            businessModel: bizModel.businessModel,
            heroPrice,
            catalogFocus: candidate.catalogFocus,
            catalogEstimate: candidate.catalogEstimate,
    ...
    if (earlySuccessFinalistCount >= discovery.earlySuccessCount) {
        earlyStopTriggered = true;
        console.log(`  Early success finalists: ${earlySuccessFinalistCount} meet full gate`);
    }
    ...
    for (const candidate of paidPool) {
        if (!canSpend(dfsBudget, discovery.estimatedSerpCostPerKeyword * 2))
            break;
    ...
    const showcaseCandidates = ranked.filter((c) => c.opportunityTier === "SHOWCASE_DESIGN_CANDIDATE");
    ...
    const waterpikUrlCheck = classifyPageEntityFromUrl("https://www.waterpik.nl/product-categorie/waterflossers/", "waterpik.nl");
    ...
    const sufficientMaterials = screened.filter((c) => (c.redesignMaterialFeasibility ?? 0) >= 65 || c.materialSweetSpot).length;
    ...
    for (const c of highGapShortlist) {
        const fam = c.productFamilyId;
        if (familyResults[fam])
            familyResults[fam].highGap += 1;
    }
    for (const c of preAuditFinalists) {
        const fam = c.productFamilyId;
        if (familyResults[fam])
            familyResults[fam].finalists += 1;
    }
    for (const c of croOnlyOpportunities) {
        const fam = c.productFamilyId;
        if (familyResults[fam])
            familyResults[fam].croOnly += 1;
    }
    const report = {
        milestone: discovery.milestone,
        version: versionLabel,
...
function printReport(report, reportPath) {
    ...
    const finalists = report.preAuditFinalists ?? [];
    console.log(`\n=== PRE-AUDIT FINALISTS (${finalists.length}) ===`);
    for (const raw of finalists) {
        const c = raw;
        console.log(`  ${c.domain} · ht_fit=${c.highTicketGapSalesFit} · showcase=${c.showcaseGapPotential}`);
    }
    const wp = report.waterpikRegression;
...
//# sourceMappingURL=runM982HighTicketPdpGapFirst.js.map