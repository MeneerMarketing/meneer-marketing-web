...
import { generateOpportunitiesFromPaidTargets, markBrandWaitingForPaidTarget, upsertPaidSearchTargets, } from "../services/supabase/paidTargetsRepository.js";
...
async function main() {
    ...
    const { data: brands, error } = await supabase
        .from("brands")
        .select("id, normalized_domain, confirmed_google_advertiser, transparency_confirmed, lead_eligible, eligibility_status, manual_excluded")
    ...
    try {
        for (const brand of targets) {
            ...
            try {
                ...
                if (withLanding > 0) {
                    oppStats = await generateOpportunitiesFromPaidTargets(supabase, brand.id);
                }
                const paidTargetStatus = oppStats.croReady > 0
                    ? "RESOLVED"
                    : labs.items.length > 0
                        ? "PARTIAL"
                        : "NOT_RESOLVED";
                ...
                if (paidTargetStatus !== "RESOLVED") {
                    await markBrandWaitingForPaidTarget(supabase, brand.id, labs.items.length);
                }
                results.push({
                    domain,
                    cost: labs.cost,
                ...
                for (const ex of examples) {
                    console.log(`  - ${ex.keyword} → ${ex.landingUrl ?? "(no url)"} · vol ${ex.volume ?? "n/a"} · rank ${ex.rank ?? "n/a"}`);
                }
                if (totalCost >= maxCost)
                    break;
                void remaining;
...
main();
//# sourceMappingURL=resolvePaidTargets.js.map