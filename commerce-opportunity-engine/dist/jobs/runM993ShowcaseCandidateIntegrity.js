...
import { M993_DISCOVERY_VERSION, M993_REPORT_PATH, M993_DASHBOARD_REPORT_PATH, M993_MAX_VALIDATED_PROSPECTS, } from "../config/showcaseIntegrity.js";
...
import { classifyVisualShowcaseSignal, classifyVisualRedesignOpportunity, } from "../services/prospect/visualRedesignSalesFit.js";
import { assessBrandOwnership, assessCrossDomainMatches, compareProductIdentity, computeValidatedVisualSalesFit, deriveCurrentSiteImpression, extractProductIdentityFingerprint, hardenShowcasePageEntity, passesShowcaseSalesCandidate, withProvenance, } from "../services/prospect/showcaseCandidateIntegrity.js";
...
function catalogProvenance(estimate, verified) {
    if (estimate != null)
        return verified ? "MEASURED" : "INFERRED";
    return "UNKNOWN";
}
function catalogFocusProvenance(value) {
    if (value == null)
        return "UNKNOWN";
    if (value === 50)
        return "DEFAULT";
...
export async function runM993ShowcaseCandidateIntegrity() {
    const env = loadEnv();
    const supabase = createSupabaseServerClient(env);
    const startedAt = new Date().toISOString();
    const crawlTimeout = 15_000;
    const m992Raw = await readFile(M992_INPUT, "utf8");
    const m992 = JSON.parse(m992Raw);
    const showcasePool = m992.showcaseDesignCandidates ?? [];
    const allScreened = m992.allScreened ?? [];
    ...
    for (const row of showcasePool) {
        ...
        const productTitle = row.productTitle;
        const heroPrice = row.heroPrice;
        const m992BusinessModel = row.businessModel;
        ...
        const maturity = row.businessMaturityScore ?? maturityProxy(light);
        ...
        const businessQualified = isProfessional && row.businessModelSalesCandidate === true;
        const cross = { match: "NONE", peers: [], evidence: [] };
        const ownership = assessBrandOwnership({
            ...
            ownBrandSignal: row.ownBrand ?? light.ownBrandSignal,
            businessModel: m992BusinessModel,
            crossDomainMatch: cross.match,
            productUrl,
        });
        const currentSiteImpression = deriveCurrentSiteImpression(row.currentVisualQualityScore);
        const visualOpp = classifyVisualRedesignOpportunity({
            currentVisualQualityScore: row.currentVisualQualityScore,
            preauditPurchaseGap: row.purchaseGap,
            mobileGap: row.mobileGap,
            businessModelSalesCandidate: row.businessModelSalesCandidate,
            businessModel: m992BusinessModel,
            redesignMaterialFeasibility: row.materialFeasibility,
        });
        const visualShowcaseSignal = classifyVisualShowcaseSignal({
            visualRedesignType: visualOpp.type,
            visualRedesignSalesFit: row.visualRedesignSalesFit,
            businessModelSalesCandidate: row.businessModelSalesCandidate,
            currentVisualQualityScore: row.currentVisualQualityScore,
        ...
        const gate = passesShowcaseSalesCandidate({
            ...
            businessQualified,
            companyScaleFit: row.companyScaleFit ?? scale.brandScaleFitScore,
            businessMaturityScore: maturity,
        ...
        const validatedFit = computeValidatedVisualSalesFit({
            currentVisualQualityScore: row.currentVisualQualityScore,
            preauditVisualGap: row.visualGap,
            preauditPurchaseGap: row.purchaseGap,
            mobileGap: row.mobileGap,
            brandOwnershipConfidence: ownership.brandOwnershipConfidence,
            companyScaleFit: row.companyScaleFit ?? scale.brandScaleFitScore,
        ...
        const catalogFocusVal = row.catalogFocus;
        ...
        reviewed.push({
            ...
            companyScaleFit: withProvenance(row.companyScaleFit ?? scale.brandScaleFitScore, null),
    ...
    for (const item of reviewed) {
        ...
        const ownership = assessBrandOwnership({
            ...
            productBrand: item.productIdentity.productBrand,
            manufacturer: item.productIdentity.manufacturer,
            catalogEstimate: item.catalogEstimate.value,
            ownBrandSignal: item.ownBrandSignal,
            businessModel: item.m992BusinessModel ?? "UNKNOWN",
            crossDomainMatch: cross.match,
        ...
        const gate = passesShowcaseSalesCandidate({
            pageEntityType: item.pageEntityType,
            isValidProductDetail: item.pageEntityType === "PRODUCT_DETAIL",
            refinedBusinessModel: ownership.refinedBusinessModel,
            brandOwnershipConfidence: ownership.brandOwnershipConfidence,
            businessQualified: item.businessQualified,
            companyScaleFit: item.companyScaleFit.value,
            businessMaturityScore: item.businessMaturityScore.value,
            currentSiteImpression: item.currentSiteImpression,
            redesignMaterialFeasibility: item.materialFeasibility,
            visualRedesignOpportunityType: item.visualRedesignOpportunityType,
            crossDomainMatch: cross.match,
        });
        item.validatedVisualSalesFit = computeValidatedVisualSalesFit({
            currentVisualQualityScore: item.currentVisualQualityScore,
            preauditVisualGap: item.visualGap,
            preauditPurchaseGap: item.purchaseGap,
            mobileGap: item.mobileGap,
            brandOwnershipConfidence: ownership.brandOwnershipConfidence,
            companyScaleFit: item.companyScaleFit.value,
            redesignMaterialFeasibility: item.materialFeasibility,
            catalogFocus: item.catalogFocus.value,
            businessMaturityScore: item.businessMaturityScore.value,
            refinedBusinessModel: ownership.refinedBusinessModel,
            currentSiteImpression: item.currentSiteImpression,
        });
    ...
    let cleanmasterVsNeduma = null;
    if (cleanmaster && neduma) {
        const cmp = compareProductIdentity(cleanmaster.productIdentity, neduma.productIdentity);
        cleanmasterVsNeduma = {
            verdict: cmp.verdict,
    ...
    const validatedProspects = reviewed
        ...
        .sort((a, b) => (b.validatedVisualSalesFit ?? 0) -
        (a.validatedVisualSalesFit ?? 0))
        ...
        whyGoodMeneerProspect: whyGoodProspect({
            ...
            refinedBusinessModel: r.businessModel,
            brandOwnershipConfidence: r.brandOwnershipConfidence,
            currentSiteImpression: r.currentSiteImpression,
            visualGap: r.visualGap,
            material: r.materialFeasibility,
            companyScaleFit: r.companyScaleFit.value,
        }),
    ...
    const m992Funnel = m992.funnel;
    const visualSignalsFromScreened = allScreened.filter((c) => c.leadType === "SHOWCASE_DESIGN" || c.visuallyUnderdesigned === true);
    const funnelConsistency = {
        ...
        root_cause: "M9.9.2 SHOWCASE_DESIGN counted visual redesign opportunity without requiring businessQualified. Visual signals can exceed business-qualified upstream pool.",
        semantic_fix: {
...
//# sourceMappingURL=runM993ShowcaseCandidateIntegrity.js.map