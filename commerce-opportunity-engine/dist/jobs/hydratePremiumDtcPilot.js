...
import { selectPremiumDtcPilot, } from "../services/concept/selectPremiumDtcPilot.js";
...
async function main() {
    ...
    const { data: rows, error } = await supabase
        ...
        .select(`id, brand_id, concept_ready_score, brand_commerce_model, catalog_focus_score,
    ...
    const mapped = (rows ?? []).map((r) => {
        ...
        return {
            ...
            catalog_focus_score: r.catalog_focus_score == null ? null : Number(r.catalog_focus_score),
            concept_asset_readiness_score: r.concept_asset_readiness_score == null
                ? null
                : Number(r.concept_asset_readiness_score),
            pdp_transformation_potential: r.pdp_transformation_potential == null
                ? null
                : Number(r.pdp_transformation_potential),
            hero_product_score: r.hero_product_score == null ? null : Number(r.hero_product_score),
            primary_concept_product_title: r.primary_concept_product_title,
            primary_concept_product_url: r.primary_concept_product_url,
            primary_concept_product_price: r.primary_concept_product_price == null
                ? null
                : Number(r.primary_concept_product_price),
            suggested_template_family: r.suggested_template_family,
            needs_assets: r.needs_assets,
            status: r.status,
...
//# sourceMappingURL=hydratePremiumDtcPilot.js.map