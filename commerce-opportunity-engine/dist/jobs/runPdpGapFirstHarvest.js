...
import { buildM98HarvestQueries, M98_DISCOVERY, M98_DISCOVERY_ROUTE, M98_DISCOVERY_VERSION, } from "../config/pdpGapFirstHarvest.js";
...
import { computeBusinessEconomicFit, computeMaterialQualityScore, computeGapFirstSalesPotential, manualReviewVerdictM98, } from "../services/prospect/gapFirstSalesPotential.js";
const scriptDir = dirname(fileURLToPath(import.meta.url));
const projectRoot = resolve(scriptDir, "../../");
config({ path: resolve(projectRoot, ".env"), quiet: true });
const REPORT_PATH = resolve(projectRoot, "reports/pdp-gap-first-report.json");
const DASHBOARD_REPORT_PATH = resolve(projectRoot, "dashboard/src/preview/concepts/data/pdp-gap-first-report.json");
...
export async function runPdpGapFirstHarvest(options) {
    const env = loadEnv();
    const supabase = createSupabaseServerClient(env);
    const dataForSeo = createDataForSeoClient(env);
    const dfsBudget = { spent: 0, cap: env.M98_MAX_DATAFORSEO_COST };
    const anthropicBudget = { spent: 0, cap: env.M98_MAX_ANTHROPIC_COST };
    ...
    const makeCandidate = (entry, input) => {
        ...
        if (candidateMap.has(key))
            return;
        if (candidateMap.size >= M98_DISCOVERY.maxRawCandidates)
            return;
    ...
    for (const entry of queries) {
        if (!canSpend(dfsBudget, M98_DISCOVERY.estimatedSerpCostPerKeyword))
            break;
        let serp;
        try {
    ...
    for (const candidate of candidateMap.values()) {
        ...
        if (candidate.observedPrice != null &&
    ...
    const highGapShortlist = screened
        .filter((c) => (c.rawPdpRedesignOpportunity ?? 0) >= M98_DISCOVERY.highGapRawThreshold ||
    ...
    for (const candidate of highGapShortlist) {
        ...
        if (heroPrice != null) {
            if (heroPrice >= 150 && heroPrice <= 750)
                heroEconomics = 94;
            else if (heroPrice >= 120 && heroPrice <= 3000)
                heroEconomics = 76;
            else if (heroPrice >= 80)
                heroEconomics = 52;
        }
    ...
    for (const candidate of paidPool) {
        if (!canSpend(dfsBudget, M98_DISCOVERY.estimatedSerpCostPerKeyword * 2))
            break;
    ...
    const sufficientMaterials = screened.filter((c) => (c.materialQualityScore ?? 0) >= M98_DISCOVERY.minSufficientMaterials ||
        c.materialSweetSpot).length;
    const potentialTargets = ranked.filter((c) => c.manualReviewVerdict === "TRUE_MANUAL_REVIEW_CANDIDATE" ||
        c.manualReviewVerdict === "PROMISING").length;
...
//# sourceMappingURL=runPdpGapFirstHarvest.js.map