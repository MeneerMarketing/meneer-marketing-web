...
import { fetchShoppingSellers, pickMatchingSellerOffer, } from "../services/dataforseo/shoppingSellers.js";
...
import { upsertCroReadyOpportunityFromTarget, upsertShoppingPaidTarget, } from "../services/supabase/paidTargetsRepository.js";
...
async function main() {
    ...
    try {
        for (const kw of targets) {
            ...
            for (const item of paidItems) {
                ...
                if (!validation.ok) {
                    ...
                    console.log(`  ! mismatch ${sellerName ?? brandDomainForUpsert} → ${finalUrl} (${validation.status})`);
                    continue;
                }
                ...
                // Upsert opportunity for paid exact OR high-confidence exact listing
                {
                    const { data: targetRow } = await supabase
                        ...
                        .select("source_type, cro_readiness_level, listing_target_confidence, paid_evidence_confidence, source_quality_score")
                        .eq("id", saved.id)
                        .maybeSingle();
                    const srcType = targetRow?.source_type ?? "GOOGLE_SHOPPING_EXACT_LISTING";
                    if (srcType === "GOOGLE_SHOPPING_PAID_EXACT" ||
                        ...
                        await upsertCroReadyOpportunityFromTarget({
                            ...
                            sourceType: srcType,
                            sourceQualityScore: Number(targetRow?.source_quality_score ?? saved.sourceQualityScore),
                            channel: "SHOPPING",
                            discoverySerpItemType: item.itemType,
                            confirmationSource: "dataforseo_merchant_shopping",
                            croReadinessLevel: targetRow?.cro_readiness_level ?? undefined,
                ...
                console.log(`  - ${sellerName ?? finalDomain} · ${item.title ?? "(no title)"} → ${finalUrl} · q=${saved.sourceQualityScore}${saved.croReady ? " · CRO-READY" : ""}`);
            }
        ...
        for (const ex of examples.slice(0, 12)) {
            console.log(`EX: ${ex.keyword} → ${ex.seller ?? ex.domain} → ${ex.product} → ${ex.landingUrl} (q=${ex.sourceQuality}, ${ex.resolvePath})`);
        }
        process.exit(0);
    }
    catch (error) {
...
main();
//# sourceMappingURL=resolveShoppingTargets.js.map