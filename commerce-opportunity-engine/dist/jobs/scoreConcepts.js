...
import { BRIEF_READY_MIN_ASSET_SCORE, HERO_PRODUCT_MIN_CONFIDENCE, } from "../config/conceptScoring.js";
...
function imageCountFromPage(page) {
    ...
    if (typeof ed.image_count === "number")
        return ed.image_count;
    return null;
}
function categoryHintsForBrand(domain, keywords) {
    const hints = [];
    for (const k of keywords) {
        if (k.category)
            hints.push(k.category);
        if (k.keyword)
            hints.push(k.keyword);
    }
    const d = (domain ?? "").toLowerCase();
    if (/huid|skin|beauty|cosmetic|body|boozy|currentbody/.test(d)) {
        hints.push("beauty", "skincare");
    }
    if (/huisdier|pet|hond|dog|dier/.test(d))
        hints.push("pets");
    if (/dekbed|slaap|sleep|kussen|matras/.test(d))
        hints.push("sleep");
    return hints;
}
function deriveStatus(score, needsAssets, existingStatus) {
...
async function main() {
    ...
    try {
        const { data: brandsRaw, error: brandsError } = await supabase
            .from("brands")
            .select(`id, name, domain, normalized_domain, business_type, platform,
         ...
         eligibility_status, crawl_metadata, do_not_contact`)
            .or("is_ecommerce.eq.true,pre_fit_score.gte.30,confirmed_google_advertiser.eq.true,platform.eq.SHOPIFY")
            .limit(500);
        if (brandsError)
            throw brandsError;
        let brands = (brandsRaw ?? []);
        const brandIdFilter = process.env.M93_SCORE_BRAND_IDS?.split(",").map((s) => s.trim()).filter(Boolean) ??
            [];
        ...
        const [{ data: opps }, { data: pages }, { data: audits }, { data: existingCandidates }, { data: keywords },] = await Promise.all([
            supabase
                .from("opportunities")
                .select(`id, brand_id, landing_url, resolved_url, resolved_page_id, ad_headline,
           ...
           meneer_marketing_fit_score, keyword_id, latest_audit_id`)
            ...
            supabase
                .from("pages")
                .select(`id, brand_id, url, product_name, price, currency, review_count, rating,
           availability, product_brand, product_description, page_type, extracted_data`)
                .in("brand_id", brandIds),
            supabase
                .from("audits")
                .select(`id, opportunity_id, brand_id, status, score, cro_scores, conversion_leaks,
        ...
        const existingByBrand = new Map((existingCandidates ?? []).map((c) => [c.brand_id, c]));
        ...
        for (const brand of brands) {
            ...
            const rankedOpps = [...brandOpps].sort((a, b) => {
                const aAud = (auditsByOpp.get(a.id) ?? []).some((x) => String(x.status).toUpperCase().includes("COMPLETED"))
                    ? 1
                    : 0;
                const bAud = (auditsByOpp.get(b.id) ?? []).some((x) => String(x.status).toUpperCase().includes("COMPLETED"))
                    ? 1
                    : 0;
                if (bAud !== aAud)
                    return bAud - aAud;
            ...
            const audit = oppAudits.find((a) => String(a.status).toUpperCase() === "COMPLETED") ??
                oppAudits[0] ??
                null;
            let page = null;
            ...
            if (!page && opp?.resolved_url) {
                page =
                    brandPages.find((p) => p.url === opp.resolved_url ||
                        p.url.includes((opp.resolved_url ?? "").split("/products/")[1] ?? "___")) ?? null;
            }
            if (!page && opp?.landing_url) {
                page =
                    brandPages.find((p) => p.url === opp.landing_url ||
                        (opp.landing_url && p.url.includes(opp.landing_url.split("/").pop() ?? "___"))) ?? null;
            }
            if (!page) {
                page =
                    brandPages
            ...
            const distinctBrands = new Set(brandPages.map((p) => p.product_brand).filter(Boolean)).size;
            const catalog = scoreCatalogIntelligence({
                ...
                hasProductPage: brandPages.some((p) => (p.page_type ?? "").toUpperCase().includes("PRODUCT") || Boolean(p.product_name)),
            ...
            const ownBrand = scoreOwnBrandIntelligence({
                businessType: brand.business_type,
                productMerchantRelationship: opp?.product_merchant_relationship ?? null,
                productMerchantConfidence: num(opp?.product_merchant_relationship_confidence),
            ...
            const existing = existingByBrand.get(brand.id);
            const kw = opp?.keyword_id ? kwById.get(opp.keyword_id) : null;
            const pageIsOpportunityTarget = Boolean(page) &&
                Boolean(opp) &&
                (page.id === opp.resolved_page_id ||
                    (opp.landing_url != null &&
                        page.url.includes(opp.landing_url.split("/").pop()?.split("?")[0] ?? "___")) ||
                    (opp.resolved_url != null && page.url === opp.resolved_url));
            const productTitle = existing?.manual_product_override && existing.primary_concept_product_title
            ...
            const heroInputs = brandPages
                ...
                hasScreenshots: Boolean(audit?.screenshot_paths &&
            ...
            if (productTitle &&
                ...
                heroInputs.unshift({
                    ...
                    productUrl: (pageIsOpportunityTarget ? page?.url : null) ??
                        opp?.resolved_url ??
                        opp?.landing_url ??
                        null,
            ...
            let primaryHero = existing?.manual_product_override && existing.primary_concept_product_title
                ? hero.candidates.find((c) => c.product_title === existing.primary_concept_product_title) ??
                    {
            ...
            if (!existing?.manual_product_override && productTitle) {
                const targetMatch = hero.candidates.find((c) => c.product_title === productTitle ||
                    (opp?.landing_url &&
                        c.product_url &&
                        c.product_url.includes(opp.landing_url.split("/").pop()?.split("?")[0] ?? "___")));
                if (targetMatch) {
                    primaryHero = {
                        ...targetMatch,
                        hero_product_score: Math.min(100, targetMatch.hero_product_score + 12),
                        hero_product_confidence: Math.max(targetMatch.hero_product_confidence, HERO_PRODUCT_MIN_CONFIDENCE),
                    ...
                    primaryHero = {
                        ...
                        hero_product_reasoning: "Selected from opportunity ad/landing target (no richer page sample).",
                        hero_product_evidence: [
                            "opportunity_target_priority",
                            "no_random_fallback",
                        ],
            ...
            const commercial = scoreProductCommercialSignal({
                ...
                paidOrDiscoveryRelevant: Boolean(opp?.paid_confirmed || brand.confirmed_google_advertiser),
            ...
            const croQuality = croQualityParts.length > 0
                ? Math.round(croQualityParts.reduce((a, b) => a + b, 0) / croQualityParts.length)
                : num(audit?.score);
            const transform = scorePdpTransformationPotential({
                ...
                retailerScaleScore: brand.retailer_scale_score,
                mmFitScore: num(opp?.meneer_marketing_fit_score) ??
                    num(audit?.meneer_marketing_fit_score) ??
                    num(brand.pre_fit_score),
                siteTechnicallyBroken: String(audit?.status ?? "")
                    .toUpperCase()
                    .includes("FAILED"),
                storytellingWeak: cro.product_storytelling_quality != null
                    ? Number(cro.product_storytelling_quality) < 55
                    : null,
                aboveFoldWeak: cro.above_fold_quality != null
                    ? Number(cro.above_fold_quality) < 55
                    : null,
                trustNearBuyblockWeak: cro.trust_quality != null ? Number(cro.trust_quality) < 55 : null,
                deepDiveWeak: cro.product_presentation_quality != null
            ...
            const mmFit = num(opp?.meneer_marketing_fit_score) ??
                num(audit?.meneer_marketing_fit_score) ??
                num(brand.pre_fit_score) ??
                40;
            ...
            const ready = scoreConceptReady({
                ...
                mobileWeak: cro.mobile_cro_quality != null
                    ? Number(cro.mobile_cro_quality) < 50
                    : null,
                buyblockWeak: cro.cta_quality != null ? Number(cro.cta_quality) < 50 : null,
                deepDiveWeak: cro.product_presentation_quality != null
                    ? Number(cro.product_presentation_quality) < 60
                    : null,
            });
            const needsAssets = assets.concept_asset_readiness_score < BRIEF_READY_MIN_ASSET_SCORE ||
                assets.missing_assets.includes("product_images");
            const status = deriveStatus(ready.concept_ready_score, needsAssets, existing?.status ?? null);
            const hints = categoryHintsForBrand(brand.normalized_domain, kw ? [{ keyword: kw.keyword, category: kw.category }] : []);
            const templateFamily = existing?.manual_template_family ??
            ...
            if (existing?.manual_product_override) {
                row.manual_product_override = true;
            }
            scoredRows.push(row);
            ranked.push({
...
//# sourceMappingURL=scoreConcepts.js.map