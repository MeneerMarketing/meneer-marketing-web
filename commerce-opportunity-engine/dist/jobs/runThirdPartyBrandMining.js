...
import { buildM97SourceQueries, M97_DISCOVERY, M97_DISCOVERY_ROUTE, M97_DISCOVERY_VERSION, } from "../config/thirdPartyBrandMining.js";
...
import { classifyThirdPartySource, isOrganicFirstPartySkip, } from "../services/prospect/thirdPartySourceClassifier.js";
...
import { classifyFirstPartyStore, passesDtcEcommerceGate, } from "../services/prospect/firstPartyStoreGate.js";
...
import { computeThirdPartyBrandOpportunityScore, manualReviewVerdictM97, } from "../services/prospect/thirdPartyBrandOpportunityScore.js";
...
const DASHBOARD_REPORT_PATH = resolve(projectRoot, "dashboard/src/preview/concepts/data/third-party-brand-mining-report.json");
...
export async function runThirdPartyBrandMining(options) {
    const env = loadEnv();
    const supabase = createSupabaseServerClient(env);
    const dataForSeo = createDataForSeoClient(env);
    const dfsBudget = { spent: 0, cap: env.M97_MAX_DATAFORSEO_COST };
    const anthropicBudget = { spent: 0, cap: env.M97_MAX_ANTHROPIC_COST };
    ...
    for (const entry of queries) {
        if (!canSpend(dfsBudget, M97_DISCOVERY.estimatedSerpCostPerKeyword))
            break;
        let serp;
        try {
        ...
        const processSource = async (input) => {
            ...
            if (!candidate) {
                if (brandMap.size >= M97_DISCOVERY.maxExtractedBrands)
                    return;
                candidate = {
                    brandKey: key,
            ...
            if (extraction.productTitle)
                candidate.discoveredProductTitle = extraction.productTitle;
            if (extraction.productModel)
                candidate.discoveredProductModel = extraction.productModel;
        };
    ...
    for (const candidate of brandMap.values()) {
        const presence = computeBrandMarketPresenceScore(candidate.sources.map((s) => ({
    ...
    const resolutionQueue = [...brandMap.values()]
        .sort((a, b) => b.brandMarketPresenceScore +
    ...
    const dtcCandidates = [];
    for (const candidate of brandMap.values()) {
        if (!candidate.officialDomain || candidate.officialDomainStatus !== "RESOLVED")
            continue;
        ...
        const catalog = await runCatalogFocusCheck(candidate.officialDomain, crawlTimeout, light.productLinks, light.categoryLinks);
        ...
        const homepageUrls = await (async () => {
            ...
            if (home.status !== "success")
                return [];
            return extractPageSignals(home.html, home.finalUrl).internalLinks.filter((l) => /\/products?\/|\/producten\//i.test(l));
        })();
        ...
        if (!hero?.url || !isUsableHeroUrl(hero.url, candidate.officialDomain)) {
            const flagship = await resolveFlagshipProduct(candidate.officialDomain, crawlTimeout, candidate.discoveredProductTitle ?? hero?.title ?? null, homepageUrls, discoveryKeywords);
            if (flagship) {
                hero = {
    ...
    for (const candidate of gapPool) {
        ...
        if (visionScreens < M97_DISCOVERY.maxVisionScreens &&
            canSpend(anthropicBudget, 0.01) &&
            paths["pdp-desktop-1440x1000"]) {
            const vision = await screenPdpViewportWithVision(env, candidate.officialDomain, paths["pdp-desktop-1440x1000"]);
    ...
    for (const candidate of rankedPrePaid) {
        if (!canSpend(dfsBudget, M97_DISCOVERY.estimatedSerpCostPerKeyword * 2))
            break;
    ...
    const report = {
        ...
        funnel: {
            ...
            official_domains_resolved: [...brandMap.values()].filter((c) => c.officialDomainStatus === "RESOLVED").length,
...
//# sourceMappingURL=runThirdPartyBrandMining.js.map