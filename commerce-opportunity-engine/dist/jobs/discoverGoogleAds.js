...
import { loadActiveKeywords, seedDevelopmentKeywords, updateKeywordLastScanned, } from "../services/supabase/keywordsRepository.js";
import { loadApprovedKeywordsForDiscovery, loadKeywordsByIds, markKeywordsScanned, } from "../services/supabase/keywordIntelligenceRepository.js";
...
import { countBrandsByBusinessCategory, countOccurrencesBySignal, getBrandActivityMetrics, } from "../services/supabase/brandMetricsService.js";
import { reclassifyAllSignals } from "../services/supabase/signalReclassificationService.js";
import { applyTransparencyResult, selectDomainsForTransparencyCheck, } from "../services/supabase/transparencyRepository.js";
...
function printRankedAdvertisers(title, advertisers, mode) {
    console.log(title);
    console.log("");
...
function printFinalReport(stats, topConfirmed, topCandidates) {
    console.log("");
...
async function processKeyword(runId, keywordId, keyword, accumulatedCost, maxCost) {
    const env = loadEnv();
    ...
    try {
        const serpResult = await fetchGooglePaidAds({ client: dataForSeoClient, env }, keyword);
...
async function runTransparencyChecks(accumulatedCost, maxCost) {
    const env = loadEnv();
    ...
    const supabase = createSupabaseServerClient(env);
    const dataForSeoClient = createDataForSeoClient(env);
    const domains = await selectDomainsForTransparencyCheck(supabase, env.GOOGLE_ADS_CONFIRMATION_MAX_DOMAINS_PER_RUN, PREFERRED_TRANSPARENCY_DOMAINS);
    let cost = 0;
    let checksRun = 0;
    for (const domain of domains) {
        ...
        try {
            const result = await checkGoogleAdsTransparency({ client: dataForSeoClient, env }, domain);
            cost += result.cost;
...
export async function runGoogleAdsDiscovery(options = {}) {
    const env = loadEnv();
    const supabase = createSupabaseServerClient(env);
    ...
    if (env.GOOGLE_DISCOVERY_SKIP_SERP_FETCH) {
        ...
        keywords = await loadActiveKeywords(supabase, maxKeywords, env.GOOGLE_DISCOVERY_KEYWORD_FILTER);
    }
    ...
    if (keywords.length > 0) {
        const results = await mapWithConcurrency(keywords, env.DATAFORSEO_CONCURRENCY, async (keywordRow) => processKeyword(run.id, keywordRow.id, keywordRow.keyword, accumulatedCost, maxSerpCost));
        const allUniqueAdvertisers = new Set();
        const allNewBrandIds = new Set();
        const allExistingBrandIds = new Set();
        ...
        if (approvedOnly || options.keywordIds?.length) {
            await markKeywordsScanned(supabase, keywords.map((k) => k.id), new Date().toISOString());
        }
    }
    if (!options.skipTransparency) {
        const transparency = await runTransparencyChecks(accumulatedCost, env.GOOGLE_ADS_CONFIRMATION_MAX_COST_PER_RUN);
    ...
    const topConfirmed = (await getBrandActivityMetrics(supabase, undefined, "CONFIRMED_PAID")).slice(0, 10);
    const topCandidates = (await getBrandActivityMetrics(supabase, undefined, "PAID_CANDIDATE")).slice(0, 10);
...
//# sourceMappingURL=discoverGoogleAds.js.map