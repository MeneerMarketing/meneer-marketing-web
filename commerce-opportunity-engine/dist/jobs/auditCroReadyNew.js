...
import { auditOpportunity, loadAuditCandidateById, } from "../services/audit/auditRunner.js";
import { classifyProductMerchantRelationship } from "../services/scoring/productMerchantRelationship.js";
import { buildInternalSalesAngle, recommendProjectType, } from "../services/scoring/projectType.js";
...
supabase, target) {
    ...
    const { data: opp } = await supabase
        ...
        .maybeSingle();
    if (opp?.cro_audit_status === "COMPLETED" &&
    ...
    if (opp?.cro_audit_status === "FAILED_TECHNICAL" ||
...
supabase, opportunityId) {
    ...
    if (!opp)
        return { opportunityId, skipped: true, reason: "not_found" };
    ...
    if (opp.latest_audit_id) {
        const { data } = await supabase
            .from("audits")
            .select("id, rebuild_potential, sales_angle, product_merchant_relationship, pdp_improvement_potential, full_rebuild_potential")
    ...
    const productBrand = page?.product_brand ?? null;
    const productName = page?.product_name ?? null;
    ...
    const fullRebuild = opp.full_rebuild_potential != null
    ...
    const pdp = opp.pdp_improvement_potential != null
        ...
        : fullRebuild != null
            ? Math.min(100, Math.round(fullRebuild +
    ...
    const salesAngle = buildInternalSalesAngle({
        ...
        pdpPotential: pdp,
        fullRebuildPotential: fullRebuild,
        aiSalesAngle: audit?.sales_angle ?? null,
      ...
      `)
        ...
        for (const result of auditResults) {
            ...
            if (result.skipped) {
                ...
                if (result.salesAngle)
                    console.log(`  Sales angle: ${result.salesAngle}`);
            }
            console.log("");
        }
        if (gatedOut.length) {
        ...
        for (const row of comparison) {
            console.log(`  ${row.domain}: Opp ${row.opportunityScore} | MM ${row.mmFit} | ${row.platform} | ${row.businessType} | ${row.productRelationship} | PDP ${row.pdpImprovement} | Rebuild ${row.fullRebuild} | ${row.recommendedProject} | conf ${row.auditConfidence}`);
        }
        console.log("");
...
main();
//# sourceMappingURL=auditCroReadyNew.js.map