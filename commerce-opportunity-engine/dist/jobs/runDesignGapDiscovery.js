...
import { DESIGN_GAP_BRANCHES, M95_DISCOVERY, M95_DISCOVERY_VERSION, M95_SCREENSHOT_CONFIG, PARKED_FOR_M95, } from "../config/designGapDiscovery.js";
...
import { computeCurrentPdpWeaknessProxy, pdpWeaknessSignalsFromHtml, } from "../services/idealProspect/pdpWeaknessProxy.js";
...
const DASHBOARD_REPORT_PATH = resolve(projectRoot, "dashboard/src/preview/concepts/data/design-gap-discovery-report.json");
...
function buildRoutedKeywords(selected) {
    ...
    return sorted.map((keyword, index) => {
        ...
        const serpQuery = shoppingFirst && !keyword.keyword.includes("kopen")
...
function computeMobileGapProxy(purchaseGap, purchaseSignals) {
    let mobile = purchaseGap * 0.55 + 18;
    if (!purchaseSignals.mobileAtcSignal)
        mobile += 12;
    if (!purchaseSignals.stickyAtcSignal)
        mobile += 8;
    if (!purchaseSignals.hasAddToCart)
        mobile += 6;
    return Math.max(0, Math.min(100, Math.round(mobile)));
}
function passesEconomicPrescreen(entry) {
    if (!entry.gateEligible)
        return false;
    if ((entry.highTicketFitScore ?? 0) < ECONOMIC_PRESCREEN_THRESHOLDS.minHighTicketFit)
        return false;
    if ((entry.ownBrandSignal ?? 0) < ECONOMIC_PRESCREEN_THRESHOLDS.minOwnBrandSignal)
        return false;
    if ((entry.assetReadinessProxy ?? 0) < ECONOMIC_PRESCREEN_THRESHOLDS.minAssetProxy)
        return false;
    const heroPrice = entry.heroes[0]?.price ?? 0;
    if (heroPrice > 0 && heroPrice < ECONOMIC_PRESCREEN_THRESHOLDS.minHeroPricePreferred)
        return false;
    if (entry.businessType !== "BRAND" && entry.businessType !== "SPECIALIST_WEBSHOP")
        return false;
    return true;
}
function toCandidate(entry, index) {
    return {
        rank: index + 1,
        domain: entry.domain,
...
export async function runDesignGapDiscovery(options) {
    const env = loadEnv();
    const supabase = createSupabaseServerClient(env);
    const dataForSeo = createDataForSeoClient(env);
    const dfsBudget = { spent: 0, cap: env.M95_MAX_DATAFORSEO_COST };
    const anthropicBudget = { spent: 0, cap: env.M95_MAX_ANTHROPIC_COST };
    const startedAt = new Date().toISOString();
    const regression = runDesignGapRegression();
    console.log(`\n=== M9.5 DESIGN-GAP-FIRST HIGH-TICKET DISCOVERY (${M95_DISCOVERY_VERSION}) ===`);
    console.log(`Regression: ${regression.passed}/${regression.total} (ranking ${regression.rankingOk ? "OK" : "FAIL"})`);
    console.log(`DataForSEO cap: $${dfsBudget.cap.toFixed(3)} · Anthropic cap: $${anthropicBudget.cap.toFixed(3)}\n`);
    ...
    console.log(`Stap 1 — ${routed.length} keywords (${routed.filter((k) => k.discoveryRoute === "shopping_first").length} shopping-first)`);
    if (options?.dryRun) {
        for (const keyword of routed) {
            console.log(`  ${keyword.discoveryRoute.padEnd(14)} ${keyword.archetypeId.padEnd(20)} · ${keyword.serpQuery}`);
        }
        console.log(`\nDry run: geschat $${(routed.length * M95_DISCOVERY.estimatedSerpCostPerKeyword + expansion.ideasCost).toFixed(4)}\n`);
        return;
    }
    const persisted = (await persistFamilyKeywords(supabase, selected)).filter((keyword) => keyword.id !== null);
    const routedByKeyword = new Map(routed.map((keyword) => [keyword.keyword, keyword]));
    ...
    for (const keyword of persisted) {
        ...
        const route = routedKeyword?.discoveryRoute ?? "ads_first";
        if (!canSpend(dfsBudget, M95_DISCOVERY.estimatedSerpCostPerKeyword))
            continue;
        let serp;
        try {
        ...
        for (const ad of paidAds) {
            ...
            if (isBlacklistedDomain(ad.normalizedDomain))
                continue;
            const isShopping = String(ad.serpItemType).toLowerCase().includes("shopping") ||
                recovery.recovered.some((item) => item.normalizedDomain === ad.normalizedDomain);
            const domainRoute = isShopping ? "shopping_first" : route;
            ...
            if (!entry) {
                entry = emptyDomain({
                    ...
                    sellerResolution: ad.rawItem?.seller_domain_resolution ??
                        null,
                });
        ...
        const quality = computeSerpProspectQuality(sample.map((domain) => ({
            domain,
            businessType: stored.get(domain)?.business_type ?? null,
        ...
        console.log(`  ${approved ? "OK" : "STOP"} ${route.padEnd(14)} ${serpQuery} · $${serp.cost.toFixed(4)}`);
    }
    ...
    for (const entry of screenPool) {
        if (designGapScreens >= M95_DISCOVERY.maxDesignGapScreens)
            break;
        let pdpUrl = entry.heroUrlIsProductPage ? entry.heroes[0]?.url : null;
        if (!pdpUrl) {
            const flagship = await resolveFlagshipProduct(entry.domain, crawlTimeout, entry.heroes[0]?.title ?? null);
            if (flagship) {
                pdpUrl = flagship.url;
        ...
        if (visionScreens < M95_DISCOVERY.maxVisionScreens &&
            canSpend(anthropicBudget, 0.01) &&
            paths["pdp-desktop-1440x1000"]) {
            try {
                const vision = await screenPdpViewportWithVision(env, entry.domain, paths["pdp-desktop-1440x1000"]);
                anthropicBudget.spent += vision.estimatedCost;
                visionScreens += 1;
                entry.preauditVisualGap = Math.max(0, Math.min(100, (entry.preauditVisualGap ?? 0) + vision.visualAdjustment));
                entry.preauditPurchaseGap = Math.max(0, Math.min(100, (entry.preauditPurchaseGap ?? 0) + vision.purchaseAdjustment));
                entry.mobileGapProxy = Math.max(0, Math.min(100, (entry.mobileGapProxy ?? 0) + vision.mobileAdjustment));
        ...
        const polishedPenalty = (entry.contentPresentationQuality ?? 0) >= 72
            ? 14
    ...
    for (const entry of domainIndex.values()) {
        if (entry.discoveryRoute === "ads_first")
            routeCounts.adsFirst += 1;
        else if (entry.discoveryRoute === "shopping_first")
            routeCounts.shoppingFirst += 1;
        else
            routeCounts.brandFirst += 1;
    }
    ...
    const report = {
        ...
        discovery: {
            ...
            keywordsRejected: expansion.rejected.slice(0, 30),
        ...
        cost: {
            ...
            visionScreens,
            designGapScreens,
            costPerCandidate: candidates.length > 0
...
function printReport(report) {
    ...
    for (const candidate of report.ranked) {
        ...
        console.log(`     route ${candidate.discoveryRoute} · visual gap ${candidate.preauditVisualGap} · purchase gap ${candidate.preauditPurchaseGap} · mobile ${candidate.mobileGapProxy}`);
        console.log(`     content avail ${candidate.assetContentAvailability} · presentation ${candidate.currentContentPresentation} · plafond ${candidate.estimatedContrastCeiling}`);
        console.log(`     hero ${candidate.heroProduct ?? "?"} ${candidate.heroPrice != null ? `EUR ${candidate.heroPrice}` : ""}`);
    ...
    console.log("\n=== COST ===");
    console.log(`  DataForSEO $${report.cost.dataForSeo.toFixed(4)} / $${report.cost.dataForSeoCap.toFixed(3)}`);
    console.log(`  Anthropic $${report.cost.anthropic.toFixed(4)} / $${report.cost.anthropicCap.toFixed(3)} (${report.cost.visionScreens} vision screens)`);
    console.log(`\nRapport: ${REPORT_PATH}\n`);
}
...
//# sourceMappingURL=runDesignGapDiscovery.js.map