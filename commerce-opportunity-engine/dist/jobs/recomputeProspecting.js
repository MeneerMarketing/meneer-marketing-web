...
async function main() {
    ...
    try {
        ...
        const { data: keywords, error } = await supabase
            .from("keywords")
            .select("id, keyword, commercial_intent_score, product_intent_score, keyword_quality_score, search_volume, competition, cpc, estimated_serp_cost, serp_cost, discovery_status, last_scanned_at")
        ...
        for (const kw of keywords ?? []) {
            ...
            const yieldMetrics = await computeKeywordYield(supabase, kw.id, kw.serp_cost ?? kw.estimated_serp_cost ?? (kw.last_scanned_at ? 0.004 : null));
            ...
            const historicalHighYield = intent.type === "PRODUCT_BRANDED" &&
            ...
            const { error: updError } = await supabase
                ...
                .eq("id", kw.id);
            if (updError)
                throw new Error(`Update ${kw.keyword}: ${updError.message}`);
            updated += 1;
        ...
        if (focusRows.length < FOCUS_KEYWORDS.length) {
            for (const focus of FOCUS_KEYWORDS) {
                if (focusRows.some((r) => String(r.keyword).toLowerCase() === focus))
                    continue;
                const match = (keywords ?? []).find((k) => k.keyword.toLowerCase().replace(/-/g, " ") === focus.replace(/-/g, " "));
                if (!match)
                    continue;
                // already updated; fetch for report
                const { data: row } = await supabase
                    .from("keywords")
                    .select("keyword, keyword_intent_type, prospecting_value_score, prospecting_tier, unique_domains_found, lead_eligible_found, shopify_found, general_retailers_found, comparison_sites_found, prospect_yield_score, placements_found")
                    .eq("id", match.id)
                    .maybeSingle();
                if (row) {
                    focusRows.push({
        ...
        console.log("keyword | intent | prospecting | tier | advertisers | eligible | shopify | retailers | yield");
        for (const row of focusRows) {
            console.log(`${row.keyword} | ${row.intent} | ${row.prospecting} | ${row.tier} | ${row.advertisers} | ${row.eligible} | ${row.shopify} | ${row.retailers} | ${row.yield ?? "—"}`);
        }
        console.log("");
...
main();
//# sourceMappingURL=recomputeProspecting.js.map