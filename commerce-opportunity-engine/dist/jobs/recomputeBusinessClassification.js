...
import { classifyProspectExclusion, structuralDomainClass, } from "../services/prospect/prospectPipelineGate.js";
import { PROSPECT_GATE_FIXTURES } from "../config/prospectExclusion.js";
import { runBusinessClassifierRegression, } from "../services/crawler/businessClassifierRegression.js";
...
export async function recomputeBusinessClassification(options) {
    ...
    let query = client
        .from("brands")
        .select("id, normalized_domain, business_type, business_type_reasoning, business_type_confidence, is_ecommerce, retailer_scale_score, business_maturity_score, lead_eligible, eligibility_status, manual_excluded, prequalified_prospect, business_classifier_version, last_crawled_at")
        .or(`business_classifier_version.is.null,business_classifier_version.neq.${BUSINESS_CLASSIFIER_VERSION}`);
    ...
    if (error)
        throw new Error(`brands load failed: ${error.message}`);
    const summary = {
        scanned: 0,
        corrected: 0,
        correctedTotal: 0,
    ...
    const { data: occurrences } = await client
        ...
        .limit(20000);
    const keywordSpread = new Map();
    const categorySpread = new Map();
    for (const occ of occurrences ?? []) {
        if (!occ.brand_id || !occ.keyword_id)
            continue;
        if (!keywordSpread.has(occ.brand_id))
            keywordSpread.set(occ.brand_id, new Set());
        ...
        if (!categorySpread.has(occ.brand_id))
            categorySpread.set(occ.brand_id, new Set());
        if (category)
            categorySpread.get(occ.brand_id).add(category);
    }
    for (const row of (data ?? [])) {
        ...
        const breadthVerdict = gate.reason === "mass_retail_breadth" || gate.reason === "mass_retail_scale"
            ? "MASS_RETAILER"
            : null;
        ...
        const fromRetiredLocaleRule = !structural &&
    ...
    for (const row of correctedRows ?? []) {
        ...
        if (summary.corrections.some((correction) => correction.domain === domain))
            continue;
        summary.corrections.push({
            domain,
            from: null,
...
async function main() {
    ...
    console.log(`  classifier-regressie: ${summary.classifierRegression.passed}/${summary.classifierRegression.total}`);
    for (const entry of summary.classifierRegression.cases) {
        console.log(`    ${entry.passed ? "ok " : "FOUT"} ${entry.label} → ${entry.verdict} (internationaal ${entry.internationalPresenceScore}, breedte ${entry.retailerBreadthScore})`);
    }
    ...
    if (summary.corrections.length > 0) {
        ...
        for (const correction of summary.corrections) {
            console.log(`  ${correction.domain}: ${correction.from ?? "onbekend"} → ${correction.to} (${correction.reason})`);
        }
    }
}
...
//# sourceMappingURL=recomputeBusinessClassification.js.map