...
import { buildAllowedClaims, } from "../services/outreach/allowedClaims.js";
function assert(cond, msg) {
    if (!cond)
        throw new Error(msg);
}
const claims = buildAllowedClaims({
    pageUrl: "https://huisdierspullen.nl/products/trixie-hondentuig-premium-touring-zwart-45-80x2-5-cm",
    ...
    findings: [
        {
            ...
            severity: "CRITICAL",
            evidence: "Both mobile and desktop screenshots show no visible price. JSON extraction confirms 'price: null'.",
...
assert(claims.some((c) => c.type === "OBSERVATION" && c.scope === "PAGE_SPECIFIC"), "price claim must be PAGE_SPECIFIC");
const good = {
...
const expanded = validateOutreachDraft({
    draft: {
        ...good,
        body: good.body.replace("Op de productpagina van het Trixie-tuig viel me op dat de prijs op mobiel niet direct duidelijk in beeld staat.", "Bij meerdere producten staat de prijs niet direct zichtbaar, ook niet als je naar beneden scrollt."),
    },
    auditType: "HIGH_CONFIDENCE_PRODUCT_TARGET",
    contactFirstName: null,
    productName: "Trixie",
...
assert(expanded.errors.some((e) => e.includes("scope_")), `expected scope error, got ${expanded.errors.join(",")}`);
...
//# sourceMappingURL=testClaimFidelity.js.map