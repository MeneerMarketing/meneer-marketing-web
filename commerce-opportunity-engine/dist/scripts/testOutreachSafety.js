...
assert(evaluateOutreachEligibility(baseEligible).eligible, "expected eligible");
assert(!evaluateOutreachEligibility({ ...baseEligible, doNotContact: true }).eligible, "DNC must block");
assert(!evaluateOutreachEligibility({ ...baseEligible, manualExcluded: true }).eligible, "manual excluded must block");
assert(!evaluateOutreachEligibility({
    ...baseEligible,
    recommendedProjectType: "DESIGN_UPGRADE",
    fullRebuildPotential: 25,
    pdpImprovementPotential: 35,
}).eligible, "strong site weak gap must block");
...
assert(sendBlocked.blockers.includes("OUTREACH_REAL_SEND_ENABLED=false"), "must cite real send lock");
const draft = {
...
//# sourceMappingURL=testOutreachSafety.js.map