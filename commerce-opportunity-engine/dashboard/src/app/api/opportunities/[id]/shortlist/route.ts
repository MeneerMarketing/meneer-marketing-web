...
import { setShortlist } from "@/lib/operatorActions";
...
export async function POST(
  ...
  try {
    ...
    await setShortlist(supabase, id, opp.brand_id, body.enabled);

    return NextResponse.json({ ok: true, enabled: body.enabled });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Shortlist update failed" },
      { status: 500 }
    );
  }
}
