...
export const FOCUSED_BRAND_GAP_FIRST_TARGET_V1 = "FOCUSED_BRAND_GAP_FIRST_V1" as const;
export const FOCUSED_BRAND_DISCOVERY_VERSION = "FOCUSED_BRAND_GAP_FIRST_V1" as const;
export const FOCUSED_BRAND_DISCOVERY_ROUTE = "focused_brand_pdp_gap_first" as const;
...
export const FOCUSED_BRAND_DASHBOARD_REPORT_PATH =
  "dashboard/src/preview/concepts/data/focused-brand-gap-rescore-report.json";

export function catalogBandForFocusedBrand(size: number | null): {
  score: number;
  label: string;
  penalty: number;
} {
  if (size == null) return { score: 50, label: "unknown", penalty: 0 };
  for (const band of FOCUSED_BRAND_CATALOG_BANDS) {
    if (size >= band.min && size <= band.max) {
      const penalty =
        size > 200 ? 28 : size > 100 ? 12 : size > 60 ? 4 : 0;
