...
export const M932_CALIBRATION_VERSION = "PROSPECT_CALIBRATION_V1" as const;

/** Branches under test in this calibration run. */
export const CALIBRATION_BRANCHES: ProductArchetypeId[] = [
  "BEAUTY_DEVICES",
  "SKINCARE_DEVICES",
  "PET_TECH",
  "SLEEP_COMFORT",
...
export const CALIBRATION_OPTIONAL_BRANCHES: ProductArchetypeId[] = [
  "RECOVERY_PRODUCTS",
  "PREMIUM_PET",
  "NICHE_HOME_COMFORT",
];
...
export function branchVerdict(prospectYieldPercent: number, specialistCount: number): BranchVerdict {
  if (specialistCount === 0) return "REJECT";
  if (prospectYieldPercent >= BRANCH_VERDICT_BANDS.strong) return "STRONG";
  if (prospectYieldPercent >= BRANCH_VERDICT_BANDS.promising) return "PROMISING";
  if (prospectYieldPercent >= BRANCH_VERDICT_BANDS.weak) return "WEAK";
  return "REJECT";
}
