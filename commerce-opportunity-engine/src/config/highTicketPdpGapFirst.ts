...
export const M982_DISCOVERY_VERSION = "HIGH_TICKET_PDP_GAP_FIRST_V1" as const;
export const M982_DISCOVERY_ROUTE = "high_ticket_pdp_gap_first" as const;
export const HIGH_TICKET_GAP_FIRST_TARGET_V1 = "HIGH_TICKET_GAP_FIRST_TARGET_V1" as const;
...
export const M982_PRODUCT_QUERIES: M982ProductQuery[] = [
  {
...
export function buildM982HarvestQueries(): M982ProductQuery[] {
  return M982_PRODUCT_QUERIES.slice(0, M982_DISCOVERY.maxSourceQueries);
}

export const M983_DISCOVERY_VERSION = "HIGH_TICKET_GAP_COMPLETION_V1" as const;
export const HAIR_SCALP_M983_STATUS = "CALIBRATED_PURCHASE_HEAVY_LOW_VISUAL" as const;
...
export function inferExecutedQueriesFromCount(count: number): string[] {
  return M982_PRODUCT_QUERIES.slice(0, count).map((q) => q.query);
}

export function buildM983CompletionQueries(executedQueries: Set<string>): M982ProductQuery[] {
  return M982_PRODUCT_QUERIES.filter(
    (q) =>
      !executedQueries.has(q.query) &&
