...
export const M98_DISCOVERY_VERSION = "PDP_GAP_FIRST_V1" as const;
export const M98_DISCOVERY_ROUTE = "pdp_gap_first" as const;

export type PdpHarvestSourceType =
  | "ORGANIC_PRODUCT_RESULT"
  | "SHOPPING_PRODUCT_RESULT"
  | "SPECIALIST_PRODUCT_RESULT";
...
export const M98_PRODUCT_FAMILIES: M98ProductFamily[] = [
  {
    id: "HAIR_SCALP_TECH",
...
export function buildM98HarvestQueries(): Array<{
  query: string;
  familyId: string;
  familyLabel: string;
  archetypeId: string;
  ...
  for (const family of M98_PRODUCT_FAMILIES) {
    for (const query of family.queries) {
      ...
      if (rows.length >= M98_DISCOVERY.maxSourceQueries) return rows;
    }
  }
  return rows;
}
