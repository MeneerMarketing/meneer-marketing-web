...
export const M934 = {
  ...
  excludedFromDesignTarget: ["tensfact.com"] as const,
} as const;
...
export const TRUE_SALES_DESIGN_FORMULA =
  "true_sales_design = 0.40*concept_contrast + 0.32*outreach_concept_fit + 0.18*project_economic_fit + 0.10*measurement_confidence. " +
...
export function catalogSweetSpotScore(size: number | null): {
  score: number;
  label: string;
} {
...
export function commerceModelFromOwnBrandSignal(signal: number | null): string {
  if (signal == null) return "UNKNOWN";
  for (const band of OWN_BRAND_MODEL_BANDS) {
    if (signal >= band.min) return band.model;
  }
