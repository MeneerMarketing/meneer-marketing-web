...
export const COPY_STYLES = ["SOFT_OBSERVATION", "DIRECT_IDEA"] as const;
...
function isStructuralSentence(sentence: string): boolean {
  if (/^(hallo|hoi|groet|beste)\b/i.test(sentence)) return true;
  if (/^meneer marketing$/i.test(sentence)) return true;
  if (/meneermarketing\.nl/i.test(sentence)) return true;
  if (/^kvk\b/i.test(sentence)) return true;
  if (/ik help vanuit meneer marketing/i.test(sentence)) return true;
  if (/ik help (webshops|bedrijven)/i.test(sentence)) return true;
  if (/ik werk inmiddels/i.test(sentence)) return true;
  if (/als je wilt/i.test(sentence)) return true;
  if (/als het interessant klinkt/i.test(sentence)) return true;
  if (/ben benieuwd hoe jullie/i.test(sentence)) return true;
  if (/ik heb daar een idee voor/i.test(sentence)) return true;
  if (/ik kwam .+ tegen/i.test(sentence)) return true;
  if (/heb (jullie|de) (shop|website|webshop) even bekeken/i.test(sentence))
    return true;
  return false;
}

function matchClaimEvidence(
  sentence: string,
  claims: AllowedClaim[]
  ...
  for (const claim of claims) {
    ...
    const hitSubject = subjectBits.some(
      (b) => b.length > 4 && lower.includes(b)
    );
    const hitTheme =
      (/prijs|price/.test(lower) && /prijs|price/.test(factBits)) ||
...
export function validateOutreachDraft(input: {
  ...
  if (wordCount > 140) errors.push(`body_too_long:${wordCount}_words_max_140`);
  if (wordCount < 50) errors.push(`body_too_short:${wordCount}_words`);

  if (FAKE_RE.test(subject.trim())) errors.push("fake_re_fwd_subject");
  if (EMOJI.test(subject) || EMOJI.test(body)) errors.push("emoji_not_allowed");
  ...
  for (const c of SALES_CLICHES) {
    if (c.test(body)) errors.push(`sales_cliche:${c.source}`);
  }
  ...
  if (
    /jullie actief zichtbaar zijn via google/i.test(body) &&
  ...
  for (const rule of UNSUPPORTED_OUTCOMES) {
    if (rule.re.test(body)) {
      ...
      if (
        rule.code === "invented_availability_causality" &&
  ...
  if (
    input.draft.personalization_used.first_name &&
  ...
  if (
    input.findingTitles.length &&
    !input.findingTitles.some(
      (t) =>
        t === input.draft.selected_finding_title ||
  ...
  if (
    input.draft.selected_strength_title &&
    input.strengthTitles.length > 0 &&
    !input.strengthTitles.some(
      (t) =>
        t.toLowerCase() ===
  ...
  if (
    /jullie (kleuren)?selectie werkt (overal |overal erg )?/i.test(body) ||
  ...
  const negativeHits = (
    body.match(
      /viel me op|opgevallen|niet (direct )?duidelijk|niet (direct )?zichtbaar|mist|ontbreekt/gi
    ) ?? []
  ).length;
  if (negativeHits > 2) {
  ...
  const sentenceEvidence: SentenceEvidenceRow[] = sentences.map((sentence) => {
