...
export function renderOutreachHtml(input: OutreachHtmlInput): string {
  ...
  const kvk =
    input.kvkNumber && input.kvkNumber.trim()
...
export function appendTextSignature(
  ...
  if (/meneermarketing\.nl/i.test(trimmed)) return trimmed;
  const kvk =
    input.kvkNumber && input.kvkNumber.trim()
