...
import {
  classifyEmailType,
  emailDomainMatchesBrand,
  emailTypePreferenceRank,
  isUsableForOutreach,
  normalizeEmail,
...
function isPlausibleEmail(email: string): boolean {
  ...
  if (!local || !domainPart) return false;
  if (local.length < 2 || domainPart.length < 4) return false;
  if (!domainPart.includes(".")) return false;
  const tld = domainPart.split(".").pop() ?? "";
  if (!/^[a-z]{2,10}$/.test(tld)) return false;
  if (
    ["css", "js", "jpg", "jpeg", "png", "gif", "svg", "webp", "woff", "map"].includes(
      tld
    )
  ) {
    return false;
...
async function fetchHtml(
  url: string,
  timeoutMs: number
): Promise<{ ok: boolean; html: string; finalUrl: string; status: number }> {
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: {
        "User-Agent":
          "Mozilla/5.0 (compatible; MeneerMarketingBot/1.0; +https://meneermarketing.nl)",
...
function extractSocial($: cheerio.CheerioAPI, host: string): {
  ...
  $("a[href]").each((_, el) => {
    ...
    if (!instagram && /instagram\.com/i.test(href)) instagram = href;
  });
  void host;
  return { linkedin, instagram };
}

function guessNameNearEmail(
  text: string,
  email: string
): { fullName: string | null; jobTitle: string | null } {
  const idx = text.toLowerCase().indexOf(email.toLowerCase());
  if (idx < 0) return { fullName: null, jobTitle: null };
  ...
  const nameMatch = window.match(
    /([A-ZÁÉÍÓÚÄËÏÖÜÀÂÊÎÔÛ][a-záéíóúäëïöüàâêîôû-]{1,20}(?:\s+[A-ZÁÉÍÓÚÄËÏÖÜÀÂÊÎÔÛ][a-záéíóúäëïöüàâêîôû-]{1,20}){0,2})\s*[-–:]?\s*$/
  );
  const fullName = nameMatch?.[1]?.trim() ?? null;
  // Reject likely nav/category blobs
  if (
    fullName &&
    /(brokjes|voer|product|populair|categorie|menu|home|shop|korting)/i.test(
      fullName
    )
  ) {
...
function scoreContact(input: {
...
}): { emailConfidence: number; contactConfidence: number } {
  ...
  if (input.hasName) contactConfidence += 15;
  if (input.hasTitle) contactConfidence += 12;
...
export async function discoverBrandContacts(input: {
  ...
  for (const origin of origins) {
    ...
    for (const path of paths) {
      ...
      $('a[href^="mailto:"]').each((_, el) => {
        ...
        if (href) mailtoEmails.add(normalizeEmail(href));
      });
      ...
      for (const email of found) {
        ...
        const candidate: DiscoveredContact = {
  ...
  let contactStatus: ContactStatus = "NOT_FOUND";
...
export function selectPreferredContact(contacts: DiscoveredContact[]): {
  contact: DiscoveredContact | null;
  reason: string | null;
} {
  const usable = contacts.filter(
    (c) => c.isUsableForOutreach && c.emailType !== "PRIVACY_LEGAL"
  );
  if (!usable.length) {
  ...
  const ranked = [...usable].sort((a, b) => {
    const titleBoost = (c: DiscoveredContact) =>
      c.jobTitle && TITLE_HINTS.some((h) => c.jobTitle!.toLowerCase().includes(h))
        ? 20
        : 0;
    const scoreA =
      emailTypePreferenceRank(a.emailType) +
      a.contactConfidence +
      titleBoost(a) +
      (a.firstName ? 10 : 0);
    const scoreB =
      emailTypePreferenceRank(b.emailType) +
      b.contactConfidence +
      titleBoost(b) +
      (b.firstName ? 10 : 0);
