...
export type OutreachBlockReason =
  | "brand_manually_excluded"
  | "brand_do_not_contact"
  | "brand_eligibility_excluded"
  | "brand_not_lead_eligible"
  | "opportunity_rejected"
  | "opportunity_excluded"
  | "outreach_eligible_false"
  | "suppression"
  | "contact_unusable";
...
export function detectBlockReasons(
  snap: BrandSafetySnapshot,
  suppressed: boolean
): OutreachBlockReason[] {
  const reasons: OutreachBlockReason[] = [];
  if (snap.manualExcluded) reasons.push("brand_manually_excluded");
  if (snap.doNotContact) reasons.push("brand_do_not_contact");
  if ((snap.eligibilityStatus ?? "").toUpperCase() === "EXCLUDED") {
    reasons.push("brand_eligibility_excluded");
  }
  if (snap.leadEligible === false) reasons.push("brand_not_lead_eligible");
  const opp = (snap.opportunityStatus ?? "").toUpperCase();
  if (opp === "REJECTED") reasons.push("opportunity_rejected");
  if (opp === "EXCLUDED") reasons.push("opportunity_excluded");
  if (snap.outreachEligible === false) reasons.push("outreach_eligible_false");
  if (suppressed) reasons.push("suppression");
  if (snap.contactEmail && !snap.contactUsable) reasons.push("contact_unusable");
  return reasons;
}

export function humanBlockReason(reasons: OutreachBlockReason[]): string {
  const map: Record<OutreachBlockReason, string> = {
...
export async function syncOutreachMessagesForBrand(
  ...
  const { data: messages, error } = await client
    ...
    .select(
      `id, status, opportunity_id, contact_id, approved_at,
  ...
  for (const msg of messages ?? []) {
    ...
    const snap: BrandSafetySnapshot = {
      brandId,
      domain,
      ...
      outreachEligible:
        opportunity?.outreach_eligible === null ||
        opportunity?.outreach_eligible === undefined
          ? null
          : Boolean(opportunity.outreach_eligible),
      opportunityStatus: (opportunity?.status as string | null) ?? null,
    ...
    const { error: updErr } = await client
      ...
      .update({
        ...
        approval_revoked_at:
          wasApproved || alreadyRevoked
            ? (msg as { approval_revoked_at?: string | null }).approval_revoked_at ??
              now
            : null,
        approved_at: null,
        approved_by: null,
...
export async function syncAllUnsafeOutreachMessages(
  client: SupabaseClient
): Promise<SyncResult[]> {
