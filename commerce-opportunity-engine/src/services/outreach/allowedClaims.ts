...
function inferScopeFromEvidence(evidence: string): EvidenceScope {
  const e = evidence.toLowerCase();
  if (
    /across (the )?site|site[- ]wide|multiple pages|meerdere pagina|door de hele/i.test(
      e
    )
  ) {
    return "SITE_WIDE";
...
function observationExternalNl(
...
function strengthExternalNl(strength: StrengthInput): string {
...
function observationAllowedFact(finding: FindingInput): string {
  const title = finding.title.toLowerCase();
...
function strengthAllowedFact(strength: StrengthInput): string {
  const title = strength.title.toLowerCase();
  if (/variant|color|kleur/i.test(title)) {
...
function pageSpecificForbiddenExpansions(): string[] {
  return [
    "meerdere producten",
    "productpagina's",
    "overal",
 ...
 */
export function buildAllowedClaims(input: BuildClaimsInput): AllowedClaim[] {
  const supportedOnly = input.supportedOnly !== false;
  const claims: AllowedClaim[] = [];

  for (const finding of input.findings) {
    const status = (finding.validationStatus ?? "SUPPORTED").toUpperCase();
    if (supportedOnly && status !== "SUPPORTED") continue;
    if (!supportedOnly && status === "REJECTED") continue;
    ...
    claims.push({
      ...
      forbidden_expansions:
        scope === "PAGE_SPECIFIC" ? pageSpecificForbiddenExpansions() : [],
...
export function pickPrimaryObservation(
  ...
  return (
    observations.find((c) => /price|prijs/i.test(c.subject)) ??
    observations[0] ??
    null
  );
}

export function pickPrimaryStrength(
  claims: AllowedClaim[]
): AllowedClaim | null {
  const strengths = claims.filter((c) => c.type === "STRENGTH");
  if (!strengths.length) return null;
  return (
    strengths.find((c) => /variant|color|kleur/i.test(c.subject)) ??
    strengths[0] ??
    null
  );
}
...
export function attachValidationStatus(
  findings: Array<{
    id: string;
    title: string;
    severity: string;
  ...
  return findings.map((f) => {
    const match = vals.find(
      (v) =>
        v &&
        typeof v === "object" &&
        String((v as Record<string, unknown>).title ?? "") === f.title
