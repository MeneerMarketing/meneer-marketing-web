...
import {
  ...
  estimateAnthropicCostFromUsage,
} from "./anthropicBudget.js";
...
export async function generatePersonalisationSnippets(input: {
  ...
  const gate = (() => {
    try {
      ...
      const e = err as Error & { code?: string; gate?: { reason?: string } };
...
{"observation":"...","strength":"..."|null}`;
  ...
  let observation = validated.data.observation
    ...
    .trim();
  let strength =
    validated.data.strength?.replace(/\s*[\u2014\u2013]\s*/g, ". ").trim() ||
    null;
  if (!input.strength) strength = null;

  // Fidelity gate via existing validator on a synthetic mini-body
  const miniDraft: OutreachDraftAi = {
...
function parseJson(text: string): unknown {
  ...
  if (!candidate) throw new Error("No JSON in personalisation response");
