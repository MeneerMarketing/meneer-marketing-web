...
export function evaluateOutreachEligibility(
  ...
  if (input.firstTouchSent) blockers.push("first_touch_already_sent");

  const biz = (input.businessType ?? "").toUpperCase();
  if (
    !biz ||
    !(t.relevantBusinessTypes as readonly string[]).includes(biz)
  ) {
  ...
  if (!input.auditValid) blockers.push("audit_invalid");

  if ((input.auditConfidence ?? 0) < t.minAuditConfidence) {
    blockers.push(
      `audit_confidence_below_${t.minAuditConfidence}:${input.auditConfidence ?? 0}`
    );
  }
  if (input.supportedFindingsCount < t.minSupportedFindings) {
    blockers.push(
      `supported_findings_below_${t.minSupportedFindings}:${input.supportedFindingsCount}`
    );
  }
  ...
  if (
    input.recommendedProjectType &&
    (t.blockedProjectTypes as readonly string[]).includes(
      input.recommendedProjectType
    )
  ) {
  ...
  if (
    input.recommendedProjectType === "DESIGN_UPGRADE" &&
  ...
  if (!input.websiteReachable) blockers.push("website_unreachable");
  ...
  if (
    ...
    blockers.push(
      `opportunity_score_too_low_without_strong_fit:${input.opportunityScore}`
    );
  }
