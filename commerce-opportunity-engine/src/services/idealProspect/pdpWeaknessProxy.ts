...
export function computeCurrentPdpWeaknessProxy(input: PdpWeaknessProxyInput): {
  score: number;
  evidence: string[];
} {
  ...
  if (input.signals) {
    ...
    if (!s.hasAddToCart && !input.hasAddToCart) weakness += 4;
...
export function pdpWeaknessSignalsFromHtml(
  html: string,
  _url: string,
  platform: string | null
): PdpWeaknessProxyInput {
  ...
  return {
    ...
    hasAddToCart:
      /add-to-cart|in winkelwagen|toevoegen aan|add to cart/i.test(lower),
