...
import {
  estimateKeywordIdeasCost,
  fetchKeywordIdeas,
  type KeywordIdeaItem,
} from "../dataforseo/keywordIdeas.js";
...
import {
  KEYWORD_LOCALE,
  ensureKeywordCategories,
  loadActiveCategorySeeds,
  upsertKeywordIntelligenceRows,
  type KeywordIntelligenceRow,
} from "../supabase/keywordIntelligenceRepository.js";
...
export interface KeywordEngineRunResult {
  ...
  rejectedSamples: Array<{ keyword: string; reason: string }>;
  stoppedReason: string | null;
}

function pickBestSeed(keyword: string, seeds: string[]): string {
...
function mapIdeaToRow(input: {
  item: KeywordIdeaItem;
  categoryId: string;
  seeds: string[];
  estimatedSerpCost: number;
  now: string;
}): { row: KeywordIntelligenceRow; rejectedReason: string | null } {
  ...
  let status: KeywordIntelligenceRow["discovery_status"] = "DISCOVERED";
  let active = true;
  let rejected = false;
...
function dedupeIdeas(items: KeywordIdeaItem[]): KeywordIdeaItem[] {
  const byKey = new Map<string, KeywordIdeaItem>();
...
export async function runKeywordGeneration(input: {
  ...
  const rows: KeywordIntelligenceRow[] = [];
  ...
  if (rejectedSamples.length < 10) {
    for (const row of rows) {
      ...
      if (rejectedSamples.length >= 10) break;
    }
  }

  const { upserted, skippedManual } = await upsertKeywordIntelligenceRows(
    input.supabase,
    rows
  );
...
export function detectKeywordQualityProblems(result: KeywordEngineRunResult): string[] {
