import {
  DISCOVERY_PRIORITY_NEUTRAL,
  DISCOVERY_PRIORITY_WEIGHTS,
  RETAILER_RATIO_PENALTY,
...
export function scoreDiscoveryPriority(input: DiscoveryPriorityInput): {
  score: number;
  ...
  const yieldScore =
    input.historicalYield === null || input.historicalYield === undefined
  ...
  const categoryRelevance =
    input.categoryRelevance === null || input.categoryRelevance === undefined
      ? 55
      : clamp01to100(input.categoryRelevance);

  let score =
    prospecting * DISCOVERY_PRIORITY_WEIGHTS.prospectingValue +
