...
import {
  CONTROLLED_SCALE_CATEGORIES,
  CONTROLLED_SCALE_CATEGORY_QUOTAS,
  CONTROLLED_SCALE_DEFAULTS,
  type ControlledScaleCategory,
...
export async function selectControlledScaleKeywords(
  ...
  const maxBrandedShare =
    options?.maxBrandedShare ?? CONTROLLED_SCALE_DEFAULTS.maxProductBrandedShare;
  ...
  if (error) throw new Error(`Failed to load keywords for selection: ${error.message}`);

  const scored: SelectableKeyword[] = [];
  ...
  for (const row of data ?? []) {
    ...
    const relevance =
      row.category_relevance_score != null ? Number(row.category_relevance_score) : null;
  ...
  const nearDupes = new Set<string>();
  let brandedCount = 0;
  const maxBranded = Math.floor(maxKeywords * maxBrandedShare);

  const tryAdd = (row: SelectableKeyword, respectQuota: boolean): boolean => {
    if (selected.length >= maxKeywords) return false;
    if (selected.some((s) => s.id === row.id)) return false;

    const cat = row.category as ControlledScaleCategory;
    const quota = CONTROLLED_SCALE_CATEGORY_QUOTAS[cat] ?? 15;
    if (respectQuota && (byCategory[cat] ?? 0) >= quota) return false;

    const clusterKey = `${row.category}:${row.cluster ?? "GENERAL"}`;
    if ((byCluster[clusterKey] ?? 0) >= maxCluster) return false;

    const nd = nearDupeKey(row.keyword);
    if (nd && nearDupes.has(nd)) return false;

    const isBranded = row.keyword_intent_type === "PRODUCT_BRANDED";
    if (isBranded && brandedCount >= maxBranded) return false;

    // PRIMARY non-branded first preference already via sort + filters
    if (row.keyword_intent_type === "RETAILER_BRANDED") return false;
    if (row.keyword_intent_type === "REVIEW_RESEARCH") return false;

    selected.push(row);
    byCategory[row.category] = (byCategory[row.category] ?? 0) + 1;
    byCluster[clusterKey] = (byCluster[clusterKey] ?? 0) + 1;
    if (nd) nearDupes.add(nd);
    if (isBranded) brandedCount += 1;
    return true;
  };

  // Pass 1: PRIMARY non-branded with quotas
  for (const row of scored) {
    if (row.prospecting_tier !== "PRIMARY") continue;
    if (row.keyword_intent_type !== "NON_BRANDED_PRODUCT") continue;
    tryAdd(row, true);
  }
  ...
  const avgProspecting =
    selected.length === 0
      ? 0
      : Math.round(
          selected.reduce((s, k) => s + (k.prospecting_value_score ?? 0), 0) / selected.length
        );
  const avgPriority =
    selected.length === 0
      ? 0
      : Math.round(
          selected.reduce((s, k) => s + k.discovery_priority_score, 0) / selected.length
        );
...
export function ensureCategoryList(): ControlledScaleCategory[] {
