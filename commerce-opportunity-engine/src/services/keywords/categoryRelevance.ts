import {
  CATEGORY_RELEVANCE_CONFIG,
  PETS_PRODUCT_CONTEXT_TOKENS,
} from "../../config/categoryRelevance.js";
...
export function scoreCategoryRelevance(
  ...
  const hasRequireStem = config.requireAllowWhenTokens.some((t) =>
    tokens.includes(normalizeKeyword(t))
  );
  ...
  if (categoryId === "PETS") {
    ...
    const hasProduct = PETS_PRODUCT_CONTEXT_TOKENS.some(
      (t) => tokens.includes(t) || normalized.includes(t)
    );
