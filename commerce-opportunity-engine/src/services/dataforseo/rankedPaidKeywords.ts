...
export async function fetchPaidRankedKeywords(input: {
  ...
  if (data.status_code && data.status_code !== 20000) {
    throw new Error(
      `DataForSEO Labs error: ${data.status_message ?? data.status_code} (cost $${cost})`
    );
  }
  if (task?.status_code && task.status_code !== 20000) {
    throw new Error(
      `DataForSEO Labs task error: ${task.status_message ?? task.status_code} (cost $${cost})`
    );
  }

  const result = task?.result?.[0];
  const items: LabsPaidKeywordItem[] = [];
