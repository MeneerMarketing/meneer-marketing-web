...
export function scoreCatalogIntelligence(
  input: CatalogIntelligenceInput
  ...
  let estimatedCategories: number | null = null;
  let estimatedBrands: number | null = null;
  let confidence = 35;
  ...
  if (input.distinctProductBrandsOnPages > 0) {
    estimatedBrands = Math.max(
      estimatedBrands ?? 0,
      input.distinctProductBrandsOnPages
    );
  ...
  let focus = 55;
  if (biz === "BRAND" || biz === "DTC") focus += 25;
  if (biz === "SPECIALIST_WEBSHOP") focus += 12;
  if (biz === "GENERAL_RETAILER") focus -= 35;
  if (biz === "MARKETPLACE") focus -= 45;
  if (rel === "OWN_BRAND") focus += 15;
  if (rel === "RESELLER_PRODUCT") focus -= 8;
  if (scale >= 70) focus -= 20;
  else if (scale >= 55) focus -= 10;
  else if (scale <= 30) focus += 10;
  // Large specialist retailers mis-tagged with low scale
  if (
    biz === "SPECIALIST_WEBSHOP" &&
  ...
  if ((estimatedBrands ?? 1) <= 2) focus += 10;
  if ((estimatedBrands ?? 99) >= 20) focus -= 15;
  if ((estimatedProducts ?? 9999) <= 100) focus += 8;
  if ((estimatedProducts ?? 0) >= 3000) focus -= 18;
  if (input.hasProductPage) focus += 3;

  focus = clamp(focus);

  let tier: CatalogSizeTier = "UNKNOWN";
  if (estimatedProducts != null) {
    if (estimatedProducts <= 20) tier = "MICRO";
    else if (estimatedProducts <= 80) tier = "SMALL";
    else if (estimatedProducts <= 250 && focus >= 60) tier = "FOCUSED";
    else if (estimatedProducts <= 600) tier = "MEDIUM";
    else if (estimatedProducts <= 3000) tier = "LARGE";
    else tier = "MASSIVE";
