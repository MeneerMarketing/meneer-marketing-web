...
function isUglyFramework(hex: string): boolean {
  const h = hex.toLowerCase();
  if (
    ["#28a745", "#198754", "#22c55e", "#16a34a", "#4ade80", "#0d6efd", "#0dcaf0"].includes(
      h
    )
  ) {
    return true;
  ...
  // success greens
  if (rgb.g > rgb.r + 40 && rgb.g > rgb.b + 40 && sat > 0.45 && max > 140) return true;
  return false;
}

function pickDarkPrimary(colors: string[]): string {
  const scored = colors
...
function pickBrandAccent(preferred: string[] | undefined, colors: string[]): string {
  const pool = [...(preferred ?? []), ...colors].filter((c) => /^#([0-9a-f]{6})$/i.test(c));
  for (const hex of pool) {
    if (isUglyFramework(hex)) continue;
    const rgb = hexToRgb(hex);
    if (!rgb) continue;
...
export function resolveProspectTheme(input: {
  ...
  const brandAccent = pickBrandAccent(
    input.brandAccentCandidates ?? undefined,
    sourceColors
  );
  const fonts = (input.fontCandidates ?? []).filter(
    (f) => !/arial|helvetica|sans-serif|system-ui|inherit/i.test(f)
  );
