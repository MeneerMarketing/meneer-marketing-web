...
export function evaluateOutreachCandidateGate(input: OutreachGateInput): {
  eligible: boolean;
  ...
  if (row.manual_excluded) blocked.push("manual_excluded");
  if (row.do_not_contact) blocked.push("do_not_contact");
  if (row.status !== "BRIEF_READY") blocked.push(`status_not_BRIEF_READY:${row.status}`);
  if (row.suggested_template_family !== "PREMIUM_DTC") {
    blocked.push("template_not_PREMIUM_DTC");
  }
  if (!row.primary_concept_product_url) blocked.push("missing_product_url");
  if (!row.primary_concept_product_title) blocked.push("missing_product_title");
  ...
  if (c.conceptContrast < OUTREACH_CONTRAST_GATE.minConceptContrast) {
    blocked.push(
      `insufficient_concept_contrast:${c.conceptContrast}:${input.outreach.contrast.band}`
    );
  }
  if (c.deepDivePdpFit < t.minDeepDivePdpFit) blocked.push("insufficient_deep_dive_fit");
  ...
  if (c.catalogFocus < t.minCatalogFocus) blocked.push("insufficient_catalog_focus");

  if (OUTREACH_PILOT_GATE.preferAudited) {
    if (input.outreach.croDataSource !== "AUDITED") {
      if (
        input.outreach.outreachScoreConfidence <
