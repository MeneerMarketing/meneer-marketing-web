...
import {
  normaliseBenefitsFromSource,
  parseSocialProof,
  parseTrustItems,
  type NormalisedBenefit,
  type NormalisedTrustItem,
} from "./contentNormalisation.js";
...
function classifyImage(url: string, alt: string | null, index = 0): MediaKind {
  const u = url.toLowerCase();
  const a = (alt ?? "").toLowerCase();
  const blob = `${u} ${a}`;
  if (/logo|brand/.test(blob)) return "logo";
  if (/vergelijk|tabel|table|chart|matrix|spec_/.test(blob)) return "comparison";
  if (/app|phone|screen|smartphone|iphone|android/.test(blob)) return "app_screen";
  if (/full_hd_feeder|feeder\.png/.test(u)) return "packshot";
  if (/lifestyle|life-style|scene|outdoor|home/.test(blob)) return "lifestyle";
  if (/use|away|stress|voedertijd|kat|hond|cat|dog/.test(a)) return "use_case";
  if (/detail|close|zoom/.test(blob)) return "detail";
  // Shopify PDP gallery without strong alt: spread editorial roles by order
  if (/cdn\.shopify\.com\/.*\/files\//.test(u)) {
    const roles: MediaKind[] = [
      "packshot",
      "feature_graphic",
      "use_case",
      "lifestyle",
...
function extractNavLabels($: cheerio.CheerioAPI): string[] {
  const out: string[] = [];
  const seen = new Set<string>();

  const push = (raw: string) => {
    const label = raw.replace(/\s+/g, " ").trim();
    if (label.length < 3 || label.length > 28) return;
    if (
      /inloggen|account|winkelwagen|cart|zoeken|search|menu|home|skip|overslaan|volg ons/i.test(
        label
      )
    ) {
      return;
    }
    if (/^(nl|en|de|fr|€|\d+)$/i.test(label)) return;
...
function extractColorsFromCss(html: string): string[] {
  ...
  for (const h of hex.slice(0, 120)) colors.add(h.toLowerCase());
  return [...colors].slice(0, 40);
}

function extractOrangeCandidates(colors: string[]): string[] {
  const out: string[] = [];
...
function extractFonts(html: string): string[] {
  ...
  for (const f of family.slice(0, 30)) {
    const cleaned = f
      ...
      ?.replace(/['"]/g, "")
      .trim();
    if (cleaned && cleaned.length > 1 && cleaned.length < 40) fonts.add(cleaned);
  }
...
function extractLogo($: cheerio.CheerioAPI, base: string): string | null {
  ...
  for (const c of candidates) {
    ...
    if (abs && !/favicon|icon-/.test(abs)) return abs;
  }
...
function extractFaq($: cheerio.CheerioAPI): Array<{ question: string; answer: string }> {
  ...
  $("details").each((_, el) => {
    ...
    const a = $(el)
      ...
      .replace(/\s+/g, " ")
      .trim();
    if (q.length > 8 && a.length > 12) faqs.push({ question: q, answer: a.slice(0, 600) });
  });
  $('script[type="application/ld+json"]').each((_, el) => {
    try {
      ...
      for (const node of nodes) {
        if (!String(node["@type"] ?? "").toLowerCase().includes("faq")) continue;
        const ents = node.mainEntity as Array<Record<string, unknown>> | undefined;
        if (!Array.isArray(ents)) continue;
        for (const e of ents) {
          const q = typeof e.name === "string" ? e.name : null;
          const ans = e.acceptedAnswer as Record<string, unknown> | undefined;
...
function walkJsonLd(
  ...
  if (Array.isArray(node)) {
    for (const n of node) walkJsonLd(n, visit, seen);
    return;
  }
  const obj = node as Record<string, unknown>;
  visit(obj);
  for (const v of Object.values(obj)) {
    if (v && typeof v === "object") walkJsonLd(v, visit, seen);
  }
}

function extractAggregateRating($: cheerio.CheerioAPI): {
  rating: number | null;
  reviewCount: number | null;
} {
  ...
  $('script[type="application/ld+json"]').each((_, el) => {
    try {
      ...
      walkJsonLd(data, (obj) => {
        ...
        if (!agg || rating != null) return;
        const v = agg.ratingValue;
        const c = agg.reviewCount ?? agg.ratingCount;
        if (typeof v === "number") rating = v;
        else if (typeof v === "string") rating = Number(v.replace(",", "."));
        if (typeof c === "number") reviewCount = c;
        else if (typeof c === "string") reviewCount = Number(c.replace(/\D/g, ""));
      });
    } catch {
      /* ignore */
    }
  ...
  if (rating == null) {
    const tp =
      $("[data-rating], .trustpilot-widget, [class*='trustpilot']").attr(
        "data-rating"
      ) ||
...
function extractReviewsFromJsonLd(
  ...
  $('script[type="application/ld+json"]').each((_, el) => {
    try {
      ...
      const data = JSON.parse(raw) as unknown;
      walkJsonLd(data, (obj) => {
        const rev = obj.review as Array<Record<string, unknown>> | undefined;
        ...
        for (const r of rev.slice(0, 8)) {
          ...
          if (reviews.some((x) => x.text === text.slice(0, 500))) continue;
          const authorNode = r.author;
          let author: string | null = null;
          if (typeof authorNode === "string") author = authorNode;
          else if (authorNode && typeof authorNode === "object") {
            const n = (authorNode as Record<string, unknown>).name;
            if (typeof n === "string") author = n;
          }
          let rating: number | null = null;
          const rr = r.reviewRating as Record<string, unknown> | undefined;
          if (rr) {
            const v = rr.ratingValue;
            if (typeof v === "number") rating = v;
            else if (typeof v === "string") rating = Number(v);
          }
          reviews.push({
            author,
            rating: rating != null && Number.isFinite(rating) ? rating : null,
...
function extractImages($: cheerio.CheerioAPI, base: string): CrawledImage[] {
  const out: CrawledImage[] = [];
  const push = (src: string | undefined | null, alt: string | null) => {
    const url = absUrl(base, src);
    if (!url) return;
    if (!/\.(jpe?g|png|webp|gif)/i.test(url) && !url.includes("cdn.shopify")) return;
    if (/pixel|spacer|1x1|sprite|icon-|favicon/.test(url)) return;
  ...
  $("script").each((_, el) => {
    const html = $(el).html() || "";
    if (!html.includes("cdn.shopify.com") || html.length > 500_000) return;
    const matches = html.match(
      /https?:\/\/cdn\.shopify\.com\/[^"'\s]+\.(?:jpe?g|png|webp)/gi
    );
    if (!matches) return;
    for (const m of matches.slice(0, 24)) push(m, null);
  });

  const deduped = uniqUrls(out.map((i) => i.url)).map(
    (url) => out.find((i) => i.url.replace(/^http:/, "https:") === url || i.url === url)!
  );
...
function parseCompareAt($: cheerio.CheerioAPI): number | null {
  const text =
    $("s, .compare-at, .price--compare, [class*='compare']").first().text() || "";
...
function firstTrustLabel(
  items: NormalisedTrustItem[],
  match: RegExp
): string | null {
...
export async function crawlPilotAssets(input: {
  ...
  try {
    ...
    if (!productCrawl.html || productCrawl.html.length < 200) {
      throw new Error(
        `Product crawl failed: ${productCrawl.status} ${productCrawl.errorMessage ?? ""}`
      );
    }
    ...
    const domDesc = $(
      ".product__description, .product-single__description, [class*='product-description'], .rte"
    )
      .first()
      .text()
      .replace(/\s+/g, " ")
    ...
    if (homeHtml) {
      ...
      if (homeNav.length > navLabels.length) navLabels = homeNav;
    }
