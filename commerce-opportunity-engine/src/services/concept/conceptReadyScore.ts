...
import {
  CONCEPT_PLATFORM_FIT,
  CONCEPT_READY_WEIGHTS,
  CONCEPT_RETAILER_SCALE_PENALTY,
  CONCEPT_TRANSFORM_SOFT_CAPS,
  OWN_BRAND_FIT_BY_MODEL,
  conceptVerdictFromScore,
  type BrandCommerceModel,
  type ConceptVerdict,
  type RecommendedConceptType,
...
export function retailerScalePenalty(scale: number | null): number {
  if (scale == null) return 0;
  const { softStart, hardStart, maxPenalty } = CONCEPT_RETAILER_SCALE_PENALTY;
  if (scale < softStart) return 0;
  if (scale >= hardStart) {
...
export function megaRetailerFitPenalty(input: {
  ...
  if (input.businessMaturity < 80) return 0;
  if (input.ownBrandFit >= 70) return 0;
  let p = 0;
  if (input.platformFit <= 40) p += 10;
  if (input.catalogFocus < 55) p += 8;
  if (input.businessMaturity >= 85 && input.ownBrandFit <= 55) p += 10;
  return Math.min(22, p);
}

export function recommendConceptType(
  input: ConceptReadyInput,
  score: number
): RecommendedConceptType {
  if (score < 50) return "NOT_SUITABLE";

  const model = input.brandCommerceModel;
  const ownHeavy =
    model === "DTC_OWN_BRAND" || model === "MOSTLY_OWN_BRAND";
  ...
  if (score >= 55) return "FULL_PDP_REDESIGN";
  return "NOT_SUITABLE";
}

export function scoreConceptReady(input: ConceptReadyInput): ConceptReadyResult {
  const platformKey = (input.platform ?? "UNKNOWN").toUpperCase();
  const platformFit =
    CONCEPT_PLATFORM_FIT[platformKey] ?? CONCEPT_PLATFORM_FIT.UNKNOWN;
  ...
  const raw =
    components.mmOrPreFit * w.mmOrPreFit +
  ...
  let transformCapApplied: number | null = null;
  ...
  const recommended_concept_type = recommendConceptType(
    input,
    concept_ready_score
  );
  ...
  return {
    ...
    formula:
      "concept_ready = clamp(0,100, round(Σ(w_i×c_i) − retailer_scale_penalty − mega_retailer_penalty)); then CONCEPT_TRANSFORM_SOFT_CAPS; CRO not a direct weight",
  };
}
