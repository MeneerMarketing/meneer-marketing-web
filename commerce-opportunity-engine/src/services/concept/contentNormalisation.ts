...
export function compactProductTitle(sourceTitle: string): {
  displayTitle: string;
  ...
  const m = t.match(
    /^(.+?)\s+met\s+(HD\s*camera)\s+en\s+(app)\s+voor\s+(\d+\s+katten)/i
  );
  if (m) {
    return {
  ...
  if (t.length <= 48) return { displayTitle: t, subheadline: null };
 ...
 */
export function buildEditorialSubline(
  benefits: NormalisedBenefit[],
  fallback: string | null
): string | null {
  const has = (re: RegExp) => benefits.some((b) => re.test(b.title));
  ...
  if (schedule && camera) return "Automatisch voeren en meekijken vanaf je telefoon.";
  if (schedule && app) return "Automatisch voeren op de tijden die jij instelt.";
  if (camera && app) return "Meekijken en bedienen vanuit de app.";
  return fallback;
}
 ...
 */
export function buildStoryBody(benefits: NormalisedBenefit[]): string | null {
  const has = (re: RegExp) => benefits.some((b) => re.test(b.title));
  const lines: string[] = [];

  if (has(/schema/i)) {
    lines.push(
      "De voerbak verdeelt de maaltijden volgens het schema dat jij instelt."
    );
  }
  if (has(/app/i)) {
...
export function parseTrustItems(rawBody: string): NormalisedTrustItem[] {
  const text = rawBody.replace(/\s+/g, " ");
  const items: NormalisedTrustItem[] = [];
  const push = (label: string, provenance: string) => {
...
export function parseSocialProof(rawBody: string): {
...
export function buildFeatureDeepDive(
  benefits: NormalisedBenefit[],
  ...
  const pick = (kinds: string[]) =>
    images.find((i) => kinds.includes(i.kind))?.url ??
    images[0]?.url ??
    null;
...
export function cleanSnippet(raw: string | null | undefined, max = 70): string | null {
  if (!raw) return null;
  ...
  if (t.length > max) return null; // refuse ellipsis truncation for UI chips
  return t;
}
