...
export type PremiumDtcViewModel = {
  meta: {
    ...
    assetUsage: Array<{ section: string; assets: string[] }>;
...
export function buildPremiumDtcViewModel(input: {
  ...
  const assetUsage: Array<{ section: string; assets: string[] }> = [];

  const priceLabel =
    formatPrice(crawl.price ?? brief.price, crawl.currency ?? brief.currency) ||
    "Prijs op aanvraag";
  const compareAtLabel = formatPrice(
    crawl.compareAtPrice,
    crawl.currency ?? brief.currency
  );
  const discount =
    crawl.price != null && crawl.compareAtPrice != null
      ? discountPercent(crawl.price, crawl.compareAtPrice)
      : null;

  const titles = compactProductTitle(crawl.title || brief.product_title);
  const brandName = (crawl.brandName || brief.brand || brief.domain).replace(
    /\.com$/i,
    ""
  );
  ...
  const promoParts = [shippingItem?.label, klarnaItem?.label, returnsItem?.label].filter(
    Boolean
  ) as string[];
   ...
   */
  const photographic = crawl.images.filter((i) =>
    ["packshot", "lifestyle"].includes(i.kind)
  );
  const graphics = crawl.images.filter(
    (i) => !["packshot", "lifestyle", "logo"].includes(i.kind)
  );

  const featureCanvas = photographic[0]?.url ?? crawl.images[0]?.url ?? null;
  if (featureCanvas) claimed.add(featureCanvas);
  ...
  const storyAsset = graphics.find(
    (i) => !claimed.has(i.url) && i.kind !== "comparison"
  );
  if (storyAsset) claimed.add(storyAsset.url);
  const storyImage = storyAsset?.url ?? null;
  const storyFit: "cover" | "contain" =
    storyAsset && ["packshot", "lifestyle"].includes(storyAsset.kind)
      ? "cover"
      : "contain";
  ...
  const immersiveImage =
    photographic.find((i) => i.url !== featureCanvas)?.url ?? null;
  ...
  const activePlan: Array<{ section: string; content_source?: ContentSource }> = [];
  const push = (section: string, source: ContentSource) => {
  ...
  if (serviceItems.length) push("TRUST_BAR", "SOURCE_CONTENT");
  else omitted.push({ section: "TRUST_BAR", reason: "No parseable service facts" });

  if (crawl.benefits.length) push("BENEFIT_GRID", "DERIVED_COPY");
  else omitted.push({ section: "BENEFIT_GRID", reason: "No grounded benefits" });
  ...
  if (storyBody) {
    ...
    if (storyImage) assetUsage.push({ section: "PRODUCT_STORY", assets: [storyImage] });
  } else {
  ...
  if (cameraFact) {
    ...
    if (!immersiveImage) {
      omitted.push({
        ...
        reason:
          "No second photographic asset without baked-in marketing text; section stays typographic",
      });
    }
  }

  if (faqs.length) push("FAQ", "SOURCE_CONTENT");
  else omitted.push({ section: "FAQ", reason: "No FAQ in source" });

  push("FINAL_PURCHASE", "DERIVED_COPY");
  if (finalImage) assetUsage.push({ section: "FINAL_PURCHASE", assets: [finalImage] });
  push("STICKY_ATC", "DERIVED_COPY");

  for (const item of input.sectionPlan) {
    if (
      (item.section === "REVIEWS" || item.section === "TESTIMONIALS") &&
    ...
    if (
      (item.section === "HOW_TO_USE" || item.section === "HOW_IT_WORKS") &&
  ...
  const klarnaLabel =
    crawl.price != null && crawl.paymentSignals.some((p) => /klarna/i.test(p))
      ? `3× ${formatPrice(Math.round((crawl.price / 3) * 100) / 100, crawl.currency)} met Klarna`
      : null;

  const paymentMethods = (
    ["ideal", "visa", "mastercard", "klarna", "apple_pay", "paypal"] as const
  ).filter((id) =>
    crawl.paymentSignals.some((p) => new RegExp(id.replace("_", "[-_ ]?"), "i").test(p))
  );
  const paymentsFinal = paymentMethods.length
    ? paymentMethods
    : /klarna/i.test(crawl.rawBodyText)
      ? (["ideal", "klarna"] as const)
      : undefined;
  ...
  const navSource: "SOURCE_CONTENT" | "DERIVED_COPY" = navFromSource.length >= 3
    ? "SOURCE_CONTENT"
    : "DERIVED_COPY";
  const navLinks =
    navSource === "SOURCE_CONTENT"
