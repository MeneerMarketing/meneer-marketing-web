...
import {
  CRO_ALREADY_STRONG_FROM_QUALITY,
  CURRENT_PDP_QUALITY_BANDS,
...
export function computeCurrentPdpQualityScore(
  cro: CroQualityScores,
  conceptSignals?: ConceptFirstPdpSignals | null
): CurrentPdpQualityResult {
  const mobileDesktop =
    (num(cro.mobile_cro_quality) + num(cro.desktop_cro_quality)) / 2;

  const baseComponents = {
    buyblock: clamp(
      (num(cro.above_fold_quality) * 0.22 +
    ...
    deepDive: clamp(
      num(cro.product_presentation_quality) * 0.35 +
  ...
  let band = "ZEER_STERK";
  if (score <= CURRENT_PDP_QUALITY_BANDS.WEAK_MAX) band = "ZWAK";
  else if (score <= CURRENT_PDP_QUALITY_BANDS.MODERATE_MAX) band = "MATIG";
  else if (score <= CURRENT_PDP_QUALITY_BANDS.REASONABLE_MAX) band = "REDELIJK";
  else if (score <= CURRENT_PDP_QUALITY_BANDS.STRONG_MAX) band = "STERK";

  return { score, band, components: baseComponents };
}

export function croAlreadyStrongPenaltyFromQuality(qualityScore: number): number {
  for (const band of CRO_ALREADY_STRONG_FROM_QUALITY) {
    if (qualityScore >= band.min) return band.penalty;
  }
  return 0;
}
