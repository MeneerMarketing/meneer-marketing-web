...
} from "../crawler/platformDetector.js";
import {
  emptyProductPageResult,
  resolveProductCandidates,
  selectBestProductCandidate,
} from "../crawler/productPageFinder.js";
...
function isInvalidProductExtraction(product: ProductPageResult): boolean {
...
function failedResult(
  ...
  startUrl: string,
  finalUrl: string,
  status: QualificationResult["crawlStatus"],
  message: string
): QualificationResult {
  return {
...
export async function qualifyBrandCandidate(
  env: Env,
  candidate: BrandQualificationCandidate
  ...
  let secondarySignals: PageExtractedSignals[] = [];
  ...
  let business = classifyBusinessFromWebsite(
    candidate.normalizedDomain,
    mergeSignalViews(homepageSignals, secondarySignals),
    ecommerce
  );

  if (env.QUALIFICATION_HAIKU_FALLBACK_ENABLED && needsHaikuFallback(business)) {
    try {
      const haiku = await classifyBusinessWithHaiku(
        env,
        candidate.normalizedDomain,
        mergeSignalViews(homepageSignals, secondarySignals)
      );
    ...
    for (const tryCandidate of tryUrls) {
      try {
        ...
        if (productCrawl.status !== "success") {
          errors.push(
            `product_crawl_${productCrawl.status}: ${productCrawl.errorMessage ?? tryCandidate.url}`
          );
          continue;
        }
...
function mergeSignalViews(
  ...
  return {
    ...
    productUrlCount:
      primary.productUrlCount + secondary.reduce((n, s) => n + s.productUrlCount, 0),
    collectionUrlCount:
      primary.collectionUrlCount + secondary.reduce((n, s) => n + s.collectionUrlCount, 0),
    ...
    compareMentions: Math.max(
      ...
      ...secondary.map((s) => s.compareMentions)
    ),
    paymentSignals: [
