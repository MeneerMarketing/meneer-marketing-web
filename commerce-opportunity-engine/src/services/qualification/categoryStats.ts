...
export function aggregateUniqueCategoryMetrics(input: {
  ...
  for (const b of input.brandsById.values()) {
    ...
    if (b.confirmed_google_advertiser || b.transparency_confirmed) conf += 1;
  }
...
export async function computeCategoryUniqueStats(
  ...
  const out: Record<string, UniqueCategoryMetrics> = {};
  ...
  for (const cat of CONTROLLED_SCALE_CATEGORIES) {
    ...
    const { data: catAds, error } = await client
      ...
      .in(
        "keyword_id",
        kwIds.length ? kwIds : ["00000000-0000-0000-0000-000000000000"]
      );

    if (error) throw new Error(`Category stats load failed (${cat}): ${error.message}`);

    const brandsById = new Map<string, BrandLite>();
    for (const ad of catAds ?? []) {
      if (!ad.brand_id) continue;
      const raw = Array.isArray(ad.brands) ? ad.brands[0] : ad.brands;
      if (!raw || typeof raw !== "object") continue;
      const b = raw as BrandLite;
...
export function categoryStatsToJson(
  metrics: UniqueCategoryMetrics
