...
function collectUrlCorpus(input: ShoppingClassificationInput): string {
  const parts: string[] = [];
  ...
  if (listingRec?.url && typeof listingRec.url === "string") parts.push(listingRec.url);
  return parts.join("\n").toLowerCase();
}

function detectFreeListingEvidence(corpus: string): string[] {
  const hits: string[] = [];
  if (corpus.includes("utm_medium=free_shopping")) hits.push("utm_medium=free_shopping");
  if (corpus.includes("freelisting=yes") || corpus.includes("freelisting=true")) {
    hits.push("freelisting=yes");
  }
  if (corpus.includes("utm_medium=organic")) hits.push("utm_medium=organic");
  return hits;
}

function detectPaidEvidence(
  input: ShoppingClassificationInput,
  corpus: string
): string[] {
  const hits: string[] = [];
  const itemType = (input.merchantItemType ?? "").toLowerCase();

  if (itemType === "google_shopping_paid") hits.push("merchant_type:google_shopping_paid");
  ...
  if (input.shopAdAclk) hits.push("shop_ad_aclk_present");
...
function titleKeywordOverlap(keyword: string | null, title: string | null): number {
 ...
 */
export function classifyShoppingTarget(
  input: ShoppingClassificationInput
  ...
  const itemType =
    input.merchantItemType ??
    (asRecord(asRecord(input.rawPayload)?.listing)?.type as string | null) ??
    null;

  // Listing target confidence: product/seller/URL relationship strength
  let listingTargetConfidence = 0;
  if (hasLanding) listingTargetConfidence += 45;
  if (domainOk) listingTargetConfidence += 25;
  if (hasSeller) listingTargetConfidence += 15;
  if (hasProductId) listingTargetConfidence += 10;
  const overlap = titleKeywordOverlap(input.keyword, input.title);
  if (overlap >= 0.35) listingTargetConfidence += 5;
  ...
  if (paidEvidence.includes("shop_ad_aclk_present")) paidEvidenceConfidence += 50;
  if (paidEvidence.includes("url_param:ps-sl-shoppingads")) paidEvidenceConfidence += 35;
  if (paidEvidence.includes("url_param:gclid+shopping")) paidEvidenceConfidence += 20;
  ...
  let sourceType: PaidSourceType;
  let croReadinessLevel: CroReadinessLevel;
  ...
  if (!hasLanding || !domainOk) {
    ...
    if (domainOk) sourceQualityScore += cfg.withDomainMatch;
    if (overlap >= 0.35) sourceQualityScore += cfg.withStrongTitleMatch;
    ...
    if (domainOk) sourceQualityScore += cfg.withDomainMatch;
    if (overlap >= 0.35) sourceQualityScore += cfg.withStrongTitleMatch;
    if (input.brandConfirmedAdvertiser) sourceQualityScore += cfg.withConfirmedAdvertiser;
    sourceQualityScore = Math.min(cfg.maxScore, Math.round(sourceQualityScore));
