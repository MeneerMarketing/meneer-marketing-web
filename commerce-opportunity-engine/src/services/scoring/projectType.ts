...
export function recommendProjectType(input: ProjectTypeInput): ProjectTypeResult {
  ...
  const ownBrandBoost =
    rel === "OWN_BRAND" || rel === "EXCLUSIVE_BRAND";
  ...
  if (
    ...
    return {
      projectType: "DESIGN_UPGRADE",
      reason:
        "Eigen merk + hoge MM Fit; huidige PDP al sterk, dus eerder design/polish dan rebuild",
    };
  }
...
export function buildInternalSalesAngle(input: {
  ...
  if (input.aiSalesAngle && input.aiSalesAngle.trim().length > 40) {
    ...
    if (/reseller|eigen merk|own brand|multi-brand/i.test(base)) return base.slice(0, 600);
    const suffix =
      input.productRelationship === "RESELLER_PRODUCT"
  ...
  if (input.productRelationship === "RESELLER_PRODUCT") {
    return `Sterke specialistische ${platform}-webshop (${input.domain}) met ${paid}. De productpagina toont externe merken (reseller). Positioneer dit eerder als Shopify CRO/PDP redesign dan volledige merk-rebuild.`.slice(
      0,
      600
    );
  }
  ...
  if (input.productRelationship === "OWN_BRAND") {
    return `Eigen merk op ${platform} (${input.domain}) met ${paid}. PDP-verbeterkans ${input.pdpPotential ?? "n/a"} / full rebuild ${input.fullRebuildPotential ?? "n/a"}. Custom Shopify rebuild is de logische hooflijn wanneer rebuild potential hoog is.`.slice(
      0,
      600
    );
  }

  return `Prospect ${input.domain} (${platform}). Aanbevolen project: ${input.projectType}. PDP ${input.pdpPotential ?? "n/a"} / rebuild ${input.fullRebuildPotential ?? "n/a"}.`.slice(
    0,
    600
  );
}
