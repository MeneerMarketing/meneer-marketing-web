import {
  FULL_REBUILD_FIT_BY_RELATIONSHIP,
  PRODUCT_MERCHANT_MM_FIT_DELTA,
  type ProductMerchantRelationship,
} from "../../config/commercialFit.js";
 ...
 */
export function computeMeneerMarketingFit(
  input: MeneerMarketingFitInput
): MeneerMarketingFitResult {
  ...
  const relationship: ProductMerchantRelationship =
    input.productMerchantRelationship ?? "UNKNOWN";
  ...
  const confirmedPaidActivity =
    input.confirmedGoogleAdvertiser || input.paidConfirmed ? 92 : 25;
  const productSeriousness = scoreProductSeriousness(
    input.productPrice,
    input.reviewCount,
    input.hasExactProductTarget
  );
  let projectSuitability = scoreProjectSuitability(input, relationship);

  const w = MENEER_MARKETING_FIT_WEIGHTS;
  let weighted =
    shopifyFit * w.shopifyFit +
  ...
  return {
    ...
    formula:
      "shopify20 + businessType20 + retailerScaleFit15 + maturity15 + paid10 + product10 + project10 + merchantDelta (OWN_BRAND+/RESELLER-)",
...
function scoreShopify(platform: string | null, candidate: string | null): number {
  if (platform === "SHOPIFY") return 100;
  if (candidate === "SHOPIFY") return 78;
  if (platform === "WOOCOMMERCE" || candidate === "WOOCOMMERCE") return 62;
  if (candidate === "MAGENTO" || platform === "MAGENTO") return 48;
  if (platform && platform !== "UNKNOWN") return 40;
  if (candidate && candidate !== "UNKNOWN") return 35;
  return 22;
}

function scoreBusinessType(type: string): number {
  if (type === "BRAND") return 100;
  if (type === "SPECIALIST_WEBSHOP") return 92;
  if (type === "DTC") return 90;
  if (type === "MULTI_BRAND_SPECIALIST") return 55;
  if (type === "RETAILER") return 30;
  if (!type) return 45;
  return 40;
}

function scoreRetailerScaleFit(scale: number | null): number {
  if (scale == null) return 55;
  if (scale <= 20) return 70;
  if (scale <= 45) return 95;
  if (scale <= 60) return 75;
  if (scale <= 75) return 40;
  return 15;
}

function scoreProductSeriousness(
  price: number | null,
  reviews: number | null,
  hasExactProductTarget: boolean
): number {
  ...
  if (price != null) {
    ...
    else if (price >= 80) score += 22;
    else if (price >= 30) score += 12;
    else score += 4;
  }
  if (reviews != null) {
...
function scoreProjectSuitability(
  input: MeneerMarketingFitInput,
  relationship: ProductMerchantRelationship
): number {
  let score = 50;
  if (input.leadEligible) score += 25;
  if (input.platform === "SHOPIFY" || input.platformCandidate === "SHOPIFY") score += 15;
  if ((input.businessType ?? "").toUpperCase() === "SPECIALIST_WEBSHOP") score += 10;
  if ((input.retailerScaleScore ?? 0) > 75) score -= 30;
...
function buildReason(input: {
  ...
  retailerScaleFit: number;
  type: string;
  leadEligible: boolean;
  relationship: ProductMerchantRelationship;
}): string {
  const bits: string[] = [];
  if (input.shopifyFit >= 90) bits.push("Shopify fit hoog");
  else if (input.shopifyFit >= 45) bits.push("Platform-kandidaat bruikbaar");
  else bits.push("Platform fit matig");
  if (input.businessTypeFit >= 85) bits.push(`type ${input.type || "onbekend"} past`);
  if (input.retailerScaleFit >= 80) bits.push("retailer scale in sweet spot");
  if (input.relationship === "OWN_BRAND") bits.push("eigen merk product");
  if (input.relationship === "RESELLER_PRODUCT") bits.push("reseller product (lagere rebuild-fit)");
  if (!input.leadEligible) bits.push("niet lead-eligible");
  return bits.join("; ");
}
