...
export function classifyKeywordIntent(
  ...
  if (hasPlace && !hasCommercial && brandOverlap === 0 && kwTokens.length <= 4) {
    return {
      ...
      confidence: 86,
      reason:
        "Plaatsnaam + bedrijfsachtige naam zonder overlap met adverteerderdomein (lokale/navigatie-zoekintentie)",
    };
  }
  ...
  if (
    ...
    return {
      ...
      confidence: 68,
      reason:
        "Korte proprium-achtige keyword zonder commerciele generieken en zonder eigen merkopverlap",
    };
  }
...
function brandTokensFrom(domain: string | null, brandName?: string | null): string[] {
