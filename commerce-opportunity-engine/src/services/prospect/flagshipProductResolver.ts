...
import {
  loadShopifyCatalogProducts,
  scoreCatalogProductsForHero,
} from "./heroSelectionScorer.js";
...
export async function resolveFlagshipProduct(
  domain: string,
  timeoutMs: number,
  ...
  try {
    ...
    if (feed.status !== "success") return null;

    const json = extractJson(feed.html);
    if (!json) return null;

    const parsed = JSON.parse(json) as { products?: ShopifyFeedProduct[] };
    const wanted = new Set(tokenize(preferredTitle ?? ""));

    let bestByPrice: FlagshipProduct | null = null;
    let bestByTitle: { product: FlagshipProduct; overlap: number } | null = null;

    for (const product of parsed.products ?? []) {
      if (!product.handle || !product.title) continue;
      const price = (product.variants ?? []).reduce<number | null>((max, variant) => {
      ...
      const candidate: FlagshipProduct = {
        url: `https://${domain}/products/${product.handle}`,
        title: product.title,
        price,
      };

      if (!bestByPrice || (bestByPrice.price ?? 0) < price) bestByPrice = candidate;
