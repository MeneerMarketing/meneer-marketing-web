...
export function computeRedesignMaterialFeasibility(input: {
...
}): { score: number; band: "HIGH" | "MEDIUM" | "LOW"; evidence: string[] } {
  ...
  const band: "HIGH" | "MEDIUM" | "LOW" =
    final >= 68 ? "HIGH" : final >= 52 ? "MEDIUM" : "LOW";
...
export function computeValidatedGapSalesFit(input: {
  ...
  let paidBonus = 0;
  if (input.paidAcquisitionLevel === "CONFIRMED") paidBonus = 5;
  else if (input.paidAcquisitionLevel === "LIKELY") paidBonus = 3;

  const score = clamp(
    input.rawPdpRedesignOpportunity * 0.28 +
...
export function passesPreAuditGate(input: {
  ...
  if (!input.isValidProductDetail) failures.push("not_product_detail");
  if (input.rawPdpRedesignOpportunity < 58) failures.push("raw_gap_low");
  if (input.redesignMaterialFeasibility < 60) failures.push("material_feasibility_low");
  ...
  const lowPriceOwnBrandException =
    (input.heroPrice ?? 0) >= 40 &&
  ...
  if (input.heroCandidateScore < 45) failures.push("hero_candidate_weak");
