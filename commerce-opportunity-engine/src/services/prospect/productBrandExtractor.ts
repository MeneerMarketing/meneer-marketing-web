...
function isPlausibleProductBrand(name: string | null): boolean {
  if (!name || name.length < 3) return false;
  const lower = name.toLowerCase();
  if (FALSE_BRAND_TOKENS.has(lower)) return false;
  if (/^(nl|be|eu|com)$/i.test(lower)) return false;
  return true;
}

export function extractProductBrandName(title: string | null): string | null {
...
export function titleSuggestsThirdPartyProduct(
  normalizedDomain: string,
  title: string | null
): boolean {
  const brand = extractProductBrandName(title);
  if (!isPlausibleProductBrand(brand)) return false;
...
export async function resolveOfficialBrandDomain(
  brandName: string,
  timeoutMs: number
): Promise<{ domain: string | null; evidence: string[] }> {
