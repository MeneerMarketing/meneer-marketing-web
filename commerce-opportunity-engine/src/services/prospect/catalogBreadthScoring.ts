...
function scoreCompactnessFromCount(size: number | null, verified: boolean): number {
  if (size == null) return verified ? 42 : 35;
  if (size <= 10) return 88;
  if (size <= 30) return 96;
  if (size <= 50) return 90;
  if (size <= 80) return 74;
  if (size <= 120) return 52;
...
function scoreCoherence(input: {
  ...
  if (input.productFamilySpread <= 2) score += 14;
  else if (input.productFamilySpread <= 4) score += 4;
  else score -= 16;

  if (input.externalBrandBreadth >= 25) score -= 20;
  else if (input.externalBrandBreadth >= 10) score -= 10;
...
export function computeBusinessBreadthScore(metrics: {
  ...
  const scalePenalty =
    (metrics.companyScaleFit ?? 50) >= 88 ? -8 : (metrics.companyScaleFit ?? 50) >= 78 ? -4 : 0;
  const categoryPenalty =
    metrics.categoryLinks >= 30 ? -18 : metrics.categoryLinks >= 20 ? -10 : 0;
  const sizePenalty =
    metrics.catalogEstimate != null && metrics.catalogEstimate >= 200
      ? -22
  ...
  const raw =
    metrics.catalogCompactnessScore * 0.45 +
...
export function computeCatalogBreadthMetrics(input: {
  ...
  for (const hint of familyHints) {
    if (html.includes(hint)) familiesHit += 1;
  }
  productFamilySpread = Math.max(1, Math.min(8, Math.round(categoryLinks / 4) + familiesHit));

  const catalogConfidence: CatalogConfidence = input.catalogVerified
    ? input.catalogEstimate != null
      ? "MEASURED"
      : "INFERRED"
    : "UNKNOWN";

  const compactness = scoreCompactnessFromCount(
  ...
  const categoryBreadth = Math.min(
    100,
    categoryLinks * 2 + Math.max(0, productFamilySpread - 2) * 8
  );
