...
export function isOrganicFirstPartySkip(
  _normalizedDomain: string,
  likelyRetailer: boolean,
  classification: ThirdPartySourceClassification
): boolean {
  if (classification.skipReason === "organic_first_party_skip") return true;
  if (!likelyRetailer && classification.sourceType === "BLOCKED") return true;
  return false;
}
