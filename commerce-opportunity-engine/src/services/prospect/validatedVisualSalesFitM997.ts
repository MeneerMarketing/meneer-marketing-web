...
export function computeValidatedVisualSalesFitM997(input: {
  ...
  let impressionAdj = 0;
  if (input.currentSiteImpression === "CLEARLY_UNDERDESIGNED") impressionAdj = 12;
  else if (input.currentSiteImpression === "BASIC_BUT_ACCEPTABLE") impressionAdj = -6;
  else if (input.currentSiteImpression === "MODERN_ENOUGH") impressionAdj = -18;
  else impressionAdj = -30;

  let likelihoodAdj = 0;
  if (input.humanShowcaseLikelihood === "STRONG") likelihoodAdj = 10;
  else if (input.humanShowcaseLikelihood === "POSSIBLE") likelihoodAdj = 2;
  else if (input.humanShowcaseLikelihood === "WEAK") likelihoodAdj = -10;
  else likelihoodAdj = -22;
