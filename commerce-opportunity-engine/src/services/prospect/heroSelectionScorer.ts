...
function keywordOverlap(tokens: string[], candidate: string): number {
  const lower = candidate.toLowerCase();
  return tokens.reduce((score, token) => (lower.includes(token) ? score + 1 : score), 0);
}

export function scoreCatalogProductsForHero(input: HeroSelectionInput): HeroSelectionResult | null {
  ...
  let best: HeroSelectionResult | null = null;
  ...
  for (const [index, product] of input.products.entries()) {
    ...
    const candidate: HeroSelectionResult = {
...
export async function loadShopifyCatalogProducts(
  domain: string,
  timeoutMs: number,
  crawlWebsite: (url: string, timeoutMs: number) => Promise<{ status: string; html: string }>
  ...
  try {
    ...
    const json = body.startsWith("{")
      ? (JSON.parse(body) as { products?: Array<Record<string, unknown>> })
      : null;
    const products = json?.products ?? [];

    return products.map((product, index) => {
      const variants = (product.variants as Array<{ price?: string | number }> | undefined) ?? [];
      const price = variants.reduce<number | null>((max, variant) => {
