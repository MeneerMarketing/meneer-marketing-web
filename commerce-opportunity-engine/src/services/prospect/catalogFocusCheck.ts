...
function parsePrintedCount(text: string): number | null {
...
function scoreFocus(size: number | null, categoryLinks: number): { score: number; evidence: string[] } {
...
export async function runCatalogFocusCheck(
  domain: string,
  timeoutMs: number,
  homepageProductLinks: number,
  homepageCategoryLinks: number
): Promise<CatalogFocusResult> {
  const base: CatalogFocusResult = {
    domain,
    collectionUrl: null,
  ...
  for (const path of COLLECTION_PATHS) {
    ...
    if (signals.estimatedProductLinks < 3) continue;
    ...
    const verified =
      printed !== null || signals.estimatedProductLinks >= MIN_TRUSTWORTHY_PRODUCT_LINKS;
