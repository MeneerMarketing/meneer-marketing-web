...
export function catalogBandPenalty(catalogEstimate: number | null): {
  band: string;
  score: number;
  penalty: number;
} {
  const c = catalogEstimate;
  if (c == null) return { band: "unknown", score: 52, penalty: 0 };
  if (c <= 30) return { band: "3_30_sweet", score: 95, penalty: 0 };
  if (c <= 60) return { band: "31_60_strong", score: 82, penalty: 0 };
  if (c <= 100) return { band: "61_100_ok", score: 68, penalty: 4 };
  if (c <= 250) return { band: "101_250_penalty", score: 45, penalty: 14 };
