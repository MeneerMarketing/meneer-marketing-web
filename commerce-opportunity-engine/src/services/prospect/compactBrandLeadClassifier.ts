...
export function classifyM997LeadType(input: {
  ...
  if (
    input.validatedVisualSalesFit >= 55 &&
    input.businessQualified &&
    material >= 55 &&
  ...
  if (input.humanShowcaseLikelihood === "NO") why = "human_likelihood_no";
  else if (input.humanShowcaseLikelihood === "WEAK") why = "before_after_not_obvious";
  else if (input.currentSiteImpression !== "CLEARLY_UNDERDESIGNED") {
    why = "not_clearly_underdesigned";
  } else if (input.businessBreadthScore < 55) why = "catalog_too_broad";
  else if (input.catalogConfidence === "UNKNOWN") why = "catalog_unverified";
...
export function passesShowcaseSalesCandidateM997(input: {
  ...
  if (input.brandOwnershipConfidence < 50) failures.push("insufficient_own_brand_evidence");
  if (!input.businessQualified) failures.push("business_not_qualified");
  if ((input.companyScaleFit ?? 0) < 32) failures.push("company_scale_low");
  if ((input.businessMaturityScore ?? 0) < 24) failures.push("amateur_maturity");
  if ((input.redesignMaterialFeasibility ?? 0) < 60) failures.push("material_feasibility_low");
  if (input.currentSiteImpression !== "CLEARLY_UNDERDESIGNED") {
    failures.push("not_clearly_underdesigned");
  }
  if (input.businessBreadthScore < 50) failures.push("catalog_or_business_too_broad");
