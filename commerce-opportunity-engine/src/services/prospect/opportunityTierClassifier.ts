...
export function classifyOpportunityTier(input: {
...
}): { tier: OpportunityTier; leadType: LeadType; reason: string } {
  ...
  const economicsOk =
    (input.focusedBrandSalesFit ?? input.productEconomicFit ?? 0) >= 50;

  if (
    input.pageEntityType === "PRODUCT_DETAIL" &&
    businessOk &&
    purchaseHeavy &&
    visualWeak &&
    economicsOk &&
