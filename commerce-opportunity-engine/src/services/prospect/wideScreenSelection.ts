...
export function evaluateHardExclusion(entry: WideScreenPoolEntry): HardExclusionResult {
...
export function passesOldEconomicPrescreen(entry: WideScreenPoolEntry): boolean {
  if ((entry.highTicketFitScore ?? 0) < 52) return false;
  if ((entry.ownBrandSignal ?? 0) < 42) return false;
  if (entry.businessType !== "BRAND" && entry.businessType !== "SPECIALIST_WEBSHOP") return false;
  const heroPrice = entry.heroPrice ?? 0;
  if (heroPrice > 0 && heroPrice < 60) return false;
  return true;
}

export function softWideScreenRank(entry: WideScreenPoolEntry): number {
  let score = 40;
  if (entry.businessType === "BRAND" || entry.businessType === "SPECIALIST_WEBSHOP") score += 12;
  if ((entry.ownBrandSignal ?? 0) >= 55) score += 10;
  else if ((entry.ownBrandSignal ?? 0) >= 42) score += 5;
  if (entry.heroPrice != null && entry.heroPrice >= 150) score += 14;
  else if (entry.heroPrice != null && entry.heroPrice >= 80) score += 10;
  else if (entry.heroPrice != null && entry.heroPrice >= 50) score += 4;
  if (entry.catalogVerified && (entry.estimatedCatalogSize ?? 999) <= 100) score += 10;
  else if (!entry.catalogVerified) score += 3;
  if ((entry.catalogFocusScore ?? 0) >= 65) score += 8;
  if (entry.adKeywordCount >= 2) score += 6;
  else if (entry.adKeywordCount >= 1) score += 3;
  if (entry.heroProductUrl) score += 8;
  if ((entry.highTicketFitScore ?? 0) >= 65) score += 8;
  return score;
}
