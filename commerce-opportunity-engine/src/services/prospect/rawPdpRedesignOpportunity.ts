...
export function computeAssetQualityProxy(input: {
  ...
  let score = 30;
  if (input.imageCount >= 6) score += 28;
  else if (input.imageCount >= 3) score += 16;
  else if (input.imageCount >= 1) score += 6;
  if (input.videoPresent) score += 14;
  if (input.featuresPresent) score += 10;
  if (input.faqPresent) score += 8;
  return clamp(score);
}
...
}): { score: number; evidence: string[]; materialSweetSpot: boolean } {
