...
export function computePreauditPurchaseGap(input: PreauditPurchaseGapInput): {
  score: number;
  evidence: string[];
} {
...
export function extractPurchaseGapSignals(html: string): Omit<
  PreauditPurchaseGapInput,
  "html"
> {
  const lower = html.toLowerCase();
  const variantSelectors =
    (html.match(/<select[^>]+name=["'][^"']*variant|product-form__input|variant-picker/gi) ?? []).length;
