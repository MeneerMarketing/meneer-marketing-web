...
async function loadCandidates(path: string): Promise<CandidateLike[]> {
  try {
    const raw = await readFile(path, "utf8");
    const parsed = JSON.parse(raw) as { candidates?: CandidateLike[] };
...
export async function runHeroDataConsistencyCheck(projectRoot: string): Promise<{
  ...
  const cases: HeroConsistencyCase[] = [];
