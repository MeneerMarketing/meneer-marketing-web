...
export function inferPriceConfidence(input: {
  ...
  if (input.observedPrice != null && input.priceFromCrawl) return "HIGH";
  if (input.heroPrice != null && input.priceFromCrawl) return "HIGH";
  if (input.observedPrice != null || input.heroPrice != null) return "MEDIUM";
  return "UNKNOWN";
}
...
}): { pass: boolean; hardReject: boolean; reason: string | null } {
...
export function computeProductEconomicFit(input: {
  ...
  if (input.purchaseMode === "DIRECT_ECOMMERCE") evidence.push("direct_ecommerce");
...
export function computeHighTicketGapSalesFit(input: {
...
}): { score: number; confidence: "HIGH" | "MEDIUM" | "LOW"; evidence: string[] } {
  const evidence: string[] = [];
  let modelScore = 40;
  ...
  let paidBonus = 0;
  if (input.paidAcquisitionLevel === "CONFIRMED") paidBonus = 4;
  else if (input.paidAcquisitionLevel === "LIKELY") paidBonus = 2;

  const score = clamp(
    input.showcaseGapPotential * 0.3 +
  ...
  if (score >= 72 && input.businessModelSalesCandidate) confidence = "HIGH";
...
export function passesHighTicketFinalistGate(input: {
  ...
  const price = input.heroPrice;
  if (price != null && price < 100) failures.push("hero_price_below_100");
  if (price != null && price < 60) failures.push("hero_price_hard_reject");

  if (input.showcaseGapPotential < 70) failures.push("showcase_gap_low");
  if (input.redesignMaterialFeasibility < 65) failures.push("material_feasibility_low");
  if (input.productEconomicFit < 55) failures.push("product_economics_weak");
  if (input.businessEconomicFit < 50) failures.push("business_economics_weak");
  ...
  if ((input.companyScaleFit ?? 0) < 40) failures.push("company_scale_low");
  if ((input.catalogFocus ?? 0) < 45) failures.push("catalog_focus_low");
  if (input.purchaseMode === "LEAD_GENERATION" || input.purchaseMode === "SHOWROOM_ASSISTED") {
    failures.push("not_direct_ecommerce");
  }
  if ((input.businessMaturityScore ?? 50) < 28) failures.push("maturity_very_low");
  if (!input.showcaseReady) failures.push("showcase_profile_weak");
...
export function meetsEarlySuccessPartialCriteria(input: {
  ...
  if (input.showcaseGapPotential < 70) return false;
  if (input.redesignMaterialFeasibility < 65) return false;
  if (!input.businessModelSalesCandidate) return false;
  if (
    input.businessModel === "GENERAL_RETAILER" ||
    input.businessModel === "GENERAL_RESELLER"
  ) {
  ...
  if (input.purchaseMode !== "DIRECT_ECOMMERCE") return false;
  if ((input.companyScaleFit ?? 0) < 40) return false;
  return (
...
export function isStrongFinalistForEarlyStop(input: {
  ...
  if (!input.preAuditGatePass) return false;
  const price = input.heroPrice;
  if (price != null && price < 150) return false;
