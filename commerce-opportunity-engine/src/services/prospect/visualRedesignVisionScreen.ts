...
export async function screenPdpVisualRedesignWithVision(
  env: Env,
  domain: string,
  desktopScreenshotPath: string
): Promise<VisualRedesignVisionResult> {
...
Positive adjustment = more room to improve visually. Negative = already premium.`,
  ...
  let parsed: Partial<VisualRedesignVisionResult> = {};
  try {
  ...
  const currentVisualQuality =
    typeof parsed.currentVisualQuality === "number"
