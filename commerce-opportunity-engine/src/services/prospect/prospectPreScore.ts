...
export function computeDeepDivePdpFitProxy(input: {
  ...
  const upside =
    deepDive.highConsideration * 0.25 +
...
export function computeProspectPreScore(input: PreScoreInput): PreScoreResult {
  ...
  const platformFit = input.platform
    ? TARGET_PROFILE.preferredPlatforms.includes(
        input.platform as (typeof TARGET_PROFILE.preferredPlatforms)[number]
      )
      ? 100
      : input.platform === "UNKNOWN"
        ...
        : 60
    : 45;
  if (platformFit === 100) evidence.push(`${input.platform}: goed te verbouwen platform`);

  // Weakness is opportunity: a PDP that already converts well needs us less.
  const pdpWeakness = input.pdpWeaknessScore ?? 50;
  if (pdpWeakness >= 65) evidence.push("huidige PDP laat veel liggen");

  const heroStrength = input.heroScore ?? 35;
  if (heroStrength >= 60) evidence.push("duidelijk heroproduct gevonden");
