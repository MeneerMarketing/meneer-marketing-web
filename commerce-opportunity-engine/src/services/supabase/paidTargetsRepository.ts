...
import {
  freshnessLabel,
  scoreLabsPaidTarget,
  scoreShoppingPaidTarget,
} from "../scoring/sourceQualityV2.js";
...
export function normalizeLandingUrl(url: string | null | undefined): string | null {
  if (!url) return null;
  try {
...
export async function upsertPaidSearchTargets(input: {
  ...
  observedAt: string;
}): Promise<{ upserted: number; croReadyLandingCount: number }> {
  let upserted = 0;
  const croReadyLandings = new Set<string>();
...
export async function upsertShoppingPaidTarget(
  client: SupabaseClient,
  input: ShoppingTargetUpsertInput
): Promise<{
  id: string;
  croReady: boolean;
...
type SourceEvidence = {
  ...
  paidShopping: {
    status: "exact" | "exact_listing" | "free_listing" | "not_found";
    sourceType?: string;
    keyword?: string;
    productTitle?: string;
    seller?: string;
  ...
  discovery: {
    popularProductsCandidate: boolean;
  };
};
...
async function buildSourceEvidence(
  ...
  const { data: targets } = await client
    .from("paid_search_targets")
    .select(
      "channel, source_type, keyword, ad_title, seller, price, currency, landing_url, source_quality_score"
    )
    .eq("brand_id", brandId)
    .eq("landing_url", landingUrl);

  const searchHit = (targets ?? []).find(
    (t) =>
      t.channel === "SEARCH" &&
      (t.source_type === "LABS_PAID_KEYWORD" || t.source_type === "LIVE_PAID_SERP")
  );
  const shoppingHit = (targets ?? []).find(
    (t) =>
      t.channel === "SHOPPING" &&
  ...
  const { data: discoveryOpp } = await client
    ...
    .eq("brand_id", brandId)
    .or(
      "source_type.eq.POPULAR_PRODUCTS_CANDIDATE,ground_truth_source_type.eq.POPULAR_PRODUCTS_CANDIDATE"
    )
    .limit(1)
    .maybeSingle();
  ...
  return {
    transparency: Boolean(
      brand?.transparency_confirmed || brand?.confirmed_google_advertiser
    ),
    paidSearch: searchHit
      ? {
          status: "exact",
          sourceType: searchHit.source_type as string,
          keyword: searchHit.keyword as string,
          landingUrl: searchHit.landing_url as string,
        }
      : { status: "not_found" },
    paidShopping: shoppingHit
      ? {
          status:
            shoppingHit.source_type === "GOOGLE_SHOPPING_PAID_EXACT"
              ? "exact"
              : shoppingHit.source_type === "GOOGLE_SHOPPING_EXACT_LISTING"
                ...
                : shoppingHit.source_type === "GOOGLE_SHOPPING_FREE_LISTING"
                  ? "free_listing"
                  : "not_found",
          sourceType: shoppingHit.source_type as string,
          keyword: shoppingHit.keyword as string,
          productTitle: shoppingHit.ad_title as string,
          seller: shoppingHit.seller as string,
          price: shoppingHit.price as number | null,
          currency: shoppingHit.currency as string | null,
          landingUrl: shoppingHit.landing_url as string,
        }
      : { status: "not_found" },
    discovery: {
...
export async function upsertCroReadyOpportunityFromTarget(input: {
  ...
  const isExactPaid =
    input.sourceType === "LIVE_PAID_SERP" ||
  ...
  const croReadinessLevel =
    input.croReadinessLevel ??
  ...
  const page =
    (pages ?? []).find(
      (p) =>
        normalizeLandingUrl(p.final_url as string | null) === landingUrl ||
        normalizeLandingUrl(p.url as string | null) === landingUrl
    ) ?? null;
  ...
  const prevEvidence = (existing?.source_evidence ?? {}) as Partial<SourceEvidence>;
  const mergedEvidence: SourceEvidence = {
    ...
    paidSearch:
      evidence.paidSearch.status === "exact"
        ...
        : prevEvidence.paidSearch?.status === "exact"
          ...
          : { status: "not_found" },
    paidShopping:
      evidence.paidShopping.status !== "not_found"
        ? evidence.paidShopping
        : prevEvidence.paidShopping && prevEvidence.paidShopping.status !== "not_found"
          ? prevEvidence.paidShopping
          : { status: "not_found" },
    discovery: {
      popularProductsCandidate:
        evidence.discovery.popularProductsCandidate ||
  ...
  const prevGround = (existing?.ground_truth_source_type as string | null) ?? "";
  const groundTruth =
    (hierarchyRank[input.sourceType] ?? 0) >= (hierarchyRank[prevGround] ?? 0)
      ? input.sourceType
 ...
 */
export async function generateOpportunitiesFromPaidTargets(
  client: SupabaseClient,
  brandId: string
): Promise<{ opportunitiesUpserted: number; croReady: number }> {
  ...
  const byLanding = new Map<string, Array<Record<string, unknown>>>();
  for (const t of targets ?? []) {
    const url = t.landing_url as string;
  ...
  for (const [landingUrl, group] of byLanding) {
    ...
    const channel = (primary.channel as string) === "SHOPPING" ? "SHOPPING" : "SEARCH";
    await upsertCroReadyOpportunityFromTarget({
      client,
      brandId,
      targetId: primary.id as string,
      keyword: primary.keyword as string,
      keywordId: (primary.keyword_id as string | null) ?? null,
      landingUrl,
      adTitle: (primary.ad_title as string | null) ?? null,
      adDescription: (primary.ad_description as string | null) ?? null,
      sourceType: primary.source_type as string,
      sourceQualityScore: Number(primary.source_quality_score ?? 0),
      channel,
      discoverySerpItemType:
        channel === "SHOPPING" ? "google_shopping_paid" : "paid",
      confirmationSource:
        channel === "SHOPPING"
          ? "dataforseo_merchant_shopping"
          : "dataforseo_labs_ranked_keywords",
...
export async function markBrandWaitingForPaidTarget(
  client: SupabaseClient,
  brandId: string,
  paidTargetsCount: number
): Promise<void> {
  const now = new Date().toISOString();
  await client
    .from("brands")
