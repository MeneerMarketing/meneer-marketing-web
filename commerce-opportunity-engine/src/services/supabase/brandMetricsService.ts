...
interface MetricsAccumulator {
  ...
  allKeywordIds: Set<string>;
  confirmedKeywordIds: Set<string>;
  candidateKeywordIds: Set<string>;
...
export async function getBrandActivityMetrics(
  client: SupabaseClient,
  brandIds?: string[],
  signalFilter?: "CONFIRMED_PAID" | "PAID_CANDIDATE"
): Promise<BrandActivityMetrics[]> {
  let query = client
    .from("ad_occurrences")
    .select(
      "brand_id, keyword_id, found_at, ad_signal_type, brands!inner(normalized_domain, lead_eligible, business_type, transparency_confirmed)"
    );
  ...
  for (const row of data ?? []) {
    ...
    const keywordId = row.keyword_id as string | null;
    const foundAt = row.found_at as string | null;
    const signalType = row.ad_signal_type as string | null;
    const brandJoin = row.brands as unknown as {
    ...
    if (!accumulator) {
      accumulator = {
        ...
        allKeywordIds: new Set<string>(),
        confirmedKeywordIds: new Set<string>(),
        candidateKeywordIds: new Set<string>(),
...
export async function getTopAdvertisersForRun(
  client: SupabaseClient,
  runId: string,
  signalFilter: "CONFIRMED_PAID" | "PAID_CANDIDATE",
  limit = 10
  ...
  const brandIds = [...new Set((data ?? []).map((row) => row.brand_id as string).filter(Boolean))];
  const metrics = await getBrandActivityMetrics(client, brandIds, signalFilter);
  return metrics.slice(0, limit);
}

export async function countBrandsByBusinessCategory(
  client: SupabaseClient
): Promise<{
  leadEligible: number;
...
export async function countOccurrencesBySignal(
  client: SupabaseClient,
  runId?: string
): Promise<{
  ...
  for (const row of data ?? []) {
    const source = row.confirmation_source as string | null;
    const serpType = row.serp_item_type as string | null;
    const signal = row.ad_signal_type as string | null;
