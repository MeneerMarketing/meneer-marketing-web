...
export async function findBrandByNormalizedDomain(
  ...
  const { data, error } = await client
    .from("brands")
    .select(
      "id, name, domain, normalized_domain, first_seen_at, last_seen_at, scan_count, confirmed_google_advertiser, confirmation_source, manual_excluded, eligibility_status, lead_eligible, business_type"
    )
...
export async function upsertBrandFromAd(
  client: SupabaseClient,
  ...
  if (existing) {
    const manualExcluded = Boolean(
      (existing as BrandRecord & { manual_excluded?: boolean }).manual_excluded
    );
    ...
    const updates: Record<string, unknown> = {
      ...
      confirmed_google_advertiser:
        ...
        false,
      confirmation_source:
        input.confirmationSource ?? existing.confirmation_source ?? null,
    ...
    if (!manualExcluded) {
      ...
      const currentType =
        (existing as BrandRecord & { business_type?: string }).business_type ?? "UNKNOWN";
    ...
    const { data, error } = await client
      ...
      .eq("id", existing.id)
      .select(
        "id, name, domain, normalized_domain, first_seen_at, last_seen_at, scan_count, confirmed_google_advertiser, confirmation_source"
      )
      .single();
    ...
    return { brand: data as BrandRecord, isNew: false };
  }
  ...
  return { brand: data as BrandRecord, isNew: true };
}
