...
export async function reclassifyAllSignals(
  ...
  const { data: brands, error: brandError } = await client
    .from("brands")
    .select(
      "id, normalized_domain, confirmed_google_advertiser, transparency_confirmed, manual_excluded, eligibility_status, business_type"
    );
