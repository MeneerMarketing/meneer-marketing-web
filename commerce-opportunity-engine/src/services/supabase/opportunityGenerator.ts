 ...
 */
export async function generateOpportunitiesFromAds(
  client: SupabaseClient
): Promise<OpportunityGenerationStats> {
  const { data: ads, error: adsError } = await client
    .from("ad_occurrences")
    .select(
      "id, brand_id, keyword_id, landing_url, headline, description, source, ad_signal_type, serp_item_type, found_at, observed_at, confirmation_source"
    )
  ...
  const { data: brands, error: brandsError } = await client
    .from("brands")
    .select(
      "id, confirmed_google_advertiser, transparency_confirmed, manual_excluded, eligibility_status, lead_eligible"
    );
  ...
  const bestProductByBrand = new Map<string, BrandPage>();
  for (const page of pages ?? []) {
    ...
    if (!bestProductByBrand.has(brandId)) {
      bestProductByBrand.set(brandId, {
        ...
        final_url: page.final_url as string | null,
        product_resolution_confidence: page.product_resolution_confidence as number | null,
        page_type: page.page_type as string | null,
      });
    }
  }
  ...
  for (const cluster of clusters.values()) {
    ...
    const paidConfirmed =
      brandPaid.get(cluster.brandId) === true ||
      cluster.ads.some(
        (a) =>
          a.ad_signal_type === "CONFIRMED_PAID" ||
    ...
    const isDiscoveryOnly =
      ...
      !cluster.landingUrl;
    const isExactPaidLanding =
      Boolean(cluster.landingUrl) &&
...
function pickPrimaryAd(ads: AdRow[]): AdRow {
  const ranked = [...ads].sort((a, b) => {
    const score = (ad: AdRow) => {
      let s = 0;
      if (ad.ad_signal_type === "CONFIRMED_PAID") s += 3;
      else if (ad.ad_signal_type === "PAID_CANDIDATE") s += 1;
      if (ad.headline) s += 1;
      return s;
    };
...
function pickStrongestSignal(ads: AdRow[]): string | null {
  if (ads.some((a) => a.ad_signal_type === "CONFIRMED_PAID")) return "CONFIRMED_PAID";
  if (ads.some((a) => a.ad_signal_type === "PAID_CANDIDATE")) return "PAID_CANDIDATE";
  if (ads.some((a) => a.ad_signal_type === "NON_PAID")) return "NON_PAID";
