...
export type CroAuditLifecycleStatus =
  | "PENDING"
  | "IN_PROGRESS"
  | "COMPLETED"
  | "FAILED_TECHNICAL"
...
export function assessPageHealth(input: PageHealthInput): PageHealthResult {
  ...
  const productHints =
    /itemprop=["']price["']|product-form|add to cart|in winkelwagen|product__title|og:type["']\s*content=["']product/i.test(
      html
    );
  const contentHints =
    productHints ||
  ...
  const captureFailed =
    input.captureErrors.length > 0 &&
    !input.hasMobileScreenshot &&
    !input.hasDesktopScreenshot;

  // --- Classify ---
  let status: PageHealthStatus = "UNKNOWN";
  let confidence = 50;
  let reason = "Onvoldoende signalen";
  let failureAuditStatus: CroAuditLifecycleStatus | null = null;
  ...
  // Screenshot quality
  let screenshotQuality: ScreenshotQuality = "INVALID";
  if (
    status === "ERROR" ||
    status === "CHALLENGE" ||
    status === "BLOCKED" ||
  ...
  const allowClaudeAudit =
    (status === "HEALTHY" || (status === "PARTIAL" && confidence >= 60 && contentHints)) &&
    screenshotQuality !== "INVALID";
...
export function auditConfidenceFromHealth(
...
function matchSignatures(html: string, patterns: RegExp[]): string[] {
  const hits: string[] = [];
...
function countDomHints(html: string): number {
  const tags = html.match(/<[a-zA-Z][^>]*>/g);
  return tags?.length ?? 0;
}

/** Map failure status to opportunity verdict when no CRO score is set. */
export function verdictForFailedAudit(
  status: CroAuditLifecycleStatus
): "NEEDS_RETRY" | "NOT_AUDITED" {
