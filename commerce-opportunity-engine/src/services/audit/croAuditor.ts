...
import {
  CroAuditAiResponseSchema,
  type CroAuditAiResponse,
...
export async function runCroAuditWithClaude(input: {
  ...
  const estimatedCost =
    (response.usage.input_tokens / 1_000_000) * INPUT_COST_PER_M_SONNET +
...
function buildPrompt(auditType: CroAuditType, rep: PageRepresentation): string {
...
function buildExactPaidPrompt(rep: PageRepresentation): string {
...
function buildHighConfidencePrompt(rep: PageRepresentation): string {
  return `You are a senior ecommerce CRO strategist for Meneer Marketing (NL).
...
function compactRep(rep: PageRepresentation): Record<string, unknown> {
  return {
    url: rep.url,
    ...
    SOURCE: rep.source,
    ADVERTISEMENT:
      rep.source.auditType === "EXACT_PAID_FUNNEL"
        ? rep.advertisement
        : {
...
function normalizeAiPayload(auditType: CroAuditType, parsed: unknown): unknown {
  ...
  if (auditType === "HIGH_CONFIDENCE_PRODUCT_TARGET") {
    leaks = leaks.filter((leak) => {
      ...
      if (
        blob.includes("advertentiebelofte") ||
  ...
  const rebuild =
    typeof obj.custom_shopify_rebuild_potential === "number"
      ? obj.custom_shopify_rebuild_potential
      : 50;
  const pdp =
    typeof obj.pdp_improvement_potential === "number"
  ...
  return {
    ...
    scores,
    ad_landing_analysis:
      auditType === "HIGH_CONFIDENCE_PRODUCT_TARGET" ? null : obj.ad_landing_analysis,
    sales_angle: clip(obj.sales_angle, 1100),
    custom_shopify_rebuild_potential: rebuild,
    pdp_improvement_potential: pdp,
    evidence_notes:
      obj.evidence_notes == null ? obj.evidence_notes : clip(obj.evidence_notes, 800),
