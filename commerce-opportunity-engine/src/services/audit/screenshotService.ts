...
import { chromium, devices, type Browser, type Page, type Response } from "playwright";
...
export async function captureOpportunityScreenshots(input: {
  ...
  let last: CaptureScreenshotsResult | null = null;
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    const looksBroken =
      /error\s*52[0-4]|checking your browser|just a moment|cf-error-details|challenge-platform/i.test(
        last.html
      ) ||
  ...
  return { ...(last as CaptureScreenshotsResult), attempts: maxRetries + 1 };
}
...
async function captureOnce(input: {
  ...
  try {
    ...
    try {
      ...
      paths.mobile = await uploadPng(
        input.supabase,
        input.bucket,
        `${input.brandId}/${input.opportunityId}/mobile${suffix}.png`,
        mobileViewport
      );
      paths.fullMobile = await uploadPng(
        input.supabase,
        input.bucket,
        `${input.brandId}/${input.opportunityId}/full-mobile${suffix}.png`,
        mobileFull
      );
      ...
      if (input.attempt === 0 || !looksLikeErrorHtml(html)) {
        paths.mobile = await uploadPng(
          input.supabase,
          input.bucket,
          `${input.brandId}/${input.opportunityId}/mobile.png`,
          mobileViewport
        );
        paths.fullMobile = await uploadPng(
          input.supabase,
          input.bucket,
          `${input.brandId}/${input.opportunityId}/full-mobile.png`,
          mobileFull
        );
      }
    ...
    try {
      ...
      paths.desktop = await uploadPng(
        input.supabase,
        input.bucket,
        `${input.brandId}/${input.opportunityId}/desktop.png`,
        desktopViewport
      );
      paths.fullDesktop = await uploadPng(
        input.supabase,
        input.bucket,
        `${input.brandId}/${input.opportunityId}/full-desktop.png`,
        desktopFull
      );
...
function looksLikeErrorHtml(html: string): boolean {
  return /error\s*52[0-4]|checking your browser|just a moment|cf-error-details|challenge-platform/i.test(
    html
  );
}
...
export function hashPageContent(html: string, representationJson: string): string {
...
export async function createSignedScreenshotUrls(
  supabase: SupabaseClient,
  bucket: string,
  paths: ScreenshotPaths,
  expiresInSeconds = 3600
): Promise<Record<string, string | null>> {
  const entries = Object.entries(paths) as Array<[keyof ScreenshotPaths, string | null]>;
...
export async function downloadScreenshotBuffers(
  supabase: SupabaseClient,
  bucket: string,
  paths: ScreenshotPaths
): Promise<{ mobilePng?: Buffer; desktopPng?: Buffer }> {
  const result: { mobilePng?: Buffer; desktopPng?: Buffer } = {};
