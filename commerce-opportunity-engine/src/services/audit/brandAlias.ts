...
export function detectBrandAliasMismatch(input: {
  ...
  const aliasCandidates = keywordBrandTokens.filter(
    (t) => !COMMERCIAL_STOP.has(t) && !PLACE_STOP.has(t) && t.length >= 4
  );
...
function domainTokens(domain: string | null): string[] {
  if (!domain) return [];
  const base = domain
    .toLowerCase()
...
function brandishTokens(value: string | null): string[] {
  return tokenize(value ?? "").filter(
    (t) => !COMMERCIAL_STOP.has(t) && t.length >= 4
  );
}
