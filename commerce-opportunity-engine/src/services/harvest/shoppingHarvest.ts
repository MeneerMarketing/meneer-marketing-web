...
import {
  ...
  pickSellerOfferForBrandDomain,
} from "../dataforseo/shoppingSellers.js";
...
} from "../supabase/paidTargetsRepository.js";
...
export function selectShoppingSourceKeywords(
  keywords: HarvestSourceKeyword[],
  max = TARGET_HARVEST_DEFAULTS.maxShoppingKeywordsPerBrand
): HarvestSourceKeyword[] {
  ...
  return [...keywords]
    ...
    .sort((a, b) => {
      const score = (k: HarvestSourceKeyword) => {
        let s = 0;
        if ((k.intent ?? "") === "NON_BRANDED_PRODUCT") s += 40;
        if ((k.tier ?? "") === "PRIMARY") s += 25;
...
export async function harvestShoppingTargetsForBrand(input: {
  ...
  const brandNorm =
    normalizeDomainFromUrl(brandDomain)?.normalizedDomain ??
    brandDomain.toLowerCase().replace(/^www\./, "");

  for (const kw of selected) {
    if (cost >= budgetRemaining) break;
    ...
    for (const item of paidItems) {
      ...
      if (!finalUrl || !finalDomain) continue;
      ...
      if (saved.sourceType === "GOOGLE_SHOPPING_PAID_EXACT") exactPaid += 1;
      else if (saved.sourceType === "GOOGLE_SHOPPING_EXACT_LISTING") exactListing += 1;
      else if (saved.sourceType === "GOOGLE_SHOPPING_FREE_LISTING") freeListing += 1;
      else candidate += 1;
