...
const CHAIN_LANGUAGE =
  /\b(filialen|vestigingen|warenhuis|hypermarkt|winkelketen|landelijke dekking|onze winkels|alle vestigingen)\b/i;
...
export function countVerticalBreadth(text: string): number {
  let hits = 0;
  for (const vertical of VERTICALS) {
    // Word boundaries, otherwise "automatisch" counts as the auto vertical.
    if (new RegExp(`\\b${vertical}\\b`, "i").test(text)) hits += 1;
  }
  return hits;
}

export function computeBreadthSignals(signals: PageExtractedSignals): BreadthSignals {
  ...
  const internationalPresenceScore = Math.min(
    100,
    Math.round((localeAlternates / BREADTH_THRESHOLDS.internationalOperatorLocales) * 100)
  );
 ...
 */
export function qualifiesAsMassRetailer(breadth: BreadthSignals): boolean {
  return (
    breadth.retailerBreadthScore >= BREADTH_THRESHOLDS.massRetailerBreadthScore &&
