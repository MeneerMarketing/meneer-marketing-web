...
export async function classifyBusinessWithHaiku(
  env: Env,
  domain: string,
  signals: PageExtractedSignals
): Promise<{ classification: BusinessClassificationResult; estimatedCost: number }> {
...
{"businessType":"BRAND|SPECIALIST_WEBSHOP|GENERAL_RETAILER|MARKETPLACE|COMPARISON_SITE|SERVICE_BUSINESS|NON_ECOMMERCE|UNKNOWN","confidence":0.0-1.0,"reasoning":"short"}`;
  ...
  let parsed: { businessType?: string; confidence?: number; reasoning?: string } = {};
  try {
