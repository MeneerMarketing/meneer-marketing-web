import type { PlatformDetectionResult, PlatformType } from "../../types/crawler.js";
...
export function detectPlatform(html: string, finalUrl: string): PlatformDetectionResult {
  ...
  if (
    htmlLower.includes("window.shopify") ||
  ...
  const scores: Array<{ platform: PlatformType; score: number }> = [
  ...
  const best = scores[0] ?? { platform: "UNKNOWN" as PlatformType, score: 0 };

  const platformCandidate: PlatformType =
    best.score >= PLATFORM_CANDIDATE_THRESHOLD ? best.platform : "UNKNOWN";

  const platform: PlatformType =
    best.score >= PLATFORM_CONFIRM_THRESHOLD ? best.platform : "UNKNOWN";
...
export function mergePlatformDetections(
  detections: PlatformDetectionResult[]
  ...
  for (const detection of detections) {
    ...
    const scores = detection.evidence.scores as Record<string, number> | undefined;
    if (scores) {
      ...
      scoreMap[detection.platformCandidate] = Math.max(
        scoreMap[detection.platformCandidate] ?? 0,
        detection.platformConfidence
      );
    }
  }

  const ranked = Object.entries(scoreMap)
    .map(([platform, score]) => ({ platform: platform as PlatformType, score }))
    .sort((a, b) => b.score - a.score);

  const best = ranked[0] ?? { platform: "UNKNOWN" as PlatformType, score: 0 };
  const platformCandidate: PlatformType =
    best.score >= PLATFORM_CANDIDATE_THRESHOLD ? best.platform : "UNKNOWN";
  const platform: PlatformType =
    best.score >= PLATFORM_CONFIRM_THRESHOLD ? best.platform : "UNKNOWN";
