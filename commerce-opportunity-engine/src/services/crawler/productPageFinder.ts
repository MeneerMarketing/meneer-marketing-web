...
export function resolveProductCandidates(input: {
  ...
  const upsert = (candidate: ProductCandidate): void => {
  ...
  const landingSources: Array<[string, ProductResolutionSource]> = [
...
export function selectBestProductCandidate(
  ...
  if (best.source === "collection_link") {
    const productCandidate = candidates.find(
      (c) => classifyUrlPageType(c.url) === "PRODUCT" && c.score >= MIN_PRODUCT_RESOLUTION_CONFIDENCE
    );
...
export function emptyProductPageResult(pageType: PageType = "UNKNOWN"): ProductPageResult {
  return {
    pageType,
    productUrl: null,
    productName: null,
...
function pickBestJsonLd(
  products: PageExtractedSignals["jsonLdProducts"],
  preferredUrl: string,
  contextText: string
): PageExtractedSignals["jsonLdProducts"][number] | null {
