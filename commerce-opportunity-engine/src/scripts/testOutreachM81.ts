...
import {
  createEmailProvider,
  evaluateSendSafety,
  getOutreachDeliverabilityChecklist,
} from "../services/outreach/emailProvider.js";
...
async function main(): Promise<void> {
  ...
  assert(
    !evaluateOutreachEligibility({ ...baseEligible, doNotContact: true })
      .eligible,
    "DNC blocks"
  );
  assert(
    !evaluateOutreachEligibility({ ...baseEligible, manualExcluded: true })
      .eligible,
    "manual exclude blocks"
  );
  assert(
...
KVK 42095913`,
  ...
  assert(
    realBlocked.blockers.includes("OUTREACH_REAL_SEND_ENABLED=false"),
    "must cite lock"
  );
  ...
  console.log(
    `✓ huisdierspullen sync: ${changed.length} message(s) updated`,
    changed.map((c) => `${c.previousStatus}→${c.nextStatus}`).join(", ") ||
      "(already safe)"
  );
  ...
  for (const m of msgs ?? []) {
    assert(
      !["APPROVED"].includes(String(m.status)),
      `message ${m.id} still APPROVED after sync`
    );
    console.log(
      `  · ${m.id.slice(0, 8)}… ${m.status} · ${m.blocked_reason ?? "—"}`
    );
  }
  ...
  if (contact?.email) {
    ...
    const { data: inserted } = await supabase
      ...
      .select("id")
      .single();
    const afterSupp = await syncOutreachMessagesForBrand(
      supabase,
      String(brand!.id),
      "suppression"
    );
    console.log(
      `✓ suppression sync touched ${afterSupp.filter((s) => s.nextStatus).length} (or already blocked)`
    );
    if (inserted?.id) {
      await supabase
        .from("coe_outreach_suppression")
