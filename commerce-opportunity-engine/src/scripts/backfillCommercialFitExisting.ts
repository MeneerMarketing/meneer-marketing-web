...
async function main(): Promise<void> {
  ...
  for (const id of IDS) {
    const { data: opp, error } = await supabase
      .from("opportunities")
      .select(
        `id, opportunity_score, meneer_marketing_fit_score, rebuild_potential,
    ...
    if (error || !opp) throw new Error(error?.message ?? "missing opp");
    ...
    console.log(
      `${brand.normalized_domain}: ${merchant.relationship} → ${project.projectType} (PDP ${pdp}, rebuild ${full})`
    );
  }
}
