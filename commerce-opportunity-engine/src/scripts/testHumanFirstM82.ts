...
assert(
  !second.allowed && second.status === "BUDGET_BLOCKED",
  "must return BUDGET_BLOCKED"
);
...
const mail = assembleHumanFirstMail({
  ...
  verifiedObservation:
    "Op mobiel viel me bij het Trixie Premium Touring hondentuig op dat de prijs niet direct duidelijk in beeld staat.",
  verifiedStrength:
    "De variantkeuze op die pagina vond ik juist netjes opgelost.",
...
assert(
  mail.personalisationCopy.includes("prijs"),
  "personalisation has observation"
);
