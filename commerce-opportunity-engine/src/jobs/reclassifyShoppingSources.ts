...
type TargetRow = {
  ...
  merchant_item_type: string | null;
};
...
async function main(): Promise<void> {
  ...
  try {
    const { data: targets, error } = await supabase
      ...
      .select(
        "id, brand_id, keyword, ad_title, landing_url, ad_url, ad_url_redirects, shop_ad_aclk, product_id, seller, domain_match_status, source_type, raw_payload, merchant_item_type"
      )
      .eq("channel", "SHOPPING");
    ...
    for (const raw of (targets ?? []) as TargetRow[]) {
      const brand = brandById.get(raw.brand_id);
      const listingType =
        raw.merchant_item_type ??
        ((raw.raw_payload as { listing?: { type?: string } } | null)?.listing?.type ??
          null);
      ...
      counts[classified.sourceType as keyof typeof counts] =
        (counts[classified.sourceType as keyof typeof counts] ?? 0) + 1;
    ...
    // Sync linked opportunities by paid_search_target_id first
      ...
      for (const opp of related) {
        const prevEvidence = (opp.source_evidence ?? {}) as Record<string, unknown>;
        const shoppingLabel =
          classified.sourceType === "GOOGLE_SHOPPING_PAID_EXACT"
            ? "paid_exact"
        ...
        await supabase
          ...
          .update({
            ...
            paid_target_status:
              classified.croReadinessLevel === "EXACT_PAID_FUNNEL"
                ? "RESOLVED"
      ...
      console.log(
        `${raw.seller ?? "?"} · ${classified.sourceType} · listing=${classified.listingTargetConfidence} paid=${classified.paidEvidenceConfidence} · ${classified.croReadinessLevel}`
      );
    }
    ...
    const focus = examples.filter(
      (e) =>
        String(e.domain).includes("currentbody") ||
    ...
    for (const f of focus) {
      console.log(
        `FOCUS: ${f.seller} → ${f.to} (listing ${f.listingConf}, paid ${f.paidConf}, ${f.croLevel})`
      );
    }
