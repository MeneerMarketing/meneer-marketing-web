...
import {
  M995_INTEGRITY_VERSION,
  M995_REPORT_PATH,
  M995_DASHBOARD_REPORT_PATH,
  M994_INPUT_REPORT_PATH,
  M995_MAX_VALIDATED_PROSPECTS,
  M995_MAX_ANTHROPIC_COST,
  M995_JOOM_REGRESSION,
  M995_TRVLMORE_REVIEW,
  M995_CLEANMASTER_FIXTURE,
  M995_SCREENSHOT_DIR,
...
import {
  classifyCaptureHealthFromScreenshot,
  nullVisualScoresWhenCaptureInvalid,
} from "../services/prospect/captureHealthClassifier.js";
...
function showcaseIntegrityPass(input: {
  ...
  if (input.preVisionHardReject || !input.visionScoreAllowed) return false;
  if (input.showcasePageEntityType !== "PRODUCT_DETAIL") return false;
  if (
...
function businessQualifiedFromRow(row: M994Row): boolean {
  const maturity = asNumber(row.businessMaturityScore) ?? 0;
  const scale = asNumber(row.companyScaleFit) ?? 0;
  return maturity >= 25 && scale >= 35;
}

function refinedModelFromRow(row: M994Row): ShowcaseOwnershipClass {
  const model = asString(row.refinedBusinessModel) ?? asString(row.businessModel) ?? "UNKNOWN";
  return model as ShowcaseOwnershipClass;
}

async function rescoreRow(row: M994Row): Promise<Record<string, unknown>> {
  const domain = String(row.domain);
  const productUrl = String(row.productUrl);
  const businessModel = (asString(row.businessModel) ?? "UNKNOWN") as BusinessModelClass;
  ...
  const manualRationale = row.manualRationale as Record<string, string> | null;
  ...
  const pdpShot =
    pickScreenshot(screenshots, "pdp-desktop-1440x1000") ??
  ...
  let currentSiteImpression: CurrentSiteImpression | null =
    (asString(row.currentSiteImpression) as CurrentSiteImpression | null) ?? null;
  ...
  const lead = classifyM994LeadType({
    ...
    visualRedesignOpportunityType:
      (asString(row.visualRedesignOpportunityType) as
...
async function runJoomRegression(): Promise<Record<string, unknown>> {
  const preVision = classifyPreVisionBusiness({
    domain: M995_JOOM_REGRESSION.domain,
    productUrl: M995_JOOM_REGRESSION.productUrl,
  });

  const joomShot = resolve(
    M994_SCREENSHOT_DIR,
    "joom-com-pdp-desktop-1440x1000.png"
  );
  const capture = await classifyCaptureHealthFromScreenshot({
  ...
  const pass =
    preVision.businessType === M995_JOOM_REGRESSION.expectedBusinessType &&
...
async function buildTrvlmoreReview(crawlTimeout: number): Promise<Record<string, unknown>> {
  ...
  const whyVisuallyUnderdesigned =
    capture.visionScoreAllowed
  ...
  const businessFits =
    !preVision.hardRejectBeforeVision &&
  ...
  return {
    ...
    screenshots: Object.fromEntries(
      Object.entries(shots).map(([k, v]) => [k, relScreenshot(v)])
    ),
    humanReview: {
      currentHomepage:
        homeCrawl.status === "success"
      ...
      whatMaterialExists:
        "Product photos, specs and buyblock present. Lifestyle and editorial layers look thin from crawl signals.",
      whatPremiumRedesignCouldChange:
        "Hero product story, gallery depth, typography system, trust blocks and mobile buyblock polish.",
...
function compareCandidates(
  ...
  trvl: Record<string, unknown>
): Record<string, unknown> {
  const tBiz = trvl.business as Record<string, unknown>;
  const tCat = trvl.catalog as Record<string, unknown>;
  const tVis = trvl.visual as Record<string, unknown>;
  ...
  const table = {
    ...
    materialFeasibility: {
      ...
      trvlmore: (trvl.material as Record<string, unknown>).redesignFeasibilityProxy,
    },
    productRelevance: {
  ...
  let better: "CLEANMASTER" | "TRVLMORE" | "NEITHER" = "NEITHER";
  if (
    clean?.leadType === "VALIDATED_SHOWCASE_PROSPECT" &&
...
export async function runM995PreVisionIntegrityRescore(): Promise<void> {
  const env = loadEnv();
  ...
  const m994 = JSON.parse(m994Raw) as Record<string, unknown>;
  const allScreened = (m994.allScreened as M994Row[]) ?? [];
  const priorValidated = (m994.validatedShowcaseProspects as M994Row[]) ?? [];
  const funnel = m994.funnel as Record<string, number>;
  ...
  const validatedShowcaseProspects = rescored
    ...
    .sort(
      (a, b) =>
        (asNumber(b.validatedVisualSalesFit) ?? 0) -
        (asNumber(a.validatedVisualSalesFit) ?? 0)
    )
    .slice(0, M995_MAX_VALIDATED_PROSPECTS);

  const cleanmasterRow =
    rescored.find((r) => r.domain === M995_CLEANMASTER_FIXTURE.domain) ??
    (priorValidated.find((r) => String(r.domain) === M995_CLEANMASTER_FIXTURE.domain) as
      | Record<string, unknown>
      | undefined);
  ...
  const report = {
    ...
    m994Rescore: {
      ...
      joomRemoved: !rescored.some(
        (r) =>
          r.domain === "joom.com" && r.leadType === "VALIDATED_SHOWCASE_PROSPECT"
      ),
      preVisionRejectCounts: rescored.reduce<Record<string, number>>((acc, row) => {
      ...
      invalidCaptureCounts: rescored.reduce<Record<string, number>>((acc, row) => {
