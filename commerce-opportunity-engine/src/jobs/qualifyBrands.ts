...
import type { QualificationResult, QualificationRunStats } from "../types/crawler.js";
...
function printReport(stats: QualificationRunStats): void {
  console.log("");
  ...
  for (const result of stats.results) {
    ...
    console.log(
      `  business_type: ${result.businessType} (${result.businessTypeConfidence})`
    );
    console.log(
      `  platform: ${result.platform} conf=${result.platformConfidence} candidate=${result.platformCandidate}`
    );
    ...
    console.log(
      `  product: ${result.productPage.productUrl ?? "null"} (conf=${result.productPage.productResolutionConfidence})`
    );
    console.log(`  product_name: ${result.productPage.productName ?? "null"}`);
    console.log(
      `  price: ${result.productPage.price != null ? `${result.productPage.currency ?? "EUR"} ${result.productPage.price}` : "null"}`
    );
    console.log(
      `  reviews: ${result.productPage.reviewCount ?? "null"} rating=${result.productPage.rating ?? "null"}`
    );
  ...
  if (eligible.length === 0) {
    ...
    eligible.forEach((result, index) => {
      ...
      if (result.productPage.price != null) {
        console.log(
          `   Price: ${result.productPage.currency ?? "EUR"} ${result.productPage.price}`
        );
      }
      console.log("");
    });
...
async function saveQualificationOutputs(
...
export async function runBrandQualification(): Promise<QualificationRunStats> {
  const env = loadEnv();
  const supabase = createSupabaseServerClient(env);

  const candidates = await loadBrandsForQualification(
    supabase,
    env.CRAWLER_MAX_BRANDS_PER_RUN,
    { forcePriorityDomains: env.QUALIFY_FORCE_PRIORITY_DOMAINS }
  );
  ...
  const results = await mapWithConcurrency(
    ...
    async (candidate) => {
      try {
        ...
        const failed: QualificationResult = {
  ...
  const stats: QualificationRunStats = {
