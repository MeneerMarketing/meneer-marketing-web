...
import {
  M921_CONSERVATIVE_AUDIT_COST,
  OUTREACH_PILOT_GATE,
} from "../config/outreachCroCoverage.js";
...
import {
  buildConceptAuditCandidate,
  selectOutreachAuditCandidates,
} from "../services/concept/selectOutreachAuditCandidates.js";
...
} from "../services/concept/currentPdpQuality.js";
import {
...
type AuditRunRecord = {
  ...
  skipped: boolean;
  skipReason?: string;
  pageHealthStatus?: string | null;
  currentPdpQuality?: number | null;
  currentPdpBand?: string | null;
...
async function patchAuditTarget(
  ...
  const findings =
    audit?.findings && typeof audit.findings === "object"
      ? { ...(audit.findings as Record<string, unknown>) }
      : {};
...
async function recomputeConceptTransformation(
  ...
  supabase: any,
  conceptId: string,
  cro: CroQualityScores,
  signals: ConceptFirstPdpSignals | null,
  entry: Awaited<ReturnType<typeof loadConceptProspectPool>>[number]
): Promise<number> {
  ...
  const transform = scorePdpTransformationPotential({
    ...
    deepDiveWeak:
      signals?.deep_dive_quality != null
...
function selectTrueOutreachPilot(
  scored: Array<{
    entry: Awaited<ReturnType<typeof loadConceptProspectPool>>[number];
    outreach: ReturnType<typeof scoreOutreachConceptFit>;
    gate: ReturnType<typeof evaluateOutreachCandidateGate>;
  ...
  if (!pick) {
    const proxyFallback = scored
      ...
      .sort(
        (a, b) => b.outreach.outreachConceptFitScore - a.outreach.outreachConceptFitScore
      )[0];

    if (!proxyFallback) return { winner: null, runnerUps: [], why: ["no_gate_eligible_candidates"] };
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const auditRuns: AuditRunRecord[] = [];
    ...
    for (const candidate of selected) {
      ...
      await recomputeConceptTransformation(
        supabase,
        row.id,
        cro,
        signals,
    ...
    const tensfactComparison = {
      ...
      top3,
      tensfactStillNumberOne:
        tensfact != null && top10[0]?.domain === "tensfact.com",
      note:
        tensfact != null && top10[0]?.domain !== "tensfact.com"
