...
import {
  M933_DISCOVERY,
  M933_DISCOVERY_VERSION,
  PARKED_FAMILIES,
  PRODUCTION_BRANCHES,
  STRONG_PROSPECT_THRESHOLD,
  TARGET_PROFILE,
  type ProspectFunnelStage,
} from "../config/productionDiscovery.js";
import {
...
const DASHBOARD_REPORT_PATH = resolve(
  projectRoot,
  "dashboard/src/preview/concepts/data/focused-production-discovery-report.json"
);
...
interface DiscoveryReport {
  ...
  discovery: {
    ...
    /** Selected but without a SERP sample: a failed call or a seed added later. */
...
async function replayStoredRun(input: {
  ...
  const storedByDomain = await loadStoredClassifications(
    input.supabase,
    [
      ...new Set(
        rows
...
export async function runFocusedProductionDiscovery(options?: {
  ...
  const budget: BudgetTracker = { spent: 0, cap: env.M933_MAX_DATAFORSEO_COST };
  ...
  console.log(
    `  ${recompute.corrected} gecorrigeerd, ${recompute.reverted} onterechte uitsluitingen teruggedraaid, ` +
  ...
  console.log(
    `  ${expansion.keywords.length} keywords met volledige lineage, ${expansion.rejected.length} geweigerd, ` +
  ...
  if (options?.dryRun) {
    for (const keyword of selected) {
      console.log(
        `  ${keyword.archetypeId.padEnd(14)} ${keyword.familyId.padEnd(26)} ${keyword.generationSource.padEnd(17)} fit ${String(keyword.archetypeFit).padStart(3)} · ${keyword.keyword}`
      );
    }
    console.log(
  ...
  for (const keyword of options?.replayRunId ? [] : persisted) {
    ...
    const sample: Array<{ domain: string; landingUrl: string | null }> = [];
  ...
  const excludedRetailers: DiscoveryReport["excludedRetailers"] = [];
  ...
  for (const entry of eligible) {
    ...
    if (catalogChecks >= M933_DISCOVERY.maxCatalogChecks) continue;
  ...
  const focused = specialists.filter(
    (entry) =>
      entry.catalogVerified &&
      (entry.catalogFocusScore ?? 0) >= 55 &&
      (entry.estimatedCatalogSize ?? 0) <= TARGET_PROFILE.catalogHardMax
  );
  const ownBrand = focused.filter(
    (entry) => (entry.ownBrandSignal ?? 0) >= TARGET_PROFILE.minOwnBrandSignal
  );
  ...
  const funnel: Record<ProspectFunnelStage, number> = {
  ...
  const costPerStrongProspect =
    strongProspects.length > 0
...
async function loadKeywordsFromRun(
  ...
  if (keywordIds.length === 0 && names.length === 0) return [];
  ...
  const data = [...byId.values()] as Array<Record<string, never>>;
...
function printReport(report: DiscoveryReport): void {
  ...
  console.log(
    `  ${report.classifier.correctedThisPass} gecorrigeerd, ${report.classifier.revertedExclusions} onterechte uitsluitingen teruggedraaid`
  );
  for (const entry of report.classifier.revertedDomains) {
    console.log(`    ${entry.domain}: ${entry.from} → opnieuw bepalen`);
  }
  console.log(
    `  regressie: gate ${report.classifier.gateRegression} · classifier ${report.classifier.classifierRegression}`
  );
  ...
  console.log(
    `  keywords: ${report.discovery.keywordsGenerated} gegenereerd, ${report.discovery.keywordsTested} getest, ${report.discovery.keywordsApproved} goedgekeurd`
  );
  if (report.discovery.keywordsNotTested.length > 0) {
    console.log(
      `  niet gemeten in deze run: ${report.discovery.keywordsNotTested.join(", ")}`
    );
  }
  console.log(
    `  advertisers: ${report.discovery.rawAdvertisers} ruw, ${report.discovery.uniqueAdvertisers} uniek`
  );
  console.log(
    `  shopping sellers: ${report.discovery.shoppingSellersResolved} herleid, ${report.discovery.shoppingSellersUnresolved} onopgelost`
  );

  console.log("\n=== KEYWORDS ===");
  for (const keyword of report.keywords) {
    console.log(
      `  ${(keyword.approvedForDiscovery ? "OK" : "STOP").padEnd(4)} ${keyword.status.padEnd(22)} ${keyword.keyword} · ${keyword.familyLabel} · ${keyword.generationSource} · quality ${keyword.serpQualityScore} · ${keyword.rawAdvertisers} advertisers`
    );
  }
  ...
  console.log(
    `  prospect yield ${report.funnelYieldPercent.prospectYield}% · strong prospect yield ${report.funnelYieldPercent.strongProspectYield}%`
  );
  ...
  console.log("\n=== COST ===");
  console.log(
    `  DataForSEO $${report.cost.dataForSeo.toFixed(4)} van cap $${report.cost.dataForSeoCap.toFixed(3)} (keyword ideas $${report.cost.keywordIdeas.toFixed(4)}, SERP $${report.cost.serp.toFixed(4)})`
  );
  console.log(`  Anthropic $0.0000`);
  console.log(
    `  kosten per sterke prospect: ${report.cost.costPerStrongProspect !== null ? `$${report.cost.costPerStrongProspect.toFixed(4)}` : "n.v.t."}`
  );
  console.log(`\nRapport: ${REPORT_PATH}\n`);
}
