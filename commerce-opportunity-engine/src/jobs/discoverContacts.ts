...
async function loadCandidateBrands(
  ...
  const mapped = (data ?? [])
    .map((row: Record<string, unknown>) => {
      ...
      if (brand.manual_excluded || brand.do_not_contact) return null;
      if (brand.first_touch_sent_at) return null;
      return {
        brandId: String(brand.id),
        domain: String(brand.normalized_domain),
        name: (brand.name as string | null) ?? null,
...
async function main(): Promise<void> {
  ...
  try {
    for (const candidate of candidates) {
      ...
      const { preferredId } = await upsertDiscoveredContacts(
        supabase,
        candidate.brandId,
        discovery.contacts,
        discovery.preferred?.email ?? null
      );
    ...
    for (const r of results) {
      ...
      if (r.skipped) {
        ...
        console.log(
          `  preferred: ${r.preferredEmail ?? "—"} (${r.preferredType ?? "n/a"}) conf ${r.preferredConfidence ?? "—"}`
        );
        const emails = (r.emails as Array<Record<string, unknown>>) ?? [];
        for (const e of emails) {
          console.log(
            `    - ${e.email} [${e.type}] conf=${e.confidence} ← ${e.sourceUrl}`
          );
        }
      }
      console.log("");
