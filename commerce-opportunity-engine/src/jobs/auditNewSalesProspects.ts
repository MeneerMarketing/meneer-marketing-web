...
import {
  preselectNewProspects,
  isUsableHeroUrl,
  type NewProspectRecord,
  type PreselectionEntry,
} from "../services/idealProspect/newProspectPreselection.js";
import { resolveHeroProducts, type ResolvedHero } from "../services/prospect/heroProductResolver.js";
...
import {
  ...
  type OutreachScoringInput,
  type OutreachScoringResult,
} from "../services/concept/outreachScoring.js";
import {
  computeTrueSalesDesignScore,
  evaluateDesignTargetGate,
  type DesignTargetGateResult,
} from "../services/concept/trueSalesDesignScore.js";
import { buildDesignCaseRationale, type DesignCaseRationale } from "../services/concept/designCaseRationale.js";
import type { BrandCommerceModel } from "../config/conceptScoring.js";
import type { ConversionLeak, CroQualityScores, PageRepresentation } from "../types/audit.js";
...
const DISCOVERY_REPORT_PATH = resolve(
  projectRoot,
  "reports/focused-production-discovery-report.json"
);
const REPORT_PATH = resolve(projectRoot, "reports/new-sales-prospect-audit-report.json");
const DASHBOARD_REPORT_PATH = resolve(
  projectRoot,
  "dashboard/src/preview/concepts/data/new-sales-prospect-audit-report.json"
);
...
interface DesignCase {
  domain: string;
  trueSalesDesignScore: number | null;
  rationale: DesignCaseRationale;
  screenshots: Record<string, string>;
}

interface PreselectionRow {
  rank: number;
  domain: string;
  preselectionScore: number;
...
interface AuditReportRow {
  ...
  adsEvidence: NewProspectRecord["googleAdsEvidence"];
  auditOutcome: string;
  auditSkipReason: string | null;
  pageHealth: string | null;
  currentPdpQuality: number | null;
...
interface M934Report {
  ...
  designTargetGate: typeof DESIGN_TARGET_GATE;
  trueSalesDesignFormula: string;
  regression: Array<Record<string, unknown>>;
  cost: {
    anthropic: number;
    anthropicCap: number;
    underlyingAuditCost: number;
    dataForSeo: number;
    auditsRun: number;
...
function conceptSignalsFrom(audit: AuditReadback | null): ConceptFirstPdpSignals | null {
  const findings = audit?.findings;
  if (!findings || typeof findings !== "object") return null;
  return (
    (findings as { concept_first_signals?: ConceptFirstPdpSignals }).concept_first_signals ?? null
  );
}

function representationFrom(audit: AuditReadback | null): PageRepresentation | null {
  const rep = audit?.page_representation;
  if (!rep || typeof rep !== "object") return null;
  return rep as unknown as PageRepresentation;
}
...
function hasCommercialProof(record: NewProspectRecord, brand: BrandRow | null): boolean {
  if (brand?.confirmed_google_advertiser) return true;
  return (record.googleAdsEvidence?.keywords?.length ?? 0) > 0;
}

function isFocusedBusiness(record: NewProspectRecord): boolean {
 ...
 */
function loadStrongProspects(): { records: NewProspectRecord[]; sources: string[] } {
  if (!existsSync(DISCOVERY_REPORT_PATH)) {
    throw new Error(
      `M9.3.3 report not found at ${DISCOVERY_REPORT_PATH}. Run the focused production discovery first.`
    );
  }
  ...
  const byDomain = new Map<string, NewProspectRecord>();
  for (const file of sources) {
    const raw = JSON.parse(readFileSync(resolve(reportDir, file), "utf8")) as {
      topProspects?: NewProspectRecord[];
    };
    for (const record of raw.topProspects ?? []) {
      if ((record.idealProspectPreScore ?? 0) < STRONG_PROSPECT_THRESHOLD) continue;
...
async function loadBrands(
  ...
  for (let index = 0; index < domains.length; index += 100) {
    ...
    const { data, error } = await supabase
      .from("brands")
      .select(
        "id, name, normalized_domain, platform, business_type, business_maturity_score, retailer_scale_score, confirmed_google_advertiser, transparency_confirmed, manual_excluded, lead_eligible"
      )
      .in("normalized_domain", chunk);
    if (error) throw new Error(error.message);
    for (const row of data ?? []) {
      result.set(row.normalized_domain as string, row as BrandRow);
    }
  }
  return result;
}
...
export async function resolvePrimarySalesProduct(
  record: NewProspectRecord
): Promise<{ hero: ResolvedHero | null; reason: string | null }> {
  const adProducts = [
    record.heroProduct
      ? {
  ...
  try {
    ...
    const usable = resolution.heroes.find(
      (hero) =>
        isUsableHeroUrl(hero.url, record.domain) && hero.heroScore >= M934.minHeroScore
    );
    if (usable) return { hero: usable, reason: null };
    ...
    return {
      hero: null,
      reason:
        resolution.heroes.length === 0
          ? "geen hero product gevonden"
          : "hero zonder betrouwbare product-URL",
    };
  } catch (error) {
    return { hero: null, reason: `hero resolutie mislukt: ${(error as Error).message}` };
  }
}
...
export async function upsertConceptCandidate(
  supabase: SupabaseClient,
  input: {
    brandId: string;
    record: NewProspectRecord;
  ...
  const patch: Record<string, unknown> = {
    ...
    own_brand_ratio_estimate:
      record.ownBrandSignal != null ? Math.round(record.ownBrandSignal) / 100 : null,
...
): Promise<AuditReadback | null> {
  const { data, error } = await supabase
    .from("audits")
    .select(
      "id, status, audit_valid, audit_confidence, cro_scores, conversion_leaks, strengths, findings, page_health_status, page_representation, meneer_marketing_fit_score, anthropic_cost, screenshot_paths"
    )
    .eq("opportunity_id", opportunityId)
    .order("created_at", { ascending: false })
    .limit(1);
  if (error) throw new Error(error.message);
  return (data?.[0] as AuditReadback | undefined) ?? null;
}
...
function isPurchasablePage(
  rep: PageRepresentation | null,
  buyblock: number | null
): boolean {
  if (!HERO_PAGE_VALIDITY.enabled) return true;
  if ((num(rep?.aboveTheFold?.price) ?? 0) > 0) return true;
  if ((rep?.page?.payments?.length ?? 0) > 0) return true;
...
export function scoreAuditedCandidate(input: {
  ...
  const auditSawContent =
    (signalScores.storytelling ?? 0) >= EXTRACTION_FALLBACK.minSignalForContent ||
  ...
  const effectiveDescriptionLength =
  ...
  const assets = scoreConceptAssetReadiness({
    ...
    lifestyleImageryLikely:
      (rep?.page?.ugc?.length ?? 0) > 0 ||
    ...
    hasScreenshots: Boolean(
      audit.screenshot_paths && Object.values(audit.screenshot_paths).some(Boolean)
    ),
  });
  ...
  let currentPdpQuality: number | null = null;
  if (cro) {
    currentPdpQuality = computeCurrentPdpQualityScore(cro as CroQualityScores, signals).score;
  }

  const subScores: AuditedCandidate["subScores"] = {
  ...
  const leaks = (Array.isArray(audit.conversion_leaks)
    ...
    : []) as ConversionLeak[];
  const strengths = (Array.isArray(audit.strengths) ? audit.strengths : []) as Array<{
    title: string;
  }>;
  ...
  const conceptReady = scoreConceptReady({
    ...
    platform: record.platform ?? brand?.platform ?? null,
    catalogFocus: record.catalogFocusScore ?? 50,
    brandCommerceModel: input.preselection.derivedCommerceModel as BrandCommerceModel,
    heroProductScore: hero.heroScore,
    productCommercialSignal: commercial.product_commercial_signal_score,
    pdpTransformationPotential: transformation.pdp_transformation_potential,
...
async function captureCurrentPdpScreenshots(
  cases: Array<{ domain: string; url: string }>
...
async function loadRegressionRows(
  supabase: SupabaseClient,
  domains: string[]
  ...
  const { data, error } = await supabase
    .from("coe_concept_candidates")
    .select(
      "concept_contrast_potential, concept_contrast_band, concept_contrast_ceiling, pdp_transformation_potential, concept_asset_readiness_score, status, brands!inner(normalized_domain, business_type)"
    )
  ...
  return (data ?? []).map((row) => {
    ...
    return {
      ...
      note:
        row.concept_contrast_band === "GEEN_CONTRAST" || (row.concept_contrast_potential ?? 0) < 40
...
async function main(): Promise<void> {
  ...
  // -------------------------------------------------------------------------
  const auditQueue: Array<{ entry: PreselectionEntry; brand: BrandRow; hero: ResolvedHero }> = [];

  for (const entry of ranked) {
    if (entry.excluded) continue;
    if (auditQueue.length >= M934.maxAuditAttempts) {
      if (!entry.skipReason) entry.skipReason = "buiten de audit-limiet";
      continue;
    }
    ...
    console.log(
      `  HERO ${entry.record.domain} → ${hero.title} (${hero.source}, score ${hero.heroScore})`
    );
  ...
  if (dryRun) {
    ...
    for (const entry of ranked) {
      ...
      console.log(
        `  ${queued > -1 && queued < M934.maxAudits ? "AUDIT" : "     "} #${String(entry.rank).padStart(2)} ${entry.record.domain.padEnd(26)} ${String(entry.preselectionScore).padStart(3)} · ${entry.derivedCommerceModel}${entry.skipReason ? ` · ${entry.skipReason}` : ""}`
      );
    }
  ...
  for (const { entry, brand, hero } of auditQueue) {
    ...
    const heroUrl = hero.url as string;
    ...
    if (!rescore) console.log(`  AUDIT ${record.domain} → ${hero.url}`);
    ...
    if (rescore) {
      console.log(
        `  RESCORE ${record.domain} → ${claudeRan ? "audit gevonden" : "geen bruikbare audit"}`
      );
    }

    const base: AuditedCandidate = {
      domain: record.domain,
      rank: entry.rank,
    ...
    if (!claudeRan || !audit) {
      ...
      console.log(
        `    geen audit: ${base.auditSkipReason} (page health ${base.pageHealth ?? "onbekend"})`
      );
      audited.push(base);
      continue;
    }
    ...
    completedAudits += 1;
    console.log(
      `    PDP ${scored.currentPdpQuality} · transformatie ${scored.transformation} · contrast ${scored.outreach.contrast.concept_contrast_potential} · sales design ${trueSales.score} · $${result.anthropicCost.toFixed(4)}`
    );
  }
  ...
  const screenshots = await captureCurrentPdpScreenshots(
    topThree
      .filter((entry) => entry.hero?.url)
      .map((entry) => ({ domain: entry.domain, url: entry.hero!.url as string }))
  );

  const designCases: DesignCase[] = topThree.map((entry) => ({
    domain: entry.domain,
  ...
  const report: M934Report = {
    ...
    preselection: {
      ...
      formula:
        "gewogen som van de M9.3.3 signalen minus penalties voor reseller-breedte, te grote catalogus, zwak materiaal, lage productwaarde en ketengedrag",
    ...
    cost: {
      ...
      underlyingAuditCost:
        Math.round(audited.reduce((sum, entry) => sum + entry.anthropicCost, 0) * 10000) /
        10000,
      dataForSeo: 0,
...
function printReport(report: M934Report): void {
  console.log("\n=== PRESELECTIE ===");
  for (const candidate of report.preselection.candidates) {
    const mark = candidate.selected ? "AUDIT" : "     ";
    console.log(
      `  ${mark} #${String(candidate.rank).padStart(2)} ${candidate.domain.padEnd(24)} score ${String(candidate.preselectionScore).padStart(3)} · ${candidate.commerceModel}${candidate.skipReason ? ` · ${candidate.skipReason}` : ""}`
    );
  }
  ...
  if (report.designTarget) {
    console.log(
      `  ${report.designTarget.domain} · sales design ${report.designTarget.trueSalesDesignScore} · contrast ${report.designTarget.conceptContrast}`
    );
  ...
  for (const row of report.regression) {
    console.log(
      `  ${row.domain}: contrast ${row.conceptContrast ?? "?"} (${row.contrastBand ?? "?"}) · ${row.note}`
    );
  }

  console.log("\n=== KOSTEN ===");
  console.log(
    `  Anthropic $${report.cost.anthropic} van cap $${report.cost.anthropicCap} · ${report.cost.auditsRun} audits · DataForSEO $0`
  );
  if (report.cost.underlyingAuditCost > report.cost.anthropic) {
    console.log(
      `  Onderliggende audits kostten eerder $${report.cost.underlyingAuditCost}`
    );
  }
}
...
if (invokedDirectly) {
  main().catch((error) => {
    logger.error("M9.3.4 audit run failed", { error: (error as Error).message });
    console.error(error);
    process.exit(1);
  });
}
