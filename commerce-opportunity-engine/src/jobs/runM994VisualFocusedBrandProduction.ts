...
import {
  type M992ProductQuery,
} from "../config/visualUnderdesignedDiscovery.js";
import {
  buildM994HarvestQueries,
  M994_DISCOVERY,
  M994_DISCOVERY_ROUTE,
  M994_DISCOVERY_VERSION,
  M994_PARKED_FAMILIES,
  M994_REPORT_PATH,
  M994_DASHBOARD_REPORT_PATH,
} from "../config/visualFocusedBrandProduction.js";
...
import {
  ...
  evaluatePreVisionBusinessNoise,
...
import { classifyPageEntity, type PageEntityType } from "../services/prospect/pageEntityClassifier.js";
import { classifyM994LeadType, type M994LeadType } from "../services/prospect/validatedShowcaseLeadClassifier.js";
import {
...
import { inferPriceConfidence, type PriceConfidence } from "../services/prospect/highTicketGapSalesFit.js";
import { assessPriceConsistency } from "../services/prospect/priceConsistencyCheck.js";
import {
  computeVisualRedesignSalesFit,
  classifyVisualRedesignOpportunity,
  type VisualRedesignOpportunityType,
} from "../services/prospect/visualRedesignSalesFit.js";
import {
  isVisuallyUnderdesigned,
  serpPositionBandExtended,
  visualQualityBand,
} from "../services/prospect/visualQualityScore.js";

const scriptDir = dirname(fileURLToPath(import.meta.url));
const projectRoot = resolve(scriptDir, "../../");
config({ path: resolve(projectRoot, ".env"), quiet: true });

const REPORT_PATH = resolve(projectRoot, M994_REPORT_PATH);
const DASHBOARD_REPORT_PATH = resolve(projectRoot, M994_DASHBOARD_REPORT_PATH);
const SCREENSHOT_DIR = resolve(projectRoot, M994_DISCOVERY.screenshotDir);
...
function screenQueuePriority(input: {
  serpPositionExtended: string;
  urlPlausibility: number;
}): number {
  let score = input.urlPlausibility;
  if (input.serpPositionExtended === "21_50") score += 12;
  else if (input.serpPositionExtended === "11_20") score += 8;
  else if (input.serpPositionExtended === "51_100") score += 14;
  else if (input.serpPositionExtended === "101_PLUS") score += 10;
  return score;
}

interface M994Candidate {
  candidateKey: string;
  domain: string;
  productUrl: string;
  sourceQuery: string;
  ...
  currentVisualQualityBand: string;
  visuallyUnderdesigned: boolean;
  templateDriven: boolean;
  visionReasoning: string | null;
  visualRedesignOpportunityType: VisualRedesignOpportunityType | null;
  visualRedesignSalesFit: number | null;
  visualRedesignConfidence: string;
  leadType: M994LeadType | string;
  opportunityTier: string;
  opportunityTierReason: string;
  showcasePageEntityType: string;
  preVisionRejected: boolean;
  preVisionRejectReason: string | null;
  refinedBusinessModel: ShowcaseOwnershipClass;
  brandOwnershipConfidence: number | null;
  brandOwnershipEvidence: string[];
  crossDomainProductMatch: CrossDomainProductMatch;
  crossDomainPeers: string[];
  validatedVisualSalesFit: number | null;
  ...
  productStoryValue: number | null;
  paidAcquisition: string;
  rank: number | null;
  screenshots: Record<string, string> | null;
  manualRationale: Record<string, string> | null;
  cachedHtml: string | null;
}

export async function runM994VisualFocusedBrandProduction(options?: {
  dryRun?: boolean;
}): Promise<void> {
  const discovery = M994_DISCOVERY;
  const env = loadEnv();
  const supabase = createSupabaseServerClient(env);
  const dataForSeo = createDataForSeoClient(env);
  const dfsBudget: Budget = { spent: 0, cap: env.M994_MAX_DATAFORSEO_COST };
  const anthropicBudget: Budget = { spent: 0, cap: env.M994_MAX_ANTHROPIC_COST };
  const serpOptions = { client: dataForSeo, env };
  const startedAt = new Date().toISOString();
  const crawlTimeout = discovery.crawlTimeoutMs;
  const queries = buildM994HarvestQueries();

  console.log("\n=== M9.9.4 VISUAL FOCUSED BRAND PRODUCTION ===");
  console.log(`Target: ${FOCUSED_BRAND_GAP_FIRST_TARGET_V1}`);
  console.log(`Queries: ${queries.length} · SERP depth ${discovery.serpDepth}`);
  console.log(`Parked: ${M994_PARKED_FAMILIES.join(", ")}`);
  ...
  const run = await createRun(supabase, "visual_focused_brand_production", {
    milestone: discovery.milestone,
    version: M994_DISCOVERY_VERSION,
    route: M994_DISCOVERY_ROUTE,
    targetProfile: FOCUSED_BRAND_GAP_FIRST_TARGET_V1,
    queries: queries.map((q) => q.query),
  });

  const candidateMap = new Map<string, M994Candidate>();
  const earlyRejectCounts: Record<string, number> = {};
  let queriesExecuted = 0;
  const positionBandHarvest: Record<string, number> = {};

  const makeCandidate = (
    entry: M992ProductQuery,
    input: {
      domain: string;
      productUrl: string;
      sourceType: PdpHarvestSourceType;
  ...
  async function fetchSerp(query: string, depth: number): Promise<Awaited<ReturnType<typeof fetchGooglePaidAds>>> {
    try {
  ...
  if (
    ...
    for (const entry of queries.slice(0, queriesExecuted)) {
      if (!canSpend(dfsBudget, discovery.estimatedSerpCostExtendedPerKeyword)) break;
      if (candidateMap.size >= discovery.maxRawCandidates) break;
      try {
        const serp = await fetchSerp(entry.query, discovery.serpDepthExtended);
        dfsBudget.spent += serp.cost;
        for (const organic of serp.organicResults) {
          if (!organic.url) continue;
          if ((organic.rank ?? 0) <= discovery.serpDepth) continue;
          const sourceType = classifyHarvestSourceType({
            sourceType: "ORGANIC_PRODUCT_RESULT",
            likelyRetailer: organic.likelyRetailer,
          });
  ...
  const validCandidates: M994Candidate[] = [];
  let invalidPdp = 0;
  const entityRejectCounts: Record<string, number> = {};
  const preVisionRejectCounts: Record<string, number> = {};
  ...
  let visionScreens = 0;
  const visualQualityDistribution: Record<string, number> = {};
  ...
  for (let i = 0; i < screenQueue.length; i += 1) {
    ...
    candidate.mobileGap = Math.max(
      ...
      Math.min(
        100,
        Math.round(purchase.score * 0.55 + 18 + (purchaseSignals.mobileAtcSignal ? 0 : 12))
      )
    );
    ...
    let quickPaths: Record<string, string> = {};
    try {
    ...
    if (
      ...
      try {
        const vision = await screenPdpVisualRedesignWithVision(
          env,
          candidate.domain,
          quickPaths["pdp-desktop-1440x1000"]
        );
        ...
        candidate.mobileGap = Math.max(
          0,
          Math.min(100, (candidate.mobileGap ?? 0) + vision.mobileAdjustment)
        );
      ...
      candidate.currentVisualQualityScore = Math.max(
        0,
        Math.min(100, 100 - (candidate.preauditVisualGap ?? 45))
      );
    }

    candidate.currentVisualQualityBand = visualQualityBand(
      candidate.currentVisualQualityScore
    );
    ...
    candidate.visuallyUnderdesigned = isVisuallyUnderdesigned(
      candidate.currentVisualQualityScore,
      discovery.currentVisualQualityShowcaseMax
    );
  ...
  const shortlistFingerprints = visuallyWeakShortlist.map((c) =>
    extractProductIdentityFingerprint(
      c.cachedHtml ?? "",
      c.productUrl,
      c.domain,
  ...
  for (let wi = 0; wi < visuallyWeakShortlist.length; wi += 1) {
    ...
    const cross = crossMap.get(candidate.domain) ?? {
      match: "NONE" as CrossDomainProductMatch,
      peers: [] as string[],
      evidence: [] as string[],
    };
    ...
    candidate.currentSiteImpression = deriveCurrentSiteImpression(
      candidate.currentVisualQualityScore
    );

    candidate.validatedVisualSalesFit = computeValidatedVisualSalesFit({
      currentVisualQualityScore: candidate.currentVisualQualityScore,
      preauditVisualGap: candidate.preauditVisualGap,
      preauditPurchaseGap: candidate.preauditPurchaseGap,
      mobileGap: candidate.mobileGap,
    ...
    const integrity = passesShowcaseSalesCandidate({
      pageEntityType: candidate.showcasePageEntityType as ShowcasePageEntityType,
      isValidProductDetail:
        candidate.showcasePageEntityType === "PRODUCT_DETAIL" &&
      ...
      currentSiteImpression: candidate.currentSiteImpression,
      redesignMaterialFeasibility: candidate.redesignMaterialFeasibility,
      visualRedesignOpportunityType: vro.type,
      crossDomainMatch: cross.match,
    });
  ...
  const strongSales = ranked
    ...
    .slice(0, discovery.maxStrongSalesListed);
  const croOnly = ranked
    .filter((c) => c.leadType === "CRO_ONLY_OPPORTUNITY")
    .slice(0, discovery.maxCroOnlyListed);

  console.log("\nStap 5 — screenshots for validated showcase prospects");
  for (const candidate of validatedProspects) {
  ...
  const report = {
    milestone: "M9.9.4",
    version: M994_DISCOVERY_VERSION,
    discoveryRoute: M994_DISCOVERY_ROUTE,
    integrityVersion: discovery.integrityVersion,
    targetProfile: FOCUSED_BRAND_GAP_FIRST_TARGET_V1,
    primaryDiscoveryHook: PRIMARY_DISCOVERY_HOOK,
    parkedFamilies: M994_PARKED_FAMILIES,
    cleanmasterRegression: discovery.cleanmasterRegression,
    startedAt,
    ...
    companyScaleCatalog: visuallyWeakShortlist.map((c) => ({
      domain: c.domain,
    ...
    materialSweetSpot: visuallyWeakShortlist
      .filter(
        (c) =>
          (c.currentVisualQualityScore ?? 60) < 55 &&
    ...
    top5Screenshots: validatedProspects.map(stripCandidate),
  ...
  for (const c of validatedProspects) {
    console.log(
      `  ${c.domain} · cvq=${c.currentVisualQualityScore} · fit=${c.validatedVisualSalesFit} · ${c.leadType}`
    );
  }
...
function computeProductStoryValueProxy(c: M994Candidate): number {
...
function buildWhyGoodProspect(c: M994Candidate): string {
  return [
...
function stripCandidate(c: M994Candidate): Record<string, unknown> {
  return {
    domain: c.domain,
...
const invokedDirectly = process.argv[1]
  ? resolve(process.argv[1]).endsWith("runM994VisualFocusedBrandProduction.js")
  : false;

if (invokedDirectly) {
  runM994VisualFocusedBrandProduction().catch(async (err) => {
    console.error(err);
