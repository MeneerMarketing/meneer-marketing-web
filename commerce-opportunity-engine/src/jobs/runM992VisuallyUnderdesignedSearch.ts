...
import {
  buildM992HarvestQueries,
  M992_DISCOVERY,
  M992_DISCOVERY_ROUTE,
  M992_DISCOVERY_VERSION,
  M992_PARKED_FAMILIES,
  M992_REPORT_PATH,
  M992_DASHBOARD_REPORT_PATH,
...
export async function runM992VisuallyUnderdesignedSearch(options?: {
  ...
  const dfsBudget: Budget = { spent: 0, cap: env.M992_MAX_DATAFORSEO_COST };
  const anthropicBudget: Budget = { spent: 0, cap: env.M992_MAX_ANTHROPIC_COST };
  ...
  const candidateMap = new Map<string, M992Candidate>();
  ...
  const validCandidates: M992Candidate[] = [];
  let invalidPdp = 0;
  ...
  const showcaseCandidates = top10.filter(
    (c) =>
      c.leadType === "VISUAL_SHOWCASE_SIGNAL" ||
  ...
  const report = {
    ...
    funnel: {
      ...
      visual_showcase_signals: visuallyWeakShortlist.filter(
        (c) => c.leadType === "VISUAL_SHOWCASE_SIGNAL" || c.leadType === "SHOWCASE_SALES_CANDIDATE"
      ).length,
      showcase_sales_candidates: visuallyWeakShortlist.filter(
        (c) => c.leadType === "SHOWCASE_SALES_CANDIDATE"
      ).length,
      showcase_design_candidates: visuallyWeakShortlist.filter(
        (c) =>
          c.leadType === "VISUAL_SHOWCASE_SIGNAL" || c.leadType === "SHOWCASE_SALES_CANDIDATE"
      ).length,
    },
  ...
  for (const c of top10.slice(0, 5)) {
    console.log(
      `  ${c.domain} · cvq=${c.currentVisualQualityScore} · fit=${c.visualRedesignSalesFit} · ${c.leadType}`
    );
  }
...
function computeProductStoryValueProxy(c: M992Candidate): number {
...
function stripCandidate(c: M992Candidate): Record<string, unknown> {
  return {
    domain: c.domain,
