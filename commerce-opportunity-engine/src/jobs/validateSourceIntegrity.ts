...
async function main(): Promise<void> {
  ...
  for (const row of result.results) {
    ...
    console.log(
      `  score: ${row.opportunityScore ?? "n/a"} (${row.verdict ?? "n/a"})${
        row.capApplied != null ? ` · capped@${row.capApplied}` : ""
      }`
    );
    const findings = (row.findingValidations as Array<{ title: string; status: string }>) ?? [];
    if (findings.length) {
      console.log(
        `  findings: ${findings.map((f) => `${f.status}:${f.title.slice(0, 40)}`).join(" | ")}`
      );
    }
    console.log("");
  }
