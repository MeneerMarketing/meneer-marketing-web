 ...
 */
async function selectAndApproveTestKeywords(
  limit: number
): Promise<Array<{ id: string; keyword: string; cluster: string | null; quality: number | null }>> {
  const env = loadEnv();
  ...
  const { data, error } = await supabase
    .from("keywords")
    .select(
      "id, keyword, cluster, keyword_quality_score, commercial_intent_score, product_intent_score, category, discovery_status, manual_review_override"
    )
  ...
  const eligible = (data ?? []).filter((row) => {
    ...
    if (blockedTokens.some((t) => kw.includes(t))) return false;
    if (row.cluster && preferredClusters.has(row.cluster)) return true;
  ...
  for (const row of picked) {
    if (row.manual_review_override) continue;
...
async function main(): Promise<void> {
  ...
  const estimatedSerp =
    maxKeywords * env.KEYWORD_SERP_ESTIMATED_COST_PER_KEYWORD;
