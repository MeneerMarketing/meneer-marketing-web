...
import type { CopyStyle } from "../services/outreach/claimValidation.js";
...
import {
  countSupportedFindings,
  findingIdFromTitle,
  isEmailOrDomainSuppressed,
} from "../services/outreach/outreachRepository.js";
import {
...
async function main(): Promise<void> {
  ...
  const { data: unsafeBrands } = await supabase
    ...
    .or(
      "manual_excluded.eq.true,do_not_contact.eq.true,eligibility_status.eq.EXCLUDED,lead_eligible.eq.false"
    );
  const preSync: Array<Record<string, unknown>> = [];
  ...
  const eligibilityReport: Array<Record<string, unknown>> = [];
  const ranked: Array<{
    ...
    findingValidations: unknown;
    salesAngle: string | null;
    copyTestOnly: boolean;
  }> = [];

  for (const row of rows ?? []) {
    const brand = one(
      row.brands as Record<string, unknown> | Record<string, unknown>[]
    );
    if (!brand) continue;
    ...
    const { data: audit } = await supabase
      .from("audits")
      .select(
        "id, audit_valid, conversion_leaks, strengths, finding_validations, sales_angle, page_health_status"
      )
    ...
    const strengths = Array.isArray(audit?.strengths)
      ...
      : [];
    const supportedCount = countSupportedFindings(
      leaks,
      audit?.finding_validations
    );
    const salesAngle = (audit?.sales_angle as string | null) ?? null;
    ...
    const suppressed = await isEmailOrDomainSuppressed(
      supabase,
      (contact?.email as string | null) ?? null,
      domain
    );

    const harvestExcluded =
    ...
    const eligibility = evaluateOutreachEligibility({
      ...
      eligibilityStatus: (brand.eligibility_status as string | null) ?? null,
      leadEligible:
        brand.lead_eligible === null || brand.lead_eligible === undefined
          ? null
          : Boolean(brand.lead_eligible),
      businessType: (brand.business_type as string | null) ?? null,
      croAuditStatus: row.cro_audit_status as string | null,
      auditValid: Boolean(audit?.audit_valid),
      auditConfidence:
        row.audit_confidence != null ? Number(row.audit_confidence) : null,
      ...
      recommendedProjectType:
        (row.recommended_project_type as string | null) ?? null,
      fullRebuildPotential:
        row.full_rebuild_potential != null
          ? Number(row.full_rebuild_potential)
          : null,
      pdpImprovementPotential:
        row.pdp_improvement_potential != null
          ? Number(row.pdp_improvement_potential)
          : null,
      websiteReachable:
        (row.page_health_status as string | null) === "HEALTHY" ||
        (row.page_health_status as string | null) === "PARTIAL",
      suppressed,
    ...
    const copyTestCandidate =
      copyTestAllowHarvestExcluded &&
  ...
  try {
    for (const item of toDraft) {
      if (draftCount >= maxDrafts || anthropicCost >= costCap) break;
      ...
      const page = one(
        item.row.pages as Record<string, unknown> | Record<string, unknown>[]
      );
      const keyword = one(
        item.row.keywords as Record<string, unknown> | Record<string, unknown>[]
      );
      ...
      const pageUrl =
        ...
        (item.row.resolved_url as string | null) ??
        (item.row.landing_url as string | null) ??
        null;

      const allowedClaims = buildAllowedClaims({
        pageUrl,
        productName: (page?.product_name as string | null) ?? null,
        brandName: (item.brand.name as string | null) ?? null,
        findings: attachValidationStatus(
          supportedFindings,
          item.findingValidations
        ),
      ...
      for (const copyStyle of OUTREACH_COPY_STYLES as readonly CopyStyle[]) {
        if (draftCount >= maxDrafts || anthropicCost >= costCap) break;
        ...
        const generated = await generateOutreachDraft({
          ...
          data: {
            ...
            brandName: (item.brand.name as string | null) ?? null,
            contactFirstName: (item.contact.first_name as string | null) ?? null,
            contactEmail: String(item.contact.email),
            productName: (page?.product_name as string | null) ?? null,
            category: (keyword?.category as string | null) ?? null,
            platform:
              (item.brand.platform as string | null) ??
              (item.brand.platform_candidate as string | null),
            auditType: (item.row.audit_type as string | null) ?? null,
            keyword: (keyword?.keyword as string | null) ?? null,
            pageUrl,
            confirmedGoogleAdvertiser: Boolean(
              item.brand.confirmed_google_advertiser
            ),
        ...
        const { data: message, error: insertError } = await supabase
          ...
          .insert({
            ...
            selected_strength_title:
              generated.draft.selected_strength_title ?? null,
        ...
        if (item.copyTestOnly) {
          const synced = await syncOutreachMessagesForBrand(
            supabase,
            item.brandId,
            "copy test on harvest-excluded brand"
          );
          const thisSync = synced.find((s) => s.messageId === message.id);
          if (thisSync?.nextStatus) status = thisSync.nextStatus;
        }
    ...
    if (!preSync.length) console.log("  (geen wijzigingen)");
    for (const s of preSync) {
      console.log(
        `  ${s.domain}: ${s.from} → ${s.to} · ${JSON.stringify(s.reasons)}`
      );
    }
    console.log("");
    ...
    for (const e of eligibilityReport) {
      console.log(
        `  ${e.domain}: ${e.eligible ? "YES" : e.copyTestCandidate ? "COPY_TEST" : "NO"} · prio ${e.priority} · ${e.reason}`
      );
    }
    console.log("");
    console.log("DRAFTS");
    for (const d of drafts) {
      ...
      console.log(
        `  status: ${d.status} · validation: ${d.validation} · words: ${d.wordCount}`
      );
