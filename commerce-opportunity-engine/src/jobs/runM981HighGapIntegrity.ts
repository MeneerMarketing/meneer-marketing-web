...
const DASHBOARD_REPORT_PATH = resolve(
  projectRoot,
  "dashboard/src/preview/concepts/data/m981-high-gap-integrity-report.json"
);
const SCREENSHOT_DIR = resolve(projectRoot, M98_DISCOVERY.screenshotDir);

type M98Candidate = Record<string, unknown>;

function isHighGap(c: M98Candidate): boolean {
  const raw = c.rawPdpRedesignOpportunity as number | null;
  const visual = c.preauditVisualGap as number | null;
  const purchase = c.preauditPurchaseGap as number | null;
  return (
    c.screened === true &&
...
function heroEconomicsScore(price: number | null): number {
  if (price == null) return 38;
  if (price >= 150 && price <= 750) return 94;
  if (price >= 120 && price <= 3000) return 76;
  if (price >= 100) return 58;
  if (price >= 80) return 48;
  if (price >= 50) return 32;
  return 18;
}
...
export async function runM981HighGapIntegrity(): Promise<void> {
  const env = loadEnv();
  ...
  const m98 = JSON.parse(m98Raw) as Record<string, unknown>;
  const allCandidates = (m98.allCandidates as M98Candidate[]) ?? [];

  const highGapPool = allCandidates
    .filter(isHighGap)
    .sort(
      (a, b) =>
        ((b.rawPdpRedesignOpportunity as number) ?? 0) -
        ((a.rawPdpRedesignOpportunity as number) ?? 0)
    )
    .slice(0, 12);
  ...
  const waterpikRegression: { passed: boolean; notes: string[] } = { passed: false, notes: [] };

  for (const raw of highGapPool) {
    const domain = String(raw.domain);
    const productUrl = String(raw.productUrl);
    const productTitle = raw.productTitle as string | null;

    let html: string | null = (raw.cachedHtml as string | undefined) ?? null;
    if (!html || html.length < 500) {
      const crawl = await crawlWebsite(productUrl, crawlTimeout);
      if (crawl.status === "success") html = crawl.html;
    }
    ...
    const entity = classifyPageEntity({
      ...
      html,
      productTitle,
      observedPrice: raw.observedPrice as number | null,
    });

    if (domain.includes("waterpik")) {
      waterpikRegression.passed = entity.pageEntityType === "CATEGORY";
      waterpikRegression.notes.push(
        `waterpik entity=${entity.pageEntityType} reason=${entity.rejectReason}`
      );
    }
    ...
    const material = computeRedesignMaterialFeasibility({
      contentAvailable: raw.contentAvailable as number | null,
      contentPresentation: raw.contentPresentation as number | null,
      assetQualityProxy: raw.assetQualityProxy as number | null,
      materialSweetSpot: Boolean(raw.materialSweetSpot),
    });

    const heroTarget = raw.heroTarget as Record<string, unknown> | null;
    const heroPrice =
      (heroTarget?.heroPrice as number | null) ?? (raw.observedPrice as number | null);
    const hero = computeHeroCandidateScore({
      heroPrice,
      heroConfidence: heroTarget?.heroConfidence as number | null,
      heroProductUrl: heroTarget?.heroProductUrl as string | null,
      discoveredProductUrl: productUrl,
      isValidProductDetail: entity.isValidProductDetail,
      assetContentAvailability: raw.assetContentAvailability as number | null,
      productFamilyRelevance: true,
    });

    const economics = heroEconomicsScore(heroPrice);
    const validated = computeValidatedGapSalesFit({
      rawPdpRedesignOpportunity: raw.rawPdpRedesignOpportunity as number,
      redesignMaterialFeasibility: material.score,
      businessModel: business.businessModel,
      businessModelSalesCandidate: business.salesCandidate,
      companyScaleFit: raw.brandScaleFit as number | null,
      catalogFocus: catalog.catalogFocusScore,
      catalogBandScore: catalogBand.score,
      ownBrandSignal: light.ownBrandSignal,
      heroCandidateScore: hero.score,
      heroEconomicsScore: economics,
      paidAcquisitionLevel: (raw.paidAcquisition as PaidAcquisitionLevel) ?? "UNKNOWN",
    });
    ...
    const existingScreenshots = (raw.screenshots as Record<string, string> | null) ?? {};
  ...
  const finalists = reviewed
    ...
    .sort((a, b) => (b.validatedGapSalesFit as number) - (a.validatedGapSalesFit as number))
    .slice(0, 3);
  ...
  for (const candidate of finalists) {
    ...
    const shots = candidate.screenshots as Record<string, string>;
    const needed: Array<{ key: string; url: string }> = [];
    for (const s of [
      ...
      if (!shots[s.key] || !(await fileExists(shots[s.key]))) needed.push(s);
    }
    if (needed.length > 0) {
      try {
        ...
        console.warn(`Screenshot skip ${domain}: ${(error as Error).message}`);
      }
    }

    candidate.manualRationale = {
      whyRealPdp: `page_entity=${candidate.pageEntityType}, evidence: ${(candidate.pageEntityEvidence as string[]).join(", ")}`,
      ...
      biggestRisk: (candidate.preAuditGateFailures as string[]).length
        ? (candidate.preAuditGateFailures as string[]).join(", ")
  ...
  const sufficientMaterials = screened.filter(
    (c) =>
      (c.materialQualityScore as number | null) != null &&
      ((c.materialQualityScore as number) >= 55 || c.materialSweetSpot === true)
  );
  ...
  for (const c of screened) {
    ...
    if (!positionBandYield[band]) positionBandYield[band] = { screened: 0, highGap: 0 };
    positionBandYield[band].screened += 1;
    if (isHighGap(c)) positionBandYield[band].highGap += 1;
  }

  const m98Funnel = (m98.funnel as Record<string, number>) ?? {};
  const funnelConsistency = {
    professional_ecommerce_after_gap: m98Funnel.professional_ecommerce_after_gap ?? 0,
    gap_sales_verdict_promising:
      m98Funnel.gap_sales_verdict_promising ?? m98Funnel.potential_targets ?? 0,
    explanation:
      "gap_sales_verdict_promising = high-gap candidates met TRUE_MANUAL_REVIEW of PROMISING op gap-first score (kan hoger zijn dan professional_ecommerce wanneer business qualification faalt, bv. debeterewereld.nl). professional_ecommerce_after_gap = alleen candidates die businessQualified=true kregen in M9.8.",
    sufficient_materials_screened: sufficientMaterials.length,
    sufficient_materials_total_screened: screened.length,
    sufficient_materials_always_true:
      sufficientMaterials.length === screened.length && screened.length > 0,
    sufficient_materials_note:
      sufficientMaterials.length === screened.length
  ...
  const report = {
    ...
    businessModelBreakdown: reviewed.reduce<Record<string, unknown[]>>((acc, r) => {
  ...
  for (const f of finalists) {
    console.log(
      `  ${f.domain} · validated=${f.validatedGapSalesFit} · model=${f.businessModel} · entity=${f.pageEntityType}`
    );
  }
