...
import {
  ...
  SHOWCASE_CONTRAST,
  SCREENSHOT_CONFIG,
} from "../config/highTicketValidation.js";
import {
  isUsableHeroUrl,
...
import { buildInterventionMap, type InterventionRow } from "../services/concept/interventionMap.js";
import type { ConceptFirstPdpSignals } from "../services/concept/currentPdpQuality.js";
import type { ConversionLeak, PageRepresentation } from "../types/audit.js";
import {
  resolvePrimarySalesProduct,
  upsertConceptCandidate,
  loadLatestAudit,
  scoreAuditedCandidate,
  persistScores,
} from "./auditNewSalesProspects.js";
...
const DASHBOARD_REPORT = resolve(
  projectRoot,
  "dashboard/src/preview/concepts/data/high-ticket-design-validation-report.json"
);
const SCREENSHOT_DIR = resolve(projectRoot, SCREENSHOT_CONFIG.outputDir);

interface HighTicketCandidate {
  rank: number;
  domain: string;
  siteUrl: string;
  heroProductUrl: string | null;
  heroUrlSource: string | null;
...
interface ValidatedCandidate {
  ...
  hero: ResolvedHero;
  brand: BrandRow;
  targetVerification: ReturnType<typeof verifyPaidProductTarget> | null;
  ...
  designInterventionOpportunity: number | null;
  conceptAssetFeasibility: number | null;
  assetInventory: Record<string, boolean>;
  interventionMap: InterventionRow[];
  visualSummary: VisualSummary;
  leaks: ConversionLeak[];
  strengths: Array<{ title: string }>;
  anthropicCost: number;
  screenshots: Record<string, string>;
  showcasePreferred: boolean;
}

type FinalVerdict =
  | "TRUE_DESIGN_TARGET"
  | "PROMISING_BUT_NOT_STRONG_ENOUGH"
  | "NO_TARGET";
...
function conceptSignalsFrom(audit: AuditReadback | null): ConceptFirstPdpSignals | null {
  ...
  return (findings as { concept_first_signals?: ConceptFirstPdpSignals }).concept_first_signals ?? null;
}
...
function loadDiscoveryReport(): {
  candidates: HighTicketCandidate[];
} {
  if (!existsSync(DISCOVERY_REPORT)) {
    throw new Error(`M9.4 rapport niet gevonden: ${DISCOVERY_REPORT}`);
  }
  return JSON.parse(readFileSync(DISCOVERY_REPORT, "utf8")) as {
    candidates: HighTicketCandidate[];
  };
}

function findCandidate(candidates: HighTicketCandidate[], domain: string): HighTicketCandidate | null {
  return candidates.find((entry) => entry.domain === domain) ?? null;
}

function toProspectRecord(candidate: HighTicketCandidate): NewProspectRecord {
  return {
...
function toPreselection(record: NewProspectRecord, rank: number): PreselectionEntry {
  return {
    record,
    rank,
    ...
    derivedCommerceModel:
      record.commerceModel ?? commerceModelFromOwnBrandSignal(record.ownBrandSignal),
...
async function loadBrands(
  ...
  const { data, error } = await supabase
    .from("brands")
    .select(
      "id, name, normalized_domain, platform, business_type, business_maturity_score, retailer_scale_score, confirmed_google_advertiser, transparency_confirmed, manual_excluded"
    )
...
function buildVisualSummary(input: {
  ...
  leaks: ConversionLeak[];
  interventionMap: InterventionRow[];
  designIntervention: number | null;
}): VisualSummary {
...
function decideFinalRecommendation(
  ...
  if (eligible.length === 0) {
    ...
    if (contrasts.every((value) => value < SHOWCASE_CONTRAST.hardMin)) {
      rationale.push(
        `geen kandidaat haalt contrast ${SHOWCASE_CONTRAST.hardMin} (max ${Math.max(...contrasts)})`
      );
      return {
        verdict: "NO_TARGET",
        recommendedDomain: null,
  ...
  const best = eligible.sort(
    (a, b) => (b.trueSalesDesignScore ?? 0) - (a.trueSalesDesignScore ?? 0)
  )[0];

  const preferred = (best.conceptContrast ?? 0) >= SHOWCASE_CONTRAST.preferredMin;
  if (preferred) {
    rationale.push(
      `contrast ${best.conceptContrast} haalt showcase-voorkeur (${SHOWCASE_CONTRAST.preferredMin}+)`
    );
    return {
  ...
  rationale.push(
    `contrast ${best.conceptContrast} haalt harde gate (${SHOWCASE_CONTRAST.hardMin}) maar niet showcase-voorkeur`
  );
  return {
    verdict: "PROMISING_BUT_NOT_STRONG_ENOUGH",
    recommendedDomain: best.domain,
...
async function runAuditForCandidate(input: {
  ...
  if (input.dryRun) {
    console.log(
      `  DRY ${input.role} ${input.candidate.domain} → ${hero.title} · target confidence ${targetVerification?.targetConfidence ?? "n.v.t."}`
    );
    return null;
  }
  ...
  const audit = (await loadLatestAudit(input.supabase, opportunityId)) as AuditReadback | null;
  ...
  const showcasePreferred =
    (scored.outreach.contrast.concept_contrast_potential ?? 0) >= SHOWCASE_CONTRAST.preferredMin;
...
export async function auditHighTicketDesignTargets(options?: { dryRun?: boolean }): Promise<void> {
  const env = loadEnv();
  ...
  let challengerDomain: string = M941.challengerFallback;
  let challengerQualification: Awaited<ReturnType<typeof qualifyChallenger>> | null = null;
  ...
  if (revigurizeRaw) {
    ...
    if (challengerQualification.qualified) {
      ...
      console.log(
        `  revigurize.nl niet gekwalificeerd: ${challengerQualification.blockers.join(" · ")}`
      );
  ...
  console.log(
    `  keyword: ${primaryVerification.keywordUsed} · confidence ${primaryVerification.targetConfidence} · aligned ${primaryVerification.aligned}`
  );
  ...
  if (challengerResult) {
    ...
    if (challengerQualification) {
      challengerResult.qualification = challengerQualification as unknown as Record<string, unknown>;
    }
  ...
  for (const entry of validated) {
    ...
    console.log(
      `     intervention ${entry.designInterventionOpportunity ?? "?"} · asset feasibility ${entry.conceptAssetFeasibility ?? "?"}`
    );
    if (entry.gate) {
      console.log(
        `     gate ${entry.gate.passed ? "PASS" : "FAIL"}${entry.gate.blockers.length ? `: ${entry.gate.blockers.join(", ")}` : ""}`
      );
    }
  }
  ...
  for (const line of final.rationale) console.log(`  · ${line}`);
