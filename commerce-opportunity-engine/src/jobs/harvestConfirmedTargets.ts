...
import {
  CRO_SHORTLIST_WEIGHTS,
  TARGET_HARVEST_DEFAULTS,
} from "../config/targetHarvest.js";
...
import {
  harvestShoppingTargetsForBrand,
  type HarvestSourceKeyword,
} from "../services/harvest/shoppingHarvest.js";
import {
...
function remaining(spent: number, cap: number): number {
...
function scoreCroShortlist(input: {
  ...
  let score =
    input.targetPriority * w.targetPriority +
    (input.preFit ?? 50) * w.preFit +
    (input.maturity ?? 40) * w.maturity;
  if ((input.platform ?? "").toUpperCase() === "SHOPIFY") score += w.shopifyBonus;
  if (input.readiness === "EXACT_PAID_FUNNEL") score += w.exactPaidBonus;
  if (input.readiness === "HIGH_CONFIDENCE_TARGET") score += w.highConfidenceBonus;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const eligiblePool = (confirmedPool ?? []).filter((b) => {
      ...
      if (b.do_not_contact) return false;
      if (EXCLUDED_DOMAINS.has(String(b.normalized_domain ?? "").toLowerCase())) return false;
      if ((b.retailer_scale_score ?? 0) > TARGET_HARVEST_DEFAULTS.maxRetailerScale) return false;
      if ((b.business_maturity_score ?? 0) < TARGET_HARVEST_DEFAULTS.minMaturity) return false;
      if (type !== "BRAND" && type !== "SPECIALIST_WEBSHOP") return false;
      return Boolean(b.is_ecommerce);
    });

    // Attach source keyword stats
    type BrandCand = (typeof eligiblePool)[number] & {
      sources: HarvestSourceKeyword[];
      harvestRank: number;
    };

    const candidates: BrandCand[] = [];
    for (const b of eligiblePool) {
      ...
      const sourceMap = new Map<string, HarvestSourceKeyword>();
      for (const ad of ads ?? []) {
        ...
        if (!sourceMap.has(id)) {
          sourceMap.set(id, {
            ...
            prospecting: (row.prospecting_value_score as number) ?? null,
            relevance: (row.category_relevance_score as number) ?? null,
            intent: (row.keyword_intent_type as string) ?? null,
            tier: (row.prospecting_tier as string) ?? null,
            yieldScore: (row.prospect_yield_score as number) ?? null,
          });
        }
      }
    ...
    for (const s of selected) {
      console.log(
        `  ${s.normalized_domain} rank=${s.harvestRank} preFit=${s.pre_fit_score} platform=${s.platform} sources=${s.sources.length}`
      );
    }
    ...
    let searchCost = 0;
    const brandsNeedingShopping: BrandCand[] = [];
    ...
    for (const brand of selected) {
      ...
      try {
        ...
        const labs = await fetchPaidRankedKeywords({
          client: dfs,
          env,
          target: brand.normalized_domain as string,
        ...
        if (labs.items.some((i) => i.landingUrl)) {
          generated = await generateOpportunitiesFromPaidTargets(
            supabase,
            brand.id as string
          );
        }
        ...
        if (generated.croReady > 0 || saved.upserted > 0) searchBrandsWithTargets += 1;
        ...
        const { data: opps } = await supabase
          .from("opportunities")
          .select(
            "id, source_quality_score, cro_readiness_level, landing_url, ad_headline"
          )
        ...
        for (const opp of opps ?? []) {
          ...
          const priority = scoreTargetPriority({
            sourceQuality:
              opp.source_quality_score != null ? Number(opp.source_quality_score) : null,
        ...
        const hasUsable =
          generated.croReady > 0 ||
          (opps ?? []).some(
            (o) =>
              o.cro_readiness_level === "EXACT_PAID_FUNNEL" ||
        ...
        if (!hasUsable) brandsNeedingShopping.push(brand);

        console.log(
          `Search ${brand.normalized_domain}: items=${labs.itemsCount} croReady=${generated.croReady} cost=$${labs.cost.toFixed(4)}`
        );
      } catch (err) {
        ...
        brandsNeedingShopping.push(brand);
        if (classified.apiStatus === "PAYMENT_REQUIRED") break;
      }
    }
    ...
    const allMismatches: Array<Record<string, unknown>> = [];
    ...
    for (const brand of brandsNeedingShopping) {
      ...
      try {
        const result = await harvestShoppingTargetsForBrand({
          ...
          env,
          brandId: brand.id as string,
          brandDomain: brand.normalized_domain as string,
        ...
        const { data: opps } = await supabase
          .from("opportunities")
          .select(
            "id, source_quality_score, cro_readiness_level, landing_url"
          )
          .eq("brand_id", brand.id)
          .in("cro_readiness_level", ["EXACT_PAID_FUNNEL", "HIGH_CONFIDENCE_TARGET"]);
        ...
        const hasUsable =
          result.exactPaid + result.exactListing > 0 || (opps ?? []).length > 0;
        ...
        console.log(
          `Shopping ${brand.normalized_domain}: paid=${result.exactPaid} listing=${result.exactListing} mismatches=${result.mismatches.length} cost=$${result.cost.toFixed(4)}`
        );
      } catch (err) {
        const classified = classifyDataForSeoError(err);
        shopUnresolved += 1;
    ...
    const { data: eligibleOpps, error: oppErr } = await supabase
      .from("opportunities")
      .select(
        `id, brand_id, landing_url, cro_readiness_level, source_quality_score,
    ...
    const byBrand = new Map<string, ShortCand>();
    for (const opp of eligibleOpps ?? []) {
      ...
      if (b.manual_excluded || b.do_not_contact) continue;
      ...
      if (opp.keyword_id) {
        const { data: kwRow } = await supabase
          ...
          .maybeSingle();
        keyword = (kwRow?.keyword as string) ?? null;
        category = (kwRow?.category as string) ?? null;
      }
      ...
      const shortlistScore = scoreCroShortlist({
        ...
        readiness: opp.cro_readiness_level as string,
      });

      const cand: ShortCand = {
        ...
        category,
        keyword,
        targetUrl: opp.landing_url as string | null,
        targetType: opp.cro_readiness_level as string | null,
        sourceQuality:
          opp.source_quality_score != null ? Number(opp.source_quality_score) : null,
        targetPriority,
        shortlistScore,
    ...
    const shortlist = [...byBrand.values()]
      .sort((a, b) => {
        if (b.targetPriority !== a.targetPriority) return b.targetPriority - a.targetPriority;
    ...
    if (scaleRun) {
      const funnel = {
        ...((scaleRun.funnel as Record<string, unknown>) ?? {}),
        confirmedAdvertisers: selected.length,
        targetsResolved: selected.length - (report.unresolved as unknown[]).length,
      ...
      await supabase
        ...
        .update({
          funnel,
          noise_report: {
            ...((scaleRun.noise_report as object) ?? {}),
            m722: {
              runId: run.id,
              shortlist,
    ...
    console.log(
      JSON.stringify(
        {
          selected: (report.confirmedPool as { selected: unknown }).selected,
          search: {
            brandsWithTargets: searchBrandsWithTargets,
