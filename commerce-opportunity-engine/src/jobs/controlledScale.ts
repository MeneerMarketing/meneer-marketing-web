...
import {
  CONTROLLED_SCALE_DEFAULTS,
} from "../config/controlledScale.js";
...
import {
  detectKeywordQualityProblems,
  runKeywordGeneration,
} from "../services/keywords/keywordEngine.js";
import {
  ensureCategoryList,
  selectControlledScaleKeywords,
} from "../services/keywords/controlledKeywordSelector.js";
...
function remaining(budget: BudgetTracker): number {
  return Math.max(0, budget.cap - budget.spent);
}

function canSpend(budget: BudgetTracker, estimated: number): boolean {
...
async function main(): Promise<void> {
  ...
  const maxKeywords = Math.min(
    env.CONTROLLED_SCALE_MAX_KEYWORDS,
    CONTROLLED_SCALE_DEFAULTS.maxKeywords
  );
  ...
  try {
    ...
    if (!env.CONTROLLED_SCALE_SKIP_KEYWORD_GENERATE) {
      for (const categoryId of ensureCategoryList()) {
        ...
        const estimate =
          CONTROLLED_SCALE_DEFAULTS.estimatedKeywordIdeasCostPerCategory;
        ...
        console.log(
          `  ${categoryId}: candidates=${result.afterDedupe} qualified=${result.qualified} cost=$${result.actualCost.toFixed(4)}`
        );
        if (problems.length) {
    ...
    if (estimatedSerp > maxSerpCost) {
      throw new Error(
        `Estimated SERP $${estimatedSerp.toFixed(4)} exceeds CONTROLLED_SCALE_MAX_SERP_COST $${maxSerpCost}`
      );
    }
    if (!canSpend(totalBudget, estimatedSerp)) {
      throw new Error(
        `Estimated SERP would exceed total DataForSEO budget (remaining $${remaining(totalBudget).toFixed(4)})`
      );
    }
    ...
    console.log(
      `Discovery: keywords=${discovery.keywordsProcessed} ads=${discovery.paidAdsFound} cost=$${discovery.serpCost.toFixed(4)}`
    );
    ...
    const { data: refreshedBrands } = await supabase
      .from("brands")
      .select(
        `id, normalized_domain, business_type, platform, is_ecommerce, business_maturity_score,
    ...
    for (const brand of refreshedBrands ?? []) {
      if (brand.is_ecommerce) ecommerceCount += 1;
      const type = (brand.business_type ?? "").toUpperCase();
      if (type === "BRAND" || type === "SPECIALIST_WEBSHOP") brandSpecialist += 1;
      if (type === "GENERAL_RETAILER") retailers += 1;
      if (type === "COMPARISON_SITE" || type === "MARKETPLACE") comparisonMp += 1;
      if ((brand.platform ?? "").toUpperCase() === "SHOPIFY") shopifyCount += 1;
      ...
      if (preFit.prequalified) prequalifiedCount += 1;
    ...
    // ------------------------------------------------------------------
    const transparencyCap = Math.min(
      env.TRANSPARENCY_SCALE_MAX_COST,
    ...
    for (const brand of prequalifiedRanked) {
      ...
      try {
        const result = await checkGoogleAdsTransparency(
          { client: dfs, env },
          brand.normalized_domain as string
        );
    ...
    const { data: confirmedBrands } = await supabase
      .from("brands")
      .select(
        "id, normalized_domain, business_maturity_score, platform, retailer_scale_score, pre_fit_score, confirmed_google_advertiser, transparency_confirmed"
      )
      .in("id", brandIds.length ? brandIds : ["00000000-0000-0000-0000-000000000000"])
    ...
    for (const kw of selection.selected) {
      const { data: ads } = await supabase
        .from("ad_occurrences")
        .select(
          "brand_id, brands(business_type, lead_eligible, platform, confirmed_google_advertiser)"
        )
        .eq("keyword_id", kw.id);
      ...
      for (const ad of ads ?? []) {
        ...
        if (ad.brand_id && b) brandMap.set(ad.brand_id, b as Record<string, unknown>);
      }
      ...
      for (const b of brandMap.values()) {
        if (b.lead_eligible) eligible += 1;
        if (String(b.platform ?? "").toUpperCase() === "SHOPIFY") shopify += 1;
        const t = String(b.business_type ?? "").toUpperCase();
        if (t === "GENERAL_RETAILER") generalRetailers += 1;
        if (t === "COMPARISON_SITE") comparison += 1;
        if (t === "MARKETPLACE") marketplaces += 1;
        if (b.confirmed_google_advertiser) confirmedAdv += 1;
      }
      ...
      const perKeywordCost =
        discovery.keywordsProcessed > 0
    ...
    for (const cat of ensureCategoryList()) {
      ...
      for (const ad of catAds ?? []) {
        ...
        const b = Array.isArray(ad.brands) ? ad.brands[0] : ad.brands;
        if (!b) continue;
        brandsById.set(ad.brand_id, b as Record<string, unknown>);
      ...
      for (const b of brandsById.values()) {
        const t = String(b.business_type ?? "").toUpperCase();
        if (t === "BRAND" || t === "SPECIALIST_WEBSHOP") specialists += 1;
        if (b.prequalified_prospect) preq += 1;
        if (String(b.platform ?? "").toUpperCase() === "SHOPIFY") shop += 1;
        if (b.confirmed_google_advertiser) conf += 1;
      }
    ...
    const { data: best } = await supabase
      .from("brands")
      .select(
        `id, normalized_domain, business_type, platform, business_maturity_score, retailer_scale_score,
    ...
    for (const b of best ?? []) {
      ...
      const sourceKeywords = (sources ?? [])
        .map((s) => {
          ...
          return kw as { keyword?: string; category?: string } | null;
        })
        .filter(Boolean);
    ...
    bestProspects.forEach((p, i) => {
      console.log(
        `${i + 1}. ${p.domain} | ${p.businessType} | ${p.platform ?? "—"} | preFit=${p.preFit} | transparency=${p.transparencyStatus ?? "—"}`
      );
    });
  } catch (error) {
