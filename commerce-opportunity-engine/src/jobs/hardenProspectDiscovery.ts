...
import {
  ...
  IDEAL_PRODUCT_ARCHETYPES_VERSION,
  normalizedBudgetShares,
...
async function loadExistingPoolDomains(root: string): Promise<Set<string>> {
  const reportPath = path.resolve(
    root,
    "dashboard/src/preview/concepts/data/ideal-prospect-report.json"
  );
  try {
    const parsed = JSON.parse(await readFile(reportPath, "utf8")) as {
      trueSalesRanking?: { top10?: Array<{ domain?: string }> };
      prequalification?: { top20?: Array<{ domain?: string }> };
    };
    const domains = [
...
async function main(): Promise<void> {
  ...
  if (occError) throw new Error(`ad_occurrences load failed: ${occError.message}`);

  const keywordIds = [...new Set((occurrences ?? []).map((o) => o.keyword_id).filter(Boolean))] as string[];
  const brandIds = [...new Set((occurrences ?? []).map((o) => o.brand_id).filter(Boolean))] as string[];

  const { data: keywordRows } = await client
    .from("keywords")
    .select(
      "id, keyword, category, search_volume, cpc, keyword_intent_type, prospecting_value_score, discovery_priority_score"
    )
    .in("id", keywordIds);
  ...
  const { data: brandRows, error: brandError } = await client
    .from("brands")
    .select(
      "id, normalized_domain, business_type, is_ecommerce, manual_excluded, retailer_scale_score, business_maturity_score, eligibility_status, lead_eligible, prequalified_prospect, pre_fit_score"
    )
    .in("id", brandIds);

  if (brandError) throw new Error(`brands load failed: ${brandError.message}`);

  const keywordById = new Map<string, KeywordRow>((keywordRows ?? []).map((k) => [k.id, k as KeywordRow]));
  const brandById = new Map<string, BrandRow>((brandRows ?? []).map((b) => [b.id, b as BrandRow]));
  ...
  const keywordBrands = new Map<string, Set<string>>();

  for (const occ of occurrences ?? []) {
    if (!occ.brand_id || !occ.keyword_id) continue;
    const keyword = keywordById.get(occ.keyword_id);
    if (!brandKeywords.has(occ.brand_id)) brandKeywords.set(occ.brand_id, new Set());
    brandKeywords.get(occ.brand_id)!.add(occ.keyword_id);
    if (!brandCategories.has(occ.brand_id)) brandCategories.set(occ.brand_id, new Set());
    if (keyword?.category) brandCategories.get(occ.brand_id)!.add(keyword.category);
    if (!keywordBrands.has(occ.keyword_id)) keywordBrands.set(occ.keyword_id, new Set());
  ...
  const poolDomainVerdicts: DomainVerdict[] = [];
  ...
  if (extraDomains.length > 0) {
    ...
    const extraByDomain = new Map<string, BrandRow>(
      (extraBrands ?? []).map((b) => [String(b.normalized_domain), b as BrandRow])
    );
  ...
  const lateExclusions = excludedDomains.filter(
    (d) =>
  ...
  type KeywordVerdict = {
    ...
    status: KeywordProspectStatus | "PRE_GATE_REJECTED";
    stopReason: string | null;
    previousIntent: string | null;
    previousProspectingValue: number | null;
    wouldRunDiscovery: boolean;
    exampleAdvertisers: string[];
  };

  const keywordVerdicts: KeywordVerdict[] = [];
  ...
  for (const [keywordId, brandSet] of keywordBrands) {
    ...
    const samples = [...brandSet]
      ...
      .filter((b): b is BrandRow => Boolean(b?.normalized_domain))
      .map((b) => ({ domain: b.normalized_domain as string, businessType: b.business_type }));

    const quality = computeSerpProspectQuality(samples, {
      archetypeTooBroad: preGate.archetype.rejectReason === "TOO_BROAD",
    });

    const status: KeywordVerdict["status"] = preGate.accepted ? quality.status : "PRE_GATE_REJECTED";
  ...
  const branchStats = IDEAL_PRODUCT_ARCHETYPES_V1.map((archetype) => {
    ...
    return {
      ...
      costPerPrequalifiedProspect:
        specialists > 0 ? Math.round((serpCost / specialists) * 1000) / 1000 : null,
  ...
  const projectedCostPerUsefulProspect =
    approvedKeywords.length > 0 && usefulProspects > 0
  ...
  const report = {
    ...
    costReview: {
      ...
      wastedSharePercent:
        keywordVerdicts.length > 0
          ? Math.round((rejectedKeywords.length / keywordVerdicts.length) * 100)
          : 0,
      costPerUsefulProspect:
        costPerUsefulProspect === null ? null : Math.round(costPerUsefulProspect * 1000) / 1000,
      projectedCostPerUsefulProspect:
        projectedCostPerUsefulProspect === null
          ? null
  ...
  for (const k of keywordVerdicts) statusCounts.set(k.status, (statusCounts.get(k.status) ?? 0) + 1);
  ...
  for (const fail of fixturesFailed) {
    console.log(
      `  FAIL ${fail.domain}: verwacht ${fail.expectEligible ? "eligible" : fail.expectReason} kreeg ${
        fail.actualEligible ? "eligible" : fail.actualReason
      }`
    );
  }
