...
async function main(): Promise<void> {
  ...
  const categoryId = (env.KEYWORD_ENGINE_CATEGORY || "BEAUTY_SKINCARE") as KeywordCategoryId;
  ...
  try {
    ...
    result.top30.forEach((k, i) => {
      console.log(
        `${String(i + 1).padStart(2, " ")}. ${k.keyword} | q=${k.quality} c=${k.commercial} p=${k.product} vol=${k.volume ?? "—"} cpc=${k.cpc ?? "—"} [${k.cluster}] ${k.status}`
      );
    });
    console.log("");
