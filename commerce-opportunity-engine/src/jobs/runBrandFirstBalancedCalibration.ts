...
import {
  buildM961RoundRobinQueries,
  M961_DISCOVERY,
  M961_DISCOVERY_VERSION,
  M961_PARKED_FAMILY_TAGS,
  M961_PRODUCT_FAMILIES,
  M961_PROFILE_VERSION,
} from "../config/brandFirstBalancedCalibration.js";
...
import {
  computeBrandFirstOpportunityScoreV2,
  manualReviewVerdict,
} from "../services/prospect/brandFirstOpportunityScoreV2.js";
...
import {
  computeFamilyYieldAnalytics,
  recommendProductionFamilies,
} from "../services/prospect/familyYieldAnalytics.js";
...
const DASHBOARD_REPORT_PATH = resolve(
  projectRoot,
  "dashboard/src/preview/concepts/data/brand-first-balanced-report.json"
);
...
async function extractHomepageProductUrls(domain: string, timeoutMs: number): Promise<string[]> {
...
interface BrandCandidate {
  ...
  manualReviewVerdict: "TRUE_MANUAL_REVIEW_CANDIDATE" | "NO_TARGET";
...
export async function runBrandFirstBalancedCalibration(options?: { dryRun?: boolean }): Promise<void> {
  const env = loadEnv();
  const supabase = createSupabaseServerClient(env);
  const dataForSeo = createDataForSeoClient(env);
  const dfsBudget: Budget = { spent: 0, cap: env.M961_MAX_DATAFORSEO_COST };
  const anthropicBudget: Budget = { spent: 0, cap: env.M961_MAX_ANTHROPIC_COST };
  ...
  console.log(
    `Extraction regression: ${extractionRegression.passed}/${extractionRegression.total} passed`
  );
  ...
  const queriesByFamily = new Map<string, number>();
  const organicRowsByFamily = new Map<string, number>();
  ...
  for (const entry of queries) {
    if (discovered.size >= M961_DISCOVERY.maxFirstPartyCandidates) break;
    const familyCount = familyBrandCounts.get(entry.familyId) ?? 0;
    if (familyCount >= M961_DISCOVERY.maxBrandsPerFamily) continue;
    if (!canSpend(dfsBudget, M961_DISCOVERY.estimatedSerpCostPerKeyword)) break;
    ...
    for (const organic of serp.organicResults) {
      organicRows += 1;
      organicRowsByFamily.set(
        entry.familyId,
        (organicRowsByFamily.get(entry.familyId) ?? 0) + 1
      );

      const gate = classifyDiscoveryDomain(organic.normalizedDomain);
      if (gate.hardExclude) {
        if (gate.entityClass === "AGENCY_EXCLUDED") agencyExcluded += 1;
        if (
          gate.entityClass === "MEDIA_PUBLISHER" ||
      ...
      if (!officialDomain || extraction.entityRole !== "OFFICIAL_BRAND_DOMAIN") continue;

      const officialGate = classifyDiscoveryDomain(officialDomain);
      if (officialGate.hardExclude) {
        if (officialGate.entityClass === "AGENCY_EXCLUDED") agencyExcluded += 1;
        continue;
      }

      if (!discovered.has(officialDomain)) {
        const currentFamilyCount = familyBrandCounts.get(entry.familyId) ?? 0;
        if (currentFamilyCount >= M961_DISCOVERY.maxBrandsPerFamily) continue;
        discovered.set(officialDomain, {
          ...
          discoverySource:
            extraction.merchantDomain ? "RETAILER_TO_BRAND_EXTRACTION" : "ORGANIC_PRODUCT_SERP",
    ...
    console.log(
      `  ${entry.familyId} · ${entry.query} · organic ${serp.organicResults.length} · brands ${discovered.size}`
    );
  }
  ...
  const economicPool = [...discovered.values()]
    ...
    .sort(
      (a, b) =>
        b.brandScaleFit + b.firstPartyConfidence - (a.brandScaleFit + a.firstPartyConfidence)
    );
  ...
  for (const candidate of gapPool) {
    ...
    if (
      visionScreens < M961_DISCOVERY.maxVisionScreens &&
  ...
  for (const candidate of rankedPrePaid) {
    if (!canSpend(dfsBudget, M961_DISCOVERY.estimatedSerpCostPerKeyword * 2)) break;
  ...
  const purchaseModeSummary = [...discovered.values()].reduce<Record<string, number>>(
    (acc, c) => {
  ...
  const report = {
    ...
    funnel: {
      ...
      first_party_passed: [...discovered.values()].filter(
        (c) => c.firstPartyConfidence >= M961_DISCOVERY.firstPartyMinConfidence
      ).length,
...
function printReport(report: Record<string, unknown>): void {
  ...
  const familyYield = report.familyYield as Array<{ familyId: string; verdict: string }>;
  console.log("\n=== FAMILY VERDICTS ===");
  for (const row of familyYield) console.log(`  ${row.familyId}: ${row.verdict}`);

  const rec = report.recommendation as { productionWorthyFamilies: string[] };
  console.log("\n=== RECOMMENDATION ===");
