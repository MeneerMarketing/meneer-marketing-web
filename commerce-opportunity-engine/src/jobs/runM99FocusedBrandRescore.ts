...
import {
  FOCUSED_BRAND_GAP_FIRST_TARGET_V1,
  FOCUSED_BRAND_DISCOVERY_VERSION,
  FOCUSED_BRAND_REPORT_PATH,
  FOCUSED_BRAND_DASHBOARD_REPORT_PATH,
...
import type { PriceConfidence } from "../services/prospect/highTicketGapSalesFit.js";
import {
  computeFocusedBrandSalesFit,
  passesShowcaseDesignGate,
  passesStrongSalesProspectGate,
} from "../services/prospect/focusedBrandSalesFit.js";
import {
...
interface NormalizedCandidate {
  ...
  screened: boolean;
  legacyHighTicketGapSalesFit: number | null;
  legacyPreAuditGatePass: boolean;
  legacyOpportunityTier: string | null;
  legacySalesCandidate: boolean | null;
}

interface RescoredCandidate extends NormalizedCandidate {
  productCommercialValueSignal: number;
  productStoryValue: number;
  pdpDesignOpportunity: number;
  focusedBrandSalesFit: number;
  focusedBrandConfidence: "HIGH" | "MEDIUM" | "LOW";
  showcaseDesignGatePass: boolean;
  showcaseDesignGateFailures: string[];
  strongSalesGatePass: boolean;
  strongSalesGateFailures: string[];
...
function pickHeroPrice(r: RawRecord): number | null {
  const heroTarget = r.heroTarget as RawRecord | undefined;
  return (
    asNum(r.heroPrice) ??
...
function normalizeRecord(r: RawRecord, milestone: string): NormalizedCandidate | null {
  const productUrl = asStr(r.productUrl);
  const domain = asStr(r.domain);
  if (!productUrl || !domain) return null;

  const heroTarget = r.heroTarget as RawRecord | undefined;
  const purchaseMode = (asStr(r.purchaseMode) ?? "UNKNOWN") as PurchaseMode;
  const paid = (asStr(r.paidAcquisition) ?? "UNKNOWN") as PaidAcquisitionLevel;
  ...
  return {
    ...
    productTitle:
      asStr(r.productTitle) ?? asStr(r.heroTitle) ?? asStr(heroTarget?.heroTitle),
    platform: asStr(r.platform),
    pageEntityType: asStr(r.pageEntityType) ?? "UNKNOWN",
    businessModel: (asStr(r.businessModel) ?? "UNKNOWN") as BusinessModelClass,
    businessModelSalesCandidate: asBool(
      r.businessModelSalesCandidate,
      asBool(r.salesCandidate, false)
    ),
    ...
    redesignMaterialFeasibility:
      asNum(r.redesignMaterialFeasibility) ?? asNum(r.redesignMaterialFeasibility),
    ...
    legacySalesCandidate:
      typeof r.salesCandidate === "boolean"
        ? r.salesCandidate
...
function mergeCandidates(
  ...
  return {
    ...
    redesignMaterialFeasibility:
      preferScreened.redesignMaterialFeasibility ?? other.redesignMaterialFeasibility,
    legacyHighTicketGapSalesFit:
      preferScreened.legacyHighTicketGapSalesFit ?? other.legacyHighTicketGapSalesFit,
    legacyPreAuditGatePass:
      preferScreened.legacyPreAuditGatePass || other.legacyPreAuditGatePass,
    legacyOpportunityTier: preferScreened.legacyOpportunityTier ?? other.legacyOpportunityTier,
    legacySalesCandidate:
      preferScreened.legacySalesCandidate ?? other.legacySalesCandidate,
  };
}

function rescoreCandidate(c: NormalizedCandidate): RescoredCandidate {
...
function summarizeMovement(
  ...
  legacyRanked: RescoredCandidate[]
): Array<{
  domain: string;
  heroPrice: number | null;
  legacyRank: number | null;
  ...
  newLeadType: LeadType;
  focusedBrandSalesFit: number;
  legacyHighTicketGapSalesFit: number | null;
}> {
  const legacyMap = new Map(
    legacyRanked.map((c, i) => [c.candidateKey, { rank: i + 1, fit: c.legacyHighTicketGapSalesFit }])
  );
  ...
  return focusedRanked.slice(0, 25).map((c, i) => {
    ...
    if (rankDelta != null) {
      if (rankDelta > 3) movement = "UP";
      else if (rankDelta < -3) movement = "DOWN";
      else movement = "SAME";
    }
    return {
      domain: c.domain,
...
export async function runM99FocusedBrandRescore(): Promise<void> {
  const startedAt = new Date().toISOString();
  const merged = new Map<string, NormalizedCandidate>();

  for (const source of SOURCE_REPORTS) {
    try {
      const raw = JSON.parse(await readFile(source.path, "utf8")) as RawRecord;
      let loaded = 0;
      for (const section of source.sections) {
        const records = (raw[section] as RawRecord[] | undefined) ?? [];
      ...
      console.warn(`${source.milestone}: skip — ${(err as Error).message}`);
    }
  }

  const pool = [...merged.values()].filter(
    (c) => c.screened || c.preauditVisualGap != null || c.rawPdpRedesignOpportunity != null
  );
  ...
  const pricePromoted = movementSummary.filter(
    (m) =>
      (m.movement === "UP" || m.movement === "NEW") &&
      m.heroPrice != null &&
      m.heroPrice < 100 &&
      m.newLeadType === "STRONG_SALES"
  );

  const resellerDemoted = movementSummary.filter(
    (m) =>
      m.movement === "DOWN" &&
...
function stripForReport(c: RescoredCandidate): Record<string, unknown> {
  return {
    domain: c.domain,
