...
async function main(): Promise<void> {
  ...
  const { data: brand, error: brandErr } = await supabase
    .from("brands")
    .select(
      "id, name, normalized_domain, platform, platform_candidate, confirmed_google_advertiser, preferred_contact_id"
    )
    .eq("normalized_domain", "huisdierspullen.nl")
    .single();
  if (brandErr || !brand) throw new Error(brandErr?.message ?? "brand missing");
  ...
  const { data: opp, error: oppErr } = await supabase
    ...
    .single();
  if (oppErr || !opp) throw new Error(oppErr?.message ?? "opportunity missing");

  const { data: audit, error: auditErr } = await supabase
    .from("audits")
    .select(
      "id, conversion_leaks, strengths, finding_validations, sales_angle, audit_valid"
    )
    ...
    .limit(1)
    .maybeSingle();
  if (auditErr || !audit) throw new Error(auditErr?.message ?? "audit missing");
  ...
  if (!contact?.email) throw new Error("No usable contact for Huisdierspullen");
  ...
  try {
    for (const copyStyle of OUTREACH_COPY_STYLES as readonly CopyStyle[]) {
      ...
      const generated = await generateOutreachDraft({
        ...
        data: {
          ...
          platform:
            (brand.platform as string | null) ??
            (brand.platform_candidate as string | null),
          ...
          salesAngle: (audit.sales_angle as string | null) ?? null,
          copyStyle,
      ...
      const synced = await syncOutreachMessagesForBrand(
        supabase,
        String(brand.id),
        "m8.1.1 copy fidelity test on harvest-excluded brand"
      );
    ...
    for (const c of orderedClaims.filter((x) => x.type === "OBSERVATION" || x.type === "STRENGTH").slice(0, 6)) {
      console.log(
        `  [${c.type}] ${c.scope} · ${c.source_title} · ${c.validation_status}`
      );
    ...
    for (const d of drafts) {
      ...
      for (const row of (d.sentenceEvidence as Array<Record<string, unknown>>) ?? []) {
        console.log(
          `  [${row.allowed ? "OK" : "??"}] ${String(row.sentence).slice(0, 100)}`
        );
        console.log(
          `       ← ${row.notes}${
            Array.isArray(row.evidence_ids) && row.evidence_ids.length
              ? ` · ids=${(row.evidence_ids as string[]).join(",")}`
              : ""
          }`
        );
      }
      if (
        Array.isArray(d.validationErrors) &&
        (d.validationErrors as string[]).length
      ) {
        console.log("ERRORS:", (d.validationErrors as string[]).join("; "));
      }
      console.log("");
    }
