...
import { fetchAndCacheDomainMetrics } from "@/services/seo/domainMetricsService";
import {
...
export async function runCitySeoAnalysis(input: {
  ...
  let cityKeywords = generateCityKeywordUniverse(
    city.name as string,
    serviceSets,
    verticalSlug,
  ).slice(0, maxKeywords);
  ...
  for (const business of businesses) {
    try {
      ...
      const bizKeywords = generateKeywordsForCity(city.name as string, types, verticalSlug);
      ...
      const competitorDomains = scored.competitors.map((c) => c.competitor_domain);
      let competitors = scored.competitors;
      if (competitorDomains.length > 0 && apiCost < maxCost) {
        try {
          const { metrics, cost: drCost } = await fetchAndCacheDomainMetrics({
            domains: competitorDomains,
            maxCost: maxCost - apiCost,
          });
          apiCost += drCost;
          competitors = scored.competitors.map((row) => ({
            ...row,
            domain_rating: metrics.get(row.competitor_domain)?.domainRating ?? null,
          }));
        } catch {
          /* DR is optional enrichment */
        }
      }
