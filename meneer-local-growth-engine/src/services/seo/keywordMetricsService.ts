...
import { pilatesSeoKeywordStrategy } from "@/verticals/pilates/seo";
...
export async function fetchKeywordMetricsCached(input: {
  ...
  if (missing.length && cost < input.maxCostRemaining) {
    const locationCode =
      input.locationCode ?? pilatesSeoKeywordStrategy.countryLocationCode;
    const { data, cost: callCost } = await dataForSeoPost<SearchVolumeResponse>(
      "/keywords_data/google_ads/search_volume/live",
      [
        {
          location_code: locationCode,
          language_code: pilatesSeoKeywordStrategy.languageCode,
