...
/**
 * Hoe centraal Pilates in de listing staat. Een studio die zich Pilates-studio
 * noemt is iets anders dan een fysiopraktijk die er ook een les bij doet, en
 * alleen de eerste hoort een Pilates-preview te krijgen.
 */
export type PilatesFocus = "STRONG" | "MEDIUM" | "WEAK" | "NONE";

const FOCUS_ORDER: PilatesFocus[] = ["NONE", "WEAK", "MEDIUM", "STRONG"];

/** Diensten die de listing van een Pilates-studio naar iets anders trekken. */
const OTHER_TRADE =
  /fysio|physio|manuele therapie|osteopa|chiroprac|schoonheidssalon|nagelstudio|kapper|massagesalon|sportschool|crossfit|bootcamp|kickbox|tandarts|huidtherapie/i;

export function pilatesFocus(item: BusinessListingItem): PilatesFocus {
  ...
  if (!/pilates|reformer/.test(blob)) return "NONE";

  const inTitle = /pilates|reformer/.test(title);
  const inDomain = /pilates|reformer/.test(domain);
  const isPrimaryCategory = /^\s*pilates|pilates studio/.test(categories);
  ...
  if (/pilates/.test(categories)) return competing ? "WEAK" : "MEDIUM";
  return "WEAK";
}

/** Haalt de listing de gevraagde ondergrens van Pilates-focus? */
export function isRelevantPilatesListing(
  item: BusinessListingItem,
  options: { minFocus?: PilatesFocus } = {}
): boolean {
  const minimum = options.minFocus ?? "MEDIUM";
  return FOCUS_ORDER.indexOf(pilatesFocus(item)) >= FOCUS_ORDER.indexOf(minimum);
}
