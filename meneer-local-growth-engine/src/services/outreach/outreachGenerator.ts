...
/** "Deborah Pilates in Vleuten" → Deborah; "Claudia Penders Pilates" → Claudia. */
function firstNameFromBusinessName(businessName: string): string | null {
  const trimmed = businessName.trim();
  if (!trimmed) return null;

  const namedPilates = /^([A-ZÀ-ÿ][a-zà-ÿ'-]{2,15})\s+(?:Pilates|pilates)\b/.exec(trimmed);
  if (namedPilates?.[1] && !NOT_A_NAME.test(namedPilates[1])) {
    return namedPilates[1];
  }
  ...
  if (
    firstWord.length >= 2 &&
    firstWord.length <= 15 &&
    /^[A-ZÀ-ÿ][a-zà-ÿ'-]+$/.test(firstWord) &&
    !NOT_A_NAME.test(firstWord) &&
    !/pilates|yoga|studio|reformer|body|mind|flow|core/i.test(firstWord)
  ) {
    return firstWord;
  }
...
async function fetchPersonalizationSlots(input: {
  ...
  const fallback = deterministicSlots({
    ...
    secondary_keywords: input.ctx.secondary_keywords.filter((k) =>
      isClinic ? /botox|laser|huid|kliniek|intake/i.test(k) : /reformer|studio/i.test(k),
    ),
    city: input.ctx.city,
...
- subject_variant: chosen | idea. chosen = "Concept voor [studio]". idea = "Samenwerking met [studio]?". Standaard chosen.
...
Antwoord ALLEEN JSON:
{
  ...
  try {
    ...
    const subject_variant = parsed.data.subject_variant === "idea" ? "idea" : "chosen";
...
/**
 * De pagina op meneermarketing.nl met het aanbod voor deze branche.
 * @deprecated Gebruik ensureCampaignForBusiness + getCampaignLandingUrl.
 */
function buildOfferLandingUrl(_previewSlug: string | null): string | null {
  return null;
}
...
export async function generateOutreachDraft(input: {
  ...
  const campaign = await ensureCampaignForBusiness({ businessId: business.id });
  const previewUrl =
    (await getCampaignPreviewUrl(campaign)) ||
    resolvePreviewUrl({
  ...
  const landingPageUrl = await getCampaignLandingUrl(campaign);
  ...
  const fromBusiness = firstNameFromBusinessName(ctx.business_name);
  const knownFirstName = firstNameFromContact(resolved.contact.name, ctx.business_name);
  const derivedFirstName = ownerFromSite ?? fromBusiness;
  if (derivedFirstName && !knownFirstName) {
    await client
      .from("contacts")
      .update({ name: derivedFirstName })
      .eq("id", resolved.contact.id);
    resolved.contact.name = derivedFirstName;
    await writeActivity(client, {
      business_id: business.id,
      activity_type: "BUSINESS_UPDATED",
      title: `Contactnaam bijgewerkt · ${derivedFirstName}`,
      description: ownerFromSite
        ? "Naam gevonden op studio-website voor outreach-aanhef."
        : "Voornaam afgeleid van studionaam voor outreach-aanhef.",
    });
  }
  ...
  const contactFirstName =
    firstNameFromContact(resolved.contact.name, ctx.business_name) ??
    ownerFromSite ??
    fromBusiness;

  const competitionIntel = await getLeadCompetitionIntel({
    businessId: business.id,
    fetchMissingMetrics: false,
  });
  ...
  const rendered = renderHardenedOutreach({
    ...
    slots,
    verticalSlug,
    competitionSnippet: competitionIntel?.growthPlanSnippet ?? null,
  });
  ...
  const generated: GeneratedMail = {
    ...
    facts_used: [
      ...
      competitionIntel?.headline ? `Competition: ${competitionIntel.headline}` : null,
