...
import {
  countOutreachMailsByAmsterdamDay,
  validateSendSlot,
...
async function getBusinessVerticalSlug(business: Business): Promise<string> {
  const client = createAdminClient();
  const { data } = await client
    ...
    .maybeSingle();
  return String(data?.slug ?? "pilates");
}
...
export async function approveOutreachMessage(messageId: string): Promise<OutreachMessage> {
  ...
  const meta = (message.personalization_metadata ?? {}) as {
    ...
    context?: { landing_page_url?: string; vertical_slug?: string };
  };

  const bodyText = message.body_text ?? message.body;
  const verticalSlug =
    meta.context?.vertical_slug ?? (await getBusinessVerticalSlug(business));
  const landingPageUrl =
    meta.context?.landing_page_url ??
    extractOfferLandingUrlFromText(bodyText);

  const messageKind = (message.message_kind as "initial" | "followup") ?? "initial";

  let validation: { ok: boolean; errors: string[]; warnings: string[] };

  if (messageKind === "followup") {
    const { validateFollowupCopy } = await import("@/services/followup/followupValidation");
    validation = validateFollowupCopy({
  ...
  await writeActivity(client, {
    ...
    activity_type:
      messageKind === "followup" ? "FOLLOWUP_APPROVED" : "OUTREACH_APPROVED",
    title:
      messageKind === "followup" ? "Follow-up goedgekeurd" : "Outreach goedgekeurd",
...
export async function updateOutreachMessageContent(input: {
  ...
  const wasScheduled = message.status === "SCHEDULED";
  ...
  const bodyHtml =
    ...
    renderOutreachHtml({
      ...
      landingPageUrl:
        extractOfferLandingUrlFromText(input.bodyText) ?? undefined,
      brand,
    });
  ...
  const { data, error } = await client
    ...
    .update({
      ...
      body_html: bodyHtml,
      status: "REVIEW_REQUIRED",
      scheduled_at: wasScheduled ? null : message.scheduled_at,
...
export async function sendTestOutreachEmail(messageId: string): Promise<{
  ...
  try {
    ...
    const verticalSlug =
      pmeta.context?.vertical_slug ?? (await getBusinessVerticalSlug(business));
    const landingPageUrl =
      pmeta.context?.landing_page_url ??
      extractOfferLandingUrlFromText(bodyText);
...
export async function sendOutreachEmail(messageId: string): Promise<{
  ...
  if (message.status !== "APPROVED" && message.status !== "SCHEDULED") {
    throw new Error("Alleen goedgekeurde of geplande mails mogen verzonden worden");
  }
  ...
  const emailConfidence = await evaluateEmailConfidence({
    email: toEmail,
    businessDomain: business.normalized_domain ?? business.domain,
    source: "outreach_send",
  });
  if (emailConfidence.skipOutreach) {
    throw new Error(
      `Send geblokkeerd: e-mail confidence skip (${emailConfidence.reasons.join(", ")})`,
    );
  }
...
function parseScheduleTime(scheduledAt: string): Date {
...
export async function scheduleOutreachEmail(
  ...
  if (!isRealSendEnabled()) {
    throw new Error(
      "Plannen geblokkeerd: OUTREACH_REAL_SEND_ENABLED is niet true."
    );
  }
  if (!isEmailProviderReady()) {
    throw new Error("E-mailprovider niet geconfigureerd.");
  }

  const when = parseScheduleTime(scheduledAt);
  const now = Date.now();
  if (when.getTime() < now + MIN_SCHEDULE_LEAD_MS) {
    throw new Error("Plan minstens 2 minuten in de toekomst.");
  }
  if (when.getTime() > now + MAX_SCHEDULE_LEAD_MS) {
    throw new Error("Maximaal 30 dagen vooruit plannen.");
  }

  const client = createAdminClient();
  const { message } = await loadMessage(messageId);

  const dayCounts = await countOutreachMailsByAmsterdamDay(messageId);
  const slotCheck = validateSendSlot(when, undefined, dayCounts);
  if (!slotCheck.ok) {
    throw new Error(slotCheck.reason ?? "Ongeldig verzendslot");
  }

  if (!["APPROVED", "SCHEDULED"].includes(message.status)) {
    throw new Error("Alleen goedgekeurde mails kunnen gepland worden.");
  }
  ...
  const { data, error } = await client
    ...
    .select("*")
    .single();
  if (error || !data) throw new Error(error?.message ?? "Plannen mislukt");

  await writeActivity(client, {
    business_id: message.business_id,
    activity_type: "OUTREACH_SCHEDULED",
    title: "Outreach gepland",
    description: new Date(scheduledIso).toLocaleString("nl-NL", {
      timeZone: "Europe/Amsterdam",
    }),
    metadata: { message_id: messageId, scheduled_at: scheduledIso },
  });

  return data as OutreachMessage;
}

export async function cancelScheduledOutreach(messageId: string): Promise<OutreachMessage> {
  const client = createAdminClient();
  const { message } = await loadMessage(messageId);
  if (message.status !== "SCHEDULED") {
    throw new Error("Alleen geplande mails kunnen worden geannuleerd.");
  }
  ...
  const { data, error } = await client
    ...
    .select("*")
    .single();
  if (error || !data) throw new Error(error?.message ?? "Annuleren mislukt");

  await writeActivity(client, {
    business_id: message.business_id,
    activity_type: "OUTREACH_SCHEDULE_CANCELLED",
    title: "Geplande outreach geannuleerd",
    metadata: { message_id: messageId },
  });
...
export async function processDueScheduledOutreach(limit = 50): Promise<{
  processed: number;
  sent: number;
  failed: number;
  errors: string[];
}> {
  if (!isRealSendEnabled() || !isEmailProviderReady()) {
  ...
  for (const row of due ?? []) {
    try {
      await sendOutreachEmail(row.id as string);
      sent += 1;
    } catch (err) {
      failed += 1;
      errors.push(
  ...
  return { processed: due?.length ?? 0, sent, failed, errors };
}
