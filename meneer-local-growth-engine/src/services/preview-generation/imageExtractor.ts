...
const SKIP =
  /pixel|tracker|sprite|icon|favicon|badge|payment|ideal|paypal|visa|mastercard|facebook|twitter|linkedin|whatsapp|cookie|1x1|spacer|arrow|button/i;

/** Ook gebruikt door de Playwright-oogst, zodat beide bronnen gelijk scoren. */
export function scoreImage(opts: {
  url: string;
  alt: string;
  width: number | null;
  height: number | null;
  ...
  if (isLogoOrBrandAsset(opts.url, opts.alt)) {
    return { score: 0, semantic_type: "other" };
  }
  if (SKIP.test(blob) && !/studio|reformer|pilates|les|class|trainer/i.test(blob)) {
    return { score: 0, semantic_type: "other" };
  }
  if (/\blogo\b/i.test(blob)) {
    return { score: 0, semantic_type: "other" };
  }
  if (isLowQualityImage(opts.url, opts.width, opts.height)) {
    return { score: 0, semantic_type: "other" };
  }
  ...
  if (/reformer/i.test(blob)) {
    score += 25;
    semantic = "reformer";
  } else if (/huid|skin|facial|behandel|kliniek|clinic|aesthetic|cabine|consult|intake|receptie|wellness|beauty|gezicht|derma/.test(blob)) {
    score += 18;
    semantic = /behandel|treatment|facial|cabine/.test(blob) ? "studio" : "atmosphere";
...
export function extractImages(
  ...
  if (extras?.google_main_image && !seen.has(extras.google_main_image)) {
    const googleUrl = extras.google_main_image;
    if (
      !isLogoOrBrandAsset(googleUrl) &&
      !isWeakHeroCandidate(googleUrl)
    ) {
      candidates.push({
        url: googleUrl,
        ...
        height: null,
        semantic_type: "hero",
        score: heroCandidateScore(googleUrl, "Google Business", null, null, "hero"),
      });
    }
  }
