...
const INTERESTING =
  /home|over|about|les|aanbod|reformer|prijs|tarief|contact|pilates|klas|class|service|team|studio|boek|book/i;
...
export async function analyzeWebsite(
  ...
  const emails = Array.from(
    new Set(blob.match(/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}/gi) ?? [])
  ).slice(0, 5);
  const phones = Array.from(
    new Set(blob.match(/(?:\+31|0)[\d\s\-()]{8,16}/g) ?? [])
  ).slice(0, 5);
