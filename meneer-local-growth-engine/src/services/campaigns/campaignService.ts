...
function buildAbsolutePreview(slug: string | null | undefined): string {
  ...
  if (!base) return `/preview/${canonical}`;
  return formatPublicPreviewUrl(base, canonical);
}
...
async function maybeReserveCityForCampaign(input: {
  ...
  // M8.4: campaign reservation is business/campaign-specific — geen city-blocking meer.
  ...
  await client
    .from("campaigns")
    .update({
      city_status_snapshot: "CAMPAIGN_ACTIVE",
      updated_at: new Date().toISOString(),
    })
  ...
  await writeActivity(client, {
    business_id: input.business.id,
    activity_type: "CAMPAIGN_RESERVATION",
    title: "Campaign-reservatie actief (business-specifiek)",
    metadata: {
      campaign_id: input.campaign.id,
      expires_at: input.expiresAt,
...
function whitelistMetadata(
  ...
  if (
    (eventType === "PREVIEW_FEEDBACK_UP" || eventType === "PREVIEW_FEEDBACK_DOWN") &&
    typeof metadata.comment === "string"
  ) {
    out.comment = metadata.comment.slice(0, 500);
  }
  return out;
}
...
export async function getCampaignPreviewUrl(campaign: CampaignRow): Promise<string> {
  ...
  return buildAbsolutePreview(slug);
}

/** @deprecated Gebruik getCampaignPreviewUrl — outreach toont geen ?ref= meer in de link. */
export async function getCampaignPreviewUrlWithRef(
  campaign: CampaignRow
): Promise<string> {
  return getCampaignPreviewUrl(campaign);
}

/** Tracking op de preview-pagina zonder ?ref= in de outreach-link. */
export async function resolveActiveCampaignRefForPreviewSlug(
  slug: string
): Promise<string | null> {
  if (!slug?.trim()) return null;
  try {
    const client = createAdminClient();
    const canonical = toCanonicalPreviewSlug(slug.trim());

    let { data: preview, error } = await client
      .from("previews")
    ...
    if (!preview?.business_id) return null;

    const { data: campaign } = await client
      .from("campaigns")
      .select("campaign_ref, status, expires_at")
    ...
    if (!campaign?.campaign_ref) return null;
    if (campaign.expires_at && new Date(campaign.expires_at) < new Date()) {
      return null;
    }

    return campaign.campaign_ref as string;
  } catch {
    return null;
  }
}
...
> {
  ...
  const steps = [
    ...
    {
      key: "preview_feedback",
      label: "Preview feedback",
      at: findRealEvent(["PREVIEW_FEEDBACK_UP", "PREVIEW_FEEDBACK_DOWN"]),
    },
    {
      key: "offer",
...
export async function getCampaignEventSplit(campaignId: string): Promise<{
  real: Array<{ event_type: string; created_at: string; metadata?: Record<string, unknown> }>;
  test: Array<{ event_type: string; created_at: string; metadata?: Record<string, unknown> }>;
}> {
  const client = createAdminClient();
  const { data } = await client
    .from("campaign_events")
    .select("event_type, created_at, is_test, metadata")
