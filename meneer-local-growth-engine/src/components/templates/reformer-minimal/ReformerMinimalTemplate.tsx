...
import { ClinicAbout } from "@/components/templates/reformer-minimal/ClinicAbout";
import {
  ClinicTopBar,
  type MegaMenu,
...
/**
 * Variant B — Clinic v2 (zoals Studio Forma demo).
 * Niet de oude FigmaServices-layout; gebruikt Clinic* secties + buildClinicModel.
 */
export function ReformerMinimalTemplate({ studio }: Props) {
  const model = buildClinicModel(studio);
  const brandStyle = buildClinicBrandStyle(studio);
  ...
  return (
    <div
      className="figma-root relative min-h-screen overflow-x-clip pb-20 md:pb-0"
      style={brandStyle}
    >
