  ...
  return (
    <section id="tarieven" className="fc-plane-025">
      <div className="mx-auto px-5 py-20 sm:px-8 lg:px-[5vw] lg:py-28">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="figma-label text-[var(--fc-label)]">
                {variant === "skin-clinic" ? "Pakketten" : "Membership"}
              </p>
              <h2 className="figma-display-l mt-4">
                {variant === "skin-clinic" ? "Kies jouw traject" : "Kies jouw ritme"}
              </h2>
            </div>
            <p className="max-w-sm text-[14px] leading-6 text-[var(--fc-ink-soft)] md:text-right">
              {variant === "skin-clinic"
                ? "Van gratis intake tot onderhoud. Details en features naast je keuze."
                    ...
                    >
                      <span className="flex min-w-0 items-center gap-4">
                        {variant === "skin-clinic" ? (
                          <span
                            aria-hidden
                            className={`inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full border text-[11px] font-semibold uppercase tracking-[0.1em] transition-opacity duration-500 ${
                              selected
                                ? "border-[var(--fc-accent-deep)] bg-[var(--fc-accent-soft)] text-[var(--fc-accent-deep)] opacity-100"
                                : "border-[var(--fc-line)] text-[var(--fc-ink-mute)] opacity-55 group-hover:opacity-85"
                            }`}
                          >
                            {plan.name.slice(0, 2)}
                          </span>
            ...
            >
              ...
              <div className="relative flex flex-wrap items-start justify-between gap-4">
                ...
                {variant === "skin-clinic" ? (
                  <span
                    aria-hidden
                    className="inline-flex h-20 w-20 items-center justify-center rounded-full border border-[var(--fc-accent-deep)]/25 bg-[var(--fc-accent-soft)] text-sm font-semibold uppercase tracking-[0.12em] text-[var(--fc-accent-deep)] sm:h-24 sm:w-24"
                  >
        ...
        {note ? (
          <p className="mt-8 text-center text-[13px] leading-6 text-[var(--fc-ink-mute)]">
            {note}
          </p>
        ) : null}
      </div>
