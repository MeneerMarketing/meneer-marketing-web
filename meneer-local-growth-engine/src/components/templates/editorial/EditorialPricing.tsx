...
/**
 * Espresso-sectie met één cream hero-paneel voor het gekozen pakket en de
 * overige pakketten als rijen eronder. Zonder abonnementsdata valt het terug op
 * het lesaanbod plus "tarieven op aanvraag". Nooit verzonnen bedragen.
 */
export function EditorialPricing({
  ...
  const restPoses = posesForLabels(rest.map((plan) => plainText(plan.name)));
  const servicePoses = posesForLabels(
    services.map((service) => plainText(service.name))
  );
                  ...
                  >
                    <div className="min-w-0">
                      <div className="flex items-center gap-4">
                        <EditorialPose
                          name={restPoses[index] ?? "boat"}
                          className="h-12 w-12 shrink-0 text-[var(--ed-accent)] opacity-75 transition-all duration-700 ease-[var(--ease-premium)] group-hover:-translate-y-0.5 group-hover:opacity-100"
                        />
                      ...
                      <p className="mt-3.5 max-w-[54ch] text-[0.93rem] leading-relaxed text-[var(--ed-fg-70)] md:pl-[3.75rem]">
                        {plainText(plan.description)}
                      </p>

                      {plan.features.length > 0 ? (
                        <ul className="mt-4 flex flex-wrap gap-x-6 gap-y-2.5 md:pl-[3.75rem]">
                    ...
                    <article className="group flex h-full flex-col rounded-[20px] border border-[var(--ed-line)] p-7 transition-colors duration-500 md:p-9 hover:border-[var(--ed-accent-line)] hover:bg-[var(--ed-bg-raised)]">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex min-w-0 items-center gap-4">
                          <EditorialPose
                            name={servicePoses[index] ?? "boat"}
                            className="h-12 w-12 shrink-0 text-[var(--ed-accent)] opacity-75 transition-all duration-700 ease-[var(--ease-premium)] group-hover:-translate-y-1 group-hover:opacity-100"
                          />
                          <h3 className="ed-serif ed-h3">{name}</h3>
                        </div>
