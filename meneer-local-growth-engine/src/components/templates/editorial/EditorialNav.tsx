...
interface Props {
  studioName: string;
  /** Korte weergavenaam in de header; valt terug op studioName. */
  brandName?: string;
  logoUrl: string | null;
    ...
    <>
      ...
      <header
        className={`sticky top-0 z-40 transition-[background-color,border-color,backdrop-filter] duration-500 ${
          onMedia
            ? "before:pointer-events-none before:absolute before:inset-0 before:z-0 before:bg-gradient-to-b before:from-[#1a140f]/78 before:via-[#1a140f]/32 before:to-transparent"
            : ""
        } ${
          condensed
      ...
      >
        {hasTopBar ? (
          <div
            className={`ed-deep relative z-10 overflow-hidden transition-[max-height,opacity] duration-500 ease-[var(--ease-premium)] ${
        ...
        <div className="relative z-10 mx-auto grid h-16 max-w-[1440px] grid-cols-[minmax(0,auto)_1fr_auto] items-center gap-3 px-5 md:h-[74px] md:gap-4 md:px-10 lg:grid-cols-[minmax(0,10.5rem)_1fr_auto] lg:px-16">
          <a href="#top" className="relative z-10 min-w-0 max-w-[10.5rem] sm:max-w-[11.5rem]">
            <StudioBrandMark
              studioName={navBrand}
              logoUrl={logoUrl}
              variant="editorial"
          ...
          <nav
            ...
            className="z-10 hidden min-w-0 items-center justify-center gap-6 overflow-hidden md:flex lg:gap-8"
          >
            {links.map((link) => (
              <a
                key={link.href}
