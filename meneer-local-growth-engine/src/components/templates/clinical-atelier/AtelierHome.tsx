...
import { ArrowRight, CircleArrow } from '@/components/templates/clinical-atelier/AtelierIcons';
import { AtelierQuizModal } from '@/components/templates/clinical-atelier/AtelierQuizModal';

export function AtelierHome({ model }: { model: ClinicalAtelierModel }) {
  const [activeStep, setActiveStep] = useState(0)
  const [quizOpen, setQuizOpen] = useState(false)
  const [spotlight, setSpotlight] = useState({ x: 0, y: 0, visible: false })
  const socialRef = useRef<HTMLDivElement>(null)
  const { images } = model
  const journeySteps = model.journeySteps
  const treatments = model.treatments
  const experts = model.experts
  const featured = model.featuredTreatment
  const secondary = model.secondaryTreatment
  const tertiary = model.tertiaryTreatment
  const bookingProps = model.bookingExternal ? { target: '_blank' as const, rel: 'noopener noreferrer' } : {}
  ...
  return (
    <>
      <AtelierQuizModal open={quizOpen} onClose={() => setQuizOpen(false)} model={model} />
      ...
      >
        ...
        <div className="absolute bottom-[36px] lg:bottom-[48px] left-0 right-0 px-6 lg:px-[64px]">
          <div className="max-w-[580px]">
            <h1
              className="text-white mb-4 anim-fade-up anim-fade-up-1"
              style={{ fontSize: 'clamp(30px, 2.8vw, 42px)', lineHeight: '1.06', letterSpacing: '-0.01em' }}
            >
              <strong className="font-extrabold">{model.hero.headlineBold}</strong>
              <br />
              <span className="font-normal">{model.hero.headlineLight}</span>
            </h1>
            ...
            <div className="flex items-center gap-4 flex-wrap anim-fade-up anim-fade-up-3">
              <a href={model.bookingHref} {...bookingProps}
                className="flex items-center gap-2 bg-primary hover:bg-primary-dark text-white text-[13.5px] font-semibold rounded-full transition-colors duration-200 group"
      ...
      <div className="max-w-[1540px] mx-auto px-6 lg:px-10 pt-6 pb-2">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full overflow-hidden bg-light-grey shrink-0">
            <img
              src={model.medicalReviewer.image.url}
              alt={model.medicalReviewer.name}
          ...
          <p className="text-[11.5px] text-muted-text leading-[1.5]">
            ...
            <br />
            <span className="font-semibold text-charcoal">{model.medicalReviewer.name}</span>
            <span className="text-muted-text"> — {model.medicalReviewer.role}, {model.medicalReviewer.clinic}</span>
          </p>
        </div>
      </div>
          ...
          <div className="grid gap-3 grid-cols-1 lg:grid-cols-[27fr_46fr_27fr]" style={{ minHeight: 'clamp(300px, 36vw, 520px)' }}>
            ...
            <div className="rounded-[24px] p-8 lg:p-10 flex flex-col justify-between relative overflow-hidden group cursor-pointer" style={{ background: '#1A3A5C' }}>
              ...
              <div className="flex items-end justify-between relative z-10">
                <div>
                  <h3 className="text-white font-extrabold leading-[1.08] mb-3" style={{ fontSize: 'clamp(22px, 2vw, 30px)' }}>
            ...
            <div className="rounded-[24px] p-8 lg:p-10 flex flex-col justify-between bg-warm-white">
              <div>
                ...
                <div className="space-y-4">
                  {model.stats.map(({ label, value, progress }) => (
                    <div key={label}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[55fr_45fr] gap-4" style={{ minHeight: 'clamp(320px, 48vw, 600px)' }}>
          ...
          <div className="relative rounded-[24px] overflow-hidden bg-charcoal" style={{ minHeight: '320px' }}>
            ...
            <div className="absolute bottom-8 left-8 right-8">
              ...
              <p className="text-white font-light leading-[1.35] mb-4" style={{ fontSize: 'clamp(18px, 1.8vw, 26px)' }}>
                {model.patientQuote.text}
              </p>
              <p className="text-[12px] font-medium" style={{ color: 'rgba(255,255,255,0.50)' }}>— {model.patientQuote.author}</p>
            </div>
          </div>
          ...
          <div className="rounded-[24px] p-8 lg:p-10 flex flex-col justify-between border border-border-subtle bg-warm-white">
            <div>
              ...
              <div className="space-y-4">
                {model.skinMetrics.map(({ label, result, sub, progress }) => (
                  <div key={label}>
            ...
            <p className="text-[11px] text-muted-text/70 leading-relaxed border-t border-border-subtle pt-4 mt-2">
              Op basis van jouw huidanalyse stelt je huidspecialist een persoonlijk en meetbaar behandelplan op.
            </p>
          </div>
        </div>
      </section>
      ...
      ════════════════════════════════════════════════════════════════ */}
      <section id="traject" style={{ background: '#F4F4F2' }} className="py-16 lg:py-24">
        <div className="max-w-[1540px] mx-auto px-6 lg:px-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-6 lg:gap-12 items-center">
            <div className="rounded-[22px] overflow-hidden bg-light-grey" style={{ height: 'clamp(280px, 38vw, 480px)' }}>
              <img
                key={activeStep}
                src={journeySteps[activeStep].image.url}
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                ...
                <a href={model.bookingHref} {...bookingProps}
                  className="ml-2 flex items-center gap-2 bg-primary text-white text-[13px] font-semibold px-5 h-[40px] rounded-full hover:bg-primary-dark transition-colors group"
                >
                  Afspraak maken
      ...
      ════════════════════════════════════════════════════════════════ */}
      <section id="behandelingen" className="max-w-[1540px] mx-auto px-6 lg:px-10 py-16 lg:py-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[40fr_60fr] gap-4 items-stretch" style={{ minHeight: '560px' }}>
          ...
          <a
            ...
            {...bookingProps}
            className="relative block rounded-[24px] overflow-hidden bg-charcoal cursor-pointer group"
            style={{ minHeight: '340px' }}
          >
            <img
              src={images.treatPortrait.url}
            ...
            <div className="absolute top-7 left-7">
              <span className="text-[10px] font-semibold tracking-[0.14em] uppercase px-3 py-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.8)' }}>{featured.name}</span>
            </div>
            <div className="absolute bottom-8 left-8 right-8 flex items-end justify-between">
              <div>
                <p className="text-[11px] font-semibold tracking-wide uppercase mb-2" style={{ color: 'rgba(255,255,255,0.50)' }}>{featured.from}</p>
                <h3 className="text-white font-extrabold leading-tight" style={{ fontSize: 'clamp(22px, 2.2vw, 30px)' }}>{featured.name}</h3>
                <p className="text-[13px] mt-1.5 font-light" style={{ color: 'rgba(255,255,255,0.65)' }}>{featured.description}</p>
              </div>
              <CircleArrow />
            </div>
          </a>
          ...
          <div className="grid grid-rows-[1fr_1fr] gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-[55fr_45fr] gap-4">
              <a
                href={model.bookingHref}
                {...bookingProps}
                className="relative block rounded-[22px] overflow-hidden bg-charcoal cursor-pointer group"
                style={{ minHeight: '200px' }}
              >
                <img src={images.treatDetail.url} alt={secondary.name} className="absolute inset-0 w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-700" />
                <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(26,58,92,0.85) 0%, transparent 55%)' }} />
                <div className="absolute bottom-5 left-5 right-5">
                  <p className="text-[10px] font-semibold tracking-wide uppercase mb-1" style={{ color: 'rgba(255,255,255,0.50)' }}>{secondary.from}</p>
                  <h4 className="text-white text-[16px] font-bold">{secondary.name}</h4>
                </div>
              </a>
              {/* Soft petrol info card */}
              <div className="rounded-[22px] p-6 flex flex-col justify-between" style={{ background: '#E4E4E0' }}>
                <p className="text-[10px] font-semibold tracking-[0.14em] uppercase text-primary">{secondary.name}</p>
                <div>
                  <p className="text-[28px] font-extrabold text-primary leading-none mb-1">{secondary.from.replace('Vanaf ', '')}</p>
                  <p className="text-[12px] text-muted-text leading-[1.5]">{secondary.description}</p>
                </div>
                <a href={model.bookingHref} {...bookingProps} className="flex items-center gap-1.5 text-primary text-[12px] font-semibold group w-fit">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-[45fr_55fr] gap-4">
              ...
              <div className="rounded-[22px] p-6 flex flex-col justify-between relative overflow-hidden" style={{ background: '#2E5B8A' }}>
                ...
                <p className="text-[10px] font-semibold tracking-[0.14em] uppercase relative z-10" style={{ color: 'rgba(255,255,255,0.60)' }}>{tertiary.name}</p>
                <div className="relative z-10">
                  <p className="text-white font-extrabold leading-[1.1] mb-2" style={{ fontSize: '20px' }}>Stralende huid.<br />Zichtbaar effect.</p>
                  <p className="text-[11px] font-medium" style={{ color: 'rgba(255,255,255,0.55)' }}>{tertiary.from}</p>
                </div>
              </div>
              <a
                href={model.bookingHref}
                ...
                style={{ minHeight: '200px' }}
              >
                <img src={images.treatHands.url} alt={treatments[3]?.name ?? tertiary.name} className="absolute inset-0 w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-700" />
                <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(26,58,92,0.85) 0%, transparent 55%)' }} />
                <div className="absolute bottom-5 left-5 right-5">
                  <p className="text-[10px] font-semibold tracking-wide uppercase mb-1" style={{ color: 'rgba(255,255,255,0.50)' }}>{treatments[3]?.from ?? tertiary.from}</p>
                  <h4 className="text-white text-[16px] font-bold">{treatments[3]?.name ?? 'Chemical peel'}</h4>
                </div>
              </a>
            </div>
          </div>
        ...
        <div className="mt-10 border-t border-border-subtle">
          {treatments.map((t) => (
            <a
              ...
              href={model.bookingHref}
              {...bookingProps}
              className="flex w-full items-center justify-between border-b border-border-subtle py-4 -mx-3 px-3 text-left transition-colors group hover:bg-off-white/60"
            >
        ...
        <div className="absolute inset-0 flex items-end pb-12 lg:pb-16">
          <div className="max-w-[1540px] mx-auto px-6 lg:px-10 w-full">
            <div className="max-w-[520px]">
              <p className="text-[10px] font-semibold tracking-[0.18em] uppercase mb-5" style={{ color: 'rgba(255,255,255,0.55)' }}>{model.campaign.eyebrow}</p>
              <h2 className="text-white leading-[1.08] mb-5 font-extrabold" style={{ fontSize: 'clamp(28px, 3.2vw, 46px)' }}>{model.campaign.titleBold}<br /><span className="font-light">{model.campaign.titleLight}</span></h2>
              <p className="text-[13.5px] leading-relaxed mb-8 max-w-[380px]" style={{ color: 'rgba(255,255,255,0.65)' }}>{model.campaign.body}</p>
      ...
      ════════════════════════════════════════════════════════════════ */}
      <section id="team" style={{ background: '#F4F4F2' }} className="py-16 lg:py-24">
        <div className="max-w-[1540px] mx-auto px-6 lg:px-10">
          <div className="flex items-end justify-between mb-10">
            <div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-[38fr_26fr_20fr_16fr] gap-3" style={{ height: 'clamp(320px, 44vw, 560px)' }}>
            ...
            {experts.slice(0, 1).map(e => (
              <a key={e.name} href="#team" className="relative rounded-[24px] overflow-hidden bg-charcoal group cursor-pointer block">
                <img src={e.image.url} alt={e.name} className="absolute inset-0 w-full h-full object-cover object-top group-hover:scale-[1.02] transition-transform duration-700" />
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  ...
                  <p className="text-white font-bold text-[17px] leading-tight">{e.name}</p>
                </div>
              </a>
            ))}
            ...
            {experts.slice(2, 3).map(e => (
              <a key={e.name} href="#team" className="relative rounded-[24px] overflow-hidden bg-charcoal group cursor-pointer block">
                ...
                <div className="absolute bottom-5 left-4 right-4">
                  ...
                  <p className="text-white font-bold text-[13px] leading-tight">{e.name}</p>
                </div>
              </a>
            ))}
            ...
            <div className="rounded-[24px] p-6 flex flex-col justify-between relative overflow-hidden" style={{ background: '#1A3A5C' }}>
              ...
              <div className="relative z-10">
                <p className="text-white font-extrabold leading-[1.1] mb-3 text-[16px]">
      ...
      ════════════════════════════════════════════════════════════════ */}
      <section id="ervaringen" className="max-w-[1540px] mx-auto px-6 lg:px-10 py-16 lg:py-24">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_440px] gap-4">

          {/* Photo with brand gradient + integrated quote */}
          <div className="relative rounded-[24px] overflow-hidden bg-charcoal" style={{ minHeight: '320px' }}>
            <img
              src={images.testimonial.url}
            ...
            <div className="absolute bottom-8 left-8 right-8">
              ...
              <p className="text-white font-light leading-[1.35] mb-4" style={{ fontSize: 'clamp(17px, 1.8vw, 24px)' }}>
                {model.testimonial.text}
              </p>
              <div className="flex items-center gap-3">
                <div className="w-px h-8 bg-white/30" />
                <div>
                  <p className="text-[13px] font-semibold text-white">{model.testimonial.author}</p>
                  <p className="text-[11px]" style={{ color: 'rgba(255,255,255,0.55)' }}>{model.testimonial.meta}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-[24px] p-8 lg:p-10 flex flex-col justify-between border border-border-subtle bg-warm-white">
            ...
            <div>
              <p className="text-[64px] font-extrabold text-primary leading-none">{model.ratingDisplay.replace('.', ',')}</p>
              <p className="text-[12px] text-muted-text mt-1 mb-6">{model.reviewCountLabel}</p>
        ...
        <div className="max-w-[1540px] mx-auto px-6 lg:px-10 pt-16 pb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-[12px] font-bold text-soft-black tracking-[-0.01em]">{model.instagram.handle}</p>
                <p className="text-[10.5px] text-muted-text">{model.instagram.followers}</p>
              </div>
            </div>
            <h2 className="font-extrabold text-soft-black leading-[1.0]" style={{ fontSize: 'clamp(26px, 2.6vw, 38px)', letterSpacing: '-0.02em' }}>
              Dagelijkse inspiratie voor jouw huid
            </h2>
          </div>
          <a
            href={model.instagram.href ?? "#"}
            className="shrink-0 inline-flex items-center gap-2 text-[13px] font-semibold h-[40px] px-5 rounded-full border border-border-subtle text-charcoal hover:bg-soft-black hover:text-white hover:border-soft-black transition-all duration-200"
          >
            Volgen op Instagram
      ...
      <div id="huidproblemen" className="sr-only" aria-hidden="true" />
      <div id="tarieven" className="sr-only" aria-hidden="true" />
      <div id="informatie" className="sr-only" aria-hidden="true" />
    </>
  )
}
