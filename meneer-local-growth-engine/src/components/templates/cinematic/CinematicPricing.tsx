 ...
 */
export function CinematicPricing({ model, variant = "pilates" }: Props) {
  const { plans, booking, primaryService, pricingNote } = model;
  ...
  return (
    <section id="tarieven" className="bg-[var(--cn-cream)]">
      <div className="mx-auto px-5 py-16 sm:px-8 sm:py-20 lg:px-10 lg:py-24">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="cine-label text-[var(--cn-muted)]">
                {variant === "skin-clinic" ? "Pakketten" : "Tarieven"}
              </p>
              <h2 className="cine-display cine-display-l cine-lower mt-5 text-[var(--cn-oxblood)]">
                {variant === "skin-clinic" ? (
                  <>
                    wat je krijgt
                    <br />
                    <span className="cine-italic">en wat het kost.</span>
                  </>
                ) : (
                  <>
                    wat het kost
            ...
            <p className="max-w-[34ch] text-[13.5px] leading-6 text-[var(--cn-body)] sm:text-[14px]">
              {variant === "skin-clinic"
                ? "Start met een gratis intake en kies daarna een traject dat bij je huid past. Pakketten zijn indicatief in dit concept."
                : `Start met een pack en stap over op een membership zodra ${primaryService ? primaryService.toLowerCase() : "pilates"} in je week past. Je plan loopt per maand, dus je kiest zelf hoe lang je blijft.`}
            </p>
          </div>
        </ScrollReveal>
        ...
        {pricingNote ? (
          <p className="cine-label mt-10 max-w-[52ch] text-[var(--cn-muted)]">
            {pricingNote}
          </p>
        ) : null}
      </div>
