...
import { PreviewShareLinkPanel } from "@/components/dashboard/PreviewShareLinkPanel";
import { BrandColorOverridePanel } from "@/components/dashboard/BrandColorOverridePanel";
import { listPreviewShareLinks } from "@/services/preview/previewShareService";
import { GenerateOutreachButton } from "@/components/dashboard/OutreachActions";
import { LeadEnrichEmailButton } from "@/components/dashboard/LeadEnrichEmailButton";
import { EmailConfidenceBadge } from "@/components/dashboard/EmailConfidenceBadge";
...
import { LeadJourneyPanel } from "@/components/dashboard/LeadJourneyPanel";
import { LeadCompetitionIntelPanel } from "@/components/dashboard/LeadCompetitionIntelPanel";
import {
...
export default async function LeadDetailPage({ params, searchParams }: Props) {
  ...
  const [verticals, cities, templates, previews, seo, contacts, messages, activity, exclusivityRows, competitionIntel, shareLinks] =
    await Promise.all([
      getVerticals(),
      getCities(),
      getTemplates(),
      ...
      getExclusivity(),
      getLeadCompetitionIntel({ businessId: business.id }),
      listPreviewShareLinks(business.id),
    ]);
    ...
    <div>
        ...
        {!business.is_demo ? (
          <div className="flex flex-col items-end gap-3">
            <LeadMailWishlistToggle
              ...
              selected={Boolean(business.selected_for_outreach)}
              studioName={business.studio_name}
              cityName={city.name}
              businessVerticalId={business.vertical_id}
              verticals={verticals}
            />
      ...
      {tab === "overview" ? (
        <div className="space-y-6">
          <LeadCompetitionIntelPanel businessId={business.id} intel={competitionIntel} />
          <div className="grid gap-6 lg:grid-cols-2">
            <Panel title="Bedrijfsgegevens">
              <dl>
                ...
                {!business.email && business.website_url ? (
                  <div className="mt-3 flex justify-end border-t border-mm-border pt-3">
                    <LeadEnrichEmailButton
                      businessId={business.id}
            ...
            <Panel title="Pipeline">
              <dl>
                ...
                <KeyValue
                  label="Mail-lijst"
                  value={business.selected_for_outreach ? "Ja" : "Nee"}
                />
                <KeyValue
                  label="Primary candidate"
                  value={business.primary_candidate ? "Ja (auto)" : "Nee"}
                />
      ...
      {tab === "preview" ? (
        <div className="space-y-4">
          ...
          {!business.is_demo ? (
            <Panel title="Brandkleuren (handmatig)">
              <BrandColorOverridePanel
                businessId={business.id}
                primaryColor={
                  (business.brand_profile as { primary_color?: string } | undefined)
                    ?.primary_color ??
                  business.primary_color
                }
                secondaryColor={
                  (business.brand_profile as { secondary_color?: string } | undefined)
                    ?.secondary_color ??
                  business.secondary_color
                }
                accentColor={
                  (business.brand_profile as { accent_color?: string } | undefined)?.accent_color ??
                  business.accent_color
                }
                previewSlug={
                  previews.find((p) => p.status === "READY" || p.status === "APPROVED")?.slug ?? null
                }
              />
            </Panel>
          ) : null}
          {!business.is_demo && (previews.some((p) => p.status === "READY" || p.status === "APPROVED")) ? (
            <Panel title="Share link (30 dagen)">
              <PreviewShareLinkPanel businessId={business.id} initialLinks={shareLinks} />
            </Panel>
          ) : null}
        </div>
      ) : null}
                    ...
                    .map((c, idx) => (
                      <li key={`${c.competitor_domain}-${idx}`} className="border border-slate-100 p-3">
                        <p className="font-semibold">
                          ...
                          {c.domain_rating != null ? (
                            <span className="ml-2 text-[10px] font-bold uppercase text-[#C2410C]">
                              DR {c.domain_rating}
                            </span>
                          ) : null}
                        </p>
              ...
              {contacts.map((c) => (
                <li key={c.id} className="border border-mm-border p-4">
                  ...
                  <EmailConfidenceBadge
                    level={c.email_confidence_level}
                    score={c.email_confidence}
                    reasons={c.email_confidence_reasons}
                  />
                </li>
              ))}
            </ul>
      ...
      {tab === "journey" ? <LeadJourneyPanel businessId={business.id} /> : null}
