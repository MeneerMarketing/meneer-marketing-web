...
import { MessageEditorActions } from "@/components/dashboard/MessageEditorActions";
...
export default async function OutreachDetailPage({ params }: Props) {
  ...
  const isFollowup = (message.message_kind ?? "initial") === "followup";

  return (
    <div>
      <div className="mb-6">
        <TextLink href={isFollowup ? "/dashboard/followups" : "/dashboard/outreach"}>
          ← Terug naar {isFollowup ? "follow-ups" : "outreach"}
        </TextLink>
      </div>
      <SectionTitle
        eyebrow={isFollowup ? "Follow-up editor" : "Outreach editor"}
        title={message.subject}
        description={
          isFollowup
            ? "Bewerk, preview, approve en verstuur je follow-up wanneer jij wilt."
            : "Review, bewerk, approve. Echte send alleen met Resend + APPROVED."
        }
      />
      <DemoBanner />

      <div className="grid gap-6 xl:grid-cols-[1fr]">
        <Panel title={isFollowup ? "Follow-up mail" : "Generated mail"}>
          <div className="mb-3 flex flex-wrap gap-2">
            <Badge tone="warn">{message.status}</Badge>
            {isFollowup ? <Badge tone="sky">Follow-up</Badge> : null}
          ...
          <MessageEditorActions
            ...
            initialBodyHtml={message.body_html}
            scheduledAt={message.scheduled_at}
            mode={isFollowup ? "followup" : "outreach"}
            followupTemplate={message.followup_template ?? null}
              ...
              <KeyValue
                ...
                value={
                  message.scheduled_at
                    ? new Date(message.scheduled_at).toLocaleString("nl-NL", {
