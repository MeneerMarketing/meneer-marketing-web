  ...
  try {
    ...
    if (data.action === "exclude_lead") {
      ...
      await applyLeadDisposition(client, {
        ...
        disposition: "dismiss",
        note: data.note ?? "Handmatige override",
      });
      return NextResponse.json({ ok: true });
    }
    ...
    if (data.action === "set_status") {
      ...
      if (data.status === "DO_NOT_CONTACT" || data.status === "REJECTED") {
        await applyLeadDisposition(client, {
          businessId: data.businessId,
          disposition: data.status === "REJECTED" ? "reject" : "dismiss",
          note: data.note ?? "Handmatige override",
        });
        return NextResponse.json({ ok: true });
      }
