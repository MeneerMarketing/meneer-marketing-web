...
Alle routes onder `/api/*` vereisen een ingelogde Supabase-sessie, **behalve**:

- `/api/public/*` — campaign tracking (rate limit in de route)
- `/api/webhooks/*` — externe webhooks (eigen signature-check)

Preview-pagina's (`/preview/[slug]`) blijven publiek; alleen discovery, outreach, scoring enz. zijn afgeschermd.

Zet `LGE_DEV_AUTH_BYPASS=false` op elke Vercel-deploy. Met `true` in productie zijn dashboard én interne API's open.
